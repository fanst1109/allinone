...
Các quy tắc về `solutions.go`, callouts, và một số chi tiết khác **áp dụng theo loại lĩnh vực**:

| Loại lĩnh vực | Ví dụ | `solutions.go` | Callouts (💡/❓/⚠/🔁/📝) |
|---------------|-------|:---:|:---:|
| **Kỹ thuật / lập trình** | `DataStructures`, `DataFoundations`, `Java`, `Python` | ✓ | ✓ |
| **Toán / khoa học trừu tượng** | `Vectors/*`, `Math`, `Physics`, `Economics` | ✓ | ✓ (bắt buộc) |
| **Ngôn ngữ** | `English`, `Japanese` | ✗ (không cần) | ✗ (không phù hợp) |
...
- [ ] Có đủ file theo cấu trúc bắt buộc: `README.md` luôn cần; `solutions.go` và `visualization.html` tùy loại lĩnh vực + yêu cầu user.
...
- [ ] (Có code) Code biên dịch / chạy được (vd `go run solutions.go`).
- [ ] (Có viz) Nav strip + readme-modal + đường dẫn `tools/` đúng độ sâu:
  - 2 cấp `<Lĩnh vực>/lesson-XX/`: `../../tools/`
  - 3 cấp `<Lĩnh vực>/<Tier>/lesson-XX/` (vd Vectors): `../../../tools/`
- [ ] (Có viz) Đã chạy `go run tools/build-readme-data.go [<đường-dẫn>]` để sinh `README.data.js`.
