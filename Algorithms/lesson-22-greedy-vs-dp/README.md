# Lesson 22 — Greedy vs Dynamic Programming
...
| Tốc độ | Nhanh, thường $O(n \log n)$ | Chậm hơn, $O(nW)$, $O(n^2)$... |
| Bộ nhớ | Ít | Thường cần bảng $O(n)$ hoặc $O(nW)$ |
...
>
> 1. Greedy thường $O(n \log n)$; DP thường $O(nW)$ hoặc $O(n^2)$ — chậm hơn và tốn bộ nhớ.
...
> - DP $O(n \cdot W)$ luôn đúng — sẽ học kỹ ở [Tier 4 Lesson 25](../tier-4-dynamic-programming/index.html).
...
| Tiêu chí | Greedy | DP |
|---|---|---|
| Thời gian điển hình | $O(n \log n)$ (do sort) | $O(n \cdot W)$ (knapsack), $O(n^2)$ (LIS naive)... |
| Bộ nhớ | $O(1)$–$O(n)$ | $O(n)$–$O(n \cdot W)$ (bảng memo) |
...
**Kết luận:** Greedy **đúng**. Độ phức tạp $O(\text{số mệnh giá})$.
...
**Kết luận:** Greedy **sai**. Hệ `{1,5,8}` không canonical → phải dùng **DP** $O(\text{amount} \times \text{số mệnh giá})$.
...
**Kết luận:** Greedy **sai**. 0/1 knapsack cần **DP** $O(n \cdot W)$ (Tier 4 L25).
...
**Kết luận:** Greedy **sai**. LIS cần **DP** $O(n^2)$ (hoặc greedy + binary search $O(n \log n)$ với cấu trúc patience sorting — nhưng đó là greedy *được chứng minh*, khác greedy ngây thơ ở trên).
