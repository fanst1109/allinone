...
**4 ví dụ số đa dạng**:
- Nhân: 3 món chính RỒI 4 tráng miệng → `3·4 = 12` bộ.
- Cộng: đi từ HN tới HCM bằng 2 chuyến bay HOẶC 3 chuyến tàu → `2+3 = 5` cách.
- Nhân nhiều giai đoạn: mã PIN 4 chữ số (0–9), mỗi vị trí 10 cách → `10·10·10·10 = 10⁴ = 10000`.
- Kết hợp: chọn (áo: 3 cái) RỒI (quần: 2 cái) cho thứ Hai HOẶC thứ Ba → `(3·2) + (3·2) = 12`.

❓ **Câu hỏi tự nhiên của người đọc**

- *"Khi nào cộng, khi nào nhân?"* Cộng khi các lựa chọn **loại trừ nhau** (chỉ chọn 1 nhóm). Nhân khi phải làm **tất cả các bước** liên tiếp. Mẹo: "hoặc" → +, "và rồi" → ×.
- *"Quy tắc cộng cần điều kiện gì?"* Các nhóm phải **không trùng** (rời nhau). Nếu trùng phải dùng bù trừ (L05): `|A∪B| = |A|+|B|−|A∩B|`.

⚠ **Lỗi thường gặp**: nhân khi đáng lẽ cộng (hoặc ngược lại). Phản ví dụ: "chọn 1 món ăn từ thực đơn 3 món Việt HOẶC 4 món Nhật" → `3+4 = 7` (cộng, vì chỉ chọn 1 món), KHÔNG phải `3·4 = 12`.
...
1. `26·10·10·10 = 26000` (nhân, các giai đoạn).
...
- "HOẶC" (rời nhau) → **cộng** `m+n`; "RỒI/VÀ" (lần lượt) → **nhân** `m·n`.
- Quy tắc cộng đòi các nhóm không trùng.
- Nhiều giai đoạn độc lập → nhân tất cả (vd PIN `10⁴`).
...
- *"Vì sao `0! = 1` chứ không phải 0?"* Để công thức `C(n,k) = n!/(k!(n−k)!)` còn đúng khi k = 0 hoặc k = n: `C(5,0) = 5!/(0!·5!) = 1` cần `0! = 1`. Đó là quy ước giữ công thức nhất quán, ngoài ra "có đúng 1 cách sắp xếp dãy rỗng".
- *"Hoán vị có vật giống nhau thì sao?"* Phải chia cho giai thừa số vật trùng. Vd sắp chữ "MISSISSIPPI" (11 chữ, 4 S, 4 I, 2 P): `11!/(4!·4!·2!)`, KHÔNG phải 11!.
...
💡 **Trực giác / Hình dung**: chỉnh hợp = "xếp hàng nhưng chỉ chọn k chỗ". Trao huy chương vàng-bạc-đồng cho 10 VĐV: chỗ vàng 10 cách, bạc 9, đồng 8 → `10·9·8`. Vẫn là xếp hàng (thứ tự quan trọng), nhưng dừng sau k bước thay vì hết n.
...
**4 ví dụ số đa dạng**:
- `A(10, 3) = 10·9·8 = 720` (huy chương V-B-Đ).
- `A(5, 2) = 5·4 = 20` (chọn + sắp 2 trong 5).
...
- *"Vì sao công thức là `n!/(n−k)!`?"* `A(n,k) = n·(n−1)·...·(n−k+1)` (k thừa số giảm dần). Viết gọn: nhân thêm rồi chia phần thừa `(n−k)!` → `n!/(n−k)!`. Vd `A(5,2) = 5!/3! = 120/6 = 20` ✓.
...
- *"`C(n,k) = A(n,k)/k!` từ đâu ra?"* Mỗi tổ hợp k phần tử có `k!` cách sắp thành chỉnh hợp. Nên `A(n,k) = C(n,k)·k!` → `C = A/k!`. Verify: `C(5,2) = A(5,2)/2! = 20/2 = 10` ✓.
...
- *"'Có lặp' nghĩa là gì?"* Là 1 phần tử được chọn lại nhiều lần. Mã PIN `1131` có lặp (số 1 dùng 3 lần) → `10⁴`. Chọn 3 người khác nhau vào ban → không lặp → C hoặc A.
...
⚠ **Lỗi thường gặp — nhầm "có lặp" và "không lặp"**. Phản ví dụ: "mã số 3 chữ số khác nhau" = `A(10,3) = 720` (không lặp), nhưng "mã số 3 chữ số bất kỳ" = `10³ = 1000` (có lặp). Đọc kỹ đề có chữ "khác nhau" hay không.
...
- Có lặp: chỉnh hợp lặp `nᵏ`, tổ hợp lặp `C(n+k−1, k)`.
...
- *"Xác suất trúng Vietlott 6/45 nhỏ cỡ nào?"* `1/C(45,6) = 1/8,145,060 ≈ 1.23·10⁻⁷` — tức ~1 phần 8 triệu. Mua 1 vé mỗi tuần, trung bình ~156,000 năm mới trúng 1 lần.
- *"Mật khẩu khác nhau với cho lặp khác bao nhiêu?"* 4 chữ số cho lặp = `10⁴ = 10000`; khác nhau = `A(10,4) = 5040` — gần một nửa.

### 📝 Tóm tắt mục 6

- Mật khẩu cho lặp: `nᵏ`; khác nhau: `A(n,k)`.
- Xổ số / chọn nhóm không vai: tổ hợp `C(n,k)`.
- Chọn có vai trò khác nhau: chỉnh hợp `A(n,k)` = `C(n,k)·k!`.
