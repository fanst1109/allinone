# Lesson 01 — Quản lý bộ nhớ (Memory Management)
...
```
Địa chỉ vật lý = Địa chỉ logic + Base
Điều kiện hợp lệ: Địa chỉ logic < Limit
```
...
- MMU dịch: vật lý = logic + base; kiểm tra: logic < limit.
...
- **Địa chỉ logic** = địa chỉ tiến trình dùng (bắt đầu từ 0). **MMU** dịch sang địa chỉ vật lý bằng `vật lý = logic + base`.
