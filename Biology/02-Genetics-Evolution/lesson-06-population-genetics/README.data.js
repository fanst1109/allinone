...
- Phát biểu và áp dụng **định luật Hardy–Weinberg (HW)**: \`p² + 2pq + q² = 1\`, đi kèm \`p + q = 1\` cho 2 allele.
...
- Vận dụng HW từ **tần số kiểu hình lặn** (q²) → suy ra q, p, và **tần số người mang gen lặn dị hợp** 2pq (ứng dụng dịch tễ bệnh di truyền lặn).
- **Kiểm tra một quần thể có ở trạng thái HW không** bằng cách so sánh kiểu gen quan sát với dự đoán p², 2pq, q².
...
- Tỉ lệ phần trăm và xác suất cơ bản (đặc biệt \`(p+q)² = p² + 2pq + q²\` — hằng đẳng thức bình phương một tổng).
...
- Tổng allele = 100 × 2 = 200.
- Số allele A = 2 × 36 + 1 × 48 = 72 + 48 = **120**. Số a = 2 × 16 + 48 = 32 + 48 = **80**.
- **p = 120/200 = 0.6**, **q = 80/200 = 0.4**. Kiểm tra p + q = 1 ✓.
- Cách 2 (qua tần số kiểu gen): D = 36/100 = 0.36, H = 0.48, R = 0.16. p = 0.36 + 0.48/2 = 0.36 + 0.24 = **0.6** ✓.

**Ví dụ 2 — Đếm từ kiểu gen (400 cá thể)**: 64 AA, 192 Aa, 144 aa.

- D = 64/400 = 0.16, H = 192/400 = 0.48, R = 144/400 = 0.36.
- p = 0.16 + 0.48/2 = 0.16 + 0.24 = **0.40**.
- q = 0.36 + 0.24 = **0.60**. Kiểm tra: 0.40 + 0.60 = 1 ✓.

**Ví dụ 3 — Quần thể chỉ có 2 kiểu gen**: 200 cá thể, gồm 50 AA và 150 Aa (không có aa).

- D = 0.25, H = 0.75, R = 0.
- p = 0.25 + 0.75/2 = 0.25 + 0.375 = **0.625**.
- q = 0 + 0.375 = **0.375**. Kiểm tra: 0.625 + 0.375 = 1 ✓.

**Ví dụ 4 — Quần thể đơn hình**: 1000 cá thể đều là aa (toàn bộ kiểu hình lặn).

- p = 0, **q = 1**. Vốn gen chỉ chứa allele a — gen này **đã cố định (fixed)** trong quần thể.
...
- **Lẫn tần số allele với tần số kiểu hình**: thấy 16% cá thể xám rồi kết luận "q = 0.16" — sai. 16% là **tần số kiểu hình lặn**, tức **q²**, không phải q. Đúng phải lấy căn: q = √0.16 = 0.4. Đây là nhầm lẫn phổ biến nhất; xem §4.
- **Quên dị hợp đóng góp cả 2 allele**: chỉ cộng "2×AA + 2×aa" rồi chia → bỏ sót Aa. Mỗi Aa cho 1 A và 1 a, không phải 0.
- **Coi p + q ≠ 1**: với gen 2 allele luôn p + q = 1; nếu tính ra p + q = 1.1 hay 0.9 → đã tính sai ở bước nào đó.
...
1. Tổng allele = 1000. Số A = 2×180 + 240 = 360 + 240 = 600. Số a = 2×80 + 240 = 160 + 240 = 400. **p = 600/1000 = 0.6**; **q = 0.4**.
2. D = 180/500 = 0.36, H = 240/500 = 0.48, R = 80/500 = 0.16. p = 0.36 + 0.48/2 = **0.6** ✓. Hai cách phải luôn ra cùng kết quả, vì chúng chỉ là 2 cách viết của cùng một phép đếm.
</details>
...
- **p²** = tần số AA (đồng hợp trội).
- **2pq** = tần số Aa (dị hợp). Hệ số 2 vì có 2 cách bốc cặp dị hợp (A trước rồi a, hoặc a trước rồi A).
- **q²** = tần số aa (đồng hợp lặn).
- Phương trình này chính là khai triển \`(p + q)² = 1\`.
...
Bắt đầu từ tần số kiểu hình lặn \`f(aa) = q²\` quan sát được. Các bước:

\`\`\`
q = √(q²)
p = 1 − q
2pq = 2 × p × q       (tần số carrier dị hợp)
p² = (1 − q)²         (tần số đồng hợp trội, không bệnh, không carrier)
\`\`\`
...
(Công thức rút ra từ: cá thể aa biến mất → chỉ tính p² AA và 2pq Aa khi tạo thế hệ kế.)
...
A: Có. Cân bằng **đột biến–chọn lọc (mutation–selection balance)** giữ q ở mức rất thấp nhưng không 0: q_eq ≈ √(μ/s), với μ = tốc độ đột biến (~10⁻⁶ tới 10⁻⁸/gen/thế hệ). Ví dụ μ = 10⁻⁶, s = 1: q_eq ≈ 0.001 — bệnh lặn lethal giữ tần số ~1/10⁶ trong dân số do đột biến mới phát sinh.
...
- Chọn lọc chống aa với s = 1: q_(n+1) = q_n / (1 + q_n). Giảm nhanh khi q lớn, rất chậm khi q nhỏ.
- Di-nhập gen có thể đổi p, q ngay trong 1 thế hệ.
- Đột biến + chọn lọc → cân bằng q_eq ≈ √(μ/s), giải thích vì sao bệnh lặn lethal vẫn tồn tại.
...
**Bài 5**: Áp dụng q_(n+1) = q_n / (1 + q_n).
- Thế hệ 0: q = 0.5.
...
- Quy luật chính xác: q_n = 1 / (n + 2) khi q_0 = 0.5. Kiểm tra: n=3 → 1/5 = 0.20 ✓; n=5 → 1/7 ≈ 0.1429 ✓.

**Bài 6**:
- Giải phương trình 2pq = 2q(1 − q) = 0.18 → q(1 − q) = 0.09 → q − q² = 0.09 → q² − q + 0.09 = 0.
- Áp dụng công thức nghiệm bậc 2: q = (1 ± √(1 − 0.36))/2 = (1 ± √0.64)/2 = (1 ± 0.8)/2.
- → q₁ = 0.9 hoặc q₂ = 0.1.
...
2. **Tần số allele** p, q với p + q = 1 (gen 2 allele); cách tính: p = D + H/2.
3. **Định luật HW**: \`p² + 2pq + q² = 1\` — khai triển \`(p + q)²\`; đạt được sau 1 thế hệ giao phối ngẫu nhiên.
...
5. **Suy ngược từ q²**: q = √q² → p → 2pq. Bệnh càng hiếm thì carrier càng đông tương đối (tỉ lệ ~ 2/q).
6. **Kiểm tra HW**: so sánh số kiểu gen quan sát với p²N, 2pq·N, q²N. Lệch → có cơ chế tiến hóa.
7. **Phá HW = tiến hóa**: chọn lọc chống aa (s=1) cho q_(n+1) = q_n/(1+q_n) — giảm chậm dần vì allele a "trốn" trong carrier.
