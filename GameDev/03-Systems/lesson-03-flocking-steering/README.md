# Lesson 13 — Flocking & Steering (đàn & hành vi lái — boids)
...
- Biết **tổng hợp có trọng số**: $F = w_1 \cdot sep + w_2 \cdot align + w_3 \cdot cohesion$, và đổi trọng số → đổi hành vi (bầy chặt / tản / cuộn).
...
> Ký hiệu vector trong bài: in **đậm** (vd **v**) hoặc dùng cặp toạ độ `(x, y)`. $|v|$ = độ dài; $\hat{v}$ = vector đơn vị (chuẩn hoá, độ dài 1). Mọi phép cộng/trừ là theo từng thành phần: $(a,b) + (c,d) = (a+c, b+d)$.
...
- **(b) Vì sao cần.** Nếu ta gán thẳng $v = v_{desired}$ thì boid **quay đầu tức thời** — trông như giật/teleport, không tự nhiên. Trừ ra rồi cộng dần (qua gia tốc) cho ra **đường cong mượt** — đó là điều khiến boids trông "sống".
- **(c) Ví dụ trực giác bằng số.** Boid đang chạy $v_{current} = (3, 0)$ (sang phải). Muốn $v_{desired} = (0, 3)$ (lên trên). Thì $steering = (0,3) - (3,0) = (-3, 3)$. Lực này kéo boid vừa giảm tốc sang phải vừa tăng tốc lên trên → frame sau `v` cong dần lên.
...
| # | pos | target | v hiện tại | $target-pos$ | $v_{desired}$ (chuẩn hoá $\times$ 5) | $desired - v$ | `steering` (kẹp ≤ 1) |
|---|-----|--------|-----------|-------------|------------------------------|---------------|----------------------|
...
Đọc kỹ ví dụ #2: $desired - v = (0,5) - (5,0) = (-5, 5)$, độ dài $\sqrt{25+25} \approx 7.07 > 1$ → kẹp: nhân $1/7.07$ → $(-0.707, 0.707)$. Ví dụ #4: vận tốc đã đúng hướng & đủ nhanh → steering = 0, không cần lái.
...
| # | pos | "kẻ thù" | v | $pos-enemy$ | $v_{desired}$ | `steering` (kẹp) |
|---|-----|---------|---|-------------|-------------|------------------|
...
1. Boid $v=(4,0)$, muốn $v_{desired}=(4,0)$ (y hệt). $steering = ?$ Có cần bẻ lái không?
...
(chia thêm cho khoảng cách lần nữa nếu muốn lực $\propto 1/d^2$ — ở đây dùng $1/d$ cho dễ tính tay).
...
Chuẩn hoá về `maxSpeed=4`: $(0,-1)$ có độ dài 1 → $v_{desired} = (0, -4)$.
$steering = desired - v_A = (0,-4) - (2,0) = (-2, -4)$, độ dài $\sqrt{4+16}=\sqrt{20} \approx 4.47 > 1$
→ **kẹp maxForce=1**: nhân $1/4.47$ → **sep $\approx (-0.447, -0.894)$**.
...
$\bar{v} = (v_B + v_C + v_D)/3 = ((1,1)+(0,2)+(3,0))/3 = (4, 3)/3 = (1.333, 1.0)$.

$steering = \bar{v} - v_A = (1.333, 1.0) - (2, 0) = (-0.667, 1.0)$, độ dài $\sqrt{0.444+1.0}=\sqrt{1.444} \approx 1.20 > 1$
→ **kẹp**: nhân $1/1.20$ → **align $\approx (-0.555, 0.832)$**.
...
Hướng từ A tới tâm: $center - pos_A = (-0.667, 1.0)$, độ dài $\sqrt{0.444+1.0} \approx 1.20$.
Chuẩn hoá $\times$ `maxSpeed=4`: $(-0.667,1.0)/1.20 \times 4 = (-0.555, 0.832) \times 4 = (-2.22, 3.33)$ = $v_{desired}$.
$steering = v_{desired} - v_A = (-2.22, 3.33) - (2,0) = (-4.22, 3.33)$, độ dài $\sqrt{17.8+11.1}=\sqrt{28.9} \approx 5.38 > 1$
→ **kẹp**: nhân $1/5.38$ → **cohesion $\approx (-0.785, 0.619)$**.
...
$F = (2 \cdot -0.447 -0.555 -0.785, \ 2 \cdot -0.894 +0.832 +0.619) = (-0.894-1.34, -1.788+1.451) = (-2.234, -0.337)$.
...
$F = (-0.447 -0.555 +3 \cdot -0.785, \ -0.894 +0.832 +3 \cdot 0.619) = (-0.447-0.555-2.355, -0.894+0.832+1.857) = (-3.357, 1.795)$.
...
**⚠ Lỗi thường gặp — cohesion mạnh → tụ một điểm.** Đặt `w_cohesion` quá lớn so với `w_separation` → mọi boid bị hút về tâm mà không gì đẩy ra → **cả đàn co thành một chấm rung lập bập**. Quy tắc ngón tay cái: giữ $w_{separation} \gtrsim w_{cohesion}$. Trong nhiều cài đặt phổ biến: `sep=1.5, align=1.0, coh=1.0`.
...
- Giữ $w_{separation} \gtrsim w_{cohesion}$ để tránh tụ một điểm.
...
Kiểm tra góc nhìn dùng **tích vô hướng (dot product, L02)**: gọi $toOther = pos_j - pos_i$ (đã chuẩn hoá), $heading = v_i$ (đã chuẩn hoá). Nếu $dot(heading, toOther) \ge \cos(viewAngle/2)$ thì j nằm trong nón nhìn.
...
**Cách ngây thơ.** Để tìm lân cận của *mỗi* boid, ta duyệt *mọi* boid khác đo khoảng cách. Với `n` boid: mỗi boid xét $n-1$ con → tổng $n \cdot (n-1)/2 \approx n^2/2$ cặp. Đây là **O(n²)**.
...
Ví dụ số: boid `pos=(0,0)`, `v=(5,0)` (đi sang phải), feeler dài 10 → đầu feeler ở `(10,0)`. Có hình tròn cản tâm `(8,1)` bán kính 2. Feeler đi qua gần tâm → lực né hướng **vuông góc ra xa** mặt cản, vd $\approx (0, -1) \cdot maxForce$ (đẩy xuống để vòng dưới vật cản).
...
- **Pursue (đuổi)** = Seek nhưng nhắm tới **vị trí tương lai** của mục tiêu, không phải vị trí hiện tại. Dự đoán: $future = pos_{target} + v_{target} \cdot T$ với $T \propto$ khoảng cách. Như hậu vệ đoán đường chuyền.
- **Evade (trốn)** = Flee khỏi vị trí tương lai của kẻ săn.

Ví dụ số (Pursue): mục tiêu `pos=(10,0)`, `v=(0,2)`, chọn `T=3` → $future=(10,0)+(0,2) \cdot 3=(10,6)$. Boid Seek tới `(10,6)` thay vì `(10,0)` → chặn đầu thay vì đuổi đuôi.
...
- **Pursue/Evade**: Seek/Flee tới vị trí **dự đoán** của mục tiêu ($pos + v \cdot T$).
...
**Bài 1.** Cho `maxSpeed = 6`, `maxForce = 2`. Boid `pos=(0,0)`, `v=(2,2)`, đích `target=(0,10)`. Tính lực **Seek** (gồm: $v_{desired}$, $desired - v$, lực sau khi kẹp `maxForce`).
...
**Bài 5.** Cùng A, B, C ở Bài 4 với `v_B=(2,0)`, `v_C=(0,4)`. Tính **Alignment** $\bar{v}$ (velocity trung bình lân cận) và **Cohesion center** (tâm khối lân cận).
...
**Bài 8.** Với `n` boid, cách ngây thơ là $n^2/2$ cặp. Một boid xét trung bình `k=8` lân cận khi dùng grid. Tính số phép đo cho `n=2000` ở (a) cách ngây thơ; (b) cách grid. Tỉ lệ tăng tốc?
...
- $target - pos = (0,10)$, độ dài $10$. Chuẩn hoá: $(0,1)$. Nhân maxSpeed: $v_{desired} = (0,6)$.
- $desired - v = (0,6) - (2,2) = (-2, 4)$, độ dài $\sqrt{4+16}=\sqrt{20} \approx 4.47$.
- $4.47 > maxForce=2$ → kẹp: nhân $2/4.47 \approx 0.447$ → **steering $\approx (-0.894, 1.789)$**.

Cách tiếp cận: luôn theo 3 bước desired → trừ v → kẹp. Độ phức tạp O(1).

### Bài 2 — Flee

Flee đảo dấu hướng: $pos - enemy = (0,0)-(0,10) = (0,-10)$, chuẩn hoá $(0,-1)$, $\times 6$ → $v_{desired}=(0,-6)$.
$desired - v = (0,-6)-(2,2) = (-2,-8)$, độ dài $\sqrt{4+64}=\sqrt{68} \approx 8.25$ → kẹp $2/8.25 \approx 0.2424$ → **steering $\approx (-0.485, -1.94)$**.
...
`maxSpeed=8, r=5`. $speed = maxSpeed$ nếu $d \ge r$, ngược lại $maxSpeed \cdot d/r$.

- $d=12 \ge 5$ → speed = 8 (full).
- $d=5 = 5$ → $8 \cdot (5/5) = 8$ (mép vùng giảm tốc).
- $d=2.5 < 5$ → $8 \cdot (2.5/5) = 4$.
- $d=0$ → $8 \cdot (0/5) = 0$ (dừng).
...
- **Alignment**: $\bar{v} = (v_B + v_C)/2 = ((2,0)+(0,4))/2 = (2,4)/2 = (1, 2)$.
- **Cohesion center**: $(pos_B + pos_C)/2 = ((1,0)+(0,2))/2 = (1,2)/2 = (0.5, 1)$.

(Đây mới là $\bar{v}$ và `center`; muốn ra *lực* thì Alignment lấy $\bar{v} - v_A$ rồi kẹp, Cohesion thì Seek tới `center`.)
...
(b) `(2,1,0.5)`: $F = (2 \cdot -1 + 0 + 0.5 \cdot 1, \ 2 \cdot 0 + 1 + 0.5 \cdot 1) = (-2+0.5, 1+0.5) = (-1.5, 1.5)$.
