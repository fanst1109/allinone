# Lesson 10 — Non-comparison Sorts (Counting, Radix, Bucket)
...
- Chứng minh được **giới hạn dưới `Ω(n log n)`** cho mọi comparison sort bằng mô hình *decision tree* (cây quyết định).
...
- Big-O cơ bản (Tier 0): `O(n)`, `O(n log n)`, `O(k)`, ký hiệu `Ω` (giới hạn dưới).
...
> 💡 **Trực giác / Hình dung.** Hãy tưởng tượng bạn chơi trò "đoán hoán vị": có một mảng `n` phần tử bị xáo trộn theo một trong `n!` cách. Mỗi lần bạn được hỏi "phần tử ở vị trí `i` có lớn hơn phần tử ở vị trí `j` không?" — câu trả lời chỉ **có/không** (1 bit thông tin). Để xác định chính xác **một** trong `n!` hoán vị, bạn cần đủ số câu hỏi để phân biệt `n!` khả năng. Mỗi câu hỏi nhị phân chia đôi không gian khả năng → cần ít nhất `log₂(n!)` câu hỏi. Đó chính là số phép so sánh tối thiểu.
...
*Chứng minh bổ đề:* Quy nạp theo `h`. `h = 0`: cây chỉ có 1 lá `= 2^0`. ✓. Giả sử cây cao `h-1` có ≤ `2^(h-1)` lá. Cây cao `h` = gốc + 2 cây con cao ≤ `h-1`, mỗi cây con ≤ `2^(h-1)` lá → tổng ≤ `2 · 2^(h-1) = 2^h` lá. ✓
...
Lấy `log₂` hai vế (log đơn điệu tăng nên giữ chiều bất đẳng thức):

```
h  ≥  log₂(n!)
```

**Ước lượng `log₂(n!)`.** Dùng cận dưới đơn giản (không cần Stirling): trong tích `n! = 1·2·3···n`, **nửa lớn** gồm `n/2` thừa số, mỗi thừa số ≥ `n/2`:
...
**Kết luận.** Mọi comparison sort thực hiện **ít nhất `Ω(n log n)`** phép so sánh trong worst case. Merge sort / heap sort đạt `O(n log n)` → **tối ưu** trong lớp comparison sort. Không thuật toán so sánh nào phá được rào này.

> 🔢 **Walk-through bằng số.** Với `n = 5`: `5! = 120`. `log₂(120) ≈ 6.9` → cần **≥ 7 phép so sánh** để sort 5 phần tử bất kỳ. Kiểm chứng: thuật toán sort 5 phần tử tốt nhất biết được cần đúng 7 so sánh worst-case → khớp `⌈log₂ 120⌉ = 7`. Với `n = 10`: `10! = 3 628 800`, `log₂ ≈ 21.8` → ≥ 22 so sánh. Với `n = 100`: `log₂(100!) ≈ 524` so sánh tối thiểu.

> ⚠ **Lỗi thường gặp.** "Lower bound này nói sort luôn cần `n log n`" — **SAI**. Nó chỉ áp dụng cho **comparison sort** (sort bằng so sánh). Counting/radix/bucket KHÔNG dựa trên so sánh nên KHÔNG bị rào này chặn → có thể `O(n)`. Rào `Ω(n log n)` là rào của **một mô hình tính toán cụ thể**, không phải của "bài toán sort" nói chung.
...
> 2. `8! = 40320`, `log₂(40320) ≈ 15.3` → tối thiểu **16** so sánh (`⌈15.3⌉`).
> </details>

> 📝 **Tóm tắt mục 1.**
> - Mọi comparison sort = một decision tree với ≥ `n!` lá.
> - Cây cao `h` có ≤ `2^h` lá → `h ≥ log₂(n!) = Ω(n log n)`.
> - Rào `Ω(n log n)` **chỉ** áp dụng cho sort bằng so sánh. Đó là lý do merge/heap sort là tối ưu trong lớp đó.
...
Rào `Ω(n log n)` chỉ chặn việc "quyết định thứ tự bằng so sánh". Nếu ta thay đổi mô hình:
...
- Độ phức tạp **`O(d · (n + b))`**. Nếu `d` và `b` là hằng → `O(n)`.
...
1. Tạo `m` bucket (rổ).
2. Với mỗi phần tử `x ∈ [0,1)`, ném vào bucket `⌊m·x⌋`.
3. Sort từng bucket (insertion sort).
4. Nối các bucket theo thứ tự 0,1,...,m−1.

- **Average `O(n)`** nếu phân phối **đều** (mỗi bucket ~`n/m` phần tử, sort mỗi rổ `O((n/m)²)`, tổng `O(n + n²/m)` → chọn `m ≈ n` cho `O(n)`).
- **Worst `O(n²)`**: nếu tất cả rơi vào **một** bucket (phân phối lệch) → insertion sort `O(n²)`.
...
> ⚠ **Lỗi thường gặp.** Bucket sort nhanh **chỉ khi phân phối đều**. Với dữ liệu lệch (vd 1000 số đều quanh 0.5), gần như mọi phần tử rơi vào 1–2 bucket → quay về `O(n²)`. Phải chọn hàm phân bucket khớp với phân phối thực tế của dữ liệu.

> 🔁 **Dừng lại tự kiểm tra.** Với `m = 5`, phần tử `0.62` rơi vào bucket nào?
> <details><summary>Đáp án</summary>
>
> `⌊5 · 0.62⌋ = ⌊3.1⌋ = 3` → bucket 3.
> </details>

> 📝 **Tóm tắt mục 5.**
> - Bucket sort = chia rổ theo range → sort từng rổ → nối.
> - Average `O(n)` khi phân phối **đều**; worst `O(n²)` khi lệch.
...
- `k` lớn (vd tới `2³²`) nhưng giá trị có ít digit → **radix sort** (counting sort sẽ tốn `O(2³²)` bộ nhớ → loại).
- Float `[0,1)` phân bố đều, hoặc dữ liệu phân bố đều biết trước → **bucket sort**.

> 🔢 **Walk-through bằng số (chọn counting hay radix).** Sort 1 triệu số nguyên 32-bit (`n = 10⁶`, range tới `~4·10⁹`).
> - Counting sort: cần mảng `count` cỡ `4·10⁹` → **~16 GB** (4 byte/ô). Không khả thi. ❌
> - Radix base 256: `d = 4` byte → 4 pass × `(10⁶ + 256)` ≈ `4·10⁶` thao tác. Bộ nhớ phụ chỉ `O(n + 256)`. ✓ **Chọn radix.**
...
- **Range giá trị khổng lồ** → counting sort tốn `O(k)` bộ nhớ. Sort 100 số nhưng giá trị tới `10⁹` → mảng count `10⁹` ô = vô lý. (Dùng radix hoặc quicksort thay thế.)
- **Phân phối lệch nặng** → bucket sort suy biến về `O(n²)`. Dữ liệu dồn cục vào vài bucket.
...
| Quicksort | `O(n log n)` | `O(n²)` | `O(log n)` | Không | Có | — |
...
| **Bucket sort** | `O(n)` | `O(n²)` | `O(n)` | Có¹ | **Không** | phân phối đều |

¹ Ổn định nếu sort con trong bucket ổn định và phân bucket giữ thứ tự.

> ⚠ **Lỗi thường gặp — "O(n) luôn nhanh hơn n log n".** Counting/radix là `O(n)` **có điều kiện**. Hằng số ẩn và `k`/`d` quan trọng. Với `n = 1000`, `k = 10⁶`: counting sort `O(n + k) ≈ 10⁶` thao tác, còn quicksort `O(n log n) ≈ 10⁴` → **quicksort nhanh hơn 100 lần**. `O(n)` chỉ thắng khi `k` (hoặc `d`) đủ nhỏ.

> 📝 **Tóm tắt mục 8.**
> - Non-comparison đạt `O(n)` nhưng kèm điều kiện (`k` nhỏ / `d` nhỏ / phân phối đều).
> - Comparison sort tổng quát, không điều kiện, nhưng chặn ở `Ω(n log n)`.
...
|---|---|---|
| Counting với range khổng lồ | Cấp phát mảng `count` tỉ phần tử → OOM/crash | Kiểm tra `k`; nếu `k ≫ n` → dùng radix hoặc quicksort |
...
| Bucket với phân phối **lệch** | Mọi phần tử dồn 1 rổ → `O(n²)` | Đo phân phối; chọn hàm phân bucket khớp dữ liệu |
...
3. **Chứng minh lower bound.** Giải thích vì sao mọi comparison sort cần `Ω(n log n)` so sánh. Tính số so sánh tối thiểu cho `n = 6` và `n = 12`.
...
6. **Khi nào counting > quicksort?** Cho `n = 10⁶` phần tử. Với những giá trị `k` nào counting sort nhanh hơn quicksort? Tính cụ thể (giả sử counting ≈ `n + k`, quicksort ≈ `n log₂ n`).
...
**Số liệu cụ thể:**
- `n = 6`: `6! = 720`, `log₂(720) ≈ 9.49` → tối thiểu **10** so sánh.
- `n = 12`: `12! = 479 001 600`, `log₂ ≈ 28.8` → tối thiểu **29** so sánh.
...
**Cách tiếp cận:** so chi phí xấp xỉ. Counting ≈ `n + k`. Quicksort ≈ `n · log₂ n`.

- `n = 10⁶`. `log₂(10⁶) ≈ 19.93 ≈ 20`.
- Quicksort ≈ `10⁶ × 20 = 2 × 10⁷` thao tác.
- Counting ≈ `10⁶ + k`.
...
**Kết luận:** khi range `k < ~1.9 × 10⁷` (gần 19 triệu), counting sort (theo mô hình thao tác) nhanh hơn quicksort. Trong thực tế hằng số của counting nhỏ hơn (chỉ tăng đếm, không so sánh + swap) nên ngưỡng thực còn rộng hơn. Nhưng **bộ nhớ** `O(k)` mới là ràng buộc chính: `k = 19 × 10⁶` ô × 8 byte ≈ 152 MB — chấp nhận được; `k = 10⁹` thì ≈ 8 GB — loại. Vậy với `k` cỡ vài triệu trở xuống, counting thắng cả tốc độ lẫn bộ nhớ.

---

## Tổng kết

- Comparison sort bị chặn ở `Ω(n log n)` (chứng minh decision tree). Merge/heap sort tối ưu trong lớp đó.
- Non-comparison sort phá rào bằng cách **dùng giá trị làm index/digit**:
  ...
  - **Bucket sort** `O(n)` average — phân phối đều, worst `O(n²)`.
