...
// Source: Programming/lesson-71-mini-project-microservices/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 71 — Mini-project: Microservices với Saga (tổng kết Tier 6)
...
### 9.1 Bảng trạng thái đầy đủ của hệ thống qua từng bước

Để "đọc một lần là hiểu", đây là bảng theo dõi cả 3 trạng thái (order, kho, payment) qua từng kịch bản. Kho ban đầu \`BOOK-GO = 5\`.

**Kịch bản thành công (ord-1, qty=2, amount=200):**

| Sau bước | order | kho | charge |
|----------|-------|-----|--------|
| (đầu) | — | 5 | — |
| CreateOrder | PENDING | 5 | — |
| ReserveInventory | PENDING | 3 | — |
| ChargePayment | PENDING | 3 | 200 |
| ConfirmOrder | **CONFIRMED** | 3 | 200 |

**Kịch bản payment từ chối (ord-3, qty=1, amount=0), kho đang 3:**

| Sau bước | order | kho | charge |
|----------|-------|-----|--------|
| (đầu) | — | 3 | — |
| CreateOrder | PENDING | 3 | — |
| ReserveInventory | PENDING | 2 | — |
| ChargePayment | (fail) | 2 | — |
| Release (bồi hoàn) | PENDING | **3** | — |
| Cancel (bồi hoàn) | **CANCELLED** | 3 | — |

Nhìn cột "kho": nó tụt xuống 2 rồi **về lại 3** — đúng như "chưa từng đặt". Đó là tính chất ta muốn của compensation.

> 🔁 **Dừng lại tự kiểm tra.** Một order có thể từ CANCELLED quay lại CONFIRMED không?
>
> <details><summary>Đáp án</summary>
> Không, trong thiết kế này trạng thái là một chiều: PENDING → (CONFIRMED hoặc CANCELLED) và dừng. Đây là một **state machine** đơn giản. Nếu muốn cho retry đặt lại, ta tạo order MỚI (orderID mới) chứ không "phục sinh" order cũ — tránh mơ hồ về lịch sử.
> </details>

### 9.2 Saga như một state machine
...
> 📝 **Tóm tắt mục 9.** Compensation đưa hệ về trạng thái nhất quán: kho tụt rồi trả lại, order về CANCELLED, không charge mồ côi. Có thể nhìn saga như state machine + event log (cầu nối tới event sourcing L67).
