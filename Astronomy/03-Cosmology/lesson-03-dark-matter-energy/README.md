# Lesson 03 — Vật chất tối & Năng lượng tối (Dark Matter & Dark Energy)
...
- Lực hấp dẫn & chuyển động tròn (định luật Newton, định luật Kepler): [`../../../Physics/`](../../../Physics/) — bài này dùng `v = √(G·M/r)`.
...
- **(c) Ví dụ số** — Dự đoán Kepler (chỉ tính khối lượng sáng) cho `v ∝ 1/√r`: ở `r = 4` lần xa hơn, `v` phải giảm `√4 = 2` lần. Thực đo: `v` gần như **không đổi** từ `r = 2 kpc` tới `r = 20 kpc`.
...
```
v ∝ 1/√r
r tăng 4 lần ⇒ v giảm √4 = 2 lần.
Vd v(5 kpc) = 200 km/s ⇒ dự đoán v(20 kpc) = 200/2 = 100 km/s.
```

Nhưng quan sát: `v(20 kpc) ≈ 200 km/s` (phẳng!). Để `v` không đổi khi `r` tăng, cần `M(r) ∝ r` — tức **khối lượng vẫn tiếp tục tăng** ra tới tận rìa, dù **không thấy** thêm sao nào ⇒ tồn tại **vật chất tối** trải rộng thành quầng (halo).
...
> 🔁 **Dừng lại tự kiểm tra.** Theo Kepler thuần, một sao ở `r = 9 kpc` so với sao ở `r = 1 kpc` (cùng khối lượng trung tâm) quay nhanh hay chậm hơn, bao nhiêu lần?
> <details><summary>Đáp án</summary>Chậm hơn. `v ∝ 1/√r`, `r` tăng 9 lần ⇒ `v` giảm `√9 = 3` lần. Nhưng thực tế đường cong phẳng nên nó quay gần bằng — đó là dấu hiệu vật chất tối.</details>
...
- **(c) Ví dụ số** — Siêu tân tinh Ia ở `z ≈ 0.5` xuất hiện **mờ hơn ~25%** so với dự đoán trong một vũ trụ chỉ hãm bởi hấp dẫn ⇒ chúng ở **xa hơn** ⇒ giãn nở đã tăng tốc.
...
> 📝 **Tóm tắt toàn bài.**
> - **Đường cong quay phẳng**: sao rìa thiên hà quay nhanh ngang sao trong ⇒ cần khối lượng vô hình ⇒ **vật chất tối** (`v ∝ 1/√r` của Kepler bị vi phạm).
...
1. **Đường cong quay.** Một thiên hà có khối lượng nhìn thấy hầu hết nằm trong `r = 5 kpc`. Theo Kepler, tốc độ tại `r = 20 kpc` sẽ bằng bao nhiêu phần tốc độ tại `r = 5 kpc`? Thực đo cho thấy hai tốc độ gần bằng nhau — kết luận gì?

2. **Cần thêm bao nhiêu khối lượng.** Tại `r = 20 kpc`, tốc độ đo được là `220 km/s` nhưng khối lượng sáng chỉ cho `100 km/s`. Vì `v² ∝ M`, khối lượng thực gấp mấy lần khối lượng sáng?
...
4. **Nến chuẩn.** Hai siêu tân tinh Ia có cùng độ sáng thật. Cái A trông sáng gấp 4 lần cái B. So sánh khoảng cách của chúng. (Gợi ý: độ sáng biểu kiến `∝ 1/d²`.)
...
Theo Kepler `v ∝ 1/√r` (khi khối lượng dồn ở trong):
```
v(20)/v(5) = √(5/20) = √(1/4) = 1/2
```
...
`v² ∝ M(r)` (vì `v = √(GM/r)` ⇒ `M = v²r/G`, cùng r):
```
M_thực / M_sáng = (v_thực / v_sáng)² = (220/100)² = 2.2² = 4.84
```
...
Cùng độ sáng thật, độ sáng biểu kiến `∝ 1/d²`. A sáng gấp 4 lần B:
```
b_A / b_B = (d_B / d_A)² = 4 ⇒ d_B/d_A = 2 ⇒ d_A = d_B/2
```
...
- **Visualization tương tác**: [visualization.html](./visualization.html) — gồm:
  - **Đường cong quay**: vẽ đồng thời đường Kepler dự đoán (`v ∝ 1/√r`, giảm dần) và đường phẳng quan sát; kéo "khối lượng vật chất tối" để thấy đường phẳng xuất hiện khi thêm quầng tối.
