...
// Source: Algorithms/lesson-26-dp-grid-2d/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 26 — Quy hoạch động 2 chiều / Trên lưới (2D DP / Grid DP)
...
- [Lesson 01 — Big-O](../lesson-01-bigo-asymptotic/): để đọc độ phức tạp \`O(m·n)\`.
...
> - Độ phức tạp điển hình \`O(m·n)\` thời gian, \`O(m·n)\` bộ nhớ (có thể giảm còn \`O(min(m,n))\`).
...
> 📝 **Tóm tắt mục 2.** \`dp[i][j] = dp[i-1][j] + dp[i][j-1]\`, biên = 1. \`O(m·n)\` time, \`O(m·n)\` space (giảm được còn \`O(n)\` — xem mục 10). Có công thức đóng \`C(m+n-2, m-1)\` nhưng DP tổng quát hoá tốt hơn (vật cản, cost...).
...
> 📝 **Tóm tắt mục 3.** \`dp[i][j] = grid[i][j] + min(trên, trái)\`. \`O(m·n)\`. Thay \`min\` bằng \`max\` để tìm đường tổng **lớn nhất**; thay phép cộng bằng đếm để quay về unique paths.
...
> 📝 **Tóm tắt mục 5.** \`dp[i][j]\` = LCS hai prefix. Khớp ký tự ⇒ \`dp[i-1][j-1]+1\`; khác ⇒ \`max\` của bỏ-A / bỏ-B. \`O(m·n)\` time & space. Truy vết bằng cách đi ngược, ưu tiên đường chéo khi khớp.
...
> 📝 **Tóm tắt mục 6.** \`dp[i][j]\` = số thao tác min. Khớp ⇒ chéo; khác ⇒ \`1 + min(chéo, trên, trái)\`. Base case = chỉ số (chèn/xoá hết). \`O(m·n)\`.
...
Cả hai đều \`O(items × capacity)\` thời gian, đều giảm được về \`dp[w]\` 1 chiều (xem [Lesson 25](../lesson-25-knapsack-family/) và mục 10 dưới đây).
...
> 💡 **Trực giác.** Nhìn lại các transition: \`dp[i][j]\` chỉ phụ thuộc **hàng \`i-1\`** (và các ô đã tính trên hàng \`i\`). Vậy không cần giữ cả \`m\` hàng — chỉ cần **2 hàng** (\`prev\`, \`curr\`), thậm chí **1 hàng + 1 biến** nếu khéo. Bộ nhớ tụt từ \`O(m·n)\` xuống \`O(n)\`.
...
> ⚠ **Lỗi thường gặp (và rất nặng).** **Tối ưu xuống 1D làm hỏng truy vết.** Khi chỉ giữ vài hàng, bạn **mất** thông tin để backtrack đường đi / chuỗi kết quả. Nếu cần **truy vết** (in ra LCS, dãy thao tác edit), phải **giữ bảng đầy đủ** \`O(m·n)\`, hoặc dùng Hirschberg (chia để trị, \`O(min(m,n))\` bộ nhớ + truy vết — nâng cao). Quy tắc: cần độ dài/đáp số ⇒ 1D OK; cần đường đi ⇒ giữ 2D.

> 📝 **Tóm tắt mục 10.** Vì \`dp[i]\` chỉ phụ thuộc \`dp[i-1]\`, giảm space \`O(m·n) → O(n)\` bằng 2 hàng, hoặc 1 hàng + biến \`diag\`. Nhưng mất khả năng truy vết — đánh đổi rõ ràng.
...
**Độ phức tạp.** Time \`O(m·n)\`; space \`O(m·n)\` (giảm còn \`O(n)\` bằng 1 hàng).
...
**Độ phức tạp.** Time \`O(m·n)\`; space \`O(m·n)\` → \`O(n)\` (mục 10).
...
**Độ phức tạp.** Time \`O(m·n)\`; space \`O(m·n)\` (bắt buộc giữ bảng để truy vết). Truy vết thêm \`O(m+n)\`.
...
**Độ phức tạp.** Time \`O(m·n)\`; space \`O(m·n)\` → \`O(n)\` (mục 10.2, nếu không cần truy vết thao tác).
...
**Độ phức tạp.** Time \`O(n²)\` (LCS hai chuỗi dài \`n\`); space \`O(n)\` nếu chỉ cần độ dài.
...
**Độ phức tạp.** Time \`O(m·n)\`; space \`O(m·n)\` → \`O(n)\` (2 hàng).
