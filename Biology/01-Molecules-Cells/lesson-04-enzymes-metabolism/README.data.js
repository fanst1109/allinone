...
// Source: Biology/01-Molecules-Cells/lesson-04-enzymes-metabolism/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 04 — Enzyme & chuyển hóa
...
- Active site = "túi" trên enzyme khớp cơ chất; tạo phức ES rồi cho sản phẩm, enzyme tái sử dụng (\`E + S ⇌ ES → E + P\`).
...
Dùng mô hình Michaelis–Menten đơn giản: \`v = Vmax · [S] / (Km + [S])\`. Giả sử \`Vmax = 100\` đơn vị/giây, \`Km = 5\` mM (Km = nồng độ cơ chất cho tốc độ bằng nửa Vmax).
...
\`\`\`
ATP  +  H₂O   →   ADP  +  Pi   +  năng lượng (~7.3 kcal/mol)   [thủy phân — xả]
ADP  +  Pi    +  năng lượng   →   ATP  +  H₂O                  [tổng hợp — nạp]
\`\`\`
