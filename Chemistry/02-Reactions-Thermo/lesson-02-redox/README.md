# Lesson 02 (Tier 2) — Phản ứng Redox (Oxidation-Reduction)

## Mục tiêu học tập

Sau bài này bạn sẽ:

- Hiểu khái niệm **oxy hóa** (mất electron) và **khử** (nhận electron), và vì sao 2 quá trình **luôn đi cặp**.
- Tính **số oxy hóa (oxidation state)** của mọi nguyên tố trong hợp chất bằng 7 quy tắc.
- Nhận diện **chất oxy hóa** (tác nhân oxy hóa = chất nhận e = bị khử) và **chất khử** (cho e = bị oxy hóa).
- Cân bằng phản ứng redox bằng **phương pháp nửa phản ứng (half-reaction)** trong môi trường acid và base.
- Liên hệ redox với các ứng dụng: hô hấp, pin, ăn mòn kim loại, công nghiệp.

## Kiến thức tiền đề

- [Lesson 06 (T1) — Mol & Phản ứng](../../01-Structure/lesson-06-mole-stoichiometry/) — biết cân bằng phương trình.
- [Lesson 01 (T2) — Acid-Base](../lesson-01-acid-base/) — biết phản ứng trong môi trường acid/base.

---

## 1. Oxy hóa và Khử

### 💡 Trực giác / Hình dung

Ban đầu, "oxy hóa" có nghĩa rất hẹp: **phản ứng với oxy**. Vd: Fe + O₂ → Fe₂O₃ (gỉ sét). "Khử" là ngược lại: lấy oxy ra khỏi hợp chất.

Hiện nay, định nghĩa **tổng quát hóa** dựa trên electron:

- **Oxy hóa (oxidation)**: **MẤT** electron. Số oxy hóa tăng.
- **Khử (reduction)**: **NHẬN** electron. Số oxy hóa giảm.

**Mnemonic OIL RIG**: **O**xidation **I**s **L**oss (electron), **R**eduction **I**s **G**ain (electron).

### 1.1. Cặp đôi luôn đi cùng nhau

Electron không tự nhiên mất đi — nó phải chuyển sang chất khác. Vậy nên:
**Trong mọi phản ứng redox, có đúng 1 chất bị oxy hóa và 1 chất bị khử.**

Ví dụ: `2Na + Cl₂ → 2NaCl`
- Na (0) → Na⁺ (+1): mất 1 e → **Na bị oxy hóa**. Na là **chất khử** (đưa e cho người khác).
- Cl₂ (0) → 2Cl⁻ (−1): mỗi Cl nhận 1 e → **Cl₂ bị khử**. Cl₂ là **chất oxy hóa**.

**Nhớ rõ**: "chất khử" là chất CHO e (chính nó bị oxy hóa). "Chất oxy hóa" là chất NHẬN e (chính nó bị khử). Dễ nhầm.

### 📝 Tóm tắt mục 1

- Oxy hóa = mất e (số ox tăng); khử = nhận e (số ox giảm).
- Luôn đi cặp: 1 chất oxy hóa + 1 chất khử trong mỗi phản ứng.
- Chất khử cho e → bị oxy hóa; chất oxy hóa nhận e → bị khử.

---

## 2. Số oxy hóa (Oxidation State)

### 2.1. Định nghĩa

**Số oxy hóa** của một nguyên tử trong hợp chất = điện tích "giả định" nếu mọi liên kết đều là ion (electron chuyển hẳn cho nguyên tử âm điện hơn).

Đây là **khái niệm hình thức**, không phải điện tích thật. Nhưng cực kỳ hữu ích để theo dõi e trong phản ứng redox.

### 2.2. Bảy quy tắc tính số oxy hóa

**Áp dụng theo thứ tự ưu tiên:**

1. **Đơn chất** (nguyên tố ở dạng tự do): số oxy hóa = 0. Vd O₂, H₂, Fe, Cu — đều = 0.
2. **Ion đơn nguyên tử**: số oxy hóa = điện tích ion. Vd Na⁺ → +1, Cl⁻ → −1, Fe³⁺ → +3.
3. **F** (fluorine): luôn = −1 (vì độ âm điện cao nhất).
4. **H**: thường = +1 (vd H₂O, HCl, CH₄). Ngoại lệ: hydride kim loại (NaH, CaH₂) → H có số ox = −1.
5. **O**: thường = −2 (vd H₂O, CO₂). Ngoại lệ: peroxide (H₂O₂, Na₂O₂) → O có số ox = −1; OF₂ → O có số ox = +2.
6. **Kim loại nhóm I, II**: luôn dương theo nhóm. Nhóm IA: +1; Nhóm IIA: +2; Al: +3.
7. **Tổng số oxy hóa = điện tích tổng** của phân tử / ion. Phân tử trung hòa → tổng = 0. Ion → tổng = điện tích ion.

### 2.3. Bốn ví dụ tính số oxy hóa

**Ví dụ 1 — H₂SO₄ (trung hòa):**
- H = +1 (×2 = +2). O = −2 (×4 = −8).
- Tổng = 0 → S + 2 − 8 = 0 → **S = +6**.

**Ví dụ 2 — KMnO₄ (trung hòa):**
- K = +1 (nhóm IA). O = −2 (×4 = −8).
- Tổng = 0 → 1 + Mn − 8 = 0 → **Mn = +7**.

**Ví dụ 3 — Cr₂O₇²⁻ (ion, điện tích = −2):**
- O = −2 (×7 = −14). 2 Cr = ?
- 2·Cr + (−14) = −2 → 2·Cr = +12 → **Cr = +6** mỗi nguyên tử.

**Ví dụ 4 — NH₄⁺ (ion, điện tích = +1):**
- H = +1 (×4 = +4).
- N + 4 = +1 → **N = −3**.

### 2.4. Phân biệt nhanh phản ứng redox

Để biết một phản ứng có phải redox không, **kiểm tra số oxy hóa của các nguyên tố trước và sau phản ứng**. Nếu có nguyên tố thay đổi số ox → là redox.

**Ví dụ — Kiểm tra**:

a) `Zn + 2HCl → ZnCl₂ + H₂`
- Zn: 0 → +2 (tăng → mất e → oxy hóa)
- H: +1 → 0 (giảm → nhận e → khử)
- → **Là redox.** Zn là chất khử, H⁺ là chất oxy hóa.

b) `HCl + NaOH → NaCl + H₂O`
- H: +1 → +1 (không đổi)
- O: −2 → −2 (không đổi)
- Cl: −1 → −1 (không đổi)
- Na: +1 → +1 (không đổi)
- → **KHÔNG phải redox** (acid-base thuần).

### 📝 Tóm tắt mục 2

- Số oxy hóa = điện tích giả định nếu liên kết đều là ion.
- 7 quy tắc theo thứ tự ưu tiên (F > H > O > nhóm I,II > tổng).
- Số ox thay đổi → là phản ứng redox.

---

## 3. Cân bằng redox — Phương pháp nửa phản ứng

### 3.1. Ý tưởng

Tách phản ứng redox thành 2 nửa:
- **Nửa oxy hóa**: chỉ có chất khử + e (e ở vế phải).
- **Nửa khử**: chỉ có chất oxy hóa + e (e ở vế trái).

Cân bằng riêng từng nửa, rồi cộng lại (nhân hệ số để e khớp nhau).

### 3.2. Quy trình 6 bước (môi trường acid)

1. **Tách** ra 2 nửa phản ứng.
2. **Cân bằng nguyên tố** (trừ O và H) trong từng nửa.
3. **Cân bằng O**: thêm H₂O.
4. **Cân bằng H**: thêm H⁺.
5. **Cân bằng điện tích**: thêm e⁻.
6. **Nhân hệ số** để số e ở 2 nửa bằng nhau, **cộng 2 nửa**.

### 3.3. Walk-through — Cân bằng MnO₄⁻ + Fe²⁺ → Mn²⁺ + Fe³⁺ (acid)

**Bước 1 — Tách 2 nửa:**
- Nửa khử: `MnO₄⁻ → Mn²⁺` (Mn: +7 → +2, nhận 5 e)
- Nửa oxy hóa: `Fe²⁺ → Fe³⁺` (Fe: +2 → +3, mất 1 e)

**Bước 2-5 — Cân bằng nửa khử:**
- Mn: 1 = 1 ✓.
- O: trái 4, phải 0 → thêm 4 H₂O bên phải: `MnO₄⁻ → Mn²⁺ + 4H₂O`.
- H: phải 8, trái 0 → thêm 8 H⁺ bên trái: `MnO₄⁻ + 8H⁺ → Mn²⁺ + 4H₂O`.
- Điện tích: trái = −1 + 8 = +7. Phải = +2 + 0 = +2. Chênh 5 → thêm 5 e bên trái: `MnO₄⁻ + 8H⁺ + 5e⁻ → Mn²⁺ + 4H₂O` ✓.

**Cân bằng nửa oxy hóa:**
- `Fe²⁺ → Fe³⁺ + e⁻` (chỉ thêm 1 e bên phải để cân bằng điện tích).

**Bước 6 — Nhân & cộng:**
- Nửa khử × 1, nửa oxy hóa × 5 (để cùng 5 e):
  - `MnO₄⁻ + 8H⁺ + 5e⁻ → Mn²⁺ + 4H₂O`
  - `5Fe²⁺ → 5Fe³⁺ + 5e⁻`
- Cộng: `MnO₄⁻ + 8H⁺ + 5Fe²⁺ → Mn²⁺ + 5Fe³⁺ + 4H₂O` ✓.

Kiểm tra: Mn 1=1, Fe 5=5, O 4=4, H 8=8. Điện tích trái = −1+8+10 = +17. Phải = +2+15+0 = +17 ✓.

### 3.4. Môi trường base — thêm bước cuối

Sau khi cân bằng như acid, thêm OH⁻ vào cả 2 vế bằng số H⁺ → H⁺ + OH⁻ → H₂O. Sau đó rút gọn H₂O dư.

### 📝 Tóm tắt mục 3

- Tách phản ứng thành 2 nửa, cân bằng riêng.
- Acid: thêm H₂O, H⁺, e theo thứ tự.
- Base: làm như acid rồi chuyển H⁺ thành H₂O bằng OH⁻.

---

## 4. Ứng dụng — Đời sống và Công nghiệp

### 4.1. Hô hấp tế bào

Glucose oxy hóa trong tế bào để tạo năng lượng:
```
C₆H₁₂O₆ + 6O₂ → 6CO₂ + 6H₂O
```
- C: 0 → +4 (oxy hóa). O₂: 0 → −2 (khử).
- Phản ứng tỏa năng lượng → ATP cho cơ thể hoạt động.

### 4.2. Pin (Galvanic cell) — chủ đề kỹ ở Lesson 03

Phản ứng redox tự xảy ra → tạo dòng điện. Vd pin Daniell: Zn (anode, oxy hóa) + Cu²⁺ → Zn²⁺ + Cu (cathode, khử). Điện áp ~ 1.1 V.

### 4.3. Ăn mòn kim loại

Fe + O₂ + H₂O → Fe₂O₃·H₂O (gỉ sét). Fe bị oxy hóa, O₂ bị khử. Chống gỉ: mạ Zn (kẽm hy sinh — Zn bị oxy hóa thay Fe).

### 4.4. Công nghiệp — Sản xuất Cl₂ và NaOH

Điện phân dung dịch NaCl:
- Anode: 2Cl⁻ → Cl₂ + 2e⁻ (oxy hóa).
- Cathode: 2H₂O + 2e⁻ → H₂ + 2OH⁻ (khử).

### 📝 Tóm tắt mục 4

Redox có mặt khắp nơi: hô hấp, pin, ăn mòn, công nghiệp.

---

## 5. Bài tập + Lời giải chi tiết

### Bài tập

**Bài 1**: Tính số oxy hóa của nguyên tố in đậm trong các chất sau:
a) **S** trong H₂SO₃
b) **N** trong HNO₃
c) **Cr** trong K₂Cr₂O₇
d) **Mn** trong KMnO₄
e) **C** trong CO₃²⁻
f) **Cl** trong ClO₄⁻

**Bài 2**: Phản ứng nào sau đây là phản ứng redox?
a) NaOH + HCl → NaCl + H₂O
b) Zn + CuSO₄ → ZnSO₄ + Cu
c) CaCO₃ → CaO + CO₂
d) Cu + 2AgNO₃ → Cu(NO₃)₂ + 2Ag

**Bài 3**: Cân bằng phản ứng (môi trường acid): `Cr₂O₇²⁻ + Fe²⁺ → Cr³⁺ + Fe³⁺`.

**Bài 4**: Trong phản ứng `Cu + 2H₂SO₄(đặc) → CuSO₄ + SO₂ + 2H₂O`, xác định chất oxy hóa, chất khử.

**Bài 5**: Cân bằng phản ứng (môi trường acid): `MnO₄⁻ + C₂O₄²⁻ → Mn²⁺ + CO₂`.

**Bài 6**: Tại sao thanh kẽm đặt vào dung dịch CuSO₄ bị mòn và xuất hiện lớp đồng đỏ trên thanh?

### Lời giải

**Bài 1**:
a) H₂SO₃ trung hòa: 2(+1) + S + 3(−2) = 0 → **S = +4**.
b) HNO₃ trung hòa: (+1) + N + 3(−2) = 0 → **N = +5**.
c) K₂Cr₂O₇ trung hòa: 2(+1) + 2Cr + 7(−2) = 0 → 2Cr = +12 → **Cr = +6**.
d) KMnO₄ trung hòa: +1 + Mn + 4(−2) = 0 → **Mn = +7**.
e) CO₃²⁻ điện tích −2: C + 3(−2) = −2 → **C = +4**.
f) ClO₄⁻ điện tích −1: Cl + 4(−2) = −1 → **Cl = +7**.

**Bài 2**:
- a) Acid-base, không redox (đã phân tích ở §2.4).
- b) **Redox**: Zn (0 → +2, oxy hóa), Cu (+2 → 0, khử).
- c) Không redox: số ox không đổi (Ca +2, C +4, O −2 trước và sau).
- d) **Redox**: Cu (0 → +2), Ag (+1 → 0).

**Bài 3**:
Tách:
- Nửa khử: Cr₂O₇²⁻ → Cr³⁺. Cr: +6 → +3, mỗi Cr nhận 3 e, 2 Cr nhận 6 e.
- Nửa oxy hóa: Fe²⁺ → Fe³⁺ + e⁻.

Cân bằng nửa khử:
- Cr: cần 2 Cr³⁺ vế phải: `Cr₂O₇²⁻ → 2Cr³⁺`.
- O: thêm 7 H₂O bên phải: `Cr₂O₇²⁻ → 2Cr³⁺ + 7H₂O`.
- H: thêm 14 H⁺ bên trái: `Cr₂O₇²⁻ + 14H⁺ → 2Cr³⁺ + 7H₂O`.
- Điện tích: trái = −2 + 14 = +12; phải = 6. Thêm 6 e bên trái: `Cr₂O₇²⁻ + 14H⁺ + 6e⁻ → 2Cr³⁺ + 7H₂O`.

Nhân nửa oxy hóa × 6:
- `6Fe²⁺ → 6Fe³⁺ + 6e⁻`.

Cộng: **`Cr₂O₇²⁻ + 14H⁺ + 6Fe²⁺ → 2Cr³⁺ + 7H₂O + 6Fe³⁺`**.

Kiểm tra điện tích: trái = −2 + 14 + 12 = +24. Phải = 6 + 0 + 18 = +24 ✓.

**Bài 4**:
- Cu: 0 → +2 (mất 2 e) → **Cu là chất khử**, bị oxy hóa.
- S trong H₂SO₄: +6 → +4 (trong SO₂), mỗi S nhận 2 e → **H₂SO₄ là chất oxy hóa**, bị khử.

**Bài 5**:
Nửa khử: MnO₄⁻ → Mn²⁺. Mn: +7 → +2, nhận 5 e.
- `MnO₄⁻ + 8H⁺ + 5e⁻ → Mn²⁺ + 4H₂O` (như bài walk-through).

Nửa oxy hóa: C₂O₄²⁻ → 2CO₂. C: +3 → +4, mỗi C mất 1 e, 2 C mất 2 e.
- `C₂O₄²⁻ → 2CO₂ + 2e⁻`. Điện tích: trái = −2, phải = 0 − 2 = −2 ✓.

Nhân: nửa khử × 2, nửa oxy hóa × 5 (LCM = 10 e):
- `2MnO₄⁻ + 16H⁺ + 10e⁻ → 2Mn²⁺ + 8H₂O`
- `5C₂O₄²⁻ → 10CO₂ + 10e⁻`

Cộng: **`2MnO₄⁻ + 16H⁺ + 5C₂O₄²⁻ → 2Mn²⁺ + 8H₂O + 10CO₂`**.

**Bài 6**: Vì Zn hoạt động hơn Cu (đứng trước Cu trong dãy hoạt động kim loại). Khi Zn vào dung dịch CuSO₄, có phản ứng:
```
Zn + CuSO₄ → ZnSO₄ + Cu↓
Zn(0) → Zn²⁺ + 2e⁻ (oxy hóa, Zn tan ra)
Cu²⁺ + 2e⁻ → Cu(0) (khử, Cu bám lên thanh)
```
Zn cho 2 e cho Cu²⁺. Kết quả: Zn bị "ăn mòn" tan vào dung dịch, Cu kim loại đỏ bám lên thanh.

---

## 6. Liên kết và bài tiếp theo

- **Bài tiếp theo**: [Lesson 03 — Điện hóa](../lesson-03-electrochemistry/) — biến phản ứng redox thành điện năng (pin).
- **Liên kết Math**: tính số oxy hóa = giải phương trình tuyến tính bậc 1.
- **Liên kết Physics**: pin = nguồn điện hóa → [`Physics/02-Thermo-Electromagnetism/lesson-06-current-circuits`](../../../Physics/02-Thermo-Electromagnetism/lesson-06-current-circuits/).

---

## 📝 Tổng kết Lesson 02 (T2)

1. **OIL RIG**: Oxidation = mất e (số ox tăng); Reduction = nhận e (giảm).
2. **2 quá trình luôn đi cặp**: 1 chất khử (cho e, bị oxy hóa) + 1 chất oxy hóa (nhận e, bị khử).
3. **7 quy tắc** tính số oxy hóa: F = −1, H = +1 (trừ hydride), O = −2 (trừ peroxide/OF₂), tổng = 0 hoặc điện tích ion.
4. **Cân bằng redox** (acid): tách nửa → cân bằng từng nửa (H₂O, H⁺, e) → nhân để e khớp → cộng.
5. **Môi trường base**: làm như acid rồi thêm OH⁻ vào 2 vế = số H⁺.
6. **Ứng dụng**: hô hấp, pin, ăn mòn, công nghiệp.

**Tiếp theo**: [Lesson 03 — Điện hóa](../lesson-03-electrochemistry/)
