# Lesson 07 — Tương đối & không-thời gian (Relativity & Spacetime)
...
- **Năng lượng, khối lượng, `E = mc²`**: [`../../../Physics/03-Optics-ModernPhysics/`](../../../Physics/03-Optics-ModernPhysics/).
...
> 💡 **Trực giác / Hình dung.** Trên xe lửa chạy 100 km/h, bạn ném quả bóng về phía trước 20 km/h → người đứng ngoài thấy bóng đi 120 km/h. Vận tốc *cộng* lại. Nhưng nếu bạn bật đèn pin, người ngoài **vẫn đo ánh sáng đúng `c`**, không phải `c + 100 km/h`. Tốc độ ánh sáng *từ chối* cộng vào. Để dung hòa điều kỳ lạ này, chính **thời gian và không gian** phải co giãn.

**Định nghĩa — bất biến tốc độ ánh sáng (3 phần):**

- **(a) Là gì** — Mọi người quan sát quán tính, dù chuyển động thế nào, đều đo tốc độ ánh sáng trong chân không bằng đúng `c ≈ 299 792 458 m/s`.
...
- **(c) Ví dụ trực giác bằng số** — Hệ số Lorentz `γ = 1/√(1 − v²/c²)`. Ở `v = 0.866c`: `γ = 1/√(1−0.75) = 1/√0.25 = 2` → đồng hồ chuyển động chạy **chậm gấp đôi** so với đồng hồ đứng yên.

**4 ví dụ số — hệ số Lorentz γ:**

| v / c | v²/c² | γ = 1/√(1−v²/c²) | Hệ quả |
|---|---|---|---|
...
> 🔁 **Dừng lại tự kiểm tra.** Ở `v = 0.6c`, tính γ.
> <details><summary>Đáp án</summary>v²/c² = 0.36; 1 − 0.36 = 0.64; √0.64 = 0.8; γ = 1/0.8 = **1.25**. Đồng hồ chuyển động chạy chậm 25%.</details>
...
- **(b) Vì sao cần** — Lý thuyết Newton mô tả hấp dẫn như lực tức thời, tỉ lệ `1/r²`. Nó cực kỳ chính xác ở vận tốc nhỏ/trường yếu, nhưng *sai* với trường mạnh (quỹ đạo Sao Thủy lệch) và không tương thích tốc độ ánh sáng hữu hạn. Tương đối tổng quát sửa cả hai.
...
- **(c) Ví dụ trực giác bằng số** — Đồng hồ vệ tinh GPS (cao ~20 200 km) chịu **hai** hiệu ứng ngược chiều:
  ...
  - Tổng: `+45 − 7 = +38 µs/ngày` đồng hồ vệ tinh chạy nhanh hơn mặt đất.
...
```
sai khoảng cách = c × Δt = 3×10⁸ m/s × 38×10⁻⁶ s ≈ 11 400 m ≈ 11.4 km
```
...
- **(c) Ví dụ trực giác bằng số** — **GW150914** (LIGO, 14/9/2015): hai lỗ đen ~36 và ~29 lần khối lượng Mặt Trời sáp nhập thành một lỗ đen ~62 M☉, cách ta ~1.3 tỉ năm ánh sáng. **~3 khối lượng Mặt Trời "biến mất"**, chuyển thành năng lượng sóng hấp dẫn (theo `E = mc²`). Biến dạng đo được tại LIGO chỉ ~10⁻²¹ — nhỏ hơn 1/1000 đường kính một proton trên cánh tay 4 km.

**Vì sao 3 M☉ ↔ năng lượng khổng lồ (`E = mc²`):**
```
ΔE = Δm·c² = 3 × (2×10³⁰ kg) × (3×10⁸ m/s)²
   = 6×10³⁰ × 9×10¹⁶ ≈ 5.4×10⁴⁷ J
```
...
> ❓ **Câu hỏi tự nhiên.** *"Đo biến dạng 10⁻²¹ kiểu gì?"* — LIGO là một giao thoa kế: hai cánh tay laser dài 4 km vuông góc. Sóng hấp dẫn co cánh này, giãn cánh kia → đường đi laser lệch → vân giao thoa đổi. Cách ly rung động cực kỳ tinh vi mới đọc nổi tín hiệu nhỏ hơn proton.
...
> 📝 **Tóm tắt toàn bài.**
> - **Tương đối hẹp**: `c` bất biến → thời gian/không gian co giãn (γ = 1/√(1−v²/c²)).
...
> - **Sóng hấp dẫn**: GW150914 — 2 lỗ đen sáp nhập, ~3 M☉ → năng lượng theo E = mc².

---

## Bài tập

1. **Hệ số Lorentz.** Tính γ cho `v = 0.8c`. Một phi hành gia bay `v = 0.8c` trong 5 năm theo đồng hồ riêng. Trên Trái Đất trôi qua bao lâu?

2. **Lệch GPS.** Nếu một hệ định vị bỏ qua *toàn bộ* hiệu chỉnh tương đối (ròng +38 µs/ngày), sau 3 ngày sai số vị trí tích lũy khoảng bao nhiêu km? (c = 3×10⁸ m/s.)
...
4. **E = mc² của GW150914.** Hai lỗ đen 36 + 29 M☉ sáp nhập thành 62 M☉. Khối lượng "mất" là bao nhiêu M☉? Đổi sang năng lượng (1 M☉ ≈ 2×10³⁰ kg, c = 3×10⁸ m/s).
...
```
t_Trái Đất = γ × t_riêng = 1.667 × 5 ≈ 8.33 năm
```
...
```
Δt = 3 × 38 µs = 114 µs = 114×10⁻⁶ s
sai khoảng cách = c × Δt = 3×10⁸ × 114×10⁻⁶ ≈ 34 200 m ≈ 34.2 km
```
...
### Bài 4 — E = mc² của GW150914

**Khối lượng mất:**
```
Δm = (36 + 29) − 62 = 65 − 62 = 3 M☉
```
**Đổi sang kg rồi năng lượng:**
```
Δm = 3 × 2×10³⁰ = 6×10³⁰ kg
E = Δm·c² = 6×10³⁰ × (3×10⁸)² = 6×10³⁰ × 9×10¹⁶
  = 5.4×10⁴⁷ J
```
...
- **Visualization tương tác**: [visualization.html](./visualization.html) — gồm:
  ...
  - **Calculator γ & GPS**: nhập v/c → γ; minh họa hai hiệu ứng tương đối cộng lại thành +38 µs/ngày và sai khoảng cách tương ứng.
...
**Tham khảo chéo:** `E = mc²` và vật lý hiện đại → [`../../../Physics/03-Optics-ModernPhysics/`](../../../Physics/03-Optics-ModernPhysics/).
