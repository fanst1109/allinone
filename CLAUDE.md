...
| `tools/fonts/` | **Font UI local (OFL, variable woff2)** cho chữ thường trong README-modal — bundle để chữ (đặc biệt **tiếng Việt**) đồng nhất mọi máy, không phụ thuộc font hệ thống. 3 font sẵn (mỗi font 4 subset latin/latin-ext/vietnamese/greek): **Roboto Flex** (đang dùng), **Inter**, **Manrope**. Khai báo `@font-face` ở `viz-base.css`; chọn font hoạt động bằng `font-family` trong `readme-modal.js` (`.rm-panel .rm-content`). KaTeX math vẫn dùng font `KaTeX_*` riêng. Auto-generated từ @fontsource-variable — KHÔNG sửa woff2 tay |
