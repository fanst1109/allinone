# Lesson 03 — Đệ quy & Hệ thức truy hồi (Recursion & Recurrence)
...
- [Lesson 01 — Big-O & phân tích tiệm cận](../lesson-01-bigo-asymptotic/README.md): ký hiệu `O / Θ / Ω`, so sánh tăng trưởng `log n < n < n log n < n² < 2ⁿ`.
...
`n! = n × (n−1) × … × 2 × 1`. Định nghĩa đệ quy: `n! = n × (n−1)!`, với `0! = 1`.
...
Số bước: `hanoi(1)=1`, `hanoi(2)=3`, `hanoi(3)=7`, `hanoi(4)=15` → quy luật `2ⁿ − 1` (sẽ chứng minh ở mục 6).
...
| `binary search(n)` | 1 nhánh, thẳng | `log₂ n` | `O(log n)` |
| `fib(n)` naive | 2 nhánh, dày | `≈ 2·F(n+1)−1` (mũ) | `O(n)` (chỉ chiều cao!) |
| `mergeSort(n)` | 2 nhánh | `O(n)` node | `O(log n)` chiều cao |

Điểm cốt lõi: **số node và độ sâu là hai đại lượng khác nhau.** `fib(n)` có cực nhiều node (chi phí *thời gian* `O(2ⁿ)`) nhưng stack tại một thời điểm chỉ chứa một đường gốc-tới-lá → *space* chỉ `O(n)`.
...
|-----|----------------|-----------|----------|
| factorial(n) | 1 lời gọi `f(n−1)`, work O(1) | `T(n) = T(n−1) + O(1)` | `Θ(n)` |
| fib(n) naive | 2 lời gọi `f(n−1)`, `f(n−2)`, work O(1) | `T(n) = T(n−1) + T(n−2) + O(1)` | `Θ(φⁿ)` ≈ `Θ(1.618ⁿ)` |
| binary search | 1 lời gọi nửa mảng, work O(1) | `T(n) = T(n/2) + O(1)` | `Θ(log n)` |
| merge sort | 2 lời gọi nửa mảng, merge O(n) | `T(n) = 2T(n/2) + O(n)` | `Θ(n log n)` |
...
**Ý tưởng:** đoán một dạng nghiệm (ví dụ `T(n) ≤ c·n log n`), rồi dùng **quy nạp toán học** chứng minh nó đúng.

Ví dụ với `T(n) = 2T(n/2) + n`, đoán `T(n) ≤ c·n log₂ n`:

- **Giả thiết quy nạp:** giả sử đúng cho mọi input nhỏ hơn, tức `T(n/2) ≤ c·(n/2)·log₂(n/2)`.
...
**Minh họa đoán SAI để thấy vì sao "không đóng được".** Thử đoán `T(n)≤c·n` cho `T(n)=2T(n/2)+n`:
...
Vế phải `(c+1)·n` luôn lớn hơn `c·n` → quy nạp **thất bại** → đoán `O(n)` sai. Phải nâng lên `O(n log n)` (mục 4.1 chứng minh đóng được). Bài học: nếu bước quy nạp ra thừa số dương không triệt tiêu được → đoán quá thấp.
...
**Walk-through `T(n) = 2T(n/2) + n` → `Θ(n log n)`:**
...
- Cây dừng khi `n/2ᵏ = 1` ⇒ `2ᵏ = n` ⇒ `k = log₂ n`. Vậy có `log₂ n + 1` level.
- **Tổng** = `n × (số level)` = `n × (log₂ n + 1)` = `Θ(n log n)`. ∎

**Verify bằng số thật `n = 8`:** level 0: `8`; level 1: `2×4=8`; level 2: `4×2=8`; level 3: `8×1=8`. Có `log₂8 + 1 = 4` level → tổng `4×8 = 32`. Công thức `n log₂ n = 8×3 = 24`, cùng bậc `Θ(n log n)` ✓ (hằng số khác nhau, bậc giống).
...
> <details><summary>Đáp án</summary>
>
> Mỗi level chỉ có **1 node** (1 nhánh), chi phí mỗi node = `1`. Số level = `log₂ n + 1` (kích thước ÷2 tới 1). Tổng = `1 × (log₂ n + 1) = Θ(log n)`.
> </details>

> 📝 **Tóm tắt mục 4.** Ba cách: (1) Substitution — đoán + quy nạp, mạnh nhưng cần đoán đúng; (2) Recursion tree — vẽ cây, tổng = `Σ chi phí mỗi level`, trực quan; (3) Master Theorem — tra bảng nhanh cho dạng `aT(n/b)+f(n)`.
...
|---|-----------|---------|---------|
| **Case 1** | `f(n) = O(n^{\log_b a - ε})` (f *nhỏ hơn* ngưỡng) | `Θ(n^{\log_b a})` | Chi phí dồn ở **lá** |
| **Case 2** | `f(n) = Θ(n^{\log_b a})` (f *bằng* ngưỡng) | `Θ(n^{\log_b a} \log n)` | Chi phí **chia đều** mọi level |
| **Case 3** | `f(n) = Ω(n^{\log_b a + ε})` (f *lớn hơn* ngưỡng) + điều kiện regularity | `Θ(f(n))` | Chi phí dồn ở **gốc** |
...
**(a) Merge sort — `T(n) = 2T(n/2) + n`.** `a=2, b=2, f(n)=n`. Ngưỡng `n^{\log_2 2}=n^1=n`. `f(n)=n=Θ(n)` → **Case 2** → `Θ(n log n)`. ✓ (khớp recursion tree mục 4.2).

**(b) Binary search — `T(n) = T(n/2) + 1`.** `a=1, b=2, f(n)=1`. Ngưỡng `n^{\log_2 1}=n^0=1`. `f(n)=1=Θ(1)=Θ(n^0)` → **Case 2** → `Θ(n^0 log n)=Θ(log n)`. ✓

**(c) Strassen (nhân ma trận) — `T(n) = 7T(n/2) + n²`.** `a=7, b=2, f(n)=n²`. Ngưỡng `n^{\log_2 7} ≈ n^{2.807}`. So sánh: `f(n)=n²=O(n^{2.807−ε})` (vì `2 < 2.807`) → **Case 1** → `Θ(n^{\log_2 7}) ≈ Θ(n^{2.81})`. (Nhanh hơn nhân ma trận ngây thơ `O(n³)`.)

**(d) `T(n) = 3T(n/2) + n`.** `a=3, b=2, f(n)=n`. Ngưỡng `n^{\log_2 3} ≈ n^{1.585}`. So sánh: `f(n)=n=O(n^{1.585−ε})` (vì `1 < 1.585`) → **Case 1** → `Θ(n^{\log_2 3}) ≈ Θ(n^{1.585})`.

**Bảng tổng hợp 4 ví dụ** (xem cùng lúc để so sánh 3 case):

| Recurrence | `a` | `b` | `f(n)` | ngưỡng `n^{log_b a}` | so sánh | Case | Kết quả |
|-----------|----:|----:|--------|----------------------|---------|:----:|---------|
| merge sort | 2 | 2 | `n` | `n^1` | bằng | 2 | `Θ(n log n)` |
| binary search | 1 | 2 | `1` | `n^0=1` | bằng | 2 | `Θ(log n)` |
| Strassen | 7 | 2 | `n²` | `n^{2.807}` | f nhỏ hơn | 1 | `Θ(n^{2.81})` |
| `3T(n/2)+n` | 3 | 2 | `n` | `n^{1.585}` | f nhỏ hơn | 1 | `Θ(n^{1.585})` |

**Một ví dụ Case 3 để đủ bộ 3 case** — `T(n)=2T(n/2)+n²`: `a=2,b=2,f=n²`, ngưỡng `n^1=n`. `f=n²=Ω(n^{1+ε})` lớn hơn theo bậc đa thức → **Case 3** → `Θ(n²)`. (Regularity: `2·(n/2)²=n²/2=(1/2)f(n)≤c·f(n)` với `c=1/2<1` ✓.)

> ⚠ **Lỗi thường gặp.** "f lớn hơn ngưỡng" của Case 3 phải lớn hơn theo **bậc đa thức** (`n^{+ε}`), không chỉ lớn hơn bằng thừa số `log`. Ví dụ `T(n)=2T(n/2)+n log n`: ngưỡng `n`, `f=n log n` lớn hơn nhưng **không** lớn hơn theo bậc đa thức (`log n` không phải `n^ε`) → **Master Theorem không áp dụng được** ở dạng cơ bản (cần phiên bản mở rộng cho ra `Θ(n log² n)`).

> ❓ **Câu hỏi tự nhiên.**
> - *"`log_b a` tính sao?"* — `log_b a = ln a / ln b`. Ví dụ `log₂7 = ln7/ln2 ≈ 1.946/0.693 ≈ 2.807`.
> - *"Vì sao Case 2 có thêm `log n`?"* — Vì mọi level đều cân nhau (cùng tốn `n^{\log_b a}`), và có `Θ(log n)` level → nhân thêm `log n`.
> - *"`b` không chia chẵn `n` thì sao?"* — Dùng `⌊n/b⌋` / `⌈n/b⌉`, kết quả tiệm cận không đổi (Master Theorem vẫn dùng được).

> 📝 **Tóm tắt mục 5.** Master Theorem cho `T(n)=aT(n/b)+f(n)`: so `f(n)` với ngưỡng `n^{\log_b a}`. Nhỏ hơn → Case 1 `Θ(n^{\log_b a})`; bằng → Case 2 thêm `log n`; lớn hơn (theo bậc đa thức) → Case 3 `Θ(f(n))`.
...
**Verify `n=4`:** `4+3+2+1 = 10 = 4·5/2` ✓ → bậc `Θ(n²)`.

### 6.2 `T(n) = 2T(n−1) + 1` → `Θ(2ⁿ)` (Tower of Hanoi)

Recursion tree là cây nhị phân **đầy** chiều cao `n` → `2ⁿ` lá. Khai triển:
...
**Verify:** `hanoi(1)=1=2¹−1`, `hanoi(2)=3=2²−1`, `hanoi(3)=7=2³−1`, `hanoi(4)=15=2⁴−1` ✓ → `Θ(2ⁿ)`.
...
> 📝 **Tóm tắt mục 6.** Recurrence trừ-hằng (`n−1`, `n−2`) → dùng telescoping/recursion tree. `T(n)=T(n−1)+n → Θ(n²)`; `T(n)=2T(n−1)+1 → Θ(2ⁿ)`.
...
Cùng cách, đếm cho `factorial(n)` luôn ra `calls == n` (1 nhánh, thẳng) — minh chứng `factorial` `Θ(n)` còn `fib` naive `Θ(2ⁿ)`. Biến đếm là công cụ rẻ tiền để kiểm chứng recurrence trên máy thật trước khi tin vào phân tích lý thuyết.
...
`fib(50)` naive ≈ `2⁵⁰` lời gọi ≈ vài trăm năm — trong khi `fibMemo(50)` chỉ ~99 lời gọi, xong tức thì. Đây là lý do *phải* nhận diện đệ quy nhiều nhánh tính trùng và thêm memo (mục 8).
...
Naive bùng nổ theo hàm mũ; memo tuyến tính `≈ 2n−1`. Đây là cửa ngõ vào **Quy hoạch động (Dynamic Programming)** — học kỹ ở [Tier 4](../tier-4-dynamic-programming/index.html).

> 📝 **Tóm tắt mục 8.** `fib` naive `O(2ⁿ)` vì tính trùng; thêm memo → mỗi giá trị tính đúng 1 lần → `O(n)`. Đây là ý tưởng cốt lõi của DP.
...
| **Fibonacci naive** | `O(2ⁿ)` với n lớn → treo | dùng memo / iterative |
...
1. **Giải 6 recurrence.** Cho biết `Θ` của mỗi recurrence và phương pháp dùng:
   (a) `T(n)=2T(n/2)+n`  (b) `T(n)=T(n/2)+1`  (c) `T(n)=4T(n/2)+n`
   (d) `T(n)=3T(n/2)+n²`  (e) `T(n)=T(n−1)+1`  (f) `T(n)=2T(n−1)+1`.
...
5. **Sửa bug stack overflow.** Tìm và sửa lỗi trong hàm sau:
   ...
   ```
6. **Chứng minh `T(n)=2T(n/2)+n = Θ(n log n)`** bằng recursion tree (viết rõ từng level, số level, tổng).
...
|--|-----------|----------------|-----------|---------|
| (a) | `2T(n/2)+n` | `a=2,b=2,f=n`; ngưỡng `n^{log₂2}=n` | `f=Θ(n)` = ngưỡng → **Master Case 2** | `Θ(n log n)` |
| (b) | `T(n/2)+1` | `a=1,b=2,f=1`; ngưỡng `n^0=1` | `f=Θ(1)` = ngưỡng → **Case 2** | `Θ(log n)` |
| (c) | `4T(n/2)+n` | `a=4,b=2,f=n`; ngưỡng `n^{log₂4}=n²` | `f=n=O(n^{2−ε})` < ngưỡng → **Case 1** | `Θ(n²)` |
| (d) | `3T(n/2)+n²` | `a=3,b=2,f=n²`; ngưỡng `n^{log₂3}≈n^{1.585}` | `f=n²=Ω(n^{1.585+ε})` > ngưỡng → **Case 3** | `Θ(n²)` |
| (e) | `T(n−1)+1` | trừ-hằng → telescoping | `=T(0)+1+1+…+1 (n lần)` | `Θ(n)` |
| (f) | `2T(n−1)+1` | trừ-hằng, 2 nhánh → cây nhị phân đầy | `=2ⁿ−1` | `Θ(2ⁿ)` |

Riêng (d) kiểm tra regularity: `a·f(n/b) = 3·(n/2)² = 3n²/4 = (3/4)·f(n) ≤ c·f(n)` với `c=3/4<1` ✓ → Case 3 hợp lệ.

### Bài 2 — Số lời gọi `fib(10)` naive

Gọi `C(n)` = số lời gọi hàm khi tính `fib(n)`. `C(0)=C(1)=1`, `C(n)=1+C(n−1)+C(n−2)`.

Tính dần: `C(2)=1+1+1=3`, `C(3)=1+3+1=5`, `C(4)=1+5+3=9`, `C(5)=1+9+5=15`, `C(6)=1+15+9=25`, `C(7)=1+25+15=41`, `C(8)=1+41+25=67`, `C(9)=1+67+41=109`, `C(10)=1+109+67=`**`177`**.

(Công thức tổng quát: `C(n)=2·F(n+1)−1`, với `F(11)=89` → `2·89−1=177` ✓.)
...
Telescoping (như mục 6.2): `T(n)=2ⁿ−1`. **Verify:** `T(3)=2³−1=7` ✓. Độ phức tạp thời gian `Θ(2ⁿ)`, space (độ sâu stack) `Θ(n)`.
...
`power(2,10)=1024`. Recurrence `T(exp)=T(exp−1)+O(1)=Θ(exp)`.
...
- **Level 1:** 2 node cỡ `n/2`, mỗi node chi phí `n/2`. Tổng level = `2·(n/2) = n`.
- **Level 2:** 4 node cỡ `n/4`, mỗi `n/4`. Tổng = `4·(n/4) = n`.
- **Level k:** `2ᵏ` node cỡ `n/2ᵏ`, mỗi `n/2ᵏ`. Tổng = `2ᵏ·(n/2ᵏ) = n`.

**Mỗi level đều tốn đúng `n`.** Cây dừng khi kích thước node = 1: `n/2ᵏ = 1 ⇒ k = log₂ n` → có `log₂ n + 1` level.

**Tổng toàn cây** = `n × (log₂ n + 1) = n log₂ n + n = Θ(n log n)`. ∎

**Verify `n=8`:** 4 level, mỗi level `8` → `4×8=32`; bậc khớp `n log₂ n = 8×3 = 24` (cùng `Θ(n log n)`) ✓.
