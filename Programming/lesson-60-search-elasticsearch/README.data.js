...
// Source: Programming/lesson-60-search-elasticsearch/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 60 — Search & Elasticsearch
...
|--------|-----------------|---------------|
| **Tốc độ** | Full scan $O(N \\times L)$ | Lookup inverted index $O(1)$ lấy posting list |
...
Posting list được lưu trong cấu trúc tra cứu nhanh (hash map hoặc cây sắp xếp). Lookup một term là **$O(1)$** (hash) hoặc **$O(\\log V)$** (V = số term phân biệt) — KHÔNG phụ thuộc số document N. So với full scan $O(N \\times L)$, với N = 1 triệu thì đây là khác biệt giữa "tức thì" và "vài giây".
...
\`\`\`
   ...
   laptop  lapland
\`\`\`
Đi theo "l→a→p" tới node, rồi DFS lấy mọi từ con bên dưới = các gợi ý. Tra prefix là $O(\\text{độ dài prefix})$. Đây chính là ý tưởng [solutions.go](./solutions.go) implement cho phần autocomplete.
