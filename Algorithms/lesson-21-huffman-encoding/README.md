# Lesson 21 — Huffman Encoding (Mã hóa Huffman)
...
Máy tính lưu ký tự bằng **mã độ dài cố định**: ASCII dùng 8 bit cho mỗi ký tự, bất kể ký tự đó xuất hiện nhiều hay ít. Nếu bảng chữ chỉ có 4 ký tự `{a, b, c, d}`, ta chỉ cần $\lceil \log_2 4 \rceil = 2$ bit mỗi ký tự:
...
Với chuỗi có $n$ ký tự, fixed-length tốn $n \times 2$ bit, **không phụ thuộc tần suất**.

**Ví dụ số 1.** Chuỗi `"aaaaabbcd"` ($n = 9$): fixed-length = $9 \times 2 = 18$ bit.

**Ví dụ số 2.** Chuỗi `"abracadabra"` ($n = 11$, bảng chữ `{a,b,r,c,d}` → 5 ký tự → cần 3 bit): fixed-length = $11 \times 3 = 33$ bit.

**Ví dụ số 3.** Một file text 1.000 ký tự thuần ASCII: $1000 \times 8 = 8000$ bit = 1000 byte.
...
**Tổng bit** = $5 \times 1 + 2 \times 2 + 1 \times 3 + 1 \times 3 = 5 + 4 + 3 + 3 = 15$ bit.

So với fixed-length 2-bit: $(5+2+1+1) \times 2 = 9 \times 2 = 18$ bit. **Tiết kiệm 3 bit (~16,7%)** — và đây mới là chuỗi tí hon. Với file lớn, độ tiết kiệm đáng kể hơn nhiều.
...
> 2. $4 \times 2 = 8$ bit. Variable-length tiết kiệm 3 bit.
> </details>

> 📝 **Tóm tắt mục 1.** Fixed-length trả cùng số bit cho mọi ký tự → lãng phí khi tần suất lệch. Variable-length gán mã ngắn cho ký tự hay gặp → ít bit hơn. Bài toán: tối thiểu **tổng bit = $\sum (\text{tần suất} \times \text{độ dài mã})$**.
...
**So sánh:**
- Fixed-length (5 ký tự → 3 bit): $11 \times 3 = 33$ bit.
- Huffman: $23$ bit.
- **Tiết kiệm: $33 - 23 = 10$ bit $\approx$ 30,3%.**
...
Gọi **chi phí** của một cây $T$ là $B(T) = \sum_{\text{ký tự } c} \text{freq}(c) \times \text{depth}_T(c)$ (đúng bằng tổng bit). Ta cần chứng minh cây Huffman cực tiểu $B(T)$.
...
2. Vì $x$ có tần suất nhỏ nhất nên $\text{freq}(x) \leq \text{freq}(a)$. Vì $a$ ở tầng sâu nhất nên $\text{depth}(a) \geq \text{depth}(x)$.
3. **Đổi chỗ** $x$ và $a$ trong cây, được cây $T'$. Thay đổi chi phí:
   ...
   B(T) − B(T') = freq(a)·depth(a) + freq(x)·depth(x)
                ...
                ≥ 0
   ```
   vì cả hai thừa số đều $\geq 0$. Nghĩa là $B(T') \leq B(T)$. Nhưng $T$ đã tối ưu nên $B(T') = B(T)$: $T'$ cũng tối ưu.
4. Tương tự đổi chỗ $y$ với người anh em còn lại của $a$ ở tầng sâu nhất, được $T''$ tối ưu trong đó $x$, $y$ là anh em ở tầng sâu nhất. ∎

> Không bước nào dùng "dễ thấy". Mấu chốt là đẳng thức $(\text{freq}(a)-\text{freq}(x))(\text{depth}(a)-\text{depth}(x)) \geq 0$: chuyển ký tự **nhẹ hơn** xuống **sâu hơn** không bao giờ làm tăng chi phí.

### 5.2 Bổ đề 2 — bài toán có cấu trúc con tối ưu

Sau khi ghép $x$, $y$ thành node $z$ với $\text{freq}(z) = \text{freq}(x) + \text{freq}(y)$, bài toán trên bảng chữ mới (thay $x$, $y$ bằng $z$) là **bài toán con cùng dạng**, nhỏ hơn 1 ký tự. Chi phí liên hệ:

```
B(T) = B(T_con) + freq(x) + freq(y)
```

(vì $x$, $y$ ở sâu hơn $z$ đúng 1 tầng; phần $\text{freq}(x)+\text{freq}(y)$ chính là chi phí thêm của 1 tầng đó). Do $\text{freq}(x)+\text{freq}(y)$ là **hằng số** với mọi cây hợp lệ, cực tiểu $B(T)$ ⟺ cực tiểu $B(T_{\text{con}})$.
...
> ❓ **Câu hỏi tự nhiên.** *"Vì sao bảng mã chạy thật (`b=111, r=10, c=1100, d=1101`) khác với walk-through tay ở mục 4 (`b=110, r=111, c=100, d=101`)?"* — Vì tie-break ở mục 4 chạy tay theo **char của lá**, còn code ở đây tie-break theo **char nhỏ nhất trong node** (so sánh node cha cũng có char), nên thứ tự ghép khác → **hình cây khác**. Nhưng để ý: **tổng bit vẫn đúng 23** ($a{:}5 \times 1 + r{:}2 \times 2 + b{:}2 \times 3 + c{:}1 \times 4 + d{:}1 \times 4 = 5+4+6+4+4 = 23$). Cả hai cây đều **tối ưu** — đúng tinh thần mục 11.1: tie-break đổi hình cây, không đổi tổng bit.
...
| Encode | nối mã `L` ký tự | $O(L)$ (hoặc $O(\text{tổng bit})$) |
| Decode | duyệt từng bit | $O(\text{tổng bit})$ |
...
Fixed-length: 6 ký tự → $\lceil \log_2 6 \rceil = 3$ bit/ký tự → $15 \times 3 = 45$ bit. **Tiết kiệm 15 bit ≈ 33,3%.**
...
| Gán mã (DFS) | $O(n)$ | $O(n)$ |
| Encode | $O(L)$ | $O(\text{tổng bit})$ |
| Decode | $O(\text{tổng bit})$ | $O(L)$ |
