...
// Source: Math/07-Mathematical-Modeling/lesson-01-modeling-cycle/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 01 — Chu trình mô hình hóa & Phân tích thứ nguyên
...
>
> **(c) Ví dụ số**: Ném một quả bóng thẳng lên với vận tốc đầu v₀ = 20 m/s. Mô hình (bỏ qua sức cản không khí): độ cao h(t) = v₀·t − ½·g·t² với g = 9.8 m/s². Tại t = 1s: h = 20·1 − ½·9.8·1² = 20 − 4.9 = **15.1 m**. Đỉnh khi h'(t) = v₀ − g·t = 0 → t = 20/9.8 ≈ 2.04s, h_max = 20² / (2·9.8) ≈ **20.4 m**. Mô hình cho ngay dự đoán bằng số — không cần thực sự ném bóng.
...
⚠ **Lỗi thường gặp — dừng ở bước 3 hoặc 4.** Rất nhiều người "lập được phương trình" rồi coi như xong. Nhưng nếu không **kiểm chứng** (bước 5), bạn không biết mô hình *đúng* hay chỉ *trông có vẻ đúng*. Một mô hình cho ra "dân số âm" hoặc "xác suất 1.3" là sai dù phương trình giải ra đẹp. Phản ví dụ: mô hình tăng trưởng mũ N = N₀·e^(rt) dự báo dân số Trái Đất năm 3000 là 10²⁰ người — vô lý vật lý → phải tinh chỉnh thành logistic (Lesson 04).
...
- **Tuyến tính hóa**: với góc nhỏ, sin θ ≈ θ (con lắc — xem [T6 L07](../../06-Advanced/lesson-07-differential-equations/)).
...
> 🧪 **Toy model & phiên bản thật.** Mô hình ném bóng h(t) = v₀t − ½gt² (bỏ sức cản) là *toy model* tốt cho minh họa. Phiên bản thật thêm lực cản tỉ lệ v hoặc v²: m·dv/dt = −mg − k·v², không còn giải tay gọn được, phải mô phỏng số. Quy tắc: nêu toy model để hiểu cơ chế, **cảnh báo hạn chế**, rồi chỉ ra phiên bản thật ở đâu.
...
>
> **(a) Là gì**: Thứ nguyên là *loại* đại lượng, biểu diễn qua các đại lượng cơ bản. Trong cơ học dùng 3 thứ nguyên cơ bản: khối lượng **M**, chiều dài **L**, thời gian **T**. Thứ nguyên của một đại lượng viết trong ngoặc vuông \`[ ]\`. Vd: vận tốc [v] = L·T⁻¹ (mét trên giây); gia tốc [a] = L·T⁻²; lực [F] = M·L·T⁻² (vì F = ma).
>
...
>
> **(c) Ví dụ số (4 ví dụ)**:
> - Quãng đường s = v·t: [v·t] = (L·T⁻¹)·T = L = [s] ✓. Số: 10 m/s · 3 s = 30 m (L) ✓.
> - Động năng E = ½mv²: [m·v²] = M·(L·T⁻¹)² = M·L²·T⁻² = [năng lượng] ✓.
> - Chu kỳ con lắc T = 2π√(L/g): [√(L/g)] = √(L / (L·T⁻²)) = √(T²) = T ✓ (thứ nguyên thời gian, đúng là chu kỳ). Số: L=1m, g=9.8 → 2π√(1/9.8) ≈ 2.0 s.
> - Sai: "v = ½·a·t²" → [a·t²] = (L·T⁻²)·T² = L (chiều dài), nhưng [v] = L·T⁻¹ → **không khớp** → công thức sai (đúng là quãng đường, không phải vận tốc).
...
2. **Hai vế phương trình cùng thứ nguyên.** \`[vế trái] = [vế phải]\`, luôn luôn.

Hệ quả: hàm như sin, cos, exp, ln **chỉ nhận đối số không thứ nguyên** (dimensionless). \`e^(kt)\` đòi \`[k·t]\` = 1 (không thứ nguyên) → \`[k] = T⁻¹\`. Vì sao? Vì khai triển e^x = 1 + x + x²/2 + ... cộng các lũy thừa khác bậc của x — chỉ cộng được khi x không thứ nguyên.
...
**Bài toán**: chu kỳ T của con lắc đơn phụ thuộc gì? Đoán nó phụ thuộc chiều dài dây ℓ (thứ nguyên L), gia tốc trọng trường g (L·T⁻²), và khối lượng m (M).

**Giả định dạng tích lũy thừa**: T = C · ℓᵃ · gᵇ · mᶜ (C là hằng số không thứ nguyên).

**Cân bằng thứ nguyên hai vế** ([T] ở vế trái là thời gian = T¹):
\`\`\`
...
Vậy T = C · ℓ^(½) · g^(−½) = C·√(ℓ/g). So với công thức thật T = 2π√(ℓ/g): **đúng dạng**, chỉ thiếu hằng số C = 2π (phân tích thứ nguyên không cho được hằng số không thứ nguyên — đó là hạn chế của phương pháp).
...
- *"Thứ nguyên cân rồi thì công thức chắc chắn đúng?"* **Không** — cân thứ nguyên là điều kiện *cần*, không *đủ*. \`s = v·t\` và \`s = 2·v·t\` đều cân thứ nguyên nhưng chỉ một đúng. Hằng số không thứ nguyên (2, π...) phương pháp không bắt được. Nhưng nếu *không* cân thứ nguyên thì *chắc chắn sai* — đó là giá trị của nó: bộ lọc loại nhanh công thức sai.
...
- *"Vì sao sin/cos/log/exp phải nhận số không thứ nguyên?"* Vì chúng định nghĩa qua chuỗi cộng các lũy thừa khác bậc (sin x = x − x³/6 + ...); chỉ cộng được khi các số hạng cùng thứ nguyên, tức x không thứ nguyên. Nên \`sin(5 mét)\` vô nghĩa; phải \`sin(5 mét / chiều_dài_chuẩn)\`.

⚠ **Lỗi thường gặp — đưa đại lượng có thứ nguyên vào exp/sin.** Viết \`N(t) = N₀·e^(−t)\` với t tính bằng giây là *thiếu hằng số tốc độ*: phải \`e^(−t/τ)\` hoặc \`e^(−kt)\` với [τ] = T, [k] = T⁻¹ để mũ không thứ nguyên. Phản ví dụ: \`e^(−5 giây)\` không có nghĩa; \`e^(−5s / 2s)\` = e^(−2.5) mới đúng.

🔁 **Dừng lại tự kiểm tra**

1. Kiểm tra thứ nguyên công thức động lượng p = m·v. Cho biết [p].
2. Một công thức cho ra "năng lượng" nhưng bạn tính được thứ nguyên vế phải là M·L·T⁻². Có vấn đề gì?
3. Đối số của \`ln(P/P₀)\` (P, P₀ là áp suất) có hợp lệ không?

<details><summary>Đáp án</summary>

1. [p] = [m·v] = M·(L·T⁻¹) = **M·L·T⁻¹**. (Đây cũng là thứ nguyên của xung lực F·t = (M·L·T⁻²)·T = M·L·T⁻¹ ✓ — định lý xung lượng.)
2. Năng lượng có thứ nguyên **M·L²·T⁻²** (từ E = ½mv²). Vế phải ra M·L·T⁻² (đó là *lực*, không phải năng lượng) → **công thức sai**, lệch một thừa số chiều dài L.
3. **Hợp lệ.** P/P₀ là tỉ số hai áp suất → thứ nguyên triệt tiêu → không thứ nguyên → ln nhận được. Đây là lý do công thức vật lý hay viết log của *tỉ số*, không phải log của đại lượng có đơn vị.
...
- Có thể *đoán dạng công thức* chỉ bằng thứ nguyên (vd chu kỳ con lắc T ∝ √(ℓ/g), không phụ thuộc khối lượng).
...
**Bước 2 — Giả định.**
- Nhiệt độ phòng T_phòng = 25°C *không đổi* (phòng lớn, cốc nhỏ).
...
\`\`\`
dT/dt = −k·(T − T_phòng),   k > 0 là hằng số tốc độ, [k] = T⁻¹ (phút⁻¹)
\`\`\`
Kiểm thứ nguyên: [dT/dt] = nhiệt độ/thời gian; [k·(T−T_phòng)] = (T⁻¹)·(nhiệt độ) = nhiệt độ/thời gian ✓.

**Bước 4 — Giải.** Đây là ODE tách biến (xem [T6 L07 mục 2](../../06-Advanced/lesson-07-differential-equations/)). Đặt u = T − 25 → du/dt = −k·u → u = u₀·e^(−kt). Với T(0) = 90 → u₀ = 65:
\`\`\`
T(t) = 25 + 65·e^(−kt)
\`\`\`
Cần một điểm dữ liệu để tìm k. Giả sử đo được: sau 5 phút cà phê còn 70°C.
- 70 = 25 + 65·e^(−5k) → 45/65 = e^(−5k) → −5k = ln(45/65) = ln(0.692) = −0.368
- → k = 0.0736 phút⁻¹.

Tìm t khi T = 60°C:
- 60 = 25 + 65·e^(−kt) → 35/65 = e^(−kt) → −kt = ln(0.538) = −0.619
- → t = 0.619 / 0.0736 ≈ **8.4 phút**.

**Bước 5 — Kiểm chứng.**
- t = 0: T = 25 + 65·1 = 90°C ✓ (đúng nhiệt độ đầu).
- t → ∞: T → 25 + 0 = 25°C ✓ (nguội về nhiệt độ phòng, hợp lý).
- t = 5: T = 25 + 65·e^(−0.368) = 25 + 65·0.692 = 70°C ✓ (khớp dữ liệu hiệu chỉnh).
...
**Bài 1.** Kiểm tra thứ nguyên công thức năng lượng tiềm năng hấp dẫn E = m·g·h. Kết quả có đúng thứ nguyên năng lượng (M·L²·T⁻²) không?

**Bài 2.** Một mô hình tính *vận tốc* cuối của vật rơi tự do từ độ cao h được viết là \`v = g·h\`. Dùng phân tích thứ nguyên chỉ ra công thức này sai, và đoán dạng đúng.

**Bài 3.** Liệt kê tối thiểu 3 giả định bạn sẽ đặt khi mô hình hóa "thời gian một thang máy đi từ tầng 1 lên tầng 10".

**Bài 4.** (End-to-end rút gọn) Một bồn nước ban đầu có 100 lít, nước chảy ra với tốc độ tỉ lệ lượng nước hiện có: dV/dt = −0.2·V (V tính bằng lít, t bằng phút). (a) Tìm V(t). (b) Sau bao lâu còn 50 lít? (c) Kiểm chứng giới hạn t→∞.

**Bài 5.** Chu kỳ dao động của một vật khối lượng m gắn vào lò xo độ cứng κ (thứ nguyên [κ] = M·T⁻², vì F = κ·x). Dùng phân tích thứ nguyên đoán dạng công thức chu kỳ T theo m và κ.

---

## 7. Lời giải chi tiết

**Bài 1.** [m·g·h] = M · (L·T⁻²) · L = **M·L²·T⁻²** ✓. Đúng bằng thứ nguyên năng lượng (so với ½mv²: M·(L·T⁻¹)² = M·L²·T⁻² — khớp). Kết luận: công thức cân thứ nguyên. Số minh họa: m=2kg, g=9.8, h=5m → E = 2·9.8·5 = 98 J (joule = kg·m²/s²) ✓.

**Bài 2.** [g·h] = (L·T⁻²)·L = L²·T⁻². Nhưng [v] = L·T⁻¹. So sánh: L²·T⁻² ≠ L·T⁻¹ → **không cân thứ nguyên → công thức sai**. Đoán dạng đúng: v phải lấy *căn* để hạ bậc. [√(g·h)] = √(L²·T⁻²) = L·T⁻¹ = [v] ✓. Vậy dạng đúng là v = C·√(g·h). (Công thức thật v = √(2gh), C = √2 — phân tích thứ nguyên cho đúng dạng, thiếu hằng số √2.)
...
**Bài 4.**
- (a) ODE tách biến: dV/V = −0.2 dt → ln V = −0.2t + C → **V(t) = 100·e^(−0.2t)** (dùng V(0)=100 → C cho hằng số 100).
- (b) 50 = 100·e^(−0.2t) → 0.5 = e^(−0.2t) → −0.2t = ln 0.5 = −0.693 → **t = 3.47 phút**. (Đây là "thời gian bán rã" của bồn nước — giống phóng xạ.)
- (c) t → ∞: V → 100·e^(−∞) = 100·0 = **0 lít** ✓ (bồn cạn dần, hợp lý). Kiểm thêm V(0) = 100·1 = 100 ✓.

**Bài 5.** Giả định T = C·mᵃ·κᵇ. Cân thứ nguyên ([T] = T¹):
\`\`\`
T¹ = Mᵃ · (M·T⁻²)ᵇ = M^(a+b) · T^(−2b)
\`\`\`
- **T**: −2b = 1 → b = −½.
- **M**: a + b = 0 → a = ½.
→ T = C·m^(½)·κ^(−½) = **C·√(m/κ)**. So công thức thật T = 2π√(m/κ): đúng dạng, C = 2π. Diễn giải: lò xo cứng hơn (κ lớn) → dao động nhanh hơn (T nhỏ); vật nặng hơn (m lớn) → dao động chậm hơn (T lớn) — khớp trực giác.
...
4. **Phân tích thứ nguyên**: hai vế cùng thứ nguyên (điều kiện cần); loại nhanh công thức sai; đoán được *dạng* công thức (vd chu kỳ con lắc ∝ √(ℓ/g), không phụ thuộc khối lượng).
