# Lesson 33 — Dijkstra (đường đi ngắn nhất, không cạnh âm)
...
**Phát biểu hình thức.** Cho đồ thị có hướng `G = (V, E)`, mỗi cạnh `(u → v)` có trọng số `w(u, v) ≥ 0`. Cho một đỉnh nguồn `src`. Tìm `dist[v]` = tổng trọng số nhỏ nhất của một đường đi từ `src` tới `v`, cho **mọi** `v ∈ V`. Nếu không tới được, `dist[v] = ∞`.

Điều kiện then chốt của bài này: **mọi trọng số không âm** (`w ≥ 0`). Đây là điều kiện sống còn để Dijkstra đúng — mục 7 sẽ giải thích vì sao.
...
|------|-------|----------------------|
| **Single-source** | 1 nguồn → mọi đỉnh | Dijkstra (w ≥ 0), Bellman-Ford (có cạnh âm) |
...
> - `dist[v] = ∞` nghĩa là không tới được.
...
> ⚠ **Lỗi thường gặp.** Bước 4 **bắt buộc** `w ≥ 0`. Nếu có cạnh âm, "phần còn lại của P ≥ 0" sai → toàn bộ chứng minh sụp. Đây chính là lý do Dijkstra **không** dùng được với cạnh âm (mục 7).
...
> - Chứng minh dựa vào **w ≥ 0** — cạnh âm phá vỡ nó.
...
| Mảng / linear scan (no heap) | O(V) tìm min | O(1) | O(V²) — tốt khi đồ thị dày `E ≈ V²` |

**Phân tích binary heap.** Mỗi cạnh relax thành công → push 1 entry → tối đa `E` lần push, mỗi lần O(log V) (heap có ≤ E phần tử). Mỗi entry pop ra 1 lần → ≤ E pop, mỗi O(log V). Cộng cả khởi tạo V đỉnh: **O((V + E) log V)**.

> ❓ **Câu hỏi tự nhiên.**
> - *"Heap có thể to tới E phần tử à? Sao không phải V?"* — Vì lazy deletion: ta push nhiều entry stale cho cùng đỉnh. Tổng push ≤ E. Một số tài liệu viết O(E log V) coi `E ≥ V` với đồ thị liên thông.
...
> - *"Khi nào dùng O(V²) array version?"* — Đồ thị **dày** (`E ≈ V²`): `O(V²)` < `O(V² log V)`. Ví dụ ma trận kề đầy đủ.

> 📝 **Tóm tắt mục 6.**
> - Binary heap: **O((V+E) log V)** — mặc định thực dụng.
> - Fibonacci heap: O(E + V log V) — lý thuyết.
> - Array version: O(V²) — tốt cho đồ thị dày.

---

## 7. Vì sao KHÔNG dùng được với cạnh âm

> 💡 **Trực giác.** Dijkstra đặt cược rằng: "đỉnh đã chốt thì không bao giờ tốt hơn được nữa." Cược này dựa vào *mọi cạnh thêm vào chỉ làm đường dài ra (w ≥ 0)*. Cạnh âm phá vỡ: một đường vòng qua cạnh âm có thể **rút ngắn** đường tới đỉnh đã chốt → quyết định "chốt sớm" trở thành sai.
...
A* mở rộng Dijkstra bằng cách ưu tiên theo `f(v) = dist[v] + h(v)`, trong đó `h(v)` là **ước lượng** khoảng cách còn lại từ `v` tới đích (heuristic). Nếu `h` **admissible** (không bao giờ ước lượng quá thật, vd khoảng cách Euclid trên bản đồ), A* tìm đúng đường ngắn nhất nhưng **mở ít đỉnh hơn** → nhanh hơn. Khi `h ≡ 0`, A* **chính là** Dijkstra. Học kỹ ở **capstone L52**.
...
> - Đúng nhờ bất biến "pop = tối ưu" (cần `w ≥ 0`).
