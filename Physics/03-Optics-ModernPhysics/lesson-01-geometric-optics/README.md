...
- Hiểu **quang hình** = mô hình ánh sáng truyền theo đường thẳng (đúng khi $\lambda$ << kích thước vật).
- Áp dụng **định luật phản xạ**: góc tới = góc phản xạ.
- Áp dụng **định luật khúc xạ Snell**: $n_1 \cdot \sin\theta_1 = n_2 \cdot \sin\theta_2$.
- Hiểu **chiết suất n** = $c/v_{\text{môi trường}}$ — và liên hệ với tốc độ ánh sáng trong môi trường.
...
- *"Góc tới đo từ đâu — từ bề mặt hay từ pháp tuyến?"* Luôn đo từ **pháp tuyến** (đường vuông góc với bề mặt tại điểm tới). Đây là quy ước thống nhất cho cả phản xạ lẫn khúc xạ. Nếu một tia "sượt" gần như song song mặt gương, góc tới của nó gần $90^\circ$ (xa pháp tuyến), không phải gần $0^\circ$.
...
- **Đo góc từ mặt phẳng thay vì từ pháp tuyến.** Một tia tới tạo $20^\circ$ với MẶT gương thì góc tới (so pháp tuyến) là $90^\circ - 20^\circ = 70^\circ$, và góc phản xạ cũng là $70^\circ$ — KHÔNG phải $20^\circ$.
- **Tưởng "góc tới = góc phản xạ" áp dụng cho khúc xạ.** Sai — đó là định luật phản xạ. Khúc xạ tuân theo Snell ($n_1 \sin\theta_1 = n_2 \sin\theta_2$), góc tới và góc khúc xạ thường KHÁC nhau.

🔁 **Dừng lại tự kiểm tra**

1. Một tia sáng đập vào gương phẳng, tạo góc $25^\circ$ với MẶT gương. Góc phản xạ (so pháp tuyến) bằng bao nhiêu?
2. Vì sao trong phòng tối hoàn toàn ta không thấy gì, dù vật vẫn ở đó?

<details><summary>Đáp án</summary>

1. Góc tới so pháp tuyến = $90^\circ - 25^\circ = 65^\circ$. Theo định luật phản xạ, góc phản xạ = $65^\circ$.
...
- **(c) Ví dụ số kèm đơn vị**: nước có $n = 1{,}33$. Tốc độ ánh sáng trong nước = $v = c/n = (3 \times 10^8 \text{ m/s})/1{,}33 = 2{,}26 \times 10^8 \text{ m/s}$. Tức ánh sáng trong nước chậm hơn trong chân không khoảng 33%.
...
trong đó $\theta$ là góc đo từ pháp tuyến.
...
- *"Tia vuông góc với mặt phân cách ($\theta_1 = 0$) có bị bẻ không?"* KHÔNG. Snell: $n_1 \sin 0 = n_2 \sin\theta_2 \to \sin\theta_2 = 0 \to \theta_2 = 0$. Tia đi thẳng, chỉ đổi tốc độ chứ không đổi hướng.
- *"Tần số ánh sáng có đổi khi qua môi trường khác không?"* KHÔNG — tần số $f$ được giữ nguyên (do nguồn quyết định). Cái thay đổi là tốc độ $v$ và bước sóng $\lambda$ (vì $v = \lambda f$, v giảm thì $\lambda$ giảm theo). Màu sắc (gắn với f) do đó không đổi.

⚠ **Lỗi thường gặp**

- **Lẫn chiều đặt $n_1$, $n_2$.** $n_1$ luôn là môi trường chứa **tia tới**, $n_2$ là môi trường chứa **tia khúc xạ**. Đặt sai → tính ra góc vô lý (vd $\sin\theta > 1$).
...
1. Ánh sáng đi từ thủy tinh ($n=1{,}5$) ra không khí ($n=1$) với góc tới $20^\circ$. Góc khúc xạ lớn hơn hay nhỏ hơn $20^\circ$?
2. Bước sóng ánh sáng đỏ trong chân không là 700 nm. Trong thủy tinh ($n=1{,}5$) bước sóng bằng bao nhiêu?

<details><summary>Đáp án</summary>

1. Đi ra môi trường LOÃNG hơn (n giảm) → bẻ XA pháp tuyến → góc khúc xạ **lớn hơn** $20^\circ$. Cụ thể: $1{,}5 \cdot \sin 20^\circ = 1 \cdot \sin\theta_2 \to \sin\theta_2 = 1{,}5 \cdot 0{,}342 = 0{,}513 \to \theta_2 \approx 30{,}9^\circ$.
2. $\lambda$ trong môi trường = $\lambda_{\text{chân không}} / n = 700/1{,}5 \approx 467 \text{ nm}$. (Tần số không đổi, chỉ bước sóng co lại theo n.)
...
- Snell: $n_1 \cdot \sin\theta_1 = n_2 \cdot \sin\theta_2$. Đi vào đậm → bẻ về pháp tuyến.
...
Khi ánh sáng đi từ môi trường **đậm hơn** sang **loãng hơn** (vd nước → không khí), nếu góc tới vượt một **góc tới hạn $\theta_c$**, ánh sáng **không xuyên qua** mà bị **phản xạ HOÀN TOÀN** vào lại môi trường đậm.

$$\sin\theta_c = \frac{n_2}{n_1} \quad (n_2 < n_1)$$

### 3.2. Ví dụ: nước → không khí

- $n_1 = 1{,}33$ (nước), $n_2 = 1{,}00$ (không khí).
- $\sin\theta_c = 1/1{,}33 = 0{,}752 \to \theta_c \approx 48{,}6^\circ$.
- Nếu nhìn lên từ dưới nước với góc $> 48{,}6^\circ$ so với pháp tuyến (= $< 41{,}4^\circ$ so với mặt nước), bạn KHÔNG thấy bầu trời mà chỉ thấy đáy bể phản chiếu.
...
- *"Tại góc tới hạn $\theta_c$ thì điều gì xảy ra?"* Tia khúc xạ đi đúng dọc mặt phân cách ($\theta_2 = 90^\circ$). Đặt $\sin 90^\circ = 1$ vào Snell: $n_1 \sin\theta_c = n_2 \cdot 1 \to \sin\theta_c = n_2/n_1$.
...
- **Áp dụng công thức $\sin\theta_c = n_2/n_1$ khi đi từ loãng sang đậm.** Vô nghĩa: khi đó $n_2 > n_1$ → $\sin\theta_c > 1$ (không tồn tại góc). Phản xạ toàn phần CHỈ xảy ra theo chiều đậm → loãng.
- **Lẫn $\theta_c$ với góc Brewster.** $\theta_c$ là ngưỡng phản xạ TOÀN PHẦN; góc Brewster là góc cho ánh sáng phản xạ phân cực hoàn toàn — hai khái niệm khác nhau.

🔁 **Dừng lại tự kiểm tra**

1. Ánh sáng đi từ kim cương ($n = 2{,}42$) ra không khí. Tính góc tới hạn $\theta_c$.
2. Một thợ lặn dưới nước ($n = 1{,}33$) chiếu đèn lên với góc $50^\circ$ so pháp tuyến. Ánh sáng có thoát ra không khí được không? ($\theta_c$ của nước $\approx 48{,}6^\circ$.)

<details><summary>Đáp án</summary>

1. $\sin\theta_c = 1/2{,}42 = 0{,}413 \to \theta_c \approx 24{,}4^\circ$. Góc tới hạn rất nhỏ → kim cương dễ "giữ" ánh sáng bên trong, tạo độ lấp lánh.
2. $50^\circ > 48{,}6^\circ = \theta_c$ → vượt góc tới hạn → **phản xạ toàn phần** → ánh sáng KHÔNG thoát ra, bị hắt ngược xuống nước.

</details>

### 📝 Tóm tắt mục 3

- Phản xạ toàn phần: từ đậm sang loãng, góc $> \theta_c$.
- $\sin\theta_c = n_2/n_1$.
...
**Bài 1**: Ánh sáng đi từ không khí ($n = 1$) vào nước ($n = 1{,}33$) với góc tới $30^\circ$. Tính góc khúc xạ.

**Bài 2**: Tốc độ ánh sáng trong thủy tinh $n = 1{,}5$ là bao nhiêu?

**Bài 3**: Tính $\theta_c$ cho ánh sáng đi từ thủy tinh ($n = 1{,}5$) sang không khí.
...
**Bài 1**: $1 \cdot \sin 30^\circ = 1{,}33 \cdot \sin\theta_2 \to \sin\theta_2 = 0{,}5/1{,}33 = 0{,}376 \to \theta_2 =$ **$22{,}1^\circ$** (bẻ về phía pháp tuyến).

**Bài 2**: $v = c/n = 3 \times 10^8 / 1{,}5 =$ **$2 \times 10^8 \text{ m/s}$**.

**Bài 3**: $\sin\theta_c = 1/1{,}5 = 0{,}667 \to \theta_c =$ **$41{,}8^\circ$**.

**Bài 4**: Kim cương có **$n = 2{,}42$, cao nhất** trong vật liệu phổ biến. Hệ quả:
- Góc tới hạn $\theta_c = \arcsin(1/2{,}42) = 24{,}4^\circ$ → rất nhỏ. Hầu như mọi tia vào đều bị phản xạ toàn phần khi gặp mặt sau → không thoát qua trừ ở góc rất hẹp. Ánh sáng bị "kẹt" bên trong, phản xạ nhiều lần.
- Tán sắc cao ($n$ thay đổi theo $\lambda$) → tách màu cầu vồng.
- Người thợ kim hoàn cắt mặt kim cương để tối đa hóa phản xạ tổng phần → "lấp lánh".

**Bài 5**: Khi nhìn từ trên gần như vuông góc, ta thấy mặt nước trong suốt. Nhưng khi nhìn từ góc xiên (xa) → góc tới lớn → vượt $\theta_c$ → phản xạ toàn phần → mặt nước "phản chiếu" bầu trời/cây cối → trông như gương.
...
3. **Snell**: $n_1 \cdot \sin\theta_1 = n_2 \cdot \sin\theta_2$. Đi vào đậm → bẻ về pháp tuyến.
4. **Phản xạ toàn phần**: từ đậm → loãng, $\theta > \theta_c$. $\sin\theta_c = n_{\text{loãng}}/n_{\text{đậm}}$.
