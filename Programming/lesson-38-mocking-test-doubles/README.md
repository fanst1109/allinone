# Lesson 38 — Mocking & Test Doubles
...
> 💡 **Mock/test double cho phép test logic mà không cần DB/API thật — và interface của Go làm việc này cực sạch.**

| Test double | Dùng thật ở đâu |
|-------------|-----------------|
| **Stub** | Trả giá trị cố định (vd `GetUser` trả user mẫu) |
| **Mock** | Kiểm tra hàm được gọi đúng (vd "gửi email đúng 1 lần") |
| **Fake** | Bản thật nhẹ (in-memory DB thay Postgres) |
| **httptest.Server** | Giả API ngoài để test HTTP client không gọi thật |
| **sqlmock** | Giả DB để test repo không cần Postgres chạy |

### 15.1. Ví dụ cụ thể — interface làm mock dễ

Service phụ thuộc `UserRepo` interface, không phụ thuộc Postgres cụ thể:

```go
type UserRepo interface { GetUser(id int) (*User, error) }

type fakeRepo struct{}  // test double, không cần DB
func (fakeRepo) GetUser(id int) (*User, error) { return &User{ID: id, Name: "test"}, nil }

func TestService(t *testing.T) {
	svc := NewService(fakeRepo{})  // tiêm fake
	// test logic svc mà không cần Postgres chạy
}
```

Vì `NewService` nhận **interface** (không phải `*PostgresRepo` cụ thể), test tiêm fake dễ dàng. Đây là **dependency injection** + interface — nền của code testable. Test chạy mili giây, không cần hạ tầng.

> ⚠ **Đừng mock quá đà.** Mock mọi thứ → test chỉ kiểm "code gọi đúng thứ tự hàm", không kiểm **hành vi thật** → pass nhưng production vẫn sai. Cân bằng: mock dependency **ngoài** (DB/API mạng, chậm/khó kiểm soát); **đừng** mock logic của chính mình. Với DB, nhiều team thích **fake in-memory** hoặc DB thật trong container (testcontainers) hơn sqlmock — gần thực tế hơn.

### 15.2. 📝 Tóm tắt mục 15

- Test double: **stub** (giá trị cố định), **mock** (kiểm gọi đúng), **fake** (bản nhẹ in-memory), httptest/sqlmock.
- **Interface + DI** làm mock sạch: service nhận interface → test tiêm fake, không cần hạ tầng.
- Đừng mock quá đà (chỉ kiểm gọi hàm ≠ kiểm hành vi); mock dependency ngoài, không mock logic mình.
