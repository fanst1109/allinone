...
5. Hiểu **collision**, vì sao phải chọn \`mod q\` nguyên tố lớn và base \`b\` tốt, và **double hashing**.
...
(thường \`m ≤ n\`), tìm **mọi** chỉ số \`i\` (\`0 ≤ i ≤ n-m\`) sao cho
...
> Tại **mỗi** vị trí \`i\`, ta khớp 4 ký tự \`'a'\` rồi mới lệch ở \`'b'\` → mỗi lần tốn ~m so sánh.
> Tổng \`≈ (n-m+1)·m = O(nm)\`. Với \`n=10^6, m=10^3\` → \`10^9\` phép so → vài giây tới phút.
...
- \`q\` (modulus): **số nguyên tố lớn**, ví dụ \`q = 1_000_000_007\`, để hash phân bố đều và tránh overflow.

> Vì sao tồn tại cách viết "đa thức" này? Vì nó cho ta công thức **cập nhật trượt O(1)** (mục 3.3).
> Một hàm băm bất kỳ (vd \`Σ s[i]\`) không trượt được rẻ vì mất thông tin vị trí — và tệ hơn, dễ
...
\`\`\`
→ \`"AB"\` (41) và \`"BA"\` (94) **khác hash** — polynomial hash phân biệt được thứ tự (khác hẳn \`Σ\`).
...
Trong đó \`b^(m-1) mod q\` được tính sẵn 1 lần (gọi là \`high\`).
...
> ⚠ **Lỗi thường gặp #1.** Quên trừ ký tự trái với **đúng hệ số** \`b^(m-1)\`. Nếu trừ \`T[i]·1\`
> thay vì \`T[i]·b^(m-1)\` thì hash hỏng hoàn toàn. Hệ số phải là số mũ cao nhất.
...
- **\`q\` nguyên tố lớn** (vd \`1_000_000_007\`, \`998_244_353\`): giảm xác suất collision còn ~\`1/q\`.
...
Dùng **hai cặp \`(b1,q1)\` và \`(b2,q2)\`** độc lập; chỉ coi là "hash khớp" khi **cả hai** hash trùng.
Xác suất collision đồng thời ≈ \`1/(q1·q2) ≈ 10^-18\` → có thể **bỏ verify** trong nhiều bài thi
...
một số trong \`[0,11)\` vì chỉ có 11 ô. Xác suất 2 chuỗi ngẫu nhiên trùng hash ≈ \`1/11 ≈ 9%\` —
quá cao. Tăng \`q\` lên \`10^9\` → xác suất ≈ \`10^-9\`, false positive gần như biến mất.
...
> lớn + \`b\` hợp lý để collision hiếm. Double hashing đẩy xác suất collision xuống ~\`1/q²\`.
...
> Rabin-Karp \`k\` lần → O(k·n). Mẹo: băm **tất cả** pattern vào một \`set\` (hash → list pattern),
...
> băm mọi substring dài \`L\`, kiểm tra có hash nào xuất hiện ≥ 2 lần (verify để chắc).
...
Tìm ma trận pattern \`P\` (\`r×c\`) trong ma trận text \`T\` (\`R×C\`). Cách Rabin-Karp 2D: băm **từng
hàng** bằng rolling hash → mỗi hàng thành 1 dãy số; rồi rolling hash **theo cột** trên các giá trị
hàng đó. Tìm khối khớp = 2 lớp rolling hash lồng nhau. Độ phức tạp ~\`O(R·C)\` trung bình.
...
| Multi-pattern RK (k pattern) | O(k·m) | **O(n + k·m)** | O(n·m + ...) | O(k·m) |
| Longest dup substring | — | O(n log n) | O(n² log n) | O(n) |
...
| 2 | **Overflow** khi tính \`s[i]·b^k\` | hash sai, kết quả loạn | lấy \`mod\` ở mọi phép nhân/cộng; dùng \`int64\` |
| 3 | **Hash âm** sau khi trừ ký tự trái | index/so sánh sai | \`((x % q) + q) % q\` |
| 4 | **Rolling update sai hệ số** (trừ \`T[i]·1\` thay vì \`T[i]·b^(m-1)\`) | hash trượt sai hoàn toàn | precompute \`high = b^(m-1) mod q\` |
| 5 | **Chọn \`q\` nhỏ / \`b\` nhỏ** | collision tràn lan → chậm | \`q\` nguyên tố ~10^9, \`b ≥ alphabet\` |
...
**Cách tiếp cận.** \`s\` lặp được từ substring dài \`d\` ⟺ \`d | n\` và \`s\` bằng \`n/d\` bản sao của \`s[0..d-1]\`.
...
1. Rolling hash **theo hàng**: với mỗi hàng của text, băm mọi cửa sổ ngang dài \`c\` → ma trận \`R×(C-c+1)\`
...
2. Rolling hash **theo cột** trên ma trận giá trị-hàng: coi mỗi cột là một "chuỗi" dài \`R\`, băm cửa sổ
   dọc dài \`r\`, so với hash của pattern (gồm \`r\` giá trị hàng). Khớp hash 2 lớp → verify khối \`r×c\`.

**Big-O:** O(R·C) trung bình (mỗi lớp rolling O(1)/bước), O(r·c) cho mỗi lần verify (hiếm).
...
\`\`\`
**Big-O:** O(n²) average (n độ dài × O(n)/độ dài). **Rủi ro collision:** hai substring khác nhau cùng
