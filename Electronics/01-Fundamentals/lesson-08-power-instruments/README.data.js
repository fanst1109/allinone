...
// Source: Electronics/01-Fundamentals/lesson-08-power-instruments/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 08 — Nguồn & Dụng cụ đo
...
- Duy trì điện áp V_nguồn = hằng số, bất kể dòng tải I bao nhiêu.
- Nội trở (internal resistance) R_nội = 0.
- Không tồn tại trong thực tế — chỉ là mô hình toán học để đơn giản bài toán.

**Nguồn điện áp thực tế:**
- Bên trong có một nội trở R_nội nối tiếp với nguồn lý tưởng.
- Khi tải kéo dòng I, có sụt áp trên R_nội: ΔV = I × R_nội.
- Điện áp tại đầu ra (terminal voltage): **V_tải = V_nguồn − I × R_nội**.
...
**Ví dụ 1 — Pin AAA 1.5 V, R_nội = 1.5 Ω, nối với đèn LED R_tải = 68 Ω:**
- Dòng tải: I = V_nguồn / (R_nội + R_tải) = 1.5 / (1.5 + 68) = 1.5 / 69.5 ≈ 0.0216 A = 21.6 mA.
- Điện áp tại đèn: V_tải = 1.5 − 0.0216 × 1.5 = 1.5 − 0.032 ≈ **1.47 V**.
- Sụt áp nhỏ vì dòng nhỏ — pin còn mới.

**Ví dụ 2 — Pin cũ R_nội tăng lên 15 Ω, cùng tải 68 Ω:**
- I = 1.5 / (15 + 68) = 1.5 / 83 ≈ 18.1 mA.
- V_tải = 1.5 − 0.0181 × 15 = 1.5 − 0.27 ≈ **1.23 V**.
- Đèn tối hơn dù pin "còn 1.5 V" khi đo không tải!

**Ví dụ 3 — Acquy 12 V xe máy, R_nội = 0.1 Ω, khởi động máy kéo 100 A:**
- V_tải = 12 − 100 × 0.1 = 12 − 10 = **2 V** (sụt áp cực lớn — đèn phải tắt trong lúc đề!).
- Sau khi máy nổ, dòng giảm còn ~2 A → V_tải = 12 − 0.2 ≈ **11.8 V** (bình thường).

**Ví dụ 4 — Nguồn 5 V (USB), R_nội = 0.5 Ω, tải 5 Ω:**
- I = 5 / (0.5 + 5) = 5 / 5.5 ≈ 0.909 A.
- V_tải = 5 − 0.909 × 0.5 = 5 − 0.45 ≈ **4.55 V**.
...
Vì đồng hồ có điện trở nội rất cao (≥ 1 MΩ) → dòng qua R_nội ≈ 0 → không sụt áp. Pin cũ chỉ "lộ mặt" khi nối tải thực (dòng lớn). Muốn biết tình trạng thật của pin: đo điện áp khi đang nối tải (load test).

> *"R_nội lấy ở đâu ra? Sao không đo được trực tiếp?"*

R_nội là điện trở của hóa chất điện phân, dây dẫn trong pin, mối hàn... Không đo trực tiếp vì pin đang có EMF. Đo gián tiếp: V_hở − V_tải = I × R_nội → R_nội = (V_hở − V_tải) / I.

⚠ **Lỗi thường gặp:**
- Nhầm "pin hết" với "pin cũ": pin cũ vẫn đủ điện áp khi đo không tải nhưng sụt áp khi tải nặng.
- Chọn nguồn 5 V nhưng cáp USB dài (R_dây cao) → V_tải < 4.75 V → Raspberry Pi reset.
...
📝 **Tóm tắt mục 1:**
- Nguồn áp lý tưởng: R_nội = 0, V_out = hằng số. Thực tế: R_nội > 0, V_tải = V_nguồn − I·R_nội.
- Sụt áp = I × R_nội — tải nặng (I lớn) → V_tải giảm đáng kể.
...
**Vì sao dùng mAh thay vì Joule?** Vì Joule = V × A × giờ, nhưng điện áp pin thay đổi trong suốt quá trình xả. Dùng mAh (chỉ đếm điện tích, không kể V) cho phép so sánh đơn giản hơn. Năng lượng thực tế = Capacity (Ah) × V_trung_bình.

### 2.2. Công thức tính thời gian xả

\`\`\`
t = C / I_tải          (đơn giản, không có hiệu suất)
t = C × η / I_tải      (thực tế, η = hiệu suất 80–95%)
\`\`\`

trong đó:
- C = dung lượng pin (mAh).
- I_tải = dòng tiêu thụ của thiết bị (mA).
- η = hiệu suất xả (loss từ nhiệt, hóa học... thường 80–90% với Li-ion, 60–80% với pin carbon).
- t = thời gian (giờ).

### 2.3. Ví dụ số

**Ví dụ 1 — Điện thoại pin 4000 mAh, màn hình sáng tiêu thụ 300 mA, η = 0.90:**
- t = 4000 × 0.90 / 300 = 3600 / 300 = **12 giờ**.

**Ví dụ 2 — Pin dự phòng 10000 mAh sạc điện thoại 4000 mAh:**
- Hiệu suất sạc thực ≈ 80%: điện tích thực sự truyền = 10000 × 0.80 = 8000 mAh.
- Sạc được 8000 / 4000 = **2 lần** đầy (thực tế ~1.8–2 lần do nhiều loss khác).

**Ví dụ 3 — Arduino Uno (80 mA) + cảm biến (20 mA) tổng 100 mA, pin 18650 2600 mAh, η = 0.85:**
- t = 2600 × 0.85 / 100 = 2210 / 100 = **22.1 giờ**.

**Ví dụ 4 — Xe máy điện pin 48V 20Ah (= 20000 mAh), motor kéo 15A trung bình:**
- t = 20000 / 15000 mA ≈ **1.33 giờ** ≈ 80 phút.
- Năng lượng tổng = 48 × 20 = **960 Wh ≈ 1 kWh**.

🔁 **Dừng lại tự kiểm tra:**
Pin 9V (dạng khối vuông) thường có dung lượng 500 mAh. Arduino Nano tiêu thụ 20 mA. Tính thời gian sử dụng (η = 0.85).

<details>
<summary>Đáp án</summary>

t = 500 × 0.85 / 20 = 425 / 20 = **21.25 giờ** ≈ 21 giờ 15 phút. Thực tế ngắn hơn vì còn có tổn hao từ bộ chuyển đổi 9V→5V.
</details>

📝 **Tóm tắt mục 2:**
- Capacity (mAh) = I × t. Thời gian xả: t = C × η / I_tải.
- Dung lượng mAh đo điện tích, không đo năng lượng (cần nhân thêm V để ra Wh).
- η = 80–95% với Li-ion, 60–80% với pin carbon thông thường.
...
**Ảnh hưởng nội trở đồng hồ**: Nếu đo điện áp trên một điện trở 1 MΩ bằng đồng hồ có R_input = 1 MΩ → nội trở đồng hồ song song tạo thành 500 kΩ → kết quả đo sai 50%! Quy tắc thực tế: R_mạch đo phải < 1/10 R_input của đồng hồ để sai số < 10%.
...
**Vì sao mắc sai là cháy cầu chì?** Đồng hồ đo dòng có điện trở nội rất thấp (≈ 0.1–2 Ω). Nếu mắc song song với nguồn 5V → R_nội đồng hồ rất nhỏ → I = 5V / 0.1Ω = **50 A** → cầu chì cháy ngay lập tức.
...
📝 **Tóm tắt mục 3:**
- Đo V: mắc song song, R_input đồng hồ phải lớn hơn R_mạch ít nhất 10×.
...
- Ví dụ V/div = 2 V → sóng chiếm 4 ô dọc → biên độ từ đỉnh đến gốc = 4 × 2 = 8 V.
- Biên độ đỉnh-đỉnh V_pp = số ô chiều cao × V/div.

**Time/div (giây per division):** mỗi ô ngang đại diện bao nhiêu giây (hoặc ms, µs).
- Ví dụ time/div = 1 ms → 1 chu kỳ chiếm 5 ô → T = 5 × 1 ms = 5 ms → f = 1/5 ms = **200 Hz**.
...
- V_pp = 6 × 0.5 = **3 V** → Biên độ đỉnh A = 3/2 = **1.5 V**.
- T = 4 × 0.2 ms = **0.8 ms** → f = 1/0.8 ms = **1250 Hz = 1.25 kHz**.
- V_RMS (sin) = A / √2 = 1.5 / 1.414 ≈ **1.06 V**.

**Ví dụ 2:** V/div = 1 V, time/div = 50 µs. Sóng vuông chiếm 3 ô dọc (đỉnh-đỉnh), 8 ô ngang/chu kỳ.
- V_pp = 3 × 1 = **3 V** (sóng vuông đi từ 0 đến 3V).
- T = 8 × 50 µs = **400 µs** → f = 1/400 µs = **2500 Hz = 2.5 kHz**.

**Ví dụ 3:** V/div = 5 V, time/div = 10 ms. Sóng sin chiếm 4 ô dọc, 2 ô ngang/chu kỳ.
- V_pp = 4 × 5 = **20 V** → A = 10 V.
- T = 2 × 10 ms = **20 ms** → f = **50 Hz** (đây là tần số điện lưới AC Việt Nam!).

**Ví dụ 4:** V/div = 200 mV, time/div = 1 µs. Sóng tam giác chiếm 5 ô, 6 ô/chu kỳ.
- V_pp = 5 × 200 mV = **1000 mV = 1 V**.
- T = 6 × 1 µs = **6 µs** → f = 1/6 µs ≈ **167 kHz**.

🔁 **Dừng lại tự kiểm tra:**
Scope hiển thị: V/div = 2 V, time/div = 5 ms. Sóng sin chiếm 5 ô dọc (đỉnh-đỉnh), 4 ô ngang/chu kỳ. Tính V_pp, A, T, f, V_RMS.

<details>
<summary>Đáp án</summary>

- V_pp = 5 × 2 = **10 V** → A = 5 V.
- T = 4 × 5 ms = **20 ms** → f = 1/20 ms = **50 Hz**.
- V_RMS = 5 / √2 ≈ **3.54 V**.
</details>
...
**Trigger**: scope bắt đầu vẽ mỗi chu kỳ tại đúng cùng một điểm của sóng (thường là lúc sóng vượt qua ngưỡng điện áp V_trig theo chiều lên hoặc xuống). Kết quả: sóng "đứng yên" trên màn.
...
- V_tải ≠ V_nguồn khi tải nặng — ảnh hưởng kết quả đo, hoạt động mạch.
- Bộ lọc RC: nếu nguồn có R_nội đáng kể → tần số cắt f_c = 1/(2π(R_nội + R)C) thay đổi.
- Mạch phân áp: điện trở tải (kể cả R_input của tầng sau) thay đổi tỉ lệ phân áp (xem [Lesson 03](../lesson-03-resistors-divider/)).

### 6.3. Giao thoa giữa các khái niệm — 4 ví dụ kết hợp

**Ví dụ kết hợp 1 — Pin cấp cho mạch lọc RC:**
Pin 9V, R_nội = 2 Ω, mạch lọc thấp RC với R = 1 kΩ, C = 100 nF. Tải sau lọc = 10 kΩ.
- f_c (không có nội trở) = 1 / (2π × 1000 × 100×10⁻⁹) ≈ 1592 Hz.
- R_nội nhỏ hơn R rất nhiều (2 Ω << 1000 Ω) → ảnh hưởng không đáng kể.
- Tải 10 kΩ song song với C: tổng trở tải thay đổi đáp ứng tần số cao.

**Ví dụ kết hợp 2 — Đo dòng qua cuộn cảm bằng scope:**
Không thể cắm ampere kế vào cuộn cảm (I không nhảy bậc — nếu cắt mạch và nối lại sẽ tạo điện áp cảm ứng rất cao). Cách đo gián tiếp: đặt điện trở nhỏ R_shunt = 1 Ω nối tiếp, đo V qua R_shunt bằng scope → I = V/R_shunt.

**Ví dụ kết hợp 3 — Xác định f_c của bộ lọc bằng scope:**
Dùng scope đo V_in và V_out của bộ lọc RC khi quét tần số. Tại f_c: |V_out| = |V_in| / √2 ≈ 0.707 × V_in, đồng thời pha lệch 45°. Đây là cách thực tế nhất để xác minh bộ lọc hoạt động đúng.

**Ví dụ kết hợp 4 — Dùng KVL kiểm tra sụt áp nội trở:**
V_nguồn (đo không tải) − V_tải (đo khi có tải) = I × R_nội. Dùng 2 que đồng hồ đo 2 điện áp → KVL cho vòng nguồn → suy ra R_nội ngay trên bench.
...
**Bài 2 — Thời gian pin:** Pin Li-ion 3000 mAh, η = 0.90. Thiết bị gồm: vi điều khiển 30 mA, màn hình LCD 60 mA, 3 LED mỗi cái 20 mA. Tính thời gian hoạt động.
...
**Bài 6 — Nguồn lý tưởng vs thực:** Nguồn điện áp 12V có nội trở 0.5 Ω. Nối với 2 tải song song: R₁ = 6 Ω và R₂ = 12 Ω. Tính điện áp tại đầu ra nguồn (terminal voltage).

**Bài 7 — Phân tích an toàn:** Người (R_người = 5 kΩ khi tay ướt) chạm vào điểm 24 VDC. Tính dòng qua người. So sánh với ngưỡng nguy hiểm và kết luận.

### Lời giải chi tiết

**Bài 1:**

(a) Dòng tải: I = V_nguồn / (R_nội + R_tải) = 1.5 / (2 + 10) = 1.5 / 12 = **0.125 A = 125 mA**.

(b) V_tải = V_nguồn − I × R_nội = 1.5 − 0.125 × 2 = 1.5 − 0.25 = **1.25 V**.

Kiểm tra: V_tải = I × R_tải = 0.125 × 10 = 1.25 V ✓.

(c) P_nội = I² × R_nội = 0.125² × 2 = 0.015625 × 2 = **0.03125 W ≈ 31.25 mW**.

(d) P_tải = I² × R_tải = 0.125² × 10 = **0.156 W = 156.25 mW**.
P_tổng = I × V_nguồn = 0.125 × 1.5 = 0.1875 W.
Hiệu suất η = P_tải / P_tổng = 156.25 / 187.5 = **83.3%**.

Hoặc η = R_tải / (R_tải + R_nội) = 10 / 12 = 83.3% (công thức nhanh khi R_tải và R_nội đã biết).

---

**Bài 2:**

Tổng dòng tiêu thụ: I_tổng = 30 + 60 + 3 × 20 = 30 + 60 + 60 = **150 mA**.

Thời gian: t = C × η / I = 3000 × 0.90 / 150 = 2700 / 150 = **18 giờ**.

---

**Bài 3:**

- V_pp = số ô dọc × V/div = 6 × 0.5 = **3 V**.
- Biên độ A = V_pp / 2 = 3 / 2 = **1.5 V**.
- Chu kỳ T = số ô ngang × time/div = 4 × 0.5 ms = **2 ms**.
- Tần số f = 1 / T = 1 / 2×10⁻³ = **500 Hz**.
- V_RMS (sóng sin) = A / √2 = 1.5 / 1.4142 ≈ **1.061 V**.
...
- Đo V_tải bằng đồng hồ chế độ V, mắc song song với R = 470 Ω.
- Tính dòng: I = V_tải / R_tải.

Tính lý thuyết: I = 12 / 470 ≈ 0.02553 A = **25.5 mA**. V_tải ≈ 12 V (giả sử nguồn lý tưởng).

Lưu ý: R_input đồng hồ (thường 1–10 MΩ) >> 470 Ω nên sai số không đáng kể (< 0.05%).

---

**Bài 5:**

Điện áp sụt trên cáp: ΔV = I × R_cáp = 1.5 × 0.8 = **1.2 V**.

V_thiết_bị = V_nguồn − ΔV = 5 − 1.2 = **3.8 V**.
...
Tải song song R₁//R₂ = (6 × 12) / (6 + 12) = 72 / 18 = **4 Ω**.

Dòng tổng: I = V_nguồn / (R_nội + R_tải_tổng) = 12 / (0.5 + 4) = 12 / 4.5 ≈ 2.667 A.

Terminal voltage: V_terminal = V_nguồn − I × R_nội = 12 − 2.667 × 0.5 = 12 − 1.333 = **10.67 V**.

Kiểm tra: V = I × R_tải_tổng = 2.667 × 4 = 10.67 V ✓.

---

**Bài 7:**

Dòng qua người: I = V / R_người = 24 / 5000 = **0.0048 A = 4.8 mA**.

Theo bảng ngưỡng: 4.8 mA nằm trong khoảng 1–5 mA → **cảm nhận nhẹ, ngứa ran, không nguy hiểm tính mạng**.

Nhưng lưu ý:
- 24 VDC với tay khô (R_người = 50 kΩ): I = 24/50000 = 0.48 mA → gần như không cảm nhận.
...
1. **Nguồn thực tế có nội trở** R_nội: V_tải = V_nguồn − I·R_nội. Tải nặng → sụt áp lớn.
2. **Dung lượng pin (mAh)**: thời gian xả t = C·η / I_tải. Đo không tải không phản ánh đúng sức pin.
3. **Multimeter**: đo V song song, đo I nối tiếp (nhầm → cháy cầu chì), đo R phải ngắt nguồn xả tụ.
4. **Oscilloscope**: V_pp = ô_dọc × V/div, T = ô_ngang × time/div, f = 1/T. Thấy dạng sóng mà multimeter không thấy.
