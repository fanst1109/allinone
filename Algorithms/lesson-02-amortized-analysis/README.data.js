...
// Source: Algorithms/lesson-02-amortized-analysis/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 02 — Phân tích Amortized (Amortized Analysis)
...
- Mảng có sức chứa cố định (capacity). Thêm phần tử vào chỗ trống: **O(1)**.
- Khi đầy, phải cấp phát mảng mới lớn hơn và **copy** mọi phần tử cũ sang: **O(n)**.
...
> ⚠ **Lỗi thường gặp.** "Append worst-case O(n) ⇒ thêm n phần tử là O(n²)." **Sai.** Như sẽ chứng minh, thêm n phần tử chỉ tốn O(n) tổng cộng, tức **trung bình O(1) mỗi append**. Nhân worst-case đơn lẻ với n là cách ước lượng **quá bi quan** khi các thao tác đắt hiếm và "bù trừ" cho nhau.
...
- *"Amortized có phải là chặn chắc chắn không, hay chỉ kỳ vọng?"* — Là chặn **chắc chắn** (deterministic) cho tổng chuỗi: tổng thực ≤ n × (chi phí amortized). Không có yếu tố may rủi.
...
1. **Quicksort (average case):** Với pivot chọn ngẫu nhiên, **kỳ vọng** O(n log n). Nhưng tồn tại input/chuỗi pivot xui khiến nó thành O(n²). Đây là average theo **xác suất** — không phải amortized.
...
1. **Một thao tác đơn lẻ** có thể chậm hơn O(1) (vd lần grow), nhưng **tổng chuỗi n thao tác luôn ≤ O(n)** — không có chuỗi nào phá vỡ chặn này. Amortized là chặn chắc chắn trên tổng.
...
Công thức: \`amortized = T(n) / n\`, trong đó \`T(n)\` là chặn trên cho tổng chi phí thực của chuỗi xấu nhất gồm n thao tác.
...
> tổng chi phí amortized ≥ tổng chi phí thực ⟹ chi phí amortized là **chặn trên hợp lệ**.

Lý do: tổng amortized = tổng thực + (credit còn dư) ≥ tổng thực, vì credit dư ≥ 0.
...
💡 **Trực giác.** Tổng quát hóa accounting. Định nghĩa một **hàm thế (potential function)** \`Φ\` ánh xạ trạng thái cấu trúc dữ liệu → một số thực (giống "thế năng" trong vật lý — năng lượng tích trữ sẵn để chi sau).

Chi phí amortized của thao tác thứ i:

\`\`\`
amortized_i = actual_i + (Φ_i − Φ_{i−1})
            = actual_i + ΔΦ
\`\`\`

trong đó \`Φ_i\` là thế **sau** thao tác i, \`Φ_{i−1}\` là thế **trước** nó.

Cộng dồn cả chuỗi (telescoping — các số hạng Φ triệt tiêu lẫn nhau):

\`\`\`
Σ amortized_i = Σ actual_i + (Φ_n − Φ_0)
\`\`\`

**Điều kiện then chốt:** chọn \`Φ\` sao cho \`Φ_0 = 0\` và \`Φ_i ≥ 0\` với mọi i. Khi đó \`Φ_n − Φ_0 ≥ 0\`, nên:

\`\`\`
Σ actual_i ≤ Σ amortized_i
\`\`\`

⟹ tổng amortized là chặn trên cho tổng thực. Đúng tinh thần của accounting, nhưng \`Φ\` cho phép định nghĩa "credit toàn cục" linh hoạt hơn.
...
- **Accounting:** gán giá amortized cho từng op; credit (≥ 0 luôn) gắn vào phần tử để trả op đắt sau.
- **Potential:** \`amortized = actual + ΔΦ\`; chọn \`Φ ≥ 0\`, \`Φ_0 = 0\` ⟹ tổng amortized ≥ tổng thực.
...
- Append vào chỗ còn trống (\`length < capacity\`): chi phí **1** (ghi 1 ô).
- Append khi đầy (\`length = capacity\`): chi phí **1 + copy length phần tử** ≈ **length + 1** (đắt).
...
Đây là tổng cấp số nhân \`1 + 2 + 4 + 8 = 2⁴ − 1 = 15 < 16 = n\`.

**Tổng chi phí** = (16 lần ghi phần tử mới) + (15 lần copy) = \`16 + 15 = 31 ≈ 2n\`.

> 💡 **Phát hiện then chốt.** Tổng copy \`1+2+4+...\` là cấp số nhân với công bội 2. Tổng của nó **luôn < số hạng cuối × 2 < 2n**. Đây là lý do toán học khiến copy "không bao giờ kịp đắt": dù mỗi cú grow tốn gấp đôi cú trước, nhưng cú trước đó đã "tích lũy" đủ thao tác rẻ để trả.
...
(Tổng cấp số nhân \`1 + 2 + ... + 2^k = 2^{k+1} − 1 < 2·2^k ≤ 2n\`.)

Vậy amortized cost = \`T(n) / n ≤ 3n / n = 3 = O(1)\`. ∎

Kiểm tra với n = 16: \`T(16) = 31\`, amortized = \`31/16 ≈ 1.94\`. Hằng số, không tăng theo n. ✓
...
\`m/2\` phần tử mới × 2 credit = \`m\` credit. Vừa đủ trả cho \`m\` phần tử cần copy. Bất biến "credit ≥ 0" được giữ. ∎
...
Credit luôn ≥ 0 ✓. Tổng amortized (3 mỗi op) ≥ tổng actual ⟹ amortized O(1). ∎
...
Kiểm tra điều kiện:
- **Φ ≥ 0?** Ngay sau một cú grow, \`length = capacity/2\` cũ... Thực ra ngay sau grow \`capacity\` mới = 2×length cũ, length = capacity_cũ; khi mảng ít nhất nửa đầy thì \`2·length ≥ capacity\`. Mảng động luôn duy trì \`length ≥ capacity/2\` ngay sau mỗi grow, nên \`Φ = 2·length − capacity ≥ 0\`. ✓ (Tại trạng thái đầy \`length = capacity\`: \`Φ = 2c − c = c ≥ 0\`.)
- **Φ_0 = 0?** Mảng rỗng \`length = 0, capacity = 0\`: \`Φ = 0\`. ✓

Tính amortized cho hai trường hợp:

**(a) Append không grow** (\`length < capacity\`): actual = 1. length tăng 1, capacity không đổi.
\`\`\`
ΔΦ = 2·(length+1) − capacity − (2·length − capacity) = 2
amortized = actual + ΔΦ = 1 + 2 = 3 = O(1)  ✓
\`\`\`

**(b) Append có grow** (\`length = capacity = c\`): actual = \`c + 1\` (copy c + ghi 1). Sau grow: length' = c+1, capacity' = 2c.
\`\`\`
Φ_trước = 2c − c = c
Φ_sau  = 2(c+1) − 2c = 2
ΔΦ = 2 − c
amortized = actual + ΔΦ = (c + 1) + (2 − c) = 3 = O(1)  ✓
\`\`\`

Cả hai trường hợp đều cho amortized = **3 = O(1)**. ∎

> 💡 **Vẻ đẹp của potential method.** Cú grow đắt \`O(c)\` được "hấp thụ" hoàn hảo: thế Φ sụt từ \`c\` xuống \`2\`, và \`ΔΦ = 2 − c\` âm vừa đủ để triệt tiêu \`c\` trong actual. Φ chính là "kho credit" đã tích từ các append rẻ trước đó.
...
\`\`\`
0 + 1 + 2 + 3 + ... + (n−1) = n(n−1)/2 = Θ(n²)
\`\`\`
Amortized = \`Θ(n²)/n = Θ(n)\` — **tệ!** Thêm n phần tử mất O(n²).
...
\`\`\`
k + 2k + 3k + ... ≈ k · (n/k)²/2 = n²/(2k) = Θ(n²)
\`\`\`
Vẫn **Θ(n²)** với mọi hằng k cố định — chỉ giảm hằng số, không đổi bậc. Amortized vẫn O(n).

**Chiến lược "×c" (nhân capacity với hằng c > 1, vd ×2, ×1.5):** Tổng copy là cấp số nhân:
\`\`\`
1 + c + c² + ... ≈ c^{log_c n} · c/(c−1) = n · c/(c−1) = Θ(n)
\`\`\`
Amortized = \`Θ(n)/n = Θ(1)\` — **O(1)!** Với mọi c > 1.

> ⚠ **Lỗi thường gặp.** Nghĩ "tăng thêm 100 mỗi lần là đủ nhanh vì 100 lớn". **Sai** — bất kỳ chiến lược **cộng hằng** nào (+1, +100, +1000) đều cho amortized **O(n)** (chỉ khác hằng số). Chỉ **nhân theo hệ số** (growth factor > 1) mới cho amortized **O(1)**. Bậc của thuật toán quyết định bởi *tỉ lệ tăng*, không phải *bước nhảy tuyệt đối*.
...
Số liệu khớp chính xác bảng walk-through Mục 4.2: tổng copy 15, tổng chi phí 31, amortized ≈ 1.94 (hằng số). ✓
...
1. Capacity đi 1→2→4→...→2048. Grow tại append thứ 2, 3, 5, 9, ..., 1025 (khi vừa đầy 2^k). Số lần grow = số lũy thừa của 2 từ 1 đến 1024 = **11 lần** (capacity cuối 2048 ≥ 1025).
2. Tổng copy = \`1 + 2 + 4 + ... + 1024 = 2¹¹ − 1 = 2047 < 2·1025\`. Vẫn ~2n. Amortized vẫn O(1). ✓

</details>

📝 **Tóm tắt Mục 4.**
- Dynamic array grow ×2: append amortized **O(1)** dù worst-case 1 op là O(n).
- Tổng copy \`1+2+4+... < 2n\` (cấp số nhân) ⟹ tổng chi phí O(n).
- Chứng minh được bằng cả aggregate (\`T(n)/n ≤ 3\`), accounting (3 credit/op), potential (\`Φ = 2·length − capacity\`).
...
- \`MultiPop(k)\`: pop \`min(k, size)\` phần tử — chi phí **min(k, size)** (đắt khi k lớn!).

Nhìn riêng lẻ, \`MultiPop(k)\` có thể tốn O(n) (pop sạch stack n phần tử). Nếu nhân worst-case với n thao tác, ta ngây thơ kết luận O(n²). Nhưng amortized thực ra là **O(1)** mỗi thao tác.
...
Credit không bao giờ âm (mỗi phần tử trên stack mang đúng 1 credit chưa tiêu). Tổng amortized = 2 × (số push) ≥ T(n). ⟹ O(1). ∎

**Potential:** Chọn \`Φ = số phần tử hiện có trong stack\`. \`Φ ≥ 0\`, \`Φ_0 = 0\`. ✓
- \`Push\`: actual 1, ΔΦ = +1 ⟹ amortized = 1 + 1 = **2**.
- \`Pop\`: actual 1, ΔΦ = −1 ⟹ amortized = 1 − 1 = **0**.
- \`MultiPop(k)\`, pop \`j = min(k,size)\` phần tử: actual = j, ΔΦ = −j ⟹ amortized = j − j = **0**.

Mọi op amortized ≤ 2 = O(1). ∎

### 5.3 Walk-through bằng số thật

Chuỗi: \`Push 1, Push 2, Push 3, Push 4, MultiPop(3), Push 5, Pop\`.

| Thao tác | actual | size sau | Φ sau | amortized = actual + ΔΦ |
|---|:--:|:--:|:--:|:--:|
| Push 1 | 1 | 1 | 1 | 1 + 1 = 2 |
| Push 2 | 1 | 2 | 2 | 1 + 1 = 2 |
| Push 3 | 1 | 3 | 3 | 1 + 1 = 2 |
...
Tổng actual = \`1+1+1+1+3+1+1 = 9\`. Tổng amortized = \`2+2+2+2+0+2+0 = 10 ≥ 9\` ✓. Trung bình amortized = \`10/7 ≈ 1.43 ≤ 2\`. MultiPop(3) "đắt" actual = 3 nhưng amortized = 0 vì nó tiêu credit đã tích từ 3 push trước. ✓
...
📝 **Tóm tắt Mục 5.**
- \`MultiPop(k)\` đắt O(n) một lần, nhưng amortized **O(1)** vì tổng pop ≤ tổng push.
- Trực giác: mỗi phần tử push 1 lần, pop tối đa 1 lần.
- \`Φ = số phần tử trong stack\`: push amortized 2, pop/multipop amortized 0.
...
Worst-case một increment lật cả k bit (vd \`0111 → 1000\`). Nhưng amortized chỉ **O(1)** (chính xác là 2 flip/increment).
...
**Tổng flip cho 16 increment = 31 < 2·16 = 32.** Amortized = \`31/16 ≈ 1.94 < 2\`. ✓

> 💡 **Vì sao tổng = ~2n?** Đếm theo bit thay vì theo increment:
> - **bit0** lật mỗi increment ⟹ n lần.
> - **bit1** lật mỗi 2 increment ⟹ n/2 lần.
> - **bit2** lật mỗi 4 increment ⟹ n/4 lần.
> - …
>
> Tổng flip = \`n + n/2 + n/4 + n/8 + ... = n·(1 + ½ + ¼ + ...) < 2n\`. Lại là cấp số nhân hội tụ — đúng "linh hồn" của amortized.

Kiểm tra n=16: \`16 + 8 + 4 + 2 + 1 = 31\`. Khớp tổng cột walk-through. ✓

### 6.3 Chứng minh amortized O(1) bằng potential method

Chọn \`Φ = số bit 1 trong bộ đếm\`. \`Φ ≥ 0\`, \`Φ_0 = 0\` (đếm bắt đầu từ 0). ✓

Xét một increment lật \`t\` bit 1 ở đuôi thành 0 (carry), rồi set 1 bit 0 thành 1.
- **actual** = \`t + 1\` (lật t bit thành 0, lật 1 bit thành 1).
- Số bit 1 thay đổi: mất \`t\` bit 1, thêm \`1\` bit 1 ⟹ \`ΔΦ = 1 − t\`.
- **amortized** = actual + ΔΦ = \`(t + 1) + (1 − t) = 2 = O(1)\`. ∎

> Mỗi increment amortized đúng **2 flip**, độc lập với t (số carry). Cú increment xui xẻo \`0111→1000\` có actual = 4 nhưng amortized vẫn 2: ΔΦ = \`1 − 3 = −2\` (mất 3 bit 1, được 1 bit 1) triệt tiêu 2 đơn vị actual dư.
...
Khớp chính xác bảng 6.2: tổng flip 31, amortized ≈ 1.94. ✓
...
2. Tổng = \`32 + 16 + 8 + 4 + 2 + 1 = 63 = 2·32 − 1 < 64\`. Amortized \`63/32 ≈ 1.97 < 2\`. ✓ (Tổng flip cho n increment luôn là \`2n − (số bit 1 trong n)\`, ở đây \`2·32 − 1 = 63\` vì 32 = \`100000\` có 1 bit.)

</details>

📝 **Tóm tắt Mục 6.**
- Binary counter increment: worst-case 1 op lật k bit, nhưng amortized **2 flip = O(1)**.
- Tổng flip n increment \`= n + n/2 + n/4 + ... < 2n\` (cấp số nhân).
- \`Φ = số bit 1\`: mỗi increment amortized = actual + ΔΦ = 2.
...
Hash table khi load factor vượt ngưỡng (vd 0.75) sẽ **rehash**: cấp bảng mới gấp đôi số bucket và đặt lại mọi phần tử — một thao tác O(n). Nhưng vì cũng theo chiến lược **nhân đôi**, chi phí rehash phân bổ amortized **O(1)** mỗi insert. (Lookup vẫn là *average* O(1) — chú ý đây là khái niệm khác, xem Mục 2.)

### 7.3 Splay tree

Splay tree không cân bằng cứng như AVL/Red-Black, nhưng mỗi thao tác (find/insert/delete) "splay" node vừa truy cập lên gốc. Một thao tác đơn lẻ có thể O(n), nhưng amortized **O(log n)** — chứng minh bằng potential method tinh vi (Φ dựa trên kích thước cây con).

### 7.4 Union-Find với path compression (tease Tier 5)

\`union-find\` (disjoint set) với path compression + union by rank: một dãy m thao tác trên n phần tử tốn tổng \`O(m · α(n))\`, với α là hàm Ackermann nghịch — ≤ 4 trên mọi giá trị thực tế. Đây là một trong những kết quả amortized đẹp nhất ngành. Sẽ học kỹ ở **Tier 5 (đồ thị)** khi dùng union-find cho Kruskal MST.

📝 **Tóm tắt Mục 7.**
- Go slice append, hash table resize: amortized O(1) nhờ chiến lược nhân đôi.
- Splay tree: amortized O(log n) (potential method).
- Union-Find path compression: amortized ~O(α(n)) ≈ O(1) — gặp lại ở Tier 5.
...
2. **Potential cho binary counter.** Dùng \`Φ = số bit 1\` chứng minh increment amortized = 2. Kiểm chứng cho cú \`0111 → 1000\` (actual và amortized).
...
4. **Multipop amortized.** Cho chuỗi: 50 lần \`Push\`, rồi 1 lần \`MultiPop(100)\`, rồi 30 \`Push\`, rồi \`MultiPop(40)\`. Tính tổng chi phí thực và amortized trung bình. Chứng minh chặn ≤ 2 bằng potential \`Φ = size\`.
...
6. **Phản ví dụ amortized O(1) nhưng 1 op O(n).** Đưa một chuỗi append cụ thể vào dynamic array (cap đầu = 1) và chỉ ra **một** thao tác có chi phí thực ≥ 1000, dù toàn chuỗi vẫn amortized O(1).

7. **(Mở rộng) Φ phải không âm.** Giải thích vì sao điều kiện \`Φ_i ≥ 0\` với mọi i là **bắt buộc** để potential method cho chặn trên hợp lệ. Cho ví dụ Φ vi phạm điều kiện dẫn tới kết luận sai.
...
**Vì sao credit không âm:** Xét cú grow từ cap \`m\` → \`2m\`, phải copy \`m\` phần tử. Kể từ lần grow trước (cap từ \`m/2\` → \`m\`), đã có đúng \`m/2\` phần tử **mới** được append (length đi từ \`m/2\` lên \`m\`). Mỗi phần tử mới mang **2 credit** chưa tiêu ⟹ \`m/2 × 2 = m\` credit khả dụng — vừa đủ trả \`m\` copy. Các phần tử cũ hơn đã tiêu credit ở lần grow trước, nhưng không sao vì chỉ cần credit của phần tử mới là đủ trả toàn bộ copy.

Bất biến: ngay trước mỗi grow, credit khả dụng ≥ số copy cần. Nên credit ngân hàng ≥ 0 mọi lúc.

**Kết luận:** Tổng amortized = \`3n\` ≥ tổng thực ⟹ amortized = \`3n/n = 3 = O(1)\`. **Độ phức tạp:** amortized O(1)/op, tổng O(n). ∎

### Bài 2 — Potential cho binary counter

\`Φ = số bit 1\`, \`Φ_0 = 0\`, \`Φ ≥ 0\`. ✓

Increment lật \`t\` bit 1 đuôi → 0, set 1 bit 0 → 1:
- actual = \`t + 1\`.
- ΔΦ = \`(1) − (t)\` = \`1 − t\` (thêm 1 bit 1, mất t bit 1).
- amortized = \`(t+1) + (1−t) = 2\`. ∎

**Kiểm chứng \`0111 → 1000\`:** t = 3 (ba bit 1 đuôi). actual = \`3 + 1 = 4\`. Φ trước = 3, Φ sau = 1, ΔΦ = −2. amortized = \`4 + (−2) = 2\`. ✓ Khớp.
...
\`\`\`
copy = 10 + 20 + 30 + 40 + 50 + 60 + 70 + 80 + 90 = 450
\`\`\`
(9 lần grow, copy 10,20,...,90.) Tổng chi phí ≈ \`100 + 450 = 550\`. Amortized = \`550/100 = 5.5\`. Tổng quát \`+k\`: tổng copy \`≈ k·(n/k)²/2 = n²/(2k) = Θ(n²)\` ⟹ amortized Θ(n).

**Grow ×2** (cap: 1→2→4→...→128): copy = \`1+2+4+8+16+32+64 = 127\`. Tổng ≈ \`100 + 127 = 227\`. Amortized = \`227/100 = 2.27 = O(1)\`.

**Kết luận:** +10 cho amortized **5.5** (và sẽ tăng theo n — Θ(n)); ×2 cho amortized **2.27** (hằng số — O(1)). Với n = 100, +10 đã copy gấp ~3.5 lần ×2; khoảng cách nới rộng khi n lớn. ∎
...
- MultiPop(100): pop min(100,50) = **50**. actual = 50. size = 0.
- 30 Push: actual = 30. size = 30.
- MultiPop(40): pop min(40,30) = **30**. actual = 30. size = 0.

**Tổng actual** = \`50 + 50 + 30 + 30 = 160\`. Số thao tác = \`50 + 1 + 30 + 1 = 82\`. Amortized trung bình = \`160/82 ≈ 1.95 ≤ 2\`. ✓

**Potential \`Φ = size\`:** push amortized = \`1 + 1 = 2\`; mỗi phần tử pop amortized = \`1 + (−1) = 0\`, nên multipop amortized = 0. Tổng amortized = \`80 push × 2 + 2 multipop × 0 = 160 ≥ 160\` actual. Chặn ≤ 2/op. ∎
...
**Potential \`Φ = số phần tử trong inbox\`** (\`Φ ≥ 0, Φ_0 = 0\`):
- **Enqueue:** actual 1 (push inbox), ΔΦ = +1 ⟹ amortized = 2.
- **Dequeue khi outbox không rỗng:** actual 1 (pop outbox), ΔΦ = 0 ⟹ amortized = 1.
- **Dequeue khi outbox rỗng, inbox có m phần tử:** actual = \`m (chuyển) + 1 (pop)\`. ΔΦ = \`−m\` (inbox về 0). amortized = \`(m+1) + (−m) = 1\`.
...
Lần append thứ **1025**: trước đó length = 1024 = capacity (capacity đi 1,2,4,...,1024). Append này **trigger grow** → copy **1024** phần tử. Chi phí thực lần đó = \`1024 + 1 = 1025 ≥ 1000\`. ✓

Nhưng toàn chuỗi: tổng copy = \`1 + 2 + ... + 1024 = 2047 < 2·1025\`, tổng chi phí ≈ \`1025 + 2047 = 3072\`, amortized = \`3072/1025 ≈ 3 = O(1)\`.
...
Potential method cho \`Σ actual = Σ amortized − (Φ_n − Φ_0)\`. Để \`Σ amortized\` là **chặn trên** của \`Σ actual\`, cần \`Φ_n − Φ_0 ≥ 0\`. Với \`Φ_0 = 0\`, điều này cần \`Φ_n ≥ 0\`. Nếu yêu cầu chặn đúng tại **mọi** prefix (mọi thời điểm dừng giữa chừng), cần \`Φ_i ≥ 0\` với **mọi** i.

**Ví dụ Φ vi phạm dẫn tới sai:** Giả sử với binary counter ta chọn nhầm \`Φ = −(số bit 1)\` (âm). Thì \`Φ_0 = 0\` nhưng \`Φ_i < 0\` khi có bit 1. Lúc đó \`Σ amortized = Σ actual + (Φ_n − Φ_0) = Σ actual + Φ_n < Σ actual\` — ta sẽ "chứng minh" được tổng amortized **nhỏ hơn** tổng thực, tức chặn trên **sai** (lạc quan giả tạo). Điều kiện \`Φ ≥ 0\` chặn đúng kiểu nhầm lẫn này: nó đảm bảo "kho thế" không bao giờ rút quá số đã gửi. ∎
...
- 3 phương pháp: aggregate (\`T(n)/n\`), accounting (credit gắn phần tử, ≥ 0), potential (\`actual + ΔΦ\`, \`Φ ≥ 0\`).
