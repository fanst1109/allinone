# Lesson 02 — DNS (Domain Name System)
...
> 💡 **"It's always DNS" — câu đùa của ngành vì DNS là nguyên nhân ẩn của vô số sự cố. Hiểu TTL/caching/record types cứu nhiều giờ debug.**

| Khái niệm | Gặp thật ở đâu |
|-----------|----------------|
| **TTL + caching** | Đổi IP nhưng client vẫn gọi IP cũ (cache chưa hết hạn) |
| **A / AAAA / CNAME / MX** | Trỏ domain, alias, email routing |
| **DNS-based service discovery** | K8s service (`svc.namespace.svc.cluster.local`), Consul |
| **DNS load balancing / failover** | Trỏ nhiều IP, GeoDNS, health-based routing |

### 8.1. Ví dụ cụ thể — TTL gây "deploy xong vẫn lỗi"

Đổi domain trỏ từ server cũ → server mới (đổi A record). Nhưng record cũ có **TTL 3600s** → client/resolver cache IP cũ tới 1 giờ → vẫn gọi server cũ (đã tắt) → lỗi. Bài học: **giảm TTL TRƯỚC khi migrate** (vài phút trước, hạ TTL xuống 60s), đổi record, đợi TTL cũ hết, rồi nâng lại. Tương tự: app Go cache DNS resolution → instance backend đổi IP (autoscale) nhưng app vẫn gọi IP chết → cần TTL ngắn hoặc resolve lại. Đây là "it's always DNS" thật.

> ⚠ **Bẫy DNS phổ biến.** (1) **Negative caching** — domain không tồn tại cũng được cache → tạo record mới nhưng client vẫn báo NXDOMAIN một lúc. (2) **DNS resolution chậm** trong app → mỗi request resolve lại → dùng connection pool (giữ connection theo IP đã resolve) hoặc DNS cache. (3) **CNAME chуỗi dài** → nhiều round-trip resolve → chậm. (4) K8s: pod resolve service qua CoreDNS — DNS chậm/lỗi trong cluster gây "service không gọi được nhau" (một sự cố K8s kinh điển → cần tune `ndots`/DNS cache). DNS dùng UDP ([nối UDP](../../01-Foundations-LowerLayers/lesson-07-udp/)) — một gói hỏi-đáp nhanh.

### 8.2. 📝 Tóm tắt mục 8

- "It's always DNS": **TTL/caching** gây "đổi IP nhưng vẫn gọi IP cũ" → giảm TTL trước khi migrate.
- DNS = service discovery (K8s/Consul), load balancing/failover; record A/AAAA/CNAME/MX.
- Bẫy: negative caching, DNS resolution chậm trong app (cần cache/pool), DNS cluster K8s (CoreDNS/ndots).
