...
// Source: Astronomy/03-Cosmology/lesson-06-exoplanets/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 06 — Ngoại hành tinh (Exoplanets)
...
- Tính **độ sâu quá cảnh (transit depth)** từ tỉ số bán kính: \`δ = (R_p/R_*)²\`, và đọc đường cong ánh sáng (light curve).
...
- **(c) Ví dụ trực giác bằng số** — Sao Mộc sáng hơn Mặt Trời ~10⁻⁹ lần (chỉ phản xạ ánh sao). Tách một nguồn yếu hơn tỉ lần ở sát một nguồn chói là cực khó → cần phương pháp gián tiếp.
...
- **(a) Là gì** — Tỉ lệ độ sáng bị che: \`δ = (R_p / R_*)²\`, với \`R_p\` = bán kính hành tinh, \`R_*\` = bán kính sao. Vì độ sáng ∝ diện tích đĩa, mà diện tích ∝ bán kính bình phương, nên độ che ∝ (R_p/R_*)².
- **(b) Vì sao cần** — δ đo *trực tiếp* được từ đường cong ánh sáng → cho ngay **kích thước hành tinh** so với sao. Đây là cách phát hiện hiệu quả nhất (Kepler tìm hàng nghìn hành tinh).
- **(c) Ví dụ trực giác bằng số** — Sao Mộc (R_p ≈ 0.1 R☉) trước Mặt Trời: \`δ = 0.1² = 0.01 = 1%\`. Trái Đất (R_p ≈ 0.009 R☉): \`δ = 0.009² ≈ 8×10⁻⁵ = 0.008%\`.

**Walk-through bằng số thật (verify) — Sao Mộc trước Mặt Trời:**

Bán kính Sao Mộc ≈ 71 500 km; bán kính Mặt Trời ≈ 696 000 km.
\`\`\`
R_p/R_* = 71 500 / 696 000 ≈ 0.1027
δ = (0.1027)² ≈ 0.01055 ≈ 1.06%
\`\`\`
...
> 🔁 **Dừng lại tự kiểm tra.** Một hành tinh có R_p = 2 R⊕ quá cảnh một sao cỡ Mặt Trời. Độ sâu gấp mấy lần so với Trái Đất quá cảnh cùng sao đó?
> <details><summary>Đáp án</summary>δ ∝ R_p² nên gấp 2² = **4 lần**. Trái Đất ~0.0084% → hành tinh này ~0.034%.</details>
...
|---|---|---|---|
| Transit | Độ giảm sáng δ=(R_p/R_*)² | Hành tinh sát sao, nhiều hành tinh cùng lúc | Cần quỹ đạo "nghiêng đúng" để che sao |
...
|---|---|---|
| \`R*\` | Số sao mới sinh mỗi năm trong Ngân Hà | ~1.5 /năm |
...
\`\`\`
N = 1.5 × 1 × 0.4 × 0.5 × 0.1 × 0.2 × 10 000
  = 1.5 × 0.4 = 0.6
  0.6 × 0.5 = 0.3
  0.3 × 0.1 = 0.03
  0.03 × 0.2 = 0.006
  0.006 × 10 000 = 60
\`\`\`
→ Với bộ giá trị này, \`N ≈ 60\` nền văn minh. Nhưng nếu \`L\` chỉ 100 năm thay vì 10 000: \`N ≈ 0.6\` (làm tròn < 1, gần như cô đơn). Một thừa số đổi → kết quả đổi 100 lần.
...
> - *"Phần nào ta đã biết khá chắc?"* — \`R*\` và \`f_p\` (nay biết hành tinh rất phổ biến) và phần nào \`n_e\`. Càng về cuối càng mù mờ.
...
> - **Transit**: độ sâu \`δ = (R_p/R_*)²\` — Sao Mộc ~1%, Trái Đất ~0.008%. Tỉ lệ với bình phương bán kính.
...
1. **Độ sâu quá cảnh.** Một hành tinh bán kính R_p = 1.5 R⊕ quá cảnh một sao cỡ Mặt Trời. Dùng R⊕ ≈ 0.00915 R☉, tính độ sâu δ (%) và so với Trái Đất.

2. **Suy bán kính hành tinh từ đường cong.** Kepler đo độ sâu quá cảnh δ = 0.25% trên một sao có R_* = 0.8 R☉. Tính R_p theo R☉ rồi đổi ra R⊕ (R☉ ≈ 109 R⊕). Hành tinh này cỡ gì?
...
4. **Phương trình Drake.** Lấy \`R* = 2\`, \`f_p = 1\`, \`n_e = 0.5\`, \`f_l = 0.3\`, \`f_i = 0.1\`, \`f_c = 0.1\`, \`L = 1000\`. Tính N. Sau đó tăng L lên 100 000 và tính lại — nhận xét vai trò của L.
...
\`R_p = 1.5 R⊕ = 1.5 × 0.00915 R☉ = 0.013725 R☉\`.
\`\`\`
δ = (R_p/R_*)² = (0.013725)² ≈ 1.88×10⁻⁴ ≈ 0.0188%
\`\`\`
So với Trái Đất (~0.0084%): tỉ lệ = 1.5² = **2.25 lần** sâu hơn (vì δ ∝ R_p²). Khớp: 0.0084% × 2.25 ≈ 0.0189%.

### Bài 2 — Suy bán kính từ đường cong

Từ \`δ = (R_p/R_*)²\` → \`R_p = R_* × √δ\`.
\`\`\`
√δ = √0.0025 = 0.05
R_p = 0.8 R☉ × 0.05 = 0.04 R☉
\`\`\`
Đổi ra R⊕: \`R_p = 0.04 × 109 R⊕ ≈ 4.36 R⊕\`.
...
**Hợp lý nhất: Quá cảnh (transit).** Hành tinh sát sao → xác suất quỹ đạo che đĩa sao cao hơn, và quá cảnh xảy ra thường xuyên (chu kỳ ngắn) nên dễ bắt lặp lại. Với sao **mờ và nhỏ** (lùn đỏ), R_* nhỏ → cùng một hành tinh cho δ = (R_p/R_*)² *lớn hơn* → dễ đo. (Đây chính là cách TRAPPIST-1 được khảo sát.)
...
- **Visualization tương tác**: [visualization.html](./visualization.html) — gồm:
  - **Simulator đường cong ánh sáng quá cảnh**: slider bán kính hành tinh (R_p/R_*) → vẽ "vết lõm" độ sáng và hiện độ sâu δ = (R_p/R_*)² ngay; có preset Sao Mộc, Trái Đất, sub-Neptune.
