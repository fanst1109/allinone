...
// Source: Vectors/06-AI-ML/lesson-07-vector-db-rag/README.md
...
- Khi user hỏi một câu, bạn embed câu hỏi thành vector \`q ∈ ℝ⁷⁶⁸\` và muốn tìm **top-10 chunk gần nhất với \`q\`** theo cosine similarity, để nhồi vào prompt LLM.
...
Để tính cosine similarity giữa \`q\` và 1 vector \`v ∈ ℝ⁷⁶⁸\`:

\`\`\`
cos(q, v) = (q · v) / (|q| · |v|)
\`\`\`

Phần đắt nhất là **dot product** \`q · v = Σᵢ qᵢ · vᵢ\` — cần đúng **768 phép nhân + 767 phép cộng ≈ 1536 phép cơ bản** (FLOP).
...
Cho tập điểm \`P = {p₁, p₂, …, pₙ} ⊂ ℝᵈ\` và truy vấn \`q ∈ ℝᵈ\`:

- **Exact k-NN**: trả về **chính xác** k điểm gần nhất theo khoảng cách \`dist(q, pᵢ)\`.
- **Approximate k-NN (ANN) với recall@k = r**: trả về k điểm sao cho **trung bình \`r × 100%\` trong số đó trùng với exact k-NN**.
...
|----|-----------------|--------------|
| **IVF (Inverted File)** | Cluster trước → chỉ tìm trong vài cluster gần | Dataset rất lớn (10⁸-10⁹), chấp nhận build chậm |
| **HNSW (graph)** | Graph nhiều tầng, navigate từ thô đến mịn | Dataset vừa (10⁶-10⁸), cần latency thấp |
...
2. Mỗi cluster có 1 **centroid** (tâm), gọi là \`cⱼ\` với \`j = 1..c\`.
3. Mỗi điểm \`pᵢ\` được gán vào cluster có centroid gần nhất → tạo "inverted list" \`Lⱼ = {pᵢ : pᵢ thuộc cluster j}\`.
...
Giả sử \`N = 1.000.000\`, \`d = 128\`, \`c = 1.000\` (chọn \`c ≈ √N\` là quy tắc ngón tay).

- **Số điểm trung bình mỗi cluster**: \`N/c = 1000\` điểm.
- **Brute force**: \`N · d = 10⁶ · 128 = 1.28 × 10⁸\` FLOP/query.
- **IVF với nprobe = 10**:
  - So với centroid: \`c · d = 1000 · 128 = 128.000\` FLOP.
  - So với điểm trong 10 cluster: \`10 · 1000 · d = 10⁴ · 128 = 1.28 × 10⁶\` FLOP.
  - Tổng: \`1.28 × 10⁶ + 1.28 × 10⁵ ≈ 1.4 × 10⁶\` FLOP.
  - **Speedup ≈ 91×** so với brute force.
...
>
> *"k-means có hội tụ chậm với 10⁹ điểm không?"* — Có. Trong thực tế dùng **mini-batch k-means** hoặc chỉ k-means trên một subset (vd 10⁶ điểm sample) rồi gán cluster cho phần còn lại bằng nearest centroid. FAISS có sẵn.
...
- \`c ≈ √N\` (vd \`N = 10⁹\` → \`c ≈ 30.000\`).
- \`nprobe\` chỉnh để đạt recall mục tiêu (thường 1% của \`c\`, tức 100-300).

### 3.4 Toy example: cảnh báo

Ví dụ ở mục 3.2 dùng \`c = 1000\` với \`N = 10⁶\`. **Trong production**, với \`N = 10⁹\` bạn sẽ:

- Dùng \`IVF65536\` (\`c = 2¹⁶\`), kết hợp với PQ để mỗi vector trong inverted list được nén.
...
>
> Cho \`N = 10⁸\`, \`d = 768\`. Bạn muốn search trong 50ms với CPU 1 GFLOPS. Chọn \`c\` và \`nprobe\` thế nào?
>
> <details><summary>Đáp án</summary>
>
> Budget: 50ms × 10⁹ FLOP/s = 5 × 10⁷ FLOP.
>
> Chọn \`c ≈ √N = 10⁴\`. Compare với centroid: \`c · d = 10⁴ · 768 ≈ 7.7 × 10⁶\` FLOP.
>
> Còn lại \`5 × 10⁷ - 7.7 × 10⁶ ≈ 4.2 × 10⁷\` FLOP cho probe.
>
> Mỗi cluster có \`N/c = 10⁴\` điểm, mỗi compare \`d = 768\` FLOP → 1 cluster = \`7.68 × 10⁶\` FLOP.
>
> \`nprobe ≈ 4.2 × 10⁷ / 7.68 × 10⁶ ≈ 5\`.
>
...
> - \`c ≈ √N\`, \`nprobe\` là núm chỉnh recall/speed.
...
Số tầng quyết định **stochastically** khi insert: mỗi điểm rơi vào tầng tối đa \`l = ⌊-ln(uniform()) · mL⌋\`, với \`mL\` là param. → Top layer rất thưa, bottom layer dày.
...
Giả sử HNSW với \`M = 16\`, 4 tầng. Để tìm 1 trong \`N = 10⁹\` node:

- Tầng 3 (top): ~100 node. Greedy chỉ vài hop.
- Tầng 2: ~10⁴ node. Greedy vài chục hop.
- Tầng 1: ~10⁶ node. ~100 hop.
- Tầng 0: ~10⁹ node. Mở \`ef = 50\` candidate.

Tổng số distance computation: ~vài trăm — so với \`10⁹\` brute force = **speedup 10⁶×**.
...
> ❓ **Câu hỏi tự nhiên của người đọc**
>
> *"Build HNSW có đắt không?"* — Có. Build \`N = 10⁸\` mất hàng giờ trên CPU. Nhưng chỉ build 1 lần (hoặc incremental insert sau đó). Đổi lại query cực nhanh.
>
...
>
> *"Memory tốn bao nhiêu?"* — Mỗi node lưu \`M × 4 byte\` cho ID neighbor + vector gốc. Với \`M = 16\`, \`d = 768\`, \`N = 10⁸\`: vector chiếm \`10⁸ × 768 × 4 = 307 GB\`, graph thêm \`10⁸ × 16 × 4 = 6.4 GB\`. Graph là phần phụ nhỏ. → Cần kết hợp PQ để giảm vector.
...
> ⚠ **Lỗi thường gặp**
>
> - **\`efSearch\` quá thấp**: query nhanh nhưng miss top-k thực sự. Chỉnh \`efSearch ≥ k × 2\` ít nhất.
...
>
> Empirical: HNSW đạt recall 99% với latency mili-giây trên \`N = 10⁸\`.
> </details>
...
Vector \`v ∈ ℝ⁷⁶⁸\` chiếm \`768 × 4 = 3072 byte\` (float32). Quá tốn.
...
2. Mỗi cột, chạy k-means với \`k = 256\` → ra **codebook \`Cⱼ ∈ ℝ²⁵⁶ˣ⁹⁶\`**.

Bước **encode** một vector \`v\`:

1. Chia \`v\` thành \`[v⁽¹⁾, v⁽²⁾, …, v⁽⁸⁾]\`, mỗi \`v⁽ʲ⁾ ∈ ℝ⁹⁶\`.
2. Với mỗi \`j\`, tìm centroid gần nhất trong \`Cⱼ\` → được ID \`iⱼ ∈ {0, …, 255}\`.
3. Lưu \`v\` bằng **8 byte** \`[i₁, i₂, …, i₈]\`.
...
Khi query \`q ∈ ℝ⁷⁶⁸\` đến, bạn KHÔNG quantize \`q\`. Thay vào đó:

1. Chia \`q\` thành 8 sub-query \`[q⁽¹⁾, …, q⁽⁸⁾]\`.
2. Với mỗi \`j\`, **precompute bảng \`Tⱼ ∈ ℝ²⁵⁶\`** trong đó \`Tⱼ[c] = ||q⁽ʲ⁾ - Cⱼ[c]||²\` cho \`c = 0..255\`.
3. Distance ước tính giữa \`q\` và vector đã quantize \`v = [i₁, …, i₈]\`:
   \`\`\`
   d̂(q, v)² = Σⱼ Tⱼ[iⱼ]
   \`\`\`
...
Ví dụ chia 8D thành 2 nhóm 4D (như trong viz) chỉ để dễ hình dung. **Production** dùng \`d = 768, m = 8-64\`. Càng nhiều sub-vector → càng ít sai số nhưng càng tốn memory cho codebook.
...
> <details><summary>Đáp án</summary>
>
> 24 byte = 24 sub-vector × 1 byte/sub → \`m = 24\`. Mỗi sub-vector chiều \`1536/24 = 64\`. Compression \`1536·4 / 24 = 256×\`.
> </details>
...
**Bước 1 — Embed query**: dùng cùng model embedding lúc index (vd \`text-embedding-3-small\`). Output: \`q ∈ ℝ¹⁵³⁶\`.
...
Cho **N** query, mỗi query có **ground truth** là tập documents đúng \`Gᵢ\` và hệ thống trả top-k \`Rᵢ\`.

| Metric | Công thức | Ý nghĩa |
|--------|-----------|---------|
| **Precision@k** | \`|Rᵢ ∩ Gᵢ| / k\` | Trong k trả về, bao nhiêu đúng |
| **Recall@k** | \`|Rᵢ ∩ Gᵢ| / |Gᵢ|\` | Trong tất cả đúng, bao nhiêu trả về được |
| **MRR (Mean Reciprocal Rank)** | \`(1/N) Σ 1/rankᵢ\` (rank của doc đúng đầu tiên) | Doc đúng đầu tiên ở vị trí nào |
...
Cho dataset \`N = 10⁷\`, \`d = 512\`. Cài đặt IVF với \`c = 2000\`, \`nprobe = 20\`. Tính:
...
**Brute force**:
- Mỗi compare: \`d\` phép nhân + \`d-1\` phép cộng ≈ \`2d\` FLOP. Lấy \`2d = 1024\` FLOP/compare.
- Tổng: \`N × 2d = 10⁷ × 1024 ≈ 1.024 × 10¹⁰\` FLOP.

**IVF**:
- So với centroid: \`c × 2d = 2000 × 1024 = 2.048 × 10⁶\` FLOP.
- Probe: \`nprobe × (N/c) × 2d = 20 × (10⁷/2000) × 1024 = 20 × 5000 × 1024 ≈ 1.024 × 10⁸\` FLOP.
- Tổng: \`2.048 × 10⁶ + 1.024 × 10⁸ ≈ 1.044 × 10⁸\` FLOP.

**Speedup**: \`1.024 × 10¹⁰ / 1.044 × 10⁸ ≈ 98×\`.

### Lời giải Bài 2

**Float32**:
- \`100.000.000 × 1024 × 4 byte = 4.096 × 10¹¹ byte ≈ 410 GB\`.

**PQ với m=16**:
- Mỗi vector lưu bằng \`m = 16 byte\`.
- Tổng: \`10⁸ × 16 = 1.6 × 10⁹ byte ≈ 1.6 GB\`.
- Codebook (negligible): \`16 × 256 × (1024/16) × 4 = 16 × 256 × 64 × 4 ≈ 1 MB\`.
- **Compression 256×**.

**PQ với m=32**:
- \`10⁸ × 32 = 3.2 × 10⁹ byte ≈ 3.2 GB\`.
...
| Q | \`|R ∩ G|\` | k | P@5 | R@5 | rank doc đúng đầu | RR |
|---|----------|---|-----|-----|-------------------|----|
