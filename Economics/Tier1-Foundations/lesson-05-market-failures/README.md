...
Trong Lesson 04, ta thấy thị trường cạnh tranh tự do **tối đa hóa tổng phúc lợi $W = \text{CS} + \text{PS}$**. Nhưng định lý này dựa trên một loạt *giả định*:
...
- **MSC** (Marginal Social Cost) = chi phí biên *toàn xã hội* = $\text{MPC} + \text{chi phí externality}$.
- **MPB** (Marginal Private Benefit) = lợi ích biên của *người mua*.
- **MSB** (Marginal Social Benefit) = lợi ích biên toàn xã hội.

**Cân bằng thị trường**: $\text{MPC} = \text{MPB}$.
**Cân bằng tối ưu xã hội**: $\text{MSC} = \text{MSB}$.
...
- $\to \text{MSC} = 100 + 30 = 130$.
- $\text{MPB} = \text{MSB}$ = đường cầu của thép (giả sử không có externality bên cầu).

Giả sử cầu cho thép: $P = 200 - Q$.

- **Cân bằng thị trường**: $\text{MPC} = \text{MPB} \to 100 = 200 - Q \to Q_{\text{market}} = 100$.
- **Cân bằng tối ưu xã hội**: $\text{MSC} = \text{MSB} \to 130 = 200 - Q \to Q_{\text{optimal}} = 70$.

→ Thị trường sản xuất **30 tấn thừa** so với mức tối ưu xã hội. Mỗi tấn thừa gây thiệt hại $(\text{MSC} - \text{MPB})$ cho xã hội.

**DWL của externality**: tam giác giữa MSC và MPB từ $Q_{\text{optimal}}$ đến $Q_{\text{market}}$:

$$\begin{aligned}
\text{DWL} &= 0.5 \times (Q_{\text{market}} - Q_{\text{optimal}}) \times (\text{MSC} - \text{MPB} \text{ tại } Q_{\text{market}}) \\
&= 0.5 \times 30 \times 30 = 450
\end{aligned}$$
...
Đánh thuế $T = \text{external cost}$ lên người gây externality tiêu cực. Hiệu ứng: MPC tăng lên *bằng* MSC → cân bằng mới = cân bằng xã hội tối ưu.

Walk-through ví dụ: thuế $T = 30/\text{tấn}$ lên nhà máy thép. MPC mới $= 130 = \text{MSC}$. Cân bằng mới $Q = 70 = Q_{\text{optimal}}$. **DWL $= 0$**.
...
Vắc-xin tạo externality tích cực → $\text{MSB} > \text{MPB}$. Thị trường tự do sẽ tiêm *ít hơn tối ưu xã hội*. Chính phủ trợ cấp $S = \text{external benefit}$ → cân bằng dịch lên đúng mức.
...
- **Quyền thuộc dân** (quyền không bị ô nhiễm): nhà máy phải trả $30/\text{tấn}$ cho dân để được sản xuất 70 tấn. Tổng trả $= 2100$, nhưng nhà máy thu lợi từ giao dịch nhiều hơn → win-win, dân được đền bù.
- **Quyền thuộc nhà máy** (quyền ô nhiễm tự do): dân phải trả nhà máy để nó giảm sản lượng từ 100 xuống 70 tấn. Tổng dân trả $\approx 30 \times \text{giá đền bù}$. Cũng cho cân bằng tối ưu.
...
Nhà máy hóa chất có: $\text{MPC} = 50$, external cost $= 20$. Cầu ($= \text{MPB} = \text{MSB}$): $P = 150 - Q$.

- (a) Tìm $Q_{\text{market}}$, $Q_{\text{optimal}}$.
- (b) Tính DWL khi không có chính sách.
- (c) Thuế Pigou tối ưu là bao nhiêu?

### Bài 2 — Externality tích cực

Vắc-xin: $\text{MPC} = \text{MSC} = 30$, $\text{MPB} = 100 - Q$, $\text{MSB} = 120 - Q$ (mỗi liều tiêm tạo external benefit $20$ cho xã hội).
...
(a) $Q_{\text{market}}$: $\text{MPC} = \text{MPB} \to 50 = 150 - Q \to Q_{\text{market}} = 100$.
   $Q_{\text{optimal}}$: $\text{MSC} = \text{MSB} \to 50 + 20 = 70 = 150 - Q \to Q_{\text{optimal}} = 80$.

(b) DWL = tam giác giữa MSC và MPB từ $Q_{\text{optimal}}$ tới $Q_{\text{market}}$. Chiều dài $= 100 - 80 = 20$. Chiều cao tại $Q_{\text{market}}$: $\text{MSC} - \text{MPB} = 70 - 50 = 20$. $\to \text{DWL} = 0.5 \times 20 \times 20 = 200$.

(c) Thuế Pigou $=$ external cost $= 20$. Áp thuế → MPC tăng lên $70 = \text{MSC}$ → cân bằng $= Q_{\text{optimal}} = 80$.

### Lời giải Bài 2

(a) $Q_{\text{market}}$: $\text{MPC} = \text{MPB} \to 30 = 100 - Q \to Q_{\text{market}} = 70$.
   $Q_{\text{optimal}}$: $\text{MSC} = \text{MSB} \to 30 = 120 - Q \to Q_{\text{optimal}} = 90$.

(b) DWL = tam giác giữa MSB và MSC từ $Q_{\text{market}}$ tới $Q_{\text{optimal}}$. Chiều dài $= 20$. Chiều cao tại $Q_{\text{market}}$: $\text{MSB} - \text{MSC} = 100 - 30 = 70$ so với $\text{MPB} = \text{MPC}$... 

Chính xác hơn: ở $Q = 70$, $\text{MSB} = 120 - 70 = 50$, $\text{MSC} = 30$. → khoảng cách $= 20$. Tại $Q = 90$, $\text{MSB} = \text{MSC} = 30$, khoảng cách $= 0$. Tam giác: $\text{DWL} = 0.5 \times 20 \times 20 = 200$.

(c) Trợ cấp $=$ external benefit $= 20$. Trợ cấp dịch MPB lên trùng MSB → cân bằng $= Q_{\text{optimal}}$.
...
(a) Premium $= 0.6 \times 1.000 + 0.4 \times 5.000 = 600 + 2000 = 2.600$/năm (giả định không có markup).

(b) Nhóm khỏe sẵn sàng trả tối đa $1.000 \times 1.1 = 1.100$. Premium $2.600 > 1.100$ → nhóm khỏe *rời thị trường*.
