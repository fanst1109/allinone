# Lesson 39 — Ghép cặp đồ thị 2 phía (Bipartite Matching)
...
4. Cài đặt **thuật toán Kuhn** (Hungarian dạng tìm augmenting path bằng DFS) — $O(V \cdot E)$.
5. Thấy matching chính là **một bài luồng cực đại** đặc biệt (L38), và biết khi nào nên dùng cách nào.
6. Biết **Hopcroft-Karp** nhanh hơn ($O(E\sqrt V)$) và **định lý König** (max matching = min vertex cover trong đồ thị 2 phía).
...
- **[L29 — Bitmask DP](../lesson-29-bitmask-dp/README.md)**: assignment problem nhỏ giải bằng bitmask DP $O(2^N \cdot N)$.
...
> - *"Tăng 1 mỗi lần thì có chậm không?"* → Maximum matching $\leq V/2$, nên tối đa $V/2$ lần tăng. Mỗi lần tìm đường tốn $O(E)$ → $O(V \cdot E)$ tổng (Kuhn).
...
> - RESET `used` mỗi đỉnh trái. Độ phức tạp $O(V \cdot E)$.
...
> hết. Giống Dinic cho flow. Số pha bị chặn $O(\sqrt V)$, mỗi pha $O(E)$ → $O(E\sqrt V)$.

**Khi nào cần?** Đồ thị **lớn** ($V, E$ hàng chục–trăm nghìn). Với $V = 10^5, E = 10^6$:
- Kuhn $O(V \cdot E) \approx 10^{11}$ → quá chậm.
- Hopcroft-Karp $O(E\sqrt V) \approx 10^6 \cdot 316 \approx 3 \cdot 10^8$ → chạy được.
...
> 📝 **Tóm tắt mục 6.** Hopcroft-Karp = nhiều augmenting path mỗi pha (BFS phân tầng + DFS), $O(E\sqrt V)$, dùng cho đồ thị lớn.
...
**Thuật toán Hungarian (Kuhn-Munkres)** giải assignment problem trong $O(n^3)$ ($n$ = kích thước
...
```
Độ phức tạp $O(2^n \cdot n)$. Với $n = 20 \approx 2 \cdot 10^7$ phép → ổn. Với $n$ lớn dùng Hungarian $O(n^3)$.

> Bài này chỉ giới thiệu; cài đặt đầy đủ Hungarian/Kuhn-Munkres để phần nâng cao.

> 📝 **Tóm tắt mục 9.** Có trọng số → assignment problem. $n$ nhỏ: bitmask DP $O(2^n \cdot n)$. $n$ lớn: Hungarian $O(n^3)$.
...
| **Kuhn** (augmenting DFS) | Max matching | $O(V \cdot E)$ | $V \leq$ vài nghìn |
| Matching via max flow | Max matching | $O(V \cdot E)$ (Edmonds-Karp) | Khi đã là flow / cap > 1 |
| **Hopcroft-Karp** | Max matching | $O(E\sqrt V)$ | Đồ thị lớn |
| König (DFS xen kẽ) | Min vertex cover | $O(V + E)$ sau matching | Sau khi có matching |
| Bitmask DP | Assignment (trọng số) | $O(2^n \cdot n)$ | $n \leq 20$ |
| Hungarian | Assignment (trọng số) | $O(n^3)$ | $n$ lớn |

> ⚠ Kuhn $O(V \cdot E)$: với đồ thị dày ($E \approx V^2$) là $O(V^3)$. Vẫn ổn cho $V \leq$ vài trăm–nghìn; lớn hơn dùng Hopcroft-Karp.
...
|---------|---------|-----------|
| Đồ thị **không 2 phía** | Kuhn/König **không áp dụng** | Bài general matching cần **Blossom algorithm** (Edmonds, $O(V^3)$) — KHÔNG học ở đây. König chỉ đúng cho bipartite. |
...
**Độ phức tạp:** $O(V \cdot E)$. Mỗi đỉnh trái 1 DFS $O(E)$, có $V$ đỉnh.
...
**Độ phức tạp:** $O(N \cdot E)$ với $E \leq N \cdot M$.
...
**Độ phức tạp:** $O(V \cdot E)$ cho Kuhn + $O(V+E)$ cho cấu trúc cover.
...
**Độ phức tạp:** $O(V \cdot E)$ (chi phối bởi Kuhn).
...
**Độ phức tạp:** $O(V \cdot E)$, $V$ = số ô, $E \leq 4V$ (mỗi ô tối đa 4 hàng xóm).
