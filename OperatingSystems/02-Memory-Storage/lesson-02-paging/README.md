...
**Ví dụ: Page size = 4 KB $= 2^{12}$ byte $\to n = 12$.**

Địa chỉ ảo 32-bit: 20 bit cao = page number (tối đa $2^{20} = 1{,}048{,}576$ trang), 12 bit thấp = offset (0–4095).

### 2.2. Công thức dịch địa chỉ

$$\text{page\_number} = \lfloor \text{địa\_chỉ\_ảo} / \text{page\_size} \rfloor \quad \text{(chia nguyên)}$$

$$\text{offset} = \text{địa\_chỉ\_ảo} \bmod \text{page\_size} \quad \text{(phần dư)}$$

$$\text{frame\_number} = \text{page\_table}[\text{page\_number}] \quad \text{(tra bảng trang)}$$

$$\text{địa\_chỉ\_vật\_lý} = \text{frame\_number} \cdot \text{page\_size} + \text{offset}$$
...
- $\text{page\_number} = \lfloor \text{địa\_chỉ} / \text{page\_size} \rfloor$ (chia nguyên).
- $\text{offset} = \text{địa\_chỉ} \bmod \text{page\_size}$.
- $\text{Địa chỉ vật lý} = \text{frame\_number} \cdot \text{page\_size} + \text{offset}$.
...
- Số trang $= 2^{32} / 2^{12} = \mathbf{2^{20} = 1{,}048{,}576}$ **trang**.
- Mỗi PTE thường 4 bytes.
- Kích thước bảng trang $= 2^{20} \times 4 =$ **4 MB per process** (chỉ cho bảng trang!).
...
Trung bình: mỗi tiến trình lãng phí nửa trang cuối $= \text{page\_size} / 2$. Với page 4 KB → lãng phí trung bình 2 KB / tiến trình — chấp nhận được.
...
- Mỗi PTE = 4 bytes.
- Kích thước $= 1{,}048{,}576 \times 4 =$ **4,194,304 bytes = 4 MB** / tiến trình.
- 50 tiến trình: $50 \times 4$ MB $=$ **200 MB** chỉ để lưu bảng trang.
...
- **Dịch địa chỉ**: $\text{page\_number} = \lfloor \text{logic} / \text{page\_size} \rfloor$; $\text{offset} = \text{logic} \bmod \text{page\_size}$; $\text{vật\_lý} = \text{frame} \cdot \text{page\_size} + \text{offset}$.
