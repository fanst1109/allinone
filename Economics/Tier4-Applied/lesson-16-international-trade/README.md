# Lesson 16 — International Trade (Thương mại quốc tế)
...
Trong Anh: 1 chai rượu = 2m vải (giành 1 giờ từ vải sang rượu = bỏ 10m, được 5 chai → 1 chai = 2m vải).
Trong Bồ: 1 chai rượu = 0.5m vải.

→ Rượu *rẻ hơn* ở Bồ (chi phí cơ hội nhỏ).
→ Vải *rẻ hơn* ở Anh (1m = 0.5 chai vs 1/6 chai).
...
- Anh: 1h vải = 10m, 1h rượu = 5 chai → tổng (10, 5).
- Bồ: 1h vải = 6m, 1h rượu = 12 → tổng (6, 12).
...
Đo *sức cạnh tranh hàng hóa*. `E_real` cao → hàng nội rẻ tương đối → X tăng, M giảm.
...
`E = 24.000` VND/USD. CPI VN tăng 5%, CPI Mỹ tăng 2%. `E_real` thay đổi thế nào? Hàng VN cạnh tranh hơn hay kém hơn?
...
Chuyên môn hóa:
- VN chuyên cá: 4h × 10 = 40 kg cá.
- Nhật chuyên lúa: 4h × 8 = 32 tấn lúa.
...
(b) Revenue = `T × Q_m_mới = 10 × 20 = 200`.
(c) DWL = 2 tam giác:
- Bên cung: `0.5 × (30 − 20) × (30 − 20) = 50`.
- Bên cầu: `0.5 × (60 − 50) × (30 − 20) = 50`.
- Tổng `DWL = 100`.

### Lời giải Bài 4

`E_real_t/E_real_t-1 = (E_t × P*_t/P_t) / (E_t-1 × P*_t-1/P_t-1)`. Nếu E không đổi: `E_real` thay đổi `(1 + 2%)/(1 + 5%) ≈ 0.971` → `E_real` giảm 2.9%. **Hàng VN ĐẮT hơn** tương đối → kém cạnh tranh. Cần phá giá VND (`E` tăng) để bù.
