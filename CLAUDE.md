...
| `tools/fonts/` | **Font UI local (OFL woff2)** cho chữ thường **toàn viz + README-modal** — bundle để chữ (đặc biệt **tiếng Việt**) đồng nhất mọi máy, không phụ thuộc font hệ thống. Font **đang dùng: Manrope Variable**. 6 font sẵn để đổi: **Manrope**, **Be Vietnam Pro** (static 400/700, tối ưu tiếng Việt), **Plus Jakarta Sans**, **Lexend**, **Roboto Flex**, **Inter**. Subset latin/latin-ext/vietnamese + greek (Manrope/Roboto Flex/Inter). Khai báo `@font-face` ở `viz-base.css`. Đổi font: sửa `font-family` ở **`viz-base.css` (`body`, áp toàn viz)** và **`readme-modal.js` (`.rm-panel .rm-content`, áp modal)** — đổi tên đầu tiên ở cả 2 chỗ. KaTeX math vẫn dùng font `KaTeX_*` riêng. Auto-generated từ @fontsource — KHÔNG sửa woff2 tay |
| `tools/viz-math.js` | Helper render công thức KaTeX trong **thân viz**: expose `km(tex, display?)` = wrapper `katex.renderToString` (throwOnError:false). Load sau `tools/katex/katex.min.js`. Dùng cho dòng công thức "khung" trong readout; số động để ngoài. KHÔNG auto-render quét DOM |
...
- **KaTeX CHỈ render trong README-modal, KHÔNG tự render thân `visualization.html`.** Công thức trong thân viz (tiêu đề, đoạn, ô readout do JS sinh) để nguyên unicode (`x²`, `Δt`, `→`...) — unicode gánh đủ cho biểu thức một dòng, và áp KaTeX đại trà cho thân viz (đặc biệt phần readout động) là over-engineering: ích lợi thị giác nhỏ, chi phí/rủi ro cao. **Mặc định KHÔNG áp KaTeX cho thân viz.**
  - **Ngoại lệ hiếm — "hero formula" tĩnh**: nếu một viz có đúng MỘT công thức tĩnh lớn trông xấu khi để unicode (vd tích phân/ma trận lớn đặt giữa trang), thì render **riêng phần tử đó** — viết công thức trong `$$...$$`, nạp 3 file (`tools/katex/katex.min.css`, `tools/katex/katex.min.js`, `tools/katex/auto-render.min.js`) rồi gọi `renderMathInElement(<phần-tử-hero>)` một lần lúc load. Làm **điểm**, không đại trà; KHÔNG dùng cho readout động (số cập nhật theo slider — KaTeX không thêm gì cho số).
  - **Ngoại lệ chuẩn — dòng công thức trong readout qua `tools/viz-math.js`**: khi readout có công thức nhiều tầng (cận tích phân, phân số, căn) mà unicode đọc không nổi, load 3 file trong `<head>` (`tools/katex/katex.min.css`, `tools/katex/katex.min.js`, `tools/viz-math.js`) rồi gọi `km("\\int_0^1 ...")` ngay trong template literal của readout (`katex.renderToString`, sync ~0.5ms/công thức, không quét DOM). Chỉ bọc phần "khung" công thức bằng `km()`; con số động cập nhật theo slider (`toFixed`) để text thường BÊN NGOÀI. Vẫn KHÔNG dùng auto-render cho thân viz.
