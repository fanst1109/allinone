...
// Source: Databases/02-Intermediate/lesson-06-view-procedure-trigger/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 06 — View, Procedure, Trigger
...
> 💡 **View/procedure/trigger mạnh nhưng gây tranh cãi: chúng đẩy logic xuống DB.** Biết khi nào dùng (và khi nào tránh) là quyết định kiến trúc thật.

| Công cụ | Dùng tốt khi | Cẩn thận khi |
|---------|--------------|--------------|
| **View** | Đơn giản hóa query phức tạp, ẩn cột nhạy cảm (phân quyền) | View lồng view nhiều tầng → khó tối ưu |
| **Materialized view** | Báo cáo/dashboard nặng, dữ liệu đổi chậm | Phải refresh → dữ liệu có thể cũ |
| **Stored procedure** | Logic gần dữ liệu, giảm round-trip, dùng chung nhiều app | Logic nghiệp vụ ẩn trong DB → khó test/version-control |
| **Trigger** | Audit log tự động, giữ denormalized count đồng bộ | "Tác dụng phụ vô hình" → khó debug, dễ vòng lặp |

### 6.1. Ví dụ cụ thể — trigger giữ "review_count" đồng bộ

Nhớ denormalization ([chuẩn hóa §6](../lesson-01-chuan-hoa/)): lưu sẵn \`products.review_count\` để khỏi \`COUNT(*)\` mỗi lần. Trigger giữ nó đúng tự động:

\`\`\`sql
CREATE TRIGGER inc_review AFTER INSERT ON reviews
FOR EACH ROW UPDATE products SET review_count = review_count + 1
WHERE id = NEW.product_id;
\`\`\`

Mỗi review mới → trigger tự tăng count, app không cần nhớ. Tiện, nhưng đây cũng là cái bẫy: logic chạy **vô hình** — người đọc code app không thấy count được cập nhật ở đâu.

> ⚠ **Tranh luận thật: logic trong DB hay trong app?** Trigger/procedure giảm round-trip và đảm bảo nhất quán, nhưng: khó **version control** (không nằm trong git như code app), khó **test/debug** (tác dụng phụ ẩn), khó **migrate** sang DB khác. Xu hướng nhiều team hiện đại: giữ logic nghiệp vụ ở **app** (dễ test/CI/CD), chỉ dùng trigger cho việc thuần dữ liệu (audit, đồng bộ count). Không có đáp án tuyệt đối — biết trade-off.

### 6.2. 📝 Tóm tắt mục 6

- **View** = query đặt tên (đơn giản hóa + phân quyền); **materialized view** = cache kết quả (báo cáo nặng, có thể cũ).
- **Procedure** = logic gần dữ liệu (ít round-trip); **trigger** = tự động hóa (audit, đồng bộ count) nhưng "vô hình".
- Trade-off lớn: logic trong DB (nhất quán, ít round-trip) vs trong app (dễ test/version/migrate). Nhiều team chọn app cho nghiệp vụ.
