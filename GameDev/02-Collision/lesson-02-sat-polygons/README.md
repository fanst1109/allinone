# Lesson 07 — SAT: Separating Axis Theorem (va chạm đa giác lồi)
...
AABB (hình bao thẳng trục) của nó: `x ∈ [-1.41, 1.41]`, `y ∈ [-1.41, 1.41]` — tức một **hộp 2.83 × 2.83**. Nhưng hình vuông thật chỉ chiếm phần **kim cương** bên trong hộp đó — 4 góc của hộp là **vùng trống**. Diện tích hộp ≈ 8.0, diện tích hình vuông thật = 4.0 → **một nửa hộp là không khí.**

Bây giờ đặt một hình vuông thứ hai (không xoay, cạnh 2) ở góc trên-phải, tâm tại `(2.2, 2.2)`:

- **AABB nói gì?** Hộp của hình 1 phủ tới `(1.41, 1.41)`; hộp của hình 2 bắt đầu từ `(1.2, 1.2)`. Hai hộp **chồng nhau** ở vùng `x ∈ [1.2, 1.41]`, `y ∈ [1.2, 1.41]` → AABB báo **"chạm"**.
...
> Trục nằm ngang (hướng x). Chiếu cả hai lên trục x: hình trái cho khoảng `[xL_min, xL_max]`, hình phải cho `[xR_min, xR_max]`, và `xL_max < xR_min` → có khe hở → trục x là trục tách → khẳng định không chạm. Chỉ một trục là đủ.
> </details>
...
> <details><summary>Đáp án</summary>
> `maxA = 9`, `minB = 10`. `9 ≥ 10`? Không → **rời nhau**. Gap = `minB − maxA = 10 − 9 = 1`. Trục này là một trục tách → kết luận không chạm.
> </details>

> 📝 **Tóm tắt mục 3**
> - Chiếu một điểm lên trục đơn vị = **dot product** $P \cdot \hat{n}$.
> - Chiếu đa giác = lấy `[min, max]` của dot product mọi đỉnh.
> - Hai đoạn chồng ⇔ `maxA ≥ minB và maxB ≥ minA`; overlap = `min(maxA,maxB) − max(minA,minB)`.
...
- So: `maxA = 4`, `minB = 4.59`. `4 ≥ 4.59`? **Không** → **RỜI NHAU** → **đây là TRỤC TÁCH!**
...
- `maxA=4 ≥ minB=3.39`? Có. `maxB=6.21 ≥ minA=0`? Có → **chồng**. overlap = `min(4,6.21) − max(0,3.39) = 4 − 3.39 = 0.61`.

**Trục a1 = (0, 1):** A y-toạ độ {0,0,2,2} → [0, 2]; B y-toạ độ {1, 2.41, 1, −0.41} → [−0.41, 2.41].
- `maxA=2 ≥ minB=−0.41`? Có. `maxB=2.41 ≥ minA=0`? Có → **chồng**. overlap = `min(2,2.41) − max(0,−0.41) = 2 − 0 = 2`.

**Trục b1 = (0.707, 0.707):** chiếu = `0.707(x+y)`.
- A đỉnh: A0→0, A1→`0.707·4=2.83`, A2→`0.707·6=4.24`, A3→`0.707·2=1.41` → [0, 4.24].
- B đỉnh: B0→`0.707·7.21=5.10`, B1→`0.707·7.21=5.10`, B2→`0.707·4.39=3.10`, B3→`0.707·4.39=3.10` → [3.10, 5.10].
- `maxA=4.24 ≥ minB=3.10`? Có. `maxB=5.10 ≥ minA=0`? Có → **chồng**. overlap = `min(4.24,5.10) − max(0,3.10) = 4.24 − 3.10 = 1.14`.

**Trục b2 = (0.707, −0.707):** chiếu = `0.707(x−y)`.
- A đỉnh: A0→0, A1→`0.707·4=2.83`, A2→`0.707·2=1.41`, A3→`0.707·(−2)=−1.41` → [−1.41, 2.83].
- B đỉnh: B0→`0.707·5.21=3.68`, B1→`0.707·2.39=1.69`, B2→`0.707·2.39=1.69`, B3→`0.707·5.21=3.68` → [1.69, 3.68].
- `maxA=2.83 ≥ minB=1.69`? Có. `maxB=3.68 ≥ minA=−1.41`? Có → **chồng**. overlap = `min(2.83,3.68) − max(−1.41,1.69) = 2.83 − 1.69 = 1.14`.
...
- **(c) Ví dụ trực giác bằng số:** Ở §4.4 case chạm, các overlap là `{a2: 0.61, a1: 2.0, b1: 1.14, b2: 1.14}`. Nhỏ nhất = **0.61** trên trục a2 = (1, 0). → MTV = `(1, 0) · 0.61 = (0.61, 0)`. Nghĩa là: đẩy B sang phải 0.61 (hoặc A sang trái 0.61, hoặc mỗi bên 0.305) → vừa hết chồng. Trực giác đúng: B lún vào A từ bên phải, lối thoát ngắn nhất là... lùi sang phải.
...
> <details><summary>Đáp án</summary>
> Nhỏ nhất = 0.5 trên trục `(1, 0)`. MTV = `(1,0) × 0.5 = (0.5, 0)` (giả sử hướng đã đúng). Đẩy theo trục x, quãng 0.5.
> </details>

> 📝 **Tóm tắt mục 5**
> - MTV = `trục_đơn_vị_overlap_nhỏ_nhất × overlap_nhỏ_nhất`.
...
> 💡 **Trực giác**
> Hình chiếu của một hình tròn (tâm $C$, bán kính $r$) lên trục đơn vị $\hat{n}$ luôn là đoạn `[C·n̂ − r, C·n̂ + r]` (tâm chiếu xuống, ± bán kính). Trục "nguy hiểm" nhất — nơi hình tròn có thể chạm góc nhọn của đa giác — là hướng nối tâm tới **đỉnh gần nhất**. Thêm trục đó là đủ để SAT đúng với circle.
...
> ❓ **Câu hỏi tự nhiên của người đọc**
> - *"Circle vs circle có cần SAT không?"* — Không. Hai hình tròn chạm ⇔ `khoảng cách hai tâm ≤ r1 + r2` — đã học ở [L06](../lesson-01-aabb-circle/), gọn hơn SAT nhiều. Chỉ dùng SAT khi có ít nhất một đa giác.

> 📝 **Tóm tắt mục 6**
> - SAT chỉ cho lồi → hình lõm phải tách thành mảnh lồi (triangulation / ear clipping / Hertel–Mehlhorn).
> - Đa giác vs circle: thêm trục = hướng tâm circle → đỉnh gần nhất. Hình chiếu circle = `[C·n̂ − r, C·n̂ + r]`.
...
(a) $A=[2,7]$, $B=[5,10]$: `maxA=7 ≥ minB=5`? Có. `maxB=10 ≥ minA=2`? Có → **chồng**.

(b) overlap = `min(maxA,maxB) − max(minA,minB) = min(7,10) − max(2,5) = 7 − 5 = 2`.

(c) $B=[8,10]$: `maxA=7 ≥ minB=8`? **Không** → **tách**. Gap = `minB − maxA = 8 − 7 = 1`. Trục này là trục tách → không chạm.
...
- **Trục x:** A → [0, 2]; B → [3, 5]. `maxA=2 ≥ minB=3`? **Không** → **RỜI** → **trục tách!** Gap = `3 − 2 = 1`.
...
(a) Kiểm tra:
- **Trục x:** A → [0, 4]; B → [3, 7]. `4 ≥ 3` Có; `7 ≥ 0` Có → chồng.
- **Trục y:** A → [0, 4]; B → [1, 5]. `4 ≥ 1` Có; `5 ≥ 0` Có → chồng.
- Cả hai chồng → **CHẠM**.
...
(c) minOverlap = **1** trên trục x = `(1,0)`. Hướng: $\text{center}(A)=(2,2)$, $\text{center}(B)=(5,3)$, $d=(3,1)$. $d \cdot (1,0) = 3 > 0$ → giữ hướng `(1,0)`. **MTV = (1,0) × 1 = (1, 0)** → đẩy B sang phải 1 đơn vị thì hết chồng (B mới x=[4,8], A=[0,4] → overlap 0). ✓
...
So: `maxA=4 ≥ minB=3`? Có. `maxB=5 ≥ minA=0`? Có → **chồng** trên trục x (overlap = `min(4,5) − max(0,3) = 1`).
...
(b) Hình chiếu của hình tròn lên trục $\hat{n}$ = `[C·n̂ − r, C·n̂ + r]`:
