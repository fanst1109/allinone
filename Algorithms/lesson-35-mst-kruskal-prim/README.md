# Lesson 35 — Cây khung nhỏ nhất (Minimum Spanning Tree): Kruskal & Prim
...
**Định nghĩa cut (nhát cắt).** Một **cut** $(S, V \setminus S)$ là cách chia tập đỉnh $V$ thành hai phần không rỗng $S$ và $V \setminus S$. Một cạnh **vắt qua cut** nếu một đầu thuộc $S$, đầu kia thuộc $V \setminus S$.
...
Giả sử $e = (u, v)$ là cạnh nhẹ nhất qua cut $(S, V \setminus S)$, với $u \in S, v \in V \setminus S$. Giả sử có một MST $T$ **không** chứa $e$. Ta chứng minh có thể "đổi" để được cây khung tốt hơn hoặc bằng → mâu thuẫn (nếu $e$ nhẹ nhất duy nhất).

**Bước 1.** Thêm $e$ vào $T$. Vì $T$ đã là cây khung (nối mọi đỉnh), thêm 1 cạnh nữa tạo ra **đúng một chu trình** $C$ chứa $e$.

**Bước 2.** Chu trình $C$ đi từ $u \in S$ tới $v \in V \setminus S$ rồi quay về. Vì nó bắt đầu trong $S$ và kết thúc cũng quay về $u$, nó phải **vắt qua cut ít nhất 2 lần**. Ngoài $e$, tồn tại một cạnh khác $e' \in C$ cũng vắt qua cut.

**Bước 3.** Vì $e$ là cạnh nhẹ nhất qua cut nên $w(e) \leq w(e')$ (nhẹ nhất duy nhất → $w(e) < w(e')$).
...
> **🔁 Dừng lại tự kiểm tra.** Cho cut $S = \{A, B\}$, $V \setminus S = \{C, D\}$ trong ví dụ mục 1.2. Các cạnh nào vắt qua cut? Cạnh nào nhẹ nhất?
>
> <details><summary>Đáp án</summary>
...
**Độ phức tạp:** sort $O(E \log E)$ thống trị. Phần Union-Find là $O(E \cdot \alpha(V)) \approx O(E)$ (xem mục 4). Vì $E \leq V^2$ nên $\log E \leq 2 \log V$, ta thường viết $O(E \log E)$ hoặc tương đương $O(E \log V)$.

> **⚠ Lỗi thường gặp.** Nếu KHÔNG dùng Union-Find mà kiểm tra chu trình bằng cách duyệt đồ thị (BFS/DFS) mỗi lần thêm cạnh → mỗi check tốn $O(V)$ → tổng $O(E \cdot V)$, chậm hơn hẳn. Union-Find biến mỗi check thành ~$O(1)$ amortized.
...
Kết hợp cả hai → mỗi thao tác **gần $O(1)$** (chính xác là $O(\alpha(n))$, với $\alpha$ là hàm Ackermann ngược — thực tế $\leq 4$ cho mọi $n$ trong vũ trụ).
...
> - Path compression + union by rank → gần $O(1)$ mỗi thao tác ($\alpha(n) \leq 4$).
...
| **Độ phức tạp** | $O(E \log E)$ | $O((V+E) \log V) \approx O(E \log V)$ |
| **Tốt cho** | Đồ thị **thưa** ($E \approx V$) | Đồ thị **dày** ($E \approx V^2$) |
...
> - **Prim** tốt khi đồ thị dày hoặc cho dưới dạng **adjacency matrix** (Prim phiên bản $O(V^2)$ không cần heap, rất nhanh khi $E \approx V^2$).
...
> - Prim $O(E \log V)$ + heap (hoặc $O(V^2)$), hợp đồ thị dày.
...
Travelling Salesman Problem (TSP) là NP-hard. Nhưng với metric TSP (thỏa bất đẳng thức tam giác), thuật toán dựa MST cho tour $\leq 2 \times \text{OPT}$: xây MST → DFS preorder → shortcut. MST cũng là **cận dưới** của tour tối ưu. Học kỹ ở [Lesson 49 — TSP & Approximation](../tier-8-problem-solving/index.html).
...
|-----------|----------|-----------|--------|---------|
| **Kruskal** | sort + Union-Find | $O(E \log E) = O(E \log V)$ | $O(V + E)$ | sort thống trị; UF $O(E \cdot \alpha(V)) \approx O(E)$ |
| **Prim (binary heap)** | adjacency list + heap | $O((V+E) \log V)$ | $O(V + E)$ | mỗi cạnh push/pop một lần |
| **Prim ($O(V^2)$)** | adjacency matrix | $O(V^2)$ | $O(V^2)$ | không heap; nhanh khi $E \approx V^2$ (đồ thị dày) |
| **Prim (Fibonacci heap)** | adj list + Fib heap | $O(E + V \log V)$ | $O(V + E)$ | lý thuyết tốt nhất, hằng số lớn |

- $\alpha(V)$ = hàm Ackermann ngược, $\leq 4$ cho mọi $V$ thực tế → Union-Find coi như $O(1)$.
- Vì $\log E \leq \log(V^2) = 2 \log V$, nên $O(E \log E) = O(E \log V)$.
- **Đồ thị thưa** ($E = O(V)$): Kruskal $O(V \log V)$, Prim heap $O(V \log V)$ — tương đương.
- **Đồ thị dày** ($E = O(V^2)$): Prim $O(V^2)$ (matrix) thắng Kruskal $O(V^2 \log V)$.

> **📝 Tóm tắt mục 10.** Kruskal $O(E \log E)$; Prim heap $O(E \log V)$, Prim matrix $O(V^2)$. Chọn theo mật độ đồ thị.

---

## 11. Cạm bẫy

> **⚠ Cạm bẫy 1 — Kruskal quên Union-Find.** Kiểm tra chu trình bằng BFS/DFS mỗi lần thêm cạnh → $O(V)$ mỗi check → tổng $O(E \cdot V)$. Union-Find biến thành ~$O(1)$ amortized. **Luôn dùng Union-Find cho Kruskal.**
...
> - **Kruskal** tự nhiên cho ra **rừng khung nhỏ nhất (minimum spanning forest)** — MST của mỗi thành phần. Kết quả có $V - (\text{số thành phần})$ cạnh thay vì $V-1$.
...
Đồ thị **đầy đủ ngầm**: mọi cặp điểm có cạnh, trọng số = $|x_1-x_2| + |y_1-y_2|$. Có $E = n(n-1)/2 = O(n^2)$ cạnh → đồ thị **dày** → **Prim $O(n^2)$** (không heap) tối ưu hơn Kruskal $O(n^2 \log n)$.
...
**Cách tiếp cận:** Prim phiên bản $O(V^2)$ vì đồ thị đầy đủ. **Độ phức tạp:** $O(n^2)$ thời gian, $O(n)$ bộ nhớ — tối ưu cho đồ thị dày.
...
**Cách tiếp cận:** với mỗi cạnh thử bỏ (critical) và thử ép trước (pseudo). **Độ phức tạp:** $O(E^2)$ — $E$ cạnh × $O(E \alpha)$ mỗi lần build. Đủ nhanh với $E \leq 200$ (ràng buộc đề).
