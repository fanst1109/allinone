...
// Source: GameDev/03-Systems/lesson-01-particle-systems/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 11 — Particle Systems (hệ hạt: khói, lửa, nước)
...
Theo dõi \`acc\` qua 5 frame đầu. Mỗi frame cộng $200 \\times (1/60) = 3.3333$:
...
Tổng 5 frame: 3+3+4+3+3 = **16 hạt**. Kiểm tra: 5 frame = $5/60 = 0.08333$ s, mong đợi $200 \\times 0.08333 = 16.667$ hạt. Đã phun 16, còn 0.667 "nợ" trong \`acc\` chờ frame 6 → **khớp, không mất hạt**. Số hạt/frame tự dao động 3–4 quanh trung bình 3.333 → đúng rate về dài hạn.
...
Ví dụ số: góc trung tâm $-\\pi/2$ (lên, hệ trục y hướng xuống), spread \`0.26 rad\` (~15°), speed 80–140. Một lần gọi có thể ra \`angle = −1.45 rad\`, \`speed = 112\` → \`vx = cos(−1.45)·112 ≈ 14.3\`, \`vy = sin(−1.45)·112 ≈ −111.1\`. Hạt bay gần như thẳng lên, hơi lệch phải. Lần gọi khác ra số khác → cả chùm xòe tự nhiên.
...
(a) $90 \\times (1/60) = 1.5$ hạt/frame → mỗi frame phun xen kẽ 1 hoặc 2 hạt (acc dao động 0.5 ↔ 1.0 ↔ 1.5).

(b) Sau 1 giây = 60 frame: tổng $90 \\times 1 = 90$ hạt (accumulator đảm bảo về dài hạn đúng rate; sai lệch chỉ ≤ 1 hạt đang "treo" trong acc).
</details>
...
$t = 3/4 = 0.75$. Size = $8 + (30-8)\\cdot 0.75 = 8 + 16.5 = 24.5$ px. Alpha = $0.6 + (0-0.6)\\cdot 0.75 = 0.6 - 0.45 = 0.15$ → gần như tan hết, đúng lúc cuộn khói loãng ra trước khi biến mất.
</details>
...
(a) $240 \\times (1/60) = 4.0$ hạt/frame — đúng số nguyên, hiếm gặp nhưng tiện.
...
(c) 0.5 s = 30 frame × 4 = **120 hạt**. Kiểm tra: $240 \\times 0.5 = 120$ ✓.

### Bài 2

$rate \\cdot dt = 20 \\times (1/60) = 0.3333$. \`int(0.3333) = 0\` → mỗi frame phun 0 hạt, **mãi mãi** — không hạt nào ra. Đây chính là bug "rate nhỏ hơn 1 hạt/frame".
...
Euler, mỗi frame: \`Vy += gravity·Δt\` rồi \`Px += Vx·Δt\`, \`Py += Vy·Δt\`. $gravity \\cdot \\Delta t = 400/60 = 6.667$. \`Vx\` không đổi (không có lực ngang). $Vx \\cdot \\Delta t = 200/60 = 3.333$.
...
(c) Nếu hạt ở đúng \`(300,300)\`: $dx = dy = 0$, $dist = 0$ → $dx/dist = 0/0 = \\text{NaN}$ → vận tốc thành NaN, hạt hỏng. Sửa: \`dist := math.Hypot(dx,dy) + 1e-3\` (epsilon) hoặc \`if dist < eps { return }\` bỏ qua frame đó.

### Bài 6

Bộ tham số gợi ý cho **vòi phun nước**:

- **Spawn velocity**: góc trung tâm $-\\pi/2$ (lên trên, y xuống), spread \`≈ 0.2 rad\` (~12°) cho chùm hẹp; speed 180–260 px/s (đủ cao để vọt lên rồi rơi).
