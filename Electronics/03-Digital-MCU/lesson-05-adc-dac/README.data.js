...
// Source: Electronics/03-Digital-MCU/lesson-05-adc-dac/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 05 — ADC/DAC & Lấy mẫu
...
**Định lý**: Để tái tạo chính xác một tín hiệu có thành phần tần số cao nhất là f_max, tần số lấy mẫu phải thỏa:
...
- Tai người nghe được đến ~20 kHz → f_max = 20 kHz.
- Nyquist: f_s > 2 × 20 000 = 40 000 Hz.
...
- Giọng nói có thể hiểu được ở dải 300 Hz – 3400 Hz → f_max ≈ 3.4 kHz.
...
⚠ **Lỗi thường gặp**: Nhiều người nghĩ "chỉ cần f_s = 2·f_max là đủ". Thực ra định lý yêu cầu **f_s > 2·f_max** (bất đẳng thức nghiêm), và trong thực tế phải lấy f_s ≥ 2.1 × f_max để tránh aliasing ở ranh giới, đồng thời bộ lọc anti-aliasing sẽ không lý tưởng.

### 2.3. Aliasing — hiện tượng méo khi vi phạm Nyquist

Khi f_s ≤ 2·f_max, tín hiệu tần số cao bị "hóa trang" thành tín hiệu tần số thấp hơn — gọi là **aliasing**.

**Ví dụ minh họa**: Tín hiệu sin 900 Hz được lấy mẫu với f_s = 1000 Hz:
- f_alias = |f_signal − f_s| = |900 − 1000| = **100 Hz** (thay vì 900 Hz!)
...
f_s > 2 × 150 = 300 Hz. Trong thực tế, thiết bị ECG dùng f_s = 500 Hz hoặc 1000 Hz để có biên an toàn và dạng sóng sắc nét.

</details>

📝 **Tóm tắt mục 2**:
- Lấy mẫu: đo tín hiệu tại điểm rời rạc, tần số f_s (Hz / SPS).
- Định lý Nyquist: **f_s > 2·f_max**.
...
Sau khi lấy mẫu, mỗi mẫu là giá trị thực trong khoảng [0, V_ref]. Nhưng ADC chỉ có thể biểu diễn một số hữu hạn mức — đây là **lượng tử hóa (quantization)**: chia dải biên độ thành 2^n mức rời rạc, mỗi mức cách nhau một **bước LSB** (Least Significant Bit).
...
trong đó:
- **V_ref** = điện áp tham chiếu (V) — thường là 3.3 V hoặc 5 V.
- **n** = số bit của ADC.
- **2^n** = tổng số mức lượng tử (ví dụ: 8-bit → 256 mức, 10-bit → 1024 mức).

**Sai số lượng tử tối đa** = ±1/2 LSB (làm tròn đến mức gần nhất).

### 3.3. Bốn ví dụ số

**Ví dụ 1 — ADC 8-bit, V_ref = 5 V**:
- Số mức: 2^8 = 256.
- Bước LSB = 5 / 256 = **19.53 mV**.
- Sai số tối đa = ±9.77 mV.

**Ví dụ 2 — ADC 10-bit, V_ref = 5 V** (chuẩn Arduino Uno):
- Số mức: 2^10 = 1024.
- Bước LSB = 5 / 1024 = **4.88 mV**.
- Sai số tối đa = ±2.44 mV.
- Đây là lý do đọc \`analogRead()\` trên Arduino Uno trả về 0–1023.

**Ví dụ 3 — ADC 12-bit, V_ref = 3.3 V** (chuẩn STM32, ESP32):
- Số mức: 2^12 = 4096.
- Bước LSB = 3.3 / 4096 = **0.806 mV** (< 1 mV!).
- Sai số tối đa = ±0.40 mV — đủ tốt cho nhiều ứng dụng công nghiệp.

**Ví dụ 4 — ADC 16-bit, V_ref = 5 V** (chip chuyên dụng ADS1115):
- Số mức: 2^16 = 65536.
- Bước LSB = 5 / 65536 = **76.3 µV**.
- Sai số tối đa = ±38.1 µV — dùng cho cân điện tử, đo dòng chính xác.
...
📝 **Tóm tắt mục 3**:
- Lượng tử hóa: chia dải [0, V_ref] thành 2^n mức rời rạc.
- **Bước LSB = V_ref / 2^n**.
- Sai số lượng tử: ±½ LSB (luôn có, không tránh được).
...
\`\`\`
Code_ADC = round( V_in / V_ref × (2^n − 1) )
\`\`\`

Walk-through: ADC 10-bit (n=10), V_ref = 5 V, V_in = 2.5 V:
- Code = round(2.5 / 5 × 1023) = round(0.5 × 1023) = round(511.5) = **512**

### 4.2. Công thức DAC: mã số → điện áp (ngược lại)

\`\`\`
V_out = Code_DAC / (2^n − 1) × V_ref
\`\`\`

Walk-through: DAC 10-bit, V_ref = 5 V, Code = 512:
- V_out = 512 / 1023 × 5 = **2.503 V** (gần 2.5 V, sai số do làm tròn Code)

### 4.3. Bốn ví dụ tính ADC

**Ví dụ 1 — ADC 8-bit, V_ref = 5 V, V_in = 1.0 V**:
- Code = round(1.0 / 5 × 255) = round(51) = **51**
- Kiểm tra ngược: V = 51/255 × 5 = 1.000 V ✓ (trùng khớp vì 1.0/5 đúng bội số)

**Ví dụ 2 — ADC 8-bit, V_ref = 5 V, V_in = 2.3 V**:
- Code = round(2.3 / 5 × 255) = round(117.3) = **117**
- Kiểm tra ngược: V = 117/255 × 5 = **2.294 V** (sai lệch 6 mV — đây là sai số lượng tử)

**Ví dụ 3 — ADC 10-bit, V_ref = 5 V, V_in = 3.75 V**:
- Code = round(3.75 / 5 × 1023) = round(767.25) = **767**
- Kiểm tra ngược: V = 767/1023 × 5 = **3.748 V** (sai lệch 2.4 mV ≈ 1/2 LSB)

**Ví dụ 4 — ADC 12-bit, V_ref = 3.3 V, V_in = 1.65 V** (đúng giữa dải):
- Code = round(1.65 / 3.3 × 4095) = round(2047.5) = **2048**
- Kiểm tra ngược: V = 2048/4095 × 3.3 = **1.6504 V** (sai lệch 0.4 mV)

### 4.4. Ví dụ DAC: tạo sóng sin số

Để tạo sóng sin 100 Hz bằng DAC 8-bit (V_ref = 5 V):
...
ADC 10-bit, V_ref = 5 V:
V_in = 614 / 1023 × 5 = **3.001 V** (xấp xỉ 3.0 V).

</details>

📝 **Tóm tắt mục 4**:
- ADC: \`Code = round(V_in / V_ref × (2^n − 1))\`
- DAC ngược lại: \`V_out = Code / (2^n − 1) × V_ref\`
- Luôn có sai số lượng tử ≤ ½ LSB.
...
- Bước 1: Thử MSB = 1 (tức V_ref/2). Nếu V_in > V_ref/2 → giữ bit 1; ngược lại → bit 0.
- Bước 2: Cộng thêm V_ref/4 hoặc trừ đi tùy kết quả bước trước. Tiếp tục đến bit LSB.
...
**Nguyên lý**: Dùng **2^n − 1** bộ so sánh song song, so V_in với 2^n − 1 mức điện áp cùng lúc → đọc kết quả tức thì.
...
**Ví dụ**: DAC 4-bit với V_ref = 5 V, code = 0b1010 (= 10):
- V_out = 10/15 × 5 = **3.33 V**
...
- **Độ phân giải**: 10 bit → 1024 mức → bước LSB = 5 V / 1024 = **4.88 mV**.
- **Điện áp tham chiếu mặc định**: V_ref = 5 V (hoặc 3.3 V nếu đổi cấu hình, hoặc nguồn AREF ngoài).
...
- V_A0 = 410/1023 × 5 = 2.004 V
- R_T = R × V_A0 / (5 − V_A0) = 10000 × 2.004 / 2.996 = **6.69 kΩ**
...
**Đề**: f_max = 5 kHz (đàn guitar).

**Lời giải**:
- Định lý Nyquist: f_s > 2 × f_max = 2 × 5000 = **10,000 Hz = 10 kHz**.
...
**Đề**: ADC 8-bit, V_ref = 3.3 V.

**Lời giải**:
- Số mức: 2^8 = 256.
- Bước LSB = 3.3 / 256 = **12.89 mV**.
- Sai số tối đa = ½ × 12.89 = **±6.45 mV**.

### Bài 3

**Đề**: ADC 10-bit, V_ref = 5 V, V_in = 1.8 V.

**Lời giải**:
- Bước 1: Code = round(1.8 / 5 × 1023) = round(0.36 × 1023) = round(368.28) = **368**.
- Bước 2 (kiểm tra): V_out = 368 / 1023 × 5 = 1.7987 V.
- Sai số lượng tử = 1.8 − 1.7987 = **0.0013 V = 1.3 mV**.
- Nhận xét: ½ LSB = ½ × (5/1024) = 2.44 mV; sai số thực tế 1.3 mV < ½ LSB ✓ (đúng với định lý).

### Bài 4

**Đề**: \`analogRead(A0) = 820\`, V_ref = 5 V, ADC 10-bit.

**Lời giải**:
- V_in = 820 / 1023 × 5 = **4.009 V**.
...
**Lời giải**:
- (a) SAR 10-bit, 8 kSPS: f_s = 8000 Hz >> 2 × 100 = 200 Hz ✓ về tốc độ. Độ phân giải 10-bit tạm ổn, nhưng thiếu cho ứng dụng y tế yêu cầu phân biệt tín hiệu mV cực nhỏ.
- (b) Flash 6-bit, 50 MSPS: tốc độ thừa (không cần), độ phân giải kém (6-bit = 64 mức), tiêu điện cao. **Không phù hợp**.
- (c) Sigma-Delta 16-bit, 1 kSPS: f_s = 1000 Hz > 200 Hz ✓. Độ phân giải 16-bit xuất sắc (76 µV/bit ở V_ref=5V). Nhiễu thấp, phù hợp tín hiệu sinh học.
...
**Đề**: DAC 8-bit, V_ref = 5 V, Code = 180. Sai số khi muốn xuất 3.5 V.

**Lời giải**:
- V_out = 180 / 255 × 5 = **3.529 V**.
- Code lý tưởng cho 3.5 V: Code = round(3.5 / 5 × 255) = round(178.5) = **179** (hoặc 178 hoặc 179 tùy cách làm tròn).
- Nếu Code = 179: V_out = 179/255 × 5 = 3.510 V → sai số = **0.01 V = 10 mV**.
- Nhận xét: bước LSB = 5/255 = 19.6 mV; sai số 10 mV < ½ LSB ✓.
...
- Tần số ảo (alias): f_alias = |f_signal − k × f_s| với k nguyên gần nhất.
  - k = 1: f_alias = |850 − 1000| = **150 Hz**.
...
2. **Lấy mẫu**: f_s > 2·f_max (Nyquist). Vi phạm → aliasing. Cần bộ lọc anti-aliasing trước ADC.
3. **Lượng tử hóa**: bước LSB = V_ref / 2^n. Sai số tối đa ±½ LSB. Tăng bit → tăng độ chính xác.
4. **ADC công thức**: \`Code = round(V_in / V_ref × (2^n − 1))\`. **DAC ngược**: \`V_out = Code / (2^n − 1) × V_ref\`.
5. **Kiểu ADC**: SAR (đa năng, MCU), Flash (nhanh nhất), Sigma-Delta (chính xác nhất, audio/y tế).
6. **Arduino**: \`analogRead()\` trả về 0–1023 (ADC 10-bit, V_ref=5V); \`V = raw × 5.0 / 1023.0\`.
