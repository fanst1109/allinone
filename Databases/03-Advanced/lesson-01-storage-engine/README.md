# Lesson 01 — Storage Engine (cỗ máy lưu trữ)
...
- [Nhóm 2 — Index](../../02-Intermediate/lesson-02-index/): bạn cần biết index là gì và vì sao B-tree cho tra cứu $\sim O(\log n)$.
...
Ở [Lesson 02 — Index](../../02-Intermediate/lesson-02-index/) ta nói "B-tree cho tra cứu $\sim O(\log n)$". Nhưng $O(\log n)$ **bước** ấy thực ra là **bao nhiêu lần chạm đĩa**? Và một câu rất đời thường:
...
**Vì sao không đọc từng byte.** Giả sử mỗi lần seek HDD tốn ~10 ms, không phụ thuộc đọc nhiều ít. Đọc 1 byte: 10 ms. Đọc 8 KB (một page): vẫn ~10 ms. Nhưng nếu cần 8 KB mà đọc từng byte một thì phải seek 8.192 lần $\approx \mathbf{82}$ giây. Đọc theo page biến 82 giây thành 10 ms.
...
$$8{.}168 / 80 \approx 102 \text{ row mỗi page}$$
...
| --- | --- | --- |
| 40 byte | $\lfloor 8168/40 \rfloor = 204$ | $10{.}000{.}000 / 204 \approx 49{.}020$ page |
| 80 byte | $\lfloor 8168/80 \rfloor = 102$ | $\approx 98{.}040$ page |
| 200 byte | $\lfloor 8168/200 \rfloor = 40$ | 250.000 page |
| 2.000 byte | $\lfloor 8168/2000 \rfloor = 4$ | 2.500.000 page |
...
2. $\lfloor 8168/100 \rfloor = 81$ row/page. $1{.}000{.}000 / 81 \approx \mathbf{12{.}346}$ page.
</details>
...
| --- | --- | --- |
| Đầu | — | $8192 - 100 = 8092$ |
| Thêm row0 (120 B) | slot0 → offset 8072, len 120 | $8092 - 120 - 4\ (\text{1 slot}) = 7968$ |
...
**(c) Ví dụ số.** Pool có hit ratio 95%, miss đọc đĩa SSD 100 µs, hit 100 ns. Truy cập trung bình $= 0{,}95 \cdot 0{,}1\ \text{µs} + 0{,}05 \cdot 100\ \text{µs} = 0{,}095 + 5 = \mathbf{5{,}1}$ µs. Nếu không có pool (mọi lần đều miss) = 100 µs → buffer pool nhanh hơn ~**20 lần** chỉ với 95% hit.
...
Kết quả: 6 miss, **1 hit** → hit ratio $= 1/7 \approx \mathbf{14\%}$. (Chuỗi này cố tình "bất lợi" để minh họa; tải thật có locality nên hit ratio thường 90%+.)
...
- **Đọc nhanh & ổn định:** tra một khóa = đi từ gốc xuống lá, $\sim \log n$ page. Range scan (đọc khoảng) cũng nhanh vì lá nối tiếp nhau theo thứ tự.
...
- **Read amplification (khuếch đại đọc):** số page phải đọc để trả lời một truy vấn. B-tree: $\sim \log n$ page, ổn định. LSM: có thể phải kiểm tra nhiều SSTable (giảm nhờ Bloom filter + compaction).
...
| Read amplification | Thấp & ổn định ($\sim \log n$) | Cao hơn (nhiều tầng, cần Bloom filter) |
...
> 💡 **Storage engine quyết định DB của bạn nhanh ở việc gì.** Chọn DB = chọn engine phù hợp workload: ghi nhiều hay đọc nhiều.

| Engine | Cấu trúc | Mạnh ở | DB thật |
|--------|----------|--------|---------|
| **B+ tree** | Cây cân bằng nhiều con | Đọc + range query, đọc-nhiều | PostgreSQL, MySQL InnoDB, SQLite |
| **LSM-tree** | Memtable + SSTable + compaction | Ghi nhiều tuần tự (write-heavy) | Cassandra, RocksDB, LevelDB, ScyllaDB |
| **Hash index** | Hash table | Tra cứu khóa chính xác, không range | Redis, một phần engine |
| **Column store** | Lưu theo cột | Phân tích/aggregate (OLAP) | ClickHouse, Parquet ([data warehouse](../lesson-05-data-warehouse-olap/)) |

### 7.1. Ví dụ cụ thể — vì sao Cassandra ghi nhanh hơn Postgres

Postgres (B+ tree): mỗi INSERT phải tìm đúng chỗ trong cây + cập nhật → ghi **ngẫu nhiên** trên đĩa. Cassandra (LSM-tree): ghi **tuần tự** vào memtable + WAL (rất nhanh), gộp xuống SSTable sau (compaction nền). Với log/metrics/IoT (ghi cực nhiều, đọc point), LSM thắng. Đổi lại đọc point chậm hơn (phải tra nhiều SSTable → mỗi SSTable có [Bloom filter](../../../DataStructures/03-Advanced/lesson-04-advanced-structures/) lọc nhanh). Đây là cùng cấu trúc học ở [advanced structures](../../../DataStructures/03-Advanced/lesson-04-advanced-structures/), giờ là lựa chọn DB thật.

> ❓ **"Làm sao biết app cần engine nào?"** Hỏi: **đọc nhiều hay ghi nhiều?** Web app điển hình (đọc >> ghi, cần range/join) → B+ tree (Postgres/MySQL). Log/metrics/time-series/feed (ghi >> đọc) → LSM (Cassandra/RocksDB). Phân tích trên tỉ dòng → column store (ClickHouse). Cache khóa-giá trị → hash (Redis).

### 7.2. 📝 Tóm tắt mục 7

- Storage engine = lựa chọn nền: **B+ tree** (đọc/range, web app), **LSM** (ghi nhiều, log/metrics), **hash** (tra khóa, cache), **column** (OLAP).
- Cassandra/RocksDB ghi nhanh nhờ LSM (ghi tuần tự + compaction nền), đổi lại đọc point chậm hơn (Bloom filter giúp).
- Chọn theo workload đọc/ghi — đây là quyết định chọn DB thực tế.
...
- **(a)** $\lfloor 8000 / 100 \rfloor = \mathbf{80}$ row/page.
- **(b)** $5{.}000{.}000 / 80 = \mathbf{62{.}500}$ page để lưu cả bảng.
- **(c)** $10{.}000$ frame $/ 62{.}500$ page $= 0{,}16 = \mathbf{16\%}$ số page của bảng nằm trong RAM cùng lúc. (Nếu tải có locality tốt, 16% page nóng này vẫn có thể phục vụ phần lớn truy cập → hit ratio cao hơn nhiều so với 16%.)
