...
// Source: AI-ML/01-Foundations/lesson-01-ml-pipeline/README.md
...
|------|--------|--------------------------|
| 3 — Calculus | [L06 Gradient](../../03-Calculus/lesson-06-partial-gradient/) | Step 5 (Train): gradient của loss theo $\\theta$ |
| 3 — Calculus | [L07 Gradient descent](../../03-Calculus/lesson-07-gradient-descent/) | Step 5: vòng lặp $\\theta \\leftarrow \\theta - \\eta \\cdot \\nabla L$ |
| 4 — Linear Algebra | [L01 Vectors](../../04-LinearAlgebra/lesson-01-vectors/) | Feature vector $x = (x_1, x_2, \\dots, x_n)$ |
| 4 — Linear Algebra | [L05 Matrices](../../04-LinearAlgebra/lesson-05-matrices/) | Batch data $X$ là ma trận $N \\times d$ |
...
|------|---------|--------|-------|
| **Supervised** (có giám sát) | $(x, y)$ — input + nhãn | Hàm $f(x) \\approx y$ | Phân loại email, dự đoán giá nhà |
...
| 2. Feature | Biểu diễn input thế nào để máy "hiểu"? | Vector số $x \\in \\mathbb{R}^d$ cho mỗi mẫu |
| 3. Model | Hàm dự đoán có dạng gì? | $\\hat{y} = f_\\theta(x)$ với params $\\theta$ |
| 4. Loss | Đo sai bao nhiêu? | Số thực $L(\\theta) \\geq 0$ |
| 5. Train | Điều chỉnh $\\theta$ ra sao? | $\\theta^*$ (tham số tối ưu) |
...
2. **Feature.** Đã là vector số $x = (5{,}1, 3{,}5, 1{,}4, 0{,}2) \\in \\mathbb{R}^4$. Standardize từng cột về mean 0, std 1.
3. **Model.** Dùng *logistic regression* multinomial — $\\hat{y} = \\text{softmax}(Wx + b)$ với $W \\in \\mathbb{R}^{3 \\times 4}$, $b \\in \\mathbb{R}^3$. Params $\\theta = (W, b)$ gồm 15 số.
4. **Loss.** Cross-entropy: $L = -\\sum y \\cdot \\log(\\hat{y})$.
5. **Train.** Gradient descent 500 epoch, $\\eta = 0{,}1$, batch size 32.
...
> 2. Tham số $\\theta$ xuất hiện ở bước nào?
>
...
> 2. Step 3 (định nghĩa $f_\\theta$), Step 4 (loss $L(\\theta)$ phụ thuộc $\\theta$), Step 5 (cập nhật $\\theta$). Step 1, 2, 6 không động vào $\\theta$.
> </details>
...
**Feature** = một con số (hoặc vector số) biểu diễn một khía cạnh của mẫu. Mỗi mẫu → một vector $x \\in \\mathbb{R}^d$.
...
với $\\mu$, $\\sigma$ tính trên **train set**. Sau standardize: mean ≈ 0, std ≈ 1.

**Ví dụ.** Tuổi của 5 người: $[20, 30, 40, 50, 60]$.

- $\\mu = (20+30+40+50+60)/5 = 200/5 = 40$.
- $\\sigma^2 = ((20-40)^2 + (30-40)^2 + (40-40)^2 + (50-40)^2 + (60-40)^2)/5 = (400+100+0+100+400)/5 = 200$. $\\sigma = \\sqrt{200} \\approx 14{,}14$.
- Sau standardize: $[-1{,}41, -0{,}71, 0, 0{,}71, 1{,}41]$. Mean = 0 ✓. Var = 1 ✓ ($(2 \\cdot 1{,}41^2 + 2 \\cdot 0{,}71^2)/5 = (2 \\cdot 2 + 2 \\cdot 0{,}5)/5 = 5/5 = 1$ ✓).
...
- $x_{\\text{min}} = 20$, $x_{\\text{max}} = 60$, range = 40.
- $[20, 30, 40, 50, 60]$ → $[(20-20)/40, (30-20)/40, \\dots, (60-20)/40]$ = $[0, 0{,}25, 0{,}5, 0{,}75, 1{,}0]$.
...
- $(5{,}1 - 5{,}84)/0{,}83 = -0{,}74/0{,}83 \\approx -0{,}89$
- $(3{,}5 - 3{,}05)/0{,}43 = 0{,}45/0{,}43 \\approx 1{,}05$
- $(1{,}4 - 3{,}76)/1{,}76 = -2{,}36/1{,}76 \\approx -1{,}34$
- $(0{,}2 - 1{,}20)/0{,}76 = -1{,}00/0{,}76 \\approx -1{,}32$

→ $x' = (-0{,}89, 1{,}05, -1{,}34, -1{,}32)$. Mỗi chiều giờ trong khoảng $[-2, 2]$.

> **⚠ Lỗi thường gặp 1.** Standardize VAL/TEST bằng $\\mu, \\sigma$ của chính val/test → leakage. Phải dùng $\\mu_{\\text{train}}, \\sigma_{\\text{train}}$.
>
> **⚠ Lỗi thường gặp 2.** One-hot cho feature có *quá nhiều giá trị* (ZIP code → 41.000 cột). Thay bằng embedding hoặc target encoding.

> **🔁 Dừng lại tự kiểm tra.**
> Cột \`petal_length\` có giá trị $[1{,}4, 4{,}5, 1{,}3, 5{,}1, 4{,}7]$. Standardize cột này (tính tay $\\mu$, $\\sigma$, rồi giá trị mới).
>
> <details><summary>Đáp án</summary>
>
> - $\\mu = (1{,}4 + 4{,}5 + 1{,}3 + 5{,}1 + 4{,}7)/5 = 17{,}0/5 = 3{,}4$.
> - Lệch bình phương: $(1{,}4-3{,}4)^2 + (4{,}5-3{,}4)^2 + (1{,}3-3{,}4)^2 + (5{,}1-3{,}4)^2 + (4{,}7-3{,}4)^2$ = $4 + 1{,}21 + 4{,}41 + 2{,}89 + 1{,}69 = 14{,}20$.
> - $\\sigma^2 = 14{,}20/5 = 2{,}84$, $\\sigma = \\sqrt{2{,}84} \\approx 1{,}685$.
> - Giá trị mới: $(1{,}4-3{,}4)/1{,}685 \\approx -1{,}187$, $(4{,}5-3{,}4)/1{,}685 \\approx 0{,}653$, $(1{,}3-3{,}4)/1{,}685 \\approx -1{,}246$, $(5{,}1-3{,}4)/1{,}685 \\approx 1{,}009$, $(4{,}7-3{,}4)/1{,}685 \\approx 0{,}772$.
> - Kiểm: tổng ≈ 0 ✓, var ≈ 1 ✓.
> </details>

> **📝 Tóm tắt mục 4.**
> - Feature = vector số. Mọi raw input cuối cùng phải thành $x \\in \\mathbb{R}^d$.
...
> - Tính $\\mu, \\sigma$ trên **train** rồi áp dụng cho val/test — không tính lại.
...
Một model $f_\\theta: \\mathbb{R}^d \\to \\mathbb{R}^k$ là **hàm** biến input thành prediction. $\\theta$ là vector tham số mà ta sẽ điều chỉnh.

| Model | Dạng $f_\\theta$ | Số params |
|-------|-----------|-----------|
| Linear regression | $f_\\theta(x) = w^\\top x + b$ | $d + 1$ |
| Logistic regression nhị phân | $f_\\theta(x) = \\sigma(w^\\top x + b)$ | $d + 1$ |
| Logistic regression k-lớp | $f_\\theta(x) = \\text{softmax}(Wx + b)$, $W \\in \\mathbb{R}^{k \\times d}$ | $k \\cdot d + k$ |
| Neural network 1 hidden | $f_\\theta(x) = W_2 \\cdot \\text{ReLU}(W_1 x + b_1) + b_2$ | $h \\cdot d + h + k \\cdot h + k$ |
| Transformer-base (BERT) | (rất phức tạp) | ~110 triệu |
| GPT-4 | (rất phức tạp) | ~1.7 nghìn tỷ (ước lượng) |

> **💡 Trực giác.** Mỗi giá trị $\\theta$ cụ thể là một "phiên bản model" khác. Tập tất cả model có dạng $f_\\theta$ (khi $\\theta$ chạy khắp $\\mathbb{R}^p$) gọi là **hypothesis space**. Training = chọn $\\theta^*$ tốt nhất trong không gian đó.
...
- $z_0 = 0{,}5 \\cdot (-0{,}89) + 0{,}4 \\cdot 1{,}05 + (-1{,}5) \\cdot (-1{,}34) + (-1{,}4) \\cdot (-1{,}32) + 0{,}2$
  $= -0{,}445 + 0{,}420 + 2{,}010 + 1{,}848 + 0{,}200 = 4{,}033$
- $z_1 = 0{,}1 \\cdot (-0{,}89) + (-0{,}2) \\cdot 1{,}05 + 0{,}3 \\cdot (-1{,}34) + 0{,}4 \\cdot (-1{,}32) + 0{,}0$
  $= -0{,}089 - 0{,}210 - 0{,}402 - 0{,}528 + 0{,}0 = -1{,}229$
- $z_2 = (-0{,}6) \\cdot (-0{,}89) + (-0{,}2) \\cdot 1{,}05 + 1{,}2 \\cdot (-1{,}34) + 1{,}0 \\cdot (-1{,}32) + (-0{,}2)$
  $= 0{,}534 - 0{,}210 - 1{,}608 - 1{,}320 - 0{,}200 = -2{,}804$

Tính softmax:

- $e^{z_0} = e^{4{,}033} \\approx 56{,}45$
- $e^{z_1} = e^{-1{,}229} \\approx 0{,}293$
- $e^{z_2} = e^{-2{,}804} \\approx 0{,}0607$
- Tổng $\\approx 56{,}81$.
- $\\hat{y} = (56{,}45/56{,}81, 0{,}293/56{,}81, 0{,}0607/56{,}81) \\approx (0{,}9937, 0{,}0052, 0{,}0011)$.

→ Model đoán **setosa** với xác suất 99.4%. Đúng (mẫu này thực sự là setosa).

> **❓ Người mới hay hỏi.**
> - *"Tại sao softmax mà không chia thẳng $z_i / \\sum z_j$?"* — $z_i$ có thể âm, làm chia ra số âm hoặc chia 0. $e^{z_i}$ luôn dương + có tính chất "lớn lên rất nhanh" → phân biệt rõ class thắng.
...
> **📝 Tóm tắt mục 5.**
> - Model = hàm $f_\\theta(x)$ với tham số $\\theta$.
> - Mỗi loại model có inductive bias riêng.
> - Logistic regression k-lớp: $\\text{softmax}(Wx + b)$ — sẽ học kỹ Lesson 03.
...
Để **tối ưu** ta cần một con số đo độ sai. Loss $L(\\theta)$ là hàm từ params $\\theta$ → số thực $\\geq 0$. Mục tiêu: tìm $\\theta^*$ cực tiểu hóa $L$.
...
|----------|---------------|-----------|
| Regression (đầu ra số thực) | MSE | $L = \\frac{1}{N} \\sum (\\hat{y}_i - y_i)^2$ |
| Regression (robust với outlier) | MAE | $L = \\frac{1}{N} \\sum |\\hat{y}_i - y_i|$ |
| Classification (k lớp) | Cross-entropy | $L = -\\frac{1}{N} \\sum \\sum y_{ik} \\log \\hat{y}_{ik}$ |
| Classification (margin-based) | Hinge | $L = \\max(0, 1 - y \\cdot \\hat{y})$ |

### 6.3 MSE — Mean Squared Error

Bài toán: dự đoán giá nhà. Nhãn thật $y = (300, 450, 200)$ (triệu VND), dự đoán $\\hat{y} = (310, 440, 220)$.

- Sai từng mẫu: $310-300 = 10$, $440-450 = -10$, $220-200 = 20$.
- Bình phương: $100, 100, 400$.
- Trung bình: $(100 + 100 + 400)/3 = 600/3 = 200$.

→ MSE = 200. Đơn vị: (triệu VND)². RMSE = $\\sqrt{200} \\approx 14{,}14$ — dùng cùng đơn vị gốc cho dễ diễn giải.
...
Một mẫu Iris thật là setosa, ta encode $y = (1, 0, 0)$ (one-hot). Model dự đoán $\\hat{y} = (0{,}99, 0{,}005, 0{,}005)$.
...
Giả sử model đoán sai: $\\hat{y} = (0{,}1, 0{,}8, 0{,}1)$ (đoán versicolor).
...
**Trường hợp cực đoan**: model đoán $\\hat{y}_{\\text{setosa}} = 0$ → $-\\log 0 = +\\infty$. Vì vậy thực tế softmax không bao giờ trả về 0 chính xác, và ta thường clip xuống \`1e-7\` để tránh overflow.

**Liên hệ Tầng 5 Lesson 08**: cross-entropy = $H(p) + \\text{KL}(p \\| q)$, đo "p đẹp" giữa nhãn thật $p = y$ và prediction $q = \\hat{y}$.

### 6.5 Bảng walk-through full Iris (3 mẫu)

| # | Lớp thật $y$ | Prediction $\\hat{y}$ | Cross-entropy |
|---|---|---|---|
...
Loss tổng: $L = (0{,}01005 + 0{,}16252 + 0{,}91629)/3 = 1{,}08886/3 \\approx 0{,}36295$.

→ Mẫu #3 đóng góp ~84% loss. Gradient descent sẽ "kéo" model điều chỉnh chủ yếu vì #3.

> **⚠ Lỗi thường gặp.** Dùng MSE cho classification 3+ lớp. MSE không phạt đủ nặng khi model tự tin sai, và gradient bão hòa (đi rất chậm) khi $\\hat{y}$ gần 0/1. → Cross-entropy luôn ưu việt hơn cho classification.

> **🔁 Dừng lại tự kiểm tra.**
> Nhãn thật $y = 0$ (lớp 0 trong 3 lớp). Prediction softmax $\\hat{y} = (0{,}7, 0{,}2, 0{,}1)$. Tính cross-entropy.
>
> <details><summary>Đáp án</summary>
>
> $-\\log 0{,}7 \\approx -(-0{,}3567) = 0{,}3567$. Kiểm: model gần đúng (0.7 > 0.2, 0.1 nhưng chưa "tự tin"), nên loss vừa phải.
> </details>

> **📝 Tóm tắt mục 6.**
> - Loss = thước đo "sai bao nhiêu", càng nhỏ càng tốt.
> - Regression: MSE ($(\\hat{y}-y)^2$ trung bình).
> - Classification: cross-entropy ($-\\sum y \\log \\hat{y}$), phạt nặng tự tin sai.
...
$L(\\theta)$ là hàm của params. Để minimize, lặp:

\`\`\`
θ ← θ − η · ∇L(θ)
\`\`\`

- $\\nabla L(\\theta)$ = gradient của loss theo $\\theta$ (đã học tính ở Tầng 3 L06-L07).
- $\\eta$ (eta) = **learning rate**, độ dài bước.

Tại sao hướng $-\\nabla L$? Vì $\\nabla L$ chỉ về hướng **tăng** nhanh nhất; lấy âm để đi xuống nhanh nhất.
...
| **Batch** | Một nhóm mẫu để tính gradient ước lượng | Mỗi batch tính một gradient → cập nhật $\\theta$ một lần |
| **Iteration / Step** | Một lần cập nhật $\\theta$ | 1 epoch = $\\lceil N/\\text{batch} \\rceil$ iteration |
| **Learning rate $\\eta$** | Độ dài bước | $0{,}001$ (chậm an toàn), $0{,}1$ (nhanh nhưng dễ nổ) |

#### Walk-through số: 500 epoch trên Iris

- $N = 90$, batch = 32 → 3 iter/epoch.
- 500 epoch × 3 = **1500 iteration** cập nhật $\\theta$.
- Loss curve điển hình:
  - Epoch 1: loss ≈ 1.10 (gần $\\log 3 \\approx 1{,}10$ — random).
  - Epoch 50: loss ≈ 0.30.
  - Epoch 200: loss ≈ 0.10.
...
| $\\eta$ | Hành vi | Loss curve |
|-----|---------|-----------|
| Quá nhỏ ($10^{-6}$) | Hội tụ siêu chậm | Loss giảm chậm, có thể dừng trước khi hội tụ |
| Vừa ($10^{-2} \\sim 10^{-3}$) | Hội tụ ổn định | Loss giảm trơn |
| Quá lớn ($1{,}0$) | Bước nhảy quá xa, "vọt qua" cực tiểu | Loss dao động, hoặc tăng → NaN |

**Trick thực tế**:

- **LR schedule**: bắt đầu lớn ($0{,}1$), giảm dần ($\\times 0{,}1$ mỗi 100 epoch).
- **Warmup**: 5 epoch đầu tăng $\\eta$ từ 0 lên $\\eta_{\\text{max}}$, sau đó schedule giảm.
- **Adam optimizer**: tự động điều chỉnh $\\eta$ cho từng tham số (thay cho SGD thuần).
...
Áp dụng cho mẫu #3 Iris (Mục 6.5): $y = (0, 0, 1)$, $\\hat{y} = (0{,}30, 0{,}30, 0{,}40)$. Gradient:

\`\`\`
∂L/∂z = (0.30 − 0, 0.30 − 0, 0.40 − 1) = (0.30, 0.30, −0.60)
\`\`\`

Diễn giải: nâng $z_2$ lên (gradient âm theo $z_2$ → tăng $z_2$ để giảm loss), hạ $z_0, z_1$ xuống. Đúng trực giác: model nên tự tin hơn vào lớp 2.
...
3. **Gradient norm $< \\epsilon$** (hiếm dùng trong DL vì không bao giờ thật sự = 0).
...
> **📝 Tóm tắt mục 7.**
> - Train = lặp $\\theta \\leftarrow \\theta - \\eta \\cdot \\nabla L(\\theta)$.
> - Epoch = 1 lượt qua data. Batch = nhóm để tính gradient ước lượng.
> - Learning rate $\\eta$ phải vừa: quá lớn → nổ, quá nhỏ → chậm.
...
- $\\text{TP} = 40$, $\\text{FN} = 50 - 40 = 10$, $\\text{FP} = 60 - 40 = 20$, $\\text{TN} = 1000 - 50 - 20 = 930$.
...
**⚠ Cẩn thận**: với data imbalance, accuracy rất "lừa". Model luôn đoán "bình thường" → $\\text{Acc} = 950/1000 = 95\\%$, nhưng *bỏ sót 100% gian lận*.
...
|--------|-----------|----------|
| MSE | $\\frac{1}{N}\\sum(\\hat{y}-y)^2$ | Sai bình phương trung bình |
| RMSE | $\\sqrt{\\text{MSE}}$ | Cùng đơn vị với $y$ |
| MAE | $\\frac{1}{N}\\sum|\\hat{y}-y|$ | Robust với outlier |
| R² | $1 - \\text{SS}_{\\text{res}}/\\text{SS}_{\\text{tot}}$ | Phần trăm variance giải thích được, $\\in (-\\infty, 1]$ |

**Ví dụ R²**: thật $y = (300, 450, 200)$, dự đoán $\\hat{y} = (310, 440, 220)$.

- $\\bar{y} = (300+450+200)/3 = 316{,}67$.
- $\\text{SS}_{\\text{tot}} = (300-316{,}67)^2 + (450-316{,}67)^2 + (200-316{,}67)^2 = 277{,}7 + 17777{,}7 + 13611{,}1 \\approx 31666{,}5$.
- $\\text{SS}_{\\text{res}} = (300-310)^2 + (450-440)^2 + (200-220)^2 = 100 + 100 + 400 = 600$.
- $R^2 = 1 - 600/31666{,}5 \\approx 1 - 0{,}0190 \\approx 0{,}981$ → giải thích được 98.1% biến động.
...
> <details><summary>Đáp án</summary>
>
> - $\\text{Acc} = (80 + 860)/(80+20+40+860) = 940/1000 = 0{,}940$.
> - $\\text{Prec} = 80/(80+20) = 80/100 = 0{,}800$.
> - $\\text{Rec} = 80/(80+40) = 80/120 \\approx 0{,}667$.
> - $\\text{F1} = 2 \\cdot (0{,}8 \\cdot 0{,}667)/(0{,}8+0{,}667) = 2 \\cdot 0{,}5336/1{,}467 \\approx 0{,}7273$.
> </details>
...
| **L2 regularization** | Cộng $\\lambda \\cdot \\|\\theta\\|^2$ vào loss → params bị "kéo" về 0 | $\\lambda$ |
| **L1 regularization** | Cộng $\\lambda \\cdot \\|\\theta\\|_1$ → ép một số params = 0 (sparse) | $\\lambda$ |
...
Loss mới: $L_{\\text{total}} = L_{\\text{data}} + \\lambda \\cdot \\|\\theta\\|^2$.

**Ví dụ**: linear regression $y = wx + b$. Loss data trung bình $0{,}05$, $w = 3{,}0$, $b = 0{,}5$, $\\lambda = 0{,}1$.

- Penalty L2 = $0{,}1 \\cdot (3{,}0^2 + 0{,}5^2) = 0{,}1 \\cdot 9{,}25 = 0{,}925$.
...
**Liên hệ Tầng 4 Lesson 03**: $\\|\\theta\\|^2$ chính là bình phương của norm L2 — đã học ở bài về norm và khoảng cách.

#### L1 — chi tiết

Loss mới: $L_{\\text{total}} = L_{\\text{data}} + \\lambda \\cdot \\|\\theta\\|_1 = L_{\\text{data}} + \\lambda \\cdot \\sum |\\theta_i|$.
...
Trong NN, ở mỗi training step, mỗi neuron có xác suất $p$ bị "tắt" (output × 0). Khi inference, không drop, nhưng scale output $\\times (1-p)$ để giữ kỳ vọng.
...
### 9.4 Walk-through chọn $\\lambda$ bằng val set

Thử $\\lambda \\in \\{0{,}001, 0{,}01, 0{,}1, 1{,}0, 10\\}$. Train với mỗi $\\lambda$ trên train set, đo val loss:
...
→ Chọn $\\lambda = 0{,}1$, train lại trên train+val (tùy chuẩn) rồi report test loss.

> **❓ Người mới hay hỏi.**
> - *"Sao không thử mọi $\\lambda$ rồi chọn theo test set?"* Không. Test set chỉ dùng MỘT lần ở cuối. Dùng test để chọn $\\lambda$ = data leakage cấp thấp → test loss bị biased lạc quan.
> - *"Có bắt buộc dùng regularization không?"* Không nếu model đã đơn giản hoặc data đủ lớn. Linear regression với $N=10^6$ thường không cần. Deep NN gần như luôn cần.
...
Iris 150 mẫu, val = 30. **Nếu val xui gặp 30 mẫu khó** → đo $\\text{val\\_loss} = 0{,}5$. Nếu gặp 30 mẫu dễ → $\\text{val\\_loss} = 0{,}1$. Đo không tin cậy.
...
- Lần 1: val = fold 1, train = fold 2-5 (72 mẫu) → $\\text{val\\_loss}_1 = 0{,}15$.
- Lần 2: val = fold 2, train = fold 1, 3-5 → $\\text{val\\_loss}_2 = 0{,}12$.
- Lần 3: val = fold 3, train = ... → $\\text{val\\_loss}_3 = 0{,}18$.
- Lần 4: $\\text{val\\_loss}_4 = 0{,}10$.
- Lần 5: $\\text{val\\_loss}_5 = 0{,}14$.
- CV score = $(0{,}15 + 0{,}12 + 0{,}18 + 0{,}10 + 0{,}14)/5 = 0{,}69/5 = 0{,}138$.
- Độ lệch: $\\text{std} \\approx 0{,}030$.
...
| Lesson | Đổi gì? |
|--------|---------|
| **L02 Linear regression** | Model $w^\\top x + b$, loss MSE, closed-form solution + GD. |
| **L03 Logistic regression** | Model $\\sigma(w^\\top x + b)$, loss cross-entropy binary, sigmoid. |
| **L04 Neural network** | Model nhiều tầng $f = W_2 \\cdot \\text{act}(W_1 x + b_1) + b_2$, backprop. |
...
Cột $\\text{tuổi} = [25, 30, 35, 40, 45, 50]$. Tính standardized và min-max normalized.

### Bài 3 — Tính tay (Softmax + cross-entropy)

Cho $z = (2{,}0, 1{,}0, 0{,}1)$ và nhãn thật là lớp 0 (one-hot $y = (1, 0, 0)$).

1. Tính $\\hat{y} = \\text{softmax}(z)$.
2. Tính cross-entropy.
3. Tính $\\partial L/\\partial z = \\hat{y} - y$.
...
Dataset có hàm thật $y = x^2$ (parabola). Fit bằng:
1. Linear $y = ax + b$.
2. Polynomial bậc 2 $y = ax^2 + bx + c$.
3. Polynomial bậc 20.
...
Train logistic regression với 5 giá trị $\\lambda$:
...
2. **Sai**. Phải tính $\\mu, \\sigma$ chỉ trên train set, sau đó áp dụng cùng $\\mu, \\sigma$ cho test. Tính trên toàn bộ → leakage.
...
- $\\mu = (25+30+35+40+45+50)/6 = 225/6 = 37{,}5$.
- Bình phương lệch: $(25-37{,}5)^2 + \\dots + (50-37{,}5)^2$ = $156{,}25 + 56{,}25 + 6{,}25 + 6{,}25 + 56{,}25 + 156{,}25 = 437{,}5$.
- $\\sigma^2 = 437{,}5/6 \\approx 72{,}917$, $\\sigma \\approx 8{,}539$.
- Kết quả: $[(25-37{,}5)/8{,}539, \\dots]$ = $[-1{,}464, -0{,}879, -0{,}293, 0{,}293, 0{,}879, 1{,}464]$.
- Kiểm: tổng ≈ 0 ✓, var ≈ 1 ✓.
...
1. **Softmax**:
   ...
   - Tổng $\\approx 11{,}212$.
   - $\\hat{y} = (7{,}389/11{,}212, 2{,}718/11{,}212, 1{,}105/11{,}212) \\approx (0{,}659, 0{,}242, 0{,}099)$.
   - Kiểm: tổng = 0.659 + 0.242 + 0.099 = 1.000 ✓.

2. **Cross-entropy**: $-\\log(\\hat{y}_0) = -\\log(0{,}659) \\approx -(-0{,}417) = 0{,}417$.

3. **Gradient**: $\\partial L/\\partial z = \\hat{y} - y = (0{,}659 - 1, 0{,}242 - 0, 0{,}099 - 0) = (-0{,}341, 0{,}242, 0{,}099)$.
   - Diễn giải: tăng $z_0$ (gradient âm), giảm $z_1, z_2$ (gradient dương).

### Bài 4

1. - Tổng = 120 + 30 + 80 + 770 = 1000.
   - $\\text{Acc} = (120 + 770)/1000 = 890/1000 = 0{,}890$.
   - $\\text{Prec} = 120/(120 + 30) = 120/150 = 0{,}800$.
   - $\\text{Rec} = 120/(120 + 80) = 120/200 = 0{,}600$.
   - $\\text{F1} = 2 \\cdot (0{,}8 \\cdot 0{,}6)/(0{,}8+0{,}6) = 2 \\cdot 0{,}48/1{,}4 \\approx 0{,}6857$.
...
- Linear $y = ax + b$: không bao giờ fit được $x^2$ → **bias cao**, **variance thấp** (mỗi lần resample data, đường thẳng cũng không đổi nhiều).
- Polynomial bậc 2: chính là dạng $y = x^2$ → **bias = 0** (về lý thuyết), **variance thấp** (3 tham số đủ stable).
...
- $\\lambda = 0{,}001$: train 99%, val 82% → khoảng cách 17% → **overfit nặng**.
- $\\lambda = 0{,}01$: vẫn overfit nhẹ (95% vs 88%).
- **$\\lambda = 0{,}1$**: train 92%, val 91% → khoảng cách rất nhỏ, val cao nhất → **chọn**.
- $\\lambda = 1{,}0$: bắt đầu underfit (cả train + val xuống).
- $\\lambda = 10$: underfit nặng.

→ **Chọn $\\lambda = 0{,}1$**. Retrain trên toàn bộ \`X_train\` (có thể bao gồm cả val), sau đó report accuracy trên test set.
