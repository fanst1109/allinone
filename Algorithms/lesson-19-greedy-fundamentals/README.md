# Lesson 19 — Greedy Fundamentals (Nền tảng thuật toán tham lam)
...
**Định nghĩa hình thức.** Gọi `g` là lựa chọn tham lam tại bước 1. *Greedy-choice property* đúng nếu: với mọi instance, tồn tại lời giải tối ưu `OPT` sao cho `g ∈ OPT`.
...
- Gọi `G = g₁, g₂, …, g_k` là dãy hoạt động greedy chọn (đã sắp theo `f`). Gọi `OPT = o₁, o₂, …, o_m` là một lời giải tối ưu, cũng sắp theo `f`. Ta muốn chứng minh `k = m` (greedy chọn được nhiều bằng tối ưu).
- **Bổ đề (claim):** với mọi `r`, `f(g_r) ≤ f(o_r)` — hoạt động thứ `r` mà greedy chọn kết thúc *không muộn hơn* hoạt động thứ `r` của OPT.
- **Chứng minh bổ đề bằng quy nạp theo `r`:**
  - *Cơ sở `r = 1`:* greedy chọn hoạt động kết thúc sớm nhất trong tất cả, nên `f(g₁) ≤ f(o₁)`. ✓
  - *Bước quy nạp:* giả sử `f(g_{r-1}) ≤ f(o_{r-1})`. Vì OPT hợp lệ, `o_r` bắt đầu sau khi `o_{r-1}` kết thúc: `s(o_r) ≥ f(o_{r-1}) ≥ f(g_{r-1})`. Vậy `o_r` cũng tương thích với `g_{r-1}` → `o_r` là **ứng viên** mà greedy được phép chọn tại bước `r`. Greedy chọn hoạt động tương thích **kết thúc sớm nhất**, nên `f(g_r) ≤ f(o_r)`. ✓
- **Kết luận từ bổ đề:** giả sử phản chứng `m > k`, tức OPT có hoạt động `o_{k+1}`. Vì `f(g_k) ≤ f(o_k) ≤ s(o_{k+1})`, hoạt động `o_{k+1}` tương thích với `g_k`, nên greedy lẽ ra đã chọn thêm nó (greedy chỉ dừng khi không còn hoạt động tương thích). Mâu thuẫn với việc greedy dừng ở `g_k`. Vậy `m = k`. **Greedy tối ưu.** ∎
...
**Chiến lược greedy:** sort theo `ratio = v/w` **giảm dần**; lấy hết từng món cho tới khi không đủ chỗ thì lấy **một phần** món cuối cho đầy.
...
| Tốc độ điển hình | `O(n log n)` | `O(n·W)`, `O(n²)`... |
| Bộ nhớ | thường `O(1)`–`O(n)` | bảng `O(n)`–`O(n·W)` |
...
- **Bổ đề:** `f(g_r) ≤ f(o_r)` với mọi `r ≤ k`.
  - *Cơ sở:* `g₁` kết thúc sớm nhất toàn cục → `f(g₁) ≤ f(o₁)`.
  - *Quy nạp:* giả sử `f(g_{r-1}) ≤ f(o_{r-1})`. Do OPT hợp lệ: `s(o_r) ≥ f(o_{r-1}) ≥ f(g_{r-1})`, nên `o_r` tương thích với `g_{r-1}`, là ứng viên hợp lệ ở bước `r`. Greedy chọn hoạt động *kết thúc sớm nhất* trong các ứng viên → `f(g_r) ≤ f(o_r)`.
- **Suy ra `k = m`:** giả sử `m > k`. Khi đó `o_{k+1}` tồn tại và `s(o_{k+1}) ≥ f(o_k) ≥ f(g_k)` (theo bổ đề) → `o_{k+1}` tương thích với `g_k`. Vậy còn ứng viên hợp lệ → greedy *không thể* đã dừng ở `g_k`. Mâu thuẫn. Do đó `m = k`.

**Vì sao bước "đổi" không làm lời giải tệ đi:** trong cách trình bày "stays ahead", phép đổi ngầm là thay `o_r` bằng `g_r`. Vì `f(g_r) ≤ f(o_r)`, hoạt động `g_r` kết thúc *không muộn hơn* `o_r`, nên mọi hoạt động sau (`o_{r+1}..`) vẫn tương thích với `g_r` (chúng đã tương thích với `o_r` có `f` lớn hơn). Số lượng hoạt động không đổi (`OPT'` vẫn `m` cái) → vẫn tối ưu. Lặp lại với mọi `r` → biến OPT thành G mà số lượng giữ nguyên → **G tối ưu**. ∎
