...
| **Suy luận (Inferential)** | Tổng thể *thực sự* như thế nào? | "Với CI 95%, lương trung bình cả nước $\\in [14{,}2M, 15{,}8M]$." |
...
- **Tham số (parameter)**: đại lượng mô tả *tổng thể* — ký hiệu chữ Hy Lạp: $\\mu$ (mean), $\\sigma$ (SD), $\\rho$ (correlation).
- **Thống kê lượng (statistic)**: đại lượng tính từ *mẫu* — ký hiệu chữ Latin: $\\bar{x}$ (sample mean), $s$ (sample SD), $r$ (sample correlation).
...
| **Mean** | $\\mu = ?$ (không biết) | $\\bar{x} = 58{,}2$ kg (tính được) |
| **SD** | $\\sigma = ?$ | $s = 11{,}4$ kg |
...
- Parameter $\\mu$: tỉ lệ tiêm vaccine thật trong tổng thể (unknown).
- Statistic $\\hat{p}$: tỉ lệ trong mẫu $= 4\\,212/5\\,000 = 84{,}2\\%$.

**Ví dụ 2 — Kiểm tra pin điện thoại:**
- Population: tất cả pin trong lô sản xuất 10 000 cái.
- Sample: 200 pin được lấy ra kiểm tra.
- Parameter $\\mu$: tuổi thọ trung bình thật của lô.
- Statistic $\\bar{x}$: trung bình của 200 pin được kiểm tra = 847 giờ.

**Ví dụ 3 — Điểm thi quốc gia:**
- Population: tất cả thí sinh năm 2024 (~900 000 người).
- Sample: không cần — Bộ GD có điểm của tất cả → đây là **census** (kiểm tra toàn bộ). $\\bar{x} = \\mu$ khi có census.
...
> 1. Công ty X chạy khảo sát hài lòng, nhận được 800 phản hồi từ tổng số 12 000 nhân viên. $\\bar{x} = 7{,}3/10$. Phân biệt population, sample, parameter, statistic.
> <details><summary>Đáp án</summary>
> Population: 12 000 nhân viên. Sample: 800 người trả lời. Parameter $\\mu$: điểm hài lòng trung bình thật của cả 12 000 người (unknown). Statistic $\\bar{x}$: $7{,}3/10$ (tính từ 800 phản hồi).
> </details>
>
...
> - "Interval và Ratio nhìn rất giống — thực tế có quan trọng không?" → Có. Với Ratio mới tính được Coefficient of Variation ($CV = \\dfrac{s}{\\bar{x}}$) có nghĩa, hay nói "doanh thu tháng này gấp $1{,}5$ lần tháng trước". Với Interval (nhiệt độ °C) câu "hôm nay ấm gấp đôi hôm qua" là vô nghĩa.
...
4. **Không bao giờ tính CV ($\\dfrac{s}{\\bar{x}}$) cho Interval** — vì mean phụ thuộc vào điểm gốc tùy ý. CV chỉ có nghĩa khi zero = zero thật (Ratio).
...
> <details><summary>Đáp án</summary>
> Không — nhiệt độ °C là thang Interval, không có zero tuyệt đối. "37°C gấp đôi 18.5°C" vô nghĩa vì 0°C chỉ là quy ước. Muốn tính tỉ số nhiệt độ cần dùng Kelvin (thang Ratio): $310K / 291{,}5K \\approx 1{,}06$ lần.
> </details>
...
- **Parameter**: Tỉ lệ tăng huyết áp thật trong tổng thể (ký hiệu $\\pi$, unknown).
- **Statistic**: Tỉ lệ tăng huyết áp trong mẫu $= \\hat{p} = 2\\,024/8\\,000 \\approx 25{,}3\\%$.
