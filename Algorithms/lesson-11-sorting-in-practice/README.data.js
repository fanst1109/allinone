...
// Source: Algorithms/lesson-11-sorting-in-practice/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 11 — Sorting trong thực tế
...
> 2. **Counting sort** — k = 101 rất nhỏ so với n = 10⁶, nên O(n+k) ≈ O(n), nhanh hơn O(n log n) nhiều.
> </details>
...
2. **Heapsort** khi độ sâu đệ quy vượt ngưỡng \`2·⌊log₂ n⌋\` → tránh worst case O(n²) của quicksort. (Heapsort guaranteed O(n log n).)
3. **Insertion sort** cho các đoạn nhỏ (≤ 16 phần tử) → quicksort tốn overhead trên mẩu nhỏ.

> **Walk-through ngưỡng đệ quy.** Với n = 1000: ngưỡng = 2·⌊log₂ 1000⌋ = 2·9 = **18**. Nếu quicksort chọn pivot tệ liên tục và đệ quy sâu hơn 18 tầng (dấu hiệu của worst case), introsort *chuyển* phần còn lại sang heapsort. Như vậy worst case bị chặn ở O(n log n) thay vì O(n²).
...
> <details><summary>Đáp án</summary>Khi độ sâu đệ quy vượt ~2·log₂ n (dấu hiệu pivot tệ liên tục → nguy cơ O(n²)). Heapsort guaranteed O(n log n) nên chặn worst case lại.</details>
