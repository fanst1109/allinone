...
// Source: Biology/02-Genetics-Evolution/lesson-07-natural-selection/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 07 — Chọn lọc tự nhiên
...
- Vận dụng công thức **q\\_{n+1} ≈ q\\_n − s·p\\_n·q\\_n²** để tính tần số allele lặn có hại sau 1 thế hệ chọn lọc.
...
- **[Lesson 06 — Di truyền quần thể](../lesson-06-population-genetics/)** — tần số allele, định luật Hardy-Weinberg \`p² + 2pq + q² = 1\`. Bài này giả sử bạn đã quen với việc tính p, q từ số kiểu gen.
...
**Độ thích nghi tương đối (relative fitness, w)** = W chia cho W lớn nhất trong quần thể. Kiểu gen "tốt nhất" có w = 1 (chuẩn); các kiểu gen khác có w ∈ [0, 1].
...
- **Quên fitness là TƯƠNG ĐỐI**: w = W / W\\_max, không phải con số tuyệt đối. Cùng 50 con có thể là w=1 (nếu là max) hoặc w=0.5 (nếu max là 100).
- **Tưởng s âm**: theo định nghĩa s = 1 − w ∈ [0, 1]. Nếu kiểu gen được ưu ái, ta chọn nó làm chuẩn (w=1, s=0), không viết "s âm".
...
**Ví dụ 1 — N = 1000, q = 0.5**: số bản allele a = 1000 × 0.5 = 500 (trong tổng 2000 alleles ở quần thể lưỡng bội). Độ lệch chuẩn của tần số ≈ √(p·q/2N) = √(0.5·0.5/2000) ≈ **0.011**. Tần số dao động trong khoảng [0.489, 0.511] — rất ổn định.

**Ví dụ 2 — N = 100, q = 0.5**: độ lệch chuẩn ≈ √(0.25/200) ≈ **0.035**. Dao động ±3.5% mỗi thế hệ.

**Ví dụ 3 — N = 10, q = 0.5**: độ lệch chuẩn ≈ √(0.25/20) ≈ **0.11**. Dao động ±11%/thế hệ → allele có thể "trôi" từ 0.5 xuống 0 trong vài chục thế hệ.

**Ví dụ 4 — N = 4, q = 0.5** (bottleneck cực mạnh): độ lệch chuẩn ≈ √(0.25/8) ≈ **0.18**. Allele a có khả năng cao biến mất hoàn toàn trong 1-2 thế hệ.

**Kết luận**: drift tỉ lệ nghịch với √N → quần thể nhỏ thì drift áp đảo cả chọn lọc.
...
Giả sử ở thế hệ n, tần số allele A là p\\_n và a là q\\_n (p + q = 1). Áp lực chọn lọc chỉ chống lại aa (kiểu lặn có hại, dị hợp Aa khỏe), với hệ số s:

\`\`\`
q_{n+1} = (q_n − s·q_n²) / (1 − s·q_n²)
\`\`\`

Khi s nhỏ và q nhỏ, mẫu số ≈ 1 → **xấp xỉ**:

\`\`\`
q_{n+1} ≈ q_n − s · p_n · q_n²
\`\`\`

Ý nghĩa: lượng giảm tần số allele lặn mỗi thế hệ tỉ lệ với s, với p (tần số allele "trội tốt") và với q² (tần số kiểu gen aa bị chọn lọc, từ Hardy-Weinberg).

### 5.2. Bốn ví dụ số

**Ví dụ 1 — q\\_0 = 0.4, s = 0.1**: q² = 0.16, p = 0.6.
Công thức chính xác: q\\_1 = (0.4 − 0.1·0.16) / (1 − 0.1·0.16) = (0.4 − 0.016) / (1 − 0.016) = 0.384 / 0.984 ≈ **0.390**.
Xấp xỉ: q\\_1 ≈ 0.4 − 0.1·0.6·0.16 = 0.4 − 0.0096 ≈ **0.390**. ✓ (giảm ~0.01 mỗi thế hệ).

**Ví dụ 2 — q\\_0 = 0.1, s = 0.5** (chọn lọc mạnh, allele đã hiếm):
q² = 0.01, p = 0.9. q\\_1 ≈ 0.1 − 0.5·0.9·0.01 = 0.1 − 0.0045 = **0.0955** (giảm rất chậm vì q² nhỏ → ít cá thể aa để loại).

**Ví dụ 3 — q\\_0 = 0.5, s = 1** (allele lặn gây chết hoàn toàn):
q² = 0.25, p = 0.5. Chính xác: q\\_1 = (0.5 − 0.25) / (1 − 0.25) = 0.25/0.75 = **0.333**. Giảm rõ trong 1 thế hệ.

**Ví dụ 4 — Vì sao loại bỏ allele lặn rất chậm khi q đã thấp?**
Từ ví dụ 2, dù s = 0.5 (rất mạnh), giảm chỉ 0.0045/thế hệ. Lý do: khi q = 0.1, chỉ q² = 1% dân số là kiểu hình aa lộ ra để chọn lọc tác động; 99% còn lại là AA hoặc Aa khỏe → chọn lọc không "nhìn thấy" allele a ở dạng dị hợp. Đây là lý do bệnh di truyền lặn (vd PKU, xơ nang) tồn tại dai trong dân số.

### ❓ Câu hỏi tự nhiên của người đọc

**Q: Nếu chọn lọc chống aa mạnh, vì sao a không biến mất hoàn toàn?**
A: Vì khi q → 0, q² → 0 nhanh hơn → tốc độ loại bỏ về 0. Allele lặn "ẩn náu" trong dị hợp Aa, không lộ ra để bị chọn lọc. Mặt khác, đột biến mới (A → a với μ ~ 10⁻⁵) bù lại → cân bằng ở q ≈ √(μ/s) — gọi là cân bằng đột biến-chọn lọc. Vd μ = 10⁻⁵, s = 0.5: q\\_cân-bằng = √(2·10⁻⁵) ≈ **0.0045** = ~1 trên 220 alleles.
...
1. q\\_0 = 0.3, s = 0.2. Tính q\\_1 bằng công thức xấp xỉ.
2. Theo công thức cân bằng đột biến-chọn lọc q\\_cân-bằng = √(μ/s), nếu μ = 10⁻⁶ và s = 0.01 thì q ổn định ở mức bao nhiêu?

<details>
<summary>Đáp án</summary>

1. q² = 0.09, p = 0.7. q\\_1 ≈ 0.3 − 0.2 · 0.7 · 0.09 = 0.3 − 0.0126 = **0.2874**.
2. q\\_cân-bằng = √(10⁻⁶/0.01) = √(10⁻⁴) = **0.01** (1%).
</details>

### 📝 Tóm tắt mục 5

- Công thức: q\\_{n+1} ≈ q\\_n − s·p\\_n·q\\_n² (chọn lọc chống aa, s nhỏ).
- Allele lặn có hại loại bỏ rất chậm khi q nhỏ vì q² nhỏ → ít cá thể aa lộ ra.
- Cân bằng đột biến-chọn lọc: q\\_cân-bằng ≈ √(μ/s) — giải thích vì sao bệnh di truyền lặn không biến mất hết.
...
**Ví dụ 3**: nếu 2 loài có 10⁶ base DNA so sánh, khác nhau 1.2% → khoảng 12,000 base khác. Với tốc độ đột biến trung tính ~10⁻⁹/base/năm, thời gian phân tách ≈ 12,000 / (2 × 10⁻⁹ × 10⁶) = **6 triệu năm**. (Hệ số 2 vì cả 2 nhánh đều tích lũy đột biến.)
...
**Bài 2**: Một quần thể có q\\_0 = 0.5 (tần số allele lặn có hại a). Hệ số chọn lọc chống aa là s = 0.2. Tính q\\_1 sau 1 thế hệ chọn lọc, dùng cả công thức chính xác và xấp xỉ.
...
**Bài 2**:
- q\\_0 = 0.5, p\\_0 = 0.5, q² = 0.25, s = 0.2.
- **Chính xác**: q\\_1 = (q − s·q²) / (1 − s·q²) = (0.5 − 0.2·0.25) / (1 − 0.2·0.25) = (0.5 − 0.05) / (1 − 0.05) = 0.45 / 0.95 ≈ **0.4737**.
- **Xấp xỉ**: q\\_1 ≈ q − s·p·q² = 0.5 − 0.2 · 0.5 · 0.25 = 0.5 − 0.025 = **0.475**.
...
**Bài 6**:
- (a) Tần số kiểu hình bệnh = q² = (0.02)² = **0.0004 = 4 × 10⁻⁴** = 1 trên 2500 trẻ.
- (b) Xấp xỉ: q\\_1 ≈ q − s·p·q² = 0.02 − 1 · 0.98 · 0.0004 = 0.02 − 0.000392 = **0.019608**. Lượng giảm = ~3.92 × 10⁻⁴ ⇒ chỉ giảm ~2% tương đối/thế hệ. Mặc dù s = 1 (chết hoàn toàn), tốc độ loại bỏ rất chậm vì q² nhỏ — allele "ẩn náu" trong dị hợp Aa.
- (c) Cân bằng đột biến-chọn lọc: q\\_cân-bằng² = μ/s → μ = q² · s = 0.0004 · 1 = **4 × 10⁻⁴/thế hệ**. Tốc độ đột biến này cao hơn bình thường nhiều (~10⁻⁵ ~ 10⁻⁶); thực tế ở người tốc độ thật ~10⁻⁵ và q cân bằng tự nhiên thấp hơn 0.02. Lý do q\\_người = 0.02 cao hơn cân bằng có thể do: (i) lợi thế dị hợp (Aa kháng tả/lao?), (ii) drift trong lịch sử châu Âu, (iii) cân bằng chưa đạt do thời gian.
...
5. **Công thức**: q\\_{n+1} ≈ q\\_n − s·p\\_n·q\\_n². Allele lặn giảm rất chậm khi q nhỏ vì q² nhỏ. Cân bằng đột biến-chọn lọc: q ≈ √(μ/s).
