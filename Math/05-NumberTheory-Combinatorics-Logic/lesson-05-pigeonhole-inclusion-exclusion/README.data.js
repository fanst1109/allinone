...
// Source: Math/05-NumberTheory-Combinatorics-Logic/lesson-05-pigeonhole-inclusion-exclusion/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 05 — Nguyên lý Dirichlet & Nguyên lý bù trừ
...
💡 **Trực giác (hình ảnh chim bồ câu)**: Có 10 chuồng và 11 con chim. Nhất định có ít nhất 1 chuồng chứa $\\ge 2$ con.
...
>
> **(c) Ví dụ số**: 30 sinh viên, 12 tháng: $\\lceil 30/12 \\rceil = $ **3** → ít nhất 3 cùng tháng. 5 điểm trong tam giác đều cạnh 2 → ít nhất 2 điểm cách nhau $\\le 1$ (chia tam giác thành 4 tam giác nhỏ, pigeonhole → 2 điểm cùng tam giác con). 100 số nguyên dương $\\le 200$ → có 2 số là bội nhau (chia thành 100 hộp dạng $\\{1,2,4,\\dots\\}$, $\\{3,6,12,\\dots\\}$). Trong 13 số chia 12 → 2 có cùng số dư.
...
- *"Bài khó thì 'hộp' là gì?"* Mấu chốt là **thiết kế hộp khéo**. Vd "2 số có hiệu chia hết 7": hộp = số dư $\\bmod 7$ (7 hộp); 8 số → 2 cùng dư → hiệu chia hết 7.

⚠ **Lỗi thường gặp — quên làm tròn LÊN (trần ⌈⌉)**. Công thức là $\\lceil N/k \\rceil$, KHÔNG phải $N/k$ rồi làm tròn xuống. Phản ví dụ: 30 sinh viên, 12 tháng → $\\lceil 30/12 \\rceil = \\lceil 2.5 \\rceil = 3$, KHÔNG phải 2. Có ít nhất 3 người cùng tháng (nếu chỉ 2/tháng thì tối đa $24 < 30$ người).
...
- 12 tháng (hộp) + 30 người (vật) → $\\lceil 30/12 \\rceil = 3$ người cùng tháng (ít nhất).

### Ví dụ 2 — Bắt tay

Tại 1 buổi tiệc $n$ người, chứng minh ít nhất 2 người có cùng số người đã bắt tay.
- Số bắt tay của mỗi người $\\in \\{0, 1, \\dots, n-1\\}$ ($n$ giá trị).
...
- 12 lớp dư ($0, 1, \\dots, 11$) → 13 số → $\\lceil 13/12 \\rceil = 2$.
...
Nếu $n = p \\cdot q$ (2 nguyên tố khác nhau):
- Tổng $= n$.
- Bội của $p$: $n/p$.
- Bội của $q$: $n/q$.
- Bội của $p$ và $q$ ($= pq$): 1.
- → $\\varphi(n) = n - n/p - n/q + 1 = (p-1)(q-1)$.

Tổng quát: nếu $n = p_1^{a_1} \\cdots p_k^{a_k}$ thì
...
1. **Dirichlet**: $n+1$ vật, $n$ hộp → có $\\ge 2$ vật cùng hộp. Tổng quát: $\\lceil N/k \\rceil$.
2. **Bù trừ 2 tập**: $|A \\cup B| = |A|+|B|-|A \\cap B|$.
3. **Bù trừ 3 tập**: cộng đơn, trừ đôi, cộng ba.
4. **Áp dụng**: $\\varphi(n)$ qua bù trừ; Derangement $D_n$.
