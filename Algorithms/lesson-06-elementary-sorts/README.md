# Lesson 06 — Elementary Sorts (Sắp xếp cơ bản)

> **Tier 1 — Sắp xếp · Lesson đầu tiên.** Ba thuật toán sắp xếp "kinh điển": **bubble sort**, **selection sort**, **insertion sort**. Tất cả đều `O(n²)`, nhưng mỗi cái có tính chất riêng (ổn định hay không, số lần swap, hành vi trên mảng gần-sorted) — và hiểu rõ chúng là nền để sau này hiểu vì sao merge/quick/heap sort nhanh hơn, và vì sao các thư viện thực tế vẫn dùng insertion sort ở "đáy" đệ quy.
...
- [Tier 0 — Big-O & phân tích tiệm cận](../tier-0-foundations/index.html): khái niệm `O(n²)`, `O(n)`, best/average/worst case.
...
**Bài toán sắp xếp (sorting problem):** cho một dãy `n` phần tử `a[0], a[1], ..., a[n-1]` cùng với một **quan hệ thứ tự** `≤` (so sánh được hai phần tử bất kỳ), hãy **hoán vị** (permute) chúng thành `a[π(0)], a[π(1)], ..., a[π(n-1)]` sao cho:
...
- Đã sort + binary search: tối đa `log₂(1,000,000) ≈ 20` phép so sánh.
...
> - *"Mảng đã sorted rồi thì sort lại có tốn không?"* — Tùy thuật toán! Insertion sort phát hiện "đã sorted" và chạy `O(n)`; selection sort thì **vẫn** `O(n²)`. Đây chính là một trong những điểm phân biệt quan trọng nhất của bài này.
...
> - Ba thuật toán bài này đều `O(n²)` — chậm với `n` lớn, nhưng đơn giản, in-place, và là nền lý thuyết cho các sort nhanh hơn.
...
> Lưu ý số comparison tổng quát = `(n-1) + (n-2) + ... + 1 = n(n-1)/2`. Với `n=5`: `5·4/2 = 10` ✓.
...
Đây là cải tiến quan trọng: với mảng **đã sorted sẵn**, bubble sort có early-exit chạy đúng **một pass** `n-1` comparison rồi dừng → `O(n)` thay vì `O(n²)`.
...
> **⚠ Lỗi thường gặp.** Quên cờ `swapped` (không có early-exit) là lỗi phổ biến nhất khi viết bubble sort — bạn mất luôn ưu điểm `O(n)` trên dữ liệu gần-sorted, biến bubble sort thành thuật toán "luôn `O(n²)`" vô dụng.
...
| **Average** | ngẫu nhiên | `~n²/2` | `~n²/4` | `O(n²)` |
| **Worst** (sorted ngược) | mọi cặp đều swap | `n(n-1)/2` | `n(n-1)/2` | `O(n²)` |
...
> 2. Vì sao early-exit giúp best case là `O(n)` chứ không phải `O(n²)`?
>
...
> - `O(n²)` average/worst; với **early-exit** thì best case (đã sorted) là `O(n)`.
...
Tổng comparison = `4+3+2+1 = 10` = `n(n-1)/2`. **Luôn luôn** `10` cho `n=5`, bất kể mảng đầu vào — selection sort không bao giờ "ăn may".
...
> **❓ Câu hỏi tự nhiên.** *"Nếu mảng đã sorted thì selection sort có nhanh hơn không?"* — **KHÔNG.** Để chọn được min của một đoạn, ta **bắt buộc** phải so sánh với mọi phần tử trong đoạn — không có cách nào "đoán trước" min mà không nhìn hết. Vì vậy số comparison luôn là `n(n-1)/2` dù mảng đầu vào thế nào.
...
Bù lại, selection sort có **số lần swap tối thiểu**: tối đa `n-1` swap. Điều này quan trọng khi **swap đắt** — ví dụ phần tử là struct lớn, hoặc ghi vào bộ nhớ flash (mỗi lần ghi tốn tuổi thọ). Bubble sort có thể swap tới `n(n-1)/2` lần (worst case `O(n²)` swap), selection sort luôn `≤ n-1` = `O(n)` swap.
...
> - Comparison **luôn** `n(n-1)/2` = `O(n²)` — không có best case nhanh.
> - Ưu điểm độc nhất: số swap tối thiểu (`≤ n-1`) — dùng khi swap đắt.
...
> **Chú thích đếm:** số comparison phụ thuộc cách cài. Trong code dưới, vòng `while a[j] > key` so sánh mỗi lần xét một phần tử. Worst case (sorted ngược) tổng = `n(n-1)/2`.
...
| **Average** | ngẫu nhiên | `~n²/4` | `~n²/4` | `O(n²)` |
| **Worst** (sorted ngược) | mỗi key dịch hết về đầu | `n(n-1)/2` | `n(n-1)/2` | `O(n²)` |
...
> 2. Best: mảng **đã sorted** (`O(n)`). Worst: mảng **sorted ngược** (`O(n²)`, mỗi key dịch hết).
> </details>

> **📝 Tóm tắt mục 4.**
> - Insertion sort: chèn từng phần tử vào phần đã sắp bên trái bằng cách dịch.
> - Best `O(n)` (đã/gần sorted), average & worst `O(n²)`.
...
|----------|---------------------|-----------|-----------|
| **Time — best** | `O(n)` (đã sorted) | `O(n²)` (luôn) | `O(n)` (đã sorted) |
...
| **Số swap (worst)** | `O(n²)` | `O(n)` (`≤ n-1`) | `O(n²)` shift |
...
> **❓ Câu hỏi tự nhiên.** *"Cả ba đều `O(n²)`, vậy chọn cái nào?"* — Trong thực tế **insertion sort thường tốt nhất** trong ba: hằng số nhỏ, thích nghi với dữ liệu gần-sorted (`O(n)`), ổn định, và ít di chuyển dữ liệu hơn bubble. Selection chỉ thắng khi **swap rất đắt**. Bubble gần như không có lý do dùng ngoài việc dạy học.

> **📝 Tóm tắt mục 5.** Cả ba `O(n²)` average. Khác nhau ở: best case (bubble/insertion `O(n)`, selection `O(n²)`), ổn định (selection không), số swap (selection ít nhất). Insertion thắng tổng thể trong thực tế.
...
Cả ba đều `O(n²)` — nghe có vẻ "vô dụng" so với `O(n log n)`. Nhưng chúng vẫn được dùng **rất nhiều** trong thực tế ở các tình huống sau:

### 8.1 `n` nhỏ — hằng số nhỏ thắng

> **💡 Trực giác.** `O(n²)` chỉ "thua" `O(n log n)` khi `n` đủ lớn. Với `n` nhỏ, **hằng số ẩn** trong Big-O mới quyết định. Insertion sort có hằng số rất nhỏ (vòng lặp đơn giản, không đệ quy, không cấp phát) → với `n < ~10–20` nó thực sự **nhanh hơn** quicksort/merge sort.

So sánh số phép tính thô (ước lượng):
- Insertion sort `n=10`: `~n²/4 = 25` phép so sánh, **không** chi phí đệ quy/cấp phát.
- Quicksort `n=10`: `~n log n = 33` phép so sánh **cộng** chi phí gọi đệ quy + quản lý stack → thực tế chậm hơn.
...
→ Thấy rõ: trên dữ liệu đã sorted, **bubble (early-exit) và insertion chỉ tốn `O(n)`**, còn **selection vẫn `O(n²)` comparison** vì luôn phải quét tìm min.
...
> **📝 Tóm tắt mục 9.** Cùng `O(n²)` nhưng số phép thực tế khác rõ theo loại đầu vào. Trên đã-sorted: bubble/insertion = `O(n)`, selection = `O(n²)`. Trên sorted-ngược: selection ăn điểm ở số swap.

---

## 10. Cạm bẫy thường gặp

> **⚠ Cạm bẫy 1 — Quên early-exit của bubble sort.** Không có cờ `swapped`, bubble sort luôn `O(n²)` kể cả khi mảng đã sorted → mất hết ưu điểm `O(n)`. Đây là lỗi #1.
...
> **⚠ Cạm bẫy 3 — Tưởng bubble nhanh ngang insertion.** Cùng `O(n²)` nhưng bubble thực tế **chậm hơn** vì số swap nhiều hơn hẳn (mỗi cặp sai chỗ swap ngay tại chỗ; insertion chỉ "dịch" và đặt key một lần). Trong ba thuật toán, bubble gần như luôn là cái chậm nhất thực tế.
...
> **⚠ Cạm bẫy 6 — Tưởng selection sort tốn nhiều swap.** Ngược lại! Selection sort có số swap **ít nhất** (`≤ n-1`). Người ta hay nhầm vì nó "chậm" (comparison luôn `O(n²)`), nhưng chậm là do comparison chứ không phải swap.
...
**Bài 5.** Với bubble sort có early-exit, đầu vào nào cho **best case** `O(n)`? Đầu vào nào cho **worst case** `O(n²)`? Giải thích.
...
- **Worst case `O(n²)`**: mảng **sorted ngược (giảm dần)**, ví dụ `[5,4,3,2,1]`. Mọi cặp đều sai thứ tự → mỗi pass đều swap, không bao giờ early-exit; phải chạy đủ `n-1` pass với tổng `n(n-1)/2` comparison và swap → `O(n²)`.
...
**Ưu điểm so với mảng:** trên mảng, chèn phải **dịch (shift)** mọi phần tử lớn hơn sang phải → mỗi lần chèn `O(n)` dịch. Trên linked list, chèn chỉ là **đổi con trỏ** `O(1)` — không cần dịch dữ liệu. (Tuy nhiên việc **tìm vị trí** vẫn `O(n)` vì list không random-access, nên tổng vẫn `O(n²)`; nhưng ta tiết kiệm chi phí di chuyển dữ liệu, hữu ích khi phần tử là struct lớn.)

### Lời giải Bài 7 — Inversion

**Định nghĩa.** Một **inversion** (nghịch thế) là một cặp chỉ số `(i, j)` với `i < j` nhưng `a[i] > a[j]` — tức một cặp **sai thứ tự**. Mảng đã sorted tăng dần có **0 inversion**; mảng sorted ngược có số inversion tối đa `n(n-1)/2`.
...
- **[Lesson 07 — Merge Sort](../lesson-07-merge-sort/)**: thuật toán sort `O(n log n)` đầu tiên, dùng **divide & conquer** (chia để trị), **ổn định** nhưng **không** in-place (`O(n)` bộ nhớ phụ). Sẽ thấy vì sao `O(n log n)` đánh bại `O(n²)` khi `n` lớn.
