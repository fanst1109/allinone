...
// Source: DataFoundations/01-NumberRepresentation/lesson-01-binary-hex/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 01 — Binary & Hex (hệ nhị phân và hệ thập lục phân)
...
Một số được viết bằng **các chữ số** (digit) và **vị trí**. Trong **hệ cơ số b**, mỗi vị trí có giá trị là một lũy thừa của $b$, đọc từ phải sang trái: $b^0, b^1, b^2, \\dots$
...
**Mẹo nhớ "≈"**: $2^{10} = 1024 \\approx 10^3$, nên cứ $2^{10n} \\approx 10^{3n}$. Trong thực tế người ta dùng xấp xỉ này khi nói "máy 16 GB RAM" (đúng ra là $16 \\cdot 2^{30} \\approx 1{,}7 \\cdot 10^{10}$ bytes).
...
→ Câu trả lời cho *"vì sao \`int64\` đến ~9.2 · 10¹⁸?"*: vì $2^{63} - 1 \\approx 9{,}22 \\cdot 10^{18}$.
...
- **byte** = 8 bit. Chứa được $2^8 = 256$ giá trị (0..255 hoặc -128..127).
...
Không, nếu hai số đủ lớn. $2\\,000\\,000\\,000 \\times 2 = 4\\,000\\,000\\,000$ vượt \`int32\` (max $\\approx 2{,}1 \\cdot 10^9$). Phải ép kiểu sang \`int64\` trước khi nhân: \`int64(a) * int64(b)\`.
...
Cách tính 100: $100 = 64 + 32 + 4 = 2^6 + 2^5 + 2^2 \\to$ \`01100100\`.
