...
// Source: GameDev/02-Collision/lesson-01-aabb-circle/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 06 — Collision: AABB & Circle (phát hiện va chạm hình cơ bản)
...
> <details><summary>Đáp án</summary>
> Trục x: \`10 ≤ 25\` ✓ và \`20 ≥ 15\` ✓ → chồng x. Trục y: \`10 ≤ 35\` ✓ nhưng \`20 ≥ 25\` ✗ → rời y. Có một vế sai → <strong>KHÔNG CHẠM</strong> (B nằm dưới A một khe từ y=20 tới y=25).
> </details>
...
**Ví dụ 1 — chạm.** A \`(0,0) r=3\`, B \`(4,0) r=2\`. \`dx=4, dy=0\` → \`distSq = 16+0 = 16\`. \`rSum = 5\` → \`rSum² = 25\`. So: \`16 < 25\` ✓ → **CHẠM**. (Khoảng cách thật = 4 < tổng bán kính 5.)

**Ví dụ 2 — không chạm.** A \`(0,0) r=2\`, B \`(10,0) r=3\`. \`dx=10, dy=0\` → \`distSq = 100\`. \`rSum = 5\` → \`rSum² = 25\`. So: \`100 < 25\`? **Sai** → **KHÔNG CHẠM**. (Khoảng cách 10 ≫ tổng bán kính 5.)
...
> <details><summary>Đáp án</summary>
> \`dx=0, dy=4\` → \`distSq = 0+16 = 16\`. \`rSum = 3\` → \`rSum² = 9\`. \`16 < 9\`? Sai → <strong>KHÔNG CHẠM</strong> (cách nhau theo trục y, khoảng cách 4 > tổng bán kính 3).
> </details>

> 📝 **Tóm tắt mục 3.** Hai tròn chạm ⇔ khoảng cách tâm < tổng bán kính. Luôn so **bình phương**: \`dx² + dy² < (r1+r2)²\` để **tránh \`sqrt\`**. \`<\` = không tính tiếp xúc, \`<=\` = tính.
...
> <details><summary>Đáp án</summary>
> Điểm gần nhất = góc \`(4,4)\`. \`dx=2, dy=2\` → \`distSq = 8\`. \`r² = 4\`. \`8 < 4\`? Sai → <strong>KHÔNG CHẠM</strong> (góc tròn còn cách góc hộp một chút; cần \`r ≥ √8 ≈ 2.83\` mới chạm).
> </details>
...
1. Click \`(180, 95)\` vào hộp: \`100 ≤ 180 ≤ 240\` ✓ và \`80 ≤ 95 ≤ 120\` ✓ → **trúng hộp**.
2. Click \`(260, 95)\`: \`100 ≤ 260 ≤ 240\`? \`260 > 240\` ✗ → **trượt** (ngoài cạnh phải).
...
1. \`(250, 45)\`: \`200 ≤ 250 ≤ 320\` ✓, \`20 ≤ 45 ≤ 60\` ✓ → **trúng HỘP**.
2. \`(330, 40)\`: \`200 ≤ 330 ≤ 320\`? \`330 > 320\` ✗ → không trong hộp. Tròn: \`dx=330-360=-30, dy=0\` → \`distSq = 900 > 324\` → không trong tròn → **TRƯỢT**.
...
A \`(0,0) r=4\`, B \`(3,4) r=3\`. \`dx=3, dy=4\` → \`distSq = 9+16 = 25\` → \`dist = √25 = 5\` (giờ **mới** cần \`sqrt\` vì cần con số thật). \`rSum = 7\`. Kiểm tra chồng: \`5 < 7\` ✓ → đang chồng.

- **Độ lún**: \`depth = rSum - dist = 7 - 5 = 2\`.
- **Hướng đẩy** (đơn vị, từ A tới B): \`(dx/dist, dy/dist) = (3/5, 4/5) = (0.6, 0.8)\`.
- Đẩy B đi đúng \`depth = 2\` theo hướng đó: B mới \`(3 + 2·0.6, 4 + 2·0.8) = (4.2, 5.6)\`. Kiểm tra: \`dx=4.2, dy=5.6\` → \`dist = √(17.64+31.36) = √49 = 7 = rSum\` → vừa tách ✓. (Trong response thật thường chia đôi: đẩy mỗi vật \`depth/2\`.)
...
Cặp phản chứng: \`A = [0, 0, 4, 4]\`, \`B = [1, 100, 3, 104]\`. Trục x: \`0 ≤ 3\` ✓ và \`4 ≥ 1\` ✓ → \`buggy\` trả về **true** (báo chạm). Thực tế A ở trên cùng (y 0..4), B ở tận y 100..104 — cách nhau cả trăm pixel theo trục y → **KHÔNG chạm**. Bug bỏ sót \`a.maxY ≥ b.minY\` (\`4 ≥ 100\` sai) nên báo nhầm. Bài học: **AABB phải AND đủ 4 điều kiện**, một trục thiếu là sai.
