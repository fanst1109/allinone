...
// Source: Algorithms/lesson-49-intractability-np/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 49 — Bài toán khó & P vs NP (Intractability)
...
- [Lesson 29 — Bitmask DP](../lesson-29-bitmask-dp/) — Held-Karp TSP $O(2^n \\cdot n^2)$, cách giải đúng tối ưu cho $n$ nhỏ.
...
**Định nghĩa (đa thức).** Thuật toán chạy thời gian đa thức nếu số bước $\\leq c \\cdot n^k$ cho hằng số $c, k$ không phụ thuộc $n$.

Hãy nhìn 4 ví dụ số cụ thể với $n = 50$ (số thành phố TSP, hoặc số phần tử):

| Độ phức tạp | n = 10 | n = 20 | n = 50 | Loại |
|---|---|---|---|---|
| $n^2$ | 100 | 400 | 2.500 | đa thức (dễ) |
| $n^3$ | 1.000 | 8.000 | 125.000 | đa thức (dễ) |
| $2^n$ | 1.024 | ~10⁶ | ~10¹⁵ | mũ (khó) |
| $n!$ | ~3,6 triệu | ~2,4·10¹⁸ | ~3·10⁶⁴ | giai thừa (khó kinh khủng) |

Walk-through con số $n = 50$:
- $n^3 = 50^3 = 125.000$ — máy tính làm xong trong **micro giây**.
- $2^{50} \\approx 1,13 \\cdot 10^{15}$ — ở 10⁹ thao tác/giây mất **~13 ngày**.
- $50! \\approx 3 \\cdot 10^{64}$ — vượt quá số nguyên tử trong vũ trụ quan sát được (~10⁸⁰), thực tế **không bao giờ** xong.

> ⚠ **Lỗi thường gặp.** "n¹⁰⁰ là đa thức nên nhanh, 1,0001ⁿ là mũ nên chậm" — đúng về mặt **phân loại tiệm cận**, nhưng với $n$ nhỏ thực tế thì n¹⁰⁰ chậm kinh khủng còn 1,0001ⁿ vẫn nhanh. Lý thuyết độ phức tạp nói về **hành vi khi n → ∞**. May mắn là các thuật toán đa thức thực tế đều có $k \\leq 4$.

> 🔁 **Dừng lại tự kiểm tra.** Với $n = 30$, ước lượng $2^n$ và $n^2$.
> <details><summary>Đáp án</summary>
> $2^{30} \\approx 1,07 \\cdot 10^9$ (khoảng 1 giây), $n^2 = 900$. Khoảng cách hơn 1 triệu lần — và mới chỉ n=30.
> </details>
...
1. **Khép kín dưới phép hợp thành.** Gọi một thuật toán đa thức bên trong một vòng lặp đa thức vẫn ra đa thức ($n^a \\cdot n^b = n^{a+b}$). Mũ thì không có tính chất đẹp này một cách an toàn.
...
Walk-through tính khép kín: Floyd-Warshall O(V³) gọi bên trong một thuật toán lặp O(V) lần → O(V⁴), vẫn đa thức. Đổi V từ 100 lên 200 (gấp đôi) → thời gian gấp $2^4 = 16$ lần — tăng có kiểm soát, không nổ.
...
**Định nghĩa (polynomial-time reduction $A \\leq_p B$).** Một hàm $f$ chạy đa thức biến mỗi instance $x$ của A thành instance $f(x)$ của B sao cho: $x$ là "Yes" của A ⟺ $f(x)$ là "Yes" của B.

**Cách chứng minh bài mới $B$ là NP-complete (2 bước):**
1. Chứng minh \`B ∈ NP\` (chỉ ra certificate + verifier đa thức).
2. Chọn một bài NP-complete đã biết $A$, chứng minh $A \\leq_p B$ (reduce A về B). Vì A khó, B cũng khó.
...
Vì sao? Trong independent set của G, mọi cặp **không** kề trong G → trong Ḡ chúng **kề** nhau → tạo thành clique. Phép dựng \`Ḡ\` mất $O(V^2)$ (đa thức). Vậy Independent Set ≤ₚ Clique và ngược lại.
...
Walk-through tính đa thức của reduction Independent Set → Clique: dựng đồ thị bù \`Ḡ\` cần duyệt mọi cặp đỉnh — $C(n,2) = n(n-1)/2$. Với $n=1000$: $\\approx 500.000$ thao tác $= O(n^2)$, đa thức. Vậy hộp đen giải Clique nhanh sẽ giải Independent Set nhanh.
...
Walk-through số: đường thẳng 0-1-2-3 (cạnh (0,1),(1,2),(2,3)), n=4. Vertex cover nhỏ nhất \`{1,2}\` (size 2). Phần bù \`{0,3}\` — kiểm: 0 và 3 không kề → là independent set lớn nhất (size 2). Kiểm đẳng thức: $|\\text{min VC}| + |\\text{max IS}| = 2 + 2 = 4 = n$ ✓. Đẳng thức $\\text{min VC} + \\text{max IS} = n$ luôn đúng — biết một cái suy ra cái kia.
...
> 💡 **Trực giác.** Thay vì lời giải tối ưu (đắt), chấp nhận lời giải "tệ hơn nhiều nhất một hệ số cố định" nhưng tính được nhanh. Hệ số đó là **approximation ratio** ρ: thuật toán đảm bảo $\\text{cost} \\leq \\rho \\cdot \\text{OPT}$ (với bài cực tiểu).
...
return C
\`\`\`
**Vì sao ≤ 2·OPT?** Các cạnh ta chọn là một **matching** (không chia sẻ đỉnh). Mỗi cạnh matching cần ít nhất 1 đỉnh trong vertex cover tối ưu → $\\text{OPT} \\geq \\text{số cạnh chọn} = |C|/2$ → $|C| \\leq 2 \\cdot \\text{OPT}$. Bound 2 được **chứng minh chặt chẽ**, không phải "hy vọng tốt".
...
- **Bitmask DP (Held-Karp) cho TSP**: $O(2^n \\cdot n^2)$. Với $n=15$: $2^{15} \\cdot 15^2 \\approx 7,4$ triệu — chạy tức thì. ([Lesson 29](../lesson-29-bitmask-dp/))
...
- DP $O(n \\cdot W)$ với $n=100$: $100 \\cdot 10^9 = 10^{11}$ bước — chậm, gần như không chạy nổi.
- Số bit của W: $\\log_2(10^9) \\approx 30$ bit. Input chỉ ~30 bit mà thời gian ~$10^{11}$ → **mũ theo số bit** ($2^{30} \\approx 10^9$).
...
Walk-through Held-Karp với 4 thành phố trên: tour tối ưu là $0 \\to 1 \\to 3 \\to 2 \\to 0$ = $10 + 25 + 30 + 15 = 80$. Cả hai hàm ra 80; brute-force thử $3! = 6$ hoán vị, Held-Karp duyệt $2^4 \\cdot 4 = 64$ trạng thái. Với $n=15$: brute-force $14! \\approx 8,7 \\cdot 10^{10}$ (chết), Held-Karp $2^{15} \\cdot 15^2 \\approx 7,4 \\cdot 10^6$ (tức thì).
...
- Certificate \`{3, 5}\`: cả 2 ∈ S ✓. Tổng $3 + 5 = 8 \\neq 10$ → certificate này **không hợp lệ** (không chứng minh được Yes). Lưu ý: điều này không có nghĩa câu trả lời bài toán là No — chỉ là certificate cụ thể này sai. (Thực tế \`{2,8}\` đã chứng minh đáp án là Yes.)
...
Ratio thực tế = $4/3 \\approx 1,33 \\leq 2$ ✓ (bound 2 luôn đúng).

### Bài 3 — TSP brute vs bitmask DP

- **n=12:** brute $(12-1)! = 11! = 39.916.800 \\approx 4 \\cdot 10^7$ → ~0,04 giây (khả thi). Held-Karp $2^{12} \\cdot 12^2 = 4096 \\cdot 144 \\approx 5,9 \\cdot 10^5$ → tức thì.
- **n=18:** brute $17! \\approx 3,56 \\cdot 10^{14}$ → ~4 ngày (không khả thi). Held-Karp $2^{18} \\cdot 18^2 = 262144 \\cdot 324 \\approx 8,5 \\cdot 10^7$ → ~0,08 giây (khả thi).

**Kết luận:** brute-force chỉ dùng được tới ~n=12-13; bitmask DP đẩy giới hạn tới ~n=20 ($2^{20} \\cdot 400 \\approx 4 \\cdot 10^8$). Vượt n=20 phải dùng heuristic/approximation.
...
- Số bit của $T = 2 \\cdot 10^9$: $\\log_2(2 \\cdot 10^9) \\approx 31$ bit.
- Số thao tác DP: $n \\cdot T = 50 \\cdot 2 \\cdot 10^9 = 10^{11}$ → ~100 giây (rất chậm).
- **Vì sao không đa thức:** kích thước input ≈ $n + 31$ bit ≈ vài chục bit, nhưng thời gian ~$10^{11}$ = mũ theo số bit ($T = 2^{31}$, thời gian $\\propto T = 2^{\\text{số bit}}$). Nếu thêm 1 bit vào T (T gấp đôi), thời gian gấp đôi → mũ theo bit, không phải đa thức. Đây là pseudo-polynomial.
