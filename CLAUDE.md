...
| `tools/fonts/` | **Font UI local (OFL woff2)** cho chữ thường trong README-modal — bundle để chữ (đặc biệt **tiếng Việt**) đồng nhất mọi máy, không phụ thuộc font hệ thống. 6 font sẵn để đổi: **Be Vietnam Pro** (đang dùng, tối ưu tiếng Việt; static 400/700), **Plus Jakarta Sans**, **Lexend**, **Roboto Flex**, **Inter**, **Manrope** (variable). Subset latin/latin-ext/vietnamese (3 font đầu) + greek (3 font variable gốc). Khai báo `@font-face` ở `viz-base.css`; chọn font hoạt động bằng `font-family` trong `readme-modal.js` (`.rm-panel .rm-content`). KaTeX math vẫn dùng font `KaTeX_*` riêng. Auto-generated từ @fontsource — KHÔNG sửa woff2 tay |
