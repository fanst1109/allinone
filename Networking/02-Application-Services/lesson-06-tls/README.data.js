...
// Source: Networking/02-Application-Services/lesson-06-tls/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 06 — TLS/SSL: Bảo mật lớp truyền tải
...
> 💡 **TLS (HTTPS) là mặc định bắt buộc của web hiện đại. Dev hiếm khi cài tay TLS nhưng phải hiểu cert, handshake cost, và bẫy cấu hình.**

| Khái niệm | Thực tế dev |
|-----------|-------------|
| **Certificate (cert)** | Let's Encrypt (free, auto-renew), không tự ký cho production |
| **Handshake cost** | TLS thêm ~1 RTT → connection reuse + TLS 1.3 (0-RTT) quan trọng |
| **TLS termination** | LB/reverse proxy (nginx/Envoy) giải mã TLS, service trong nói HTTP |
| **mTLS** | Cả hai phía xác thực — service-to-service nội bộ ([nối service mesh](../../../Programming/lesson-70-service-mesh-intro/)) |

### 6.1. Ví dụ cụ thể — TLS termination tại load balancer

Production thường: client → HTTPS → **load balancer giải mã TLS** → HTTP (plaintext) trong mạng nội bộ tin cậy → service. Lợi: service không phải xử lý cert/handshake (đỡ CPU), cert quản tập trung ở LB, dễ renew. Service Go chỉ cần nói HTTP, LB (nginx/ALB/Cloudflare) lo TLS. Với zero-trust nội bộ thì dùng **mTLS** (service mesh) mã hóa cả trong. Cert dùng **Let's Encrypt** (miễn phí, tự động gia hạn qua ACME) — đừng mua cert đắt hay tự ký (browser cảnh báo "not secure").

> ⚠ **Bẫy TLS thật.** (1) **Cert hết hạn** = site down toàn bộ (sự cố kinh điển, kể cả công ty lớn) → auto-renew + alert trước hạn. (2) **Verify cert** ở client — \`InsecureSkipVerify: true\` trong Go (bỏ qua verify) = mở cửa MITM, chỉ dùng test, KHÔNG production. (3) **Phiên bản cũ**: tắt TLS 1.0/1.1 (lỗ hổng), dùng TLS 1.2+ / 1.3. (4) Handshake tốn RTT → **tái dùng connection** (keep-alive) + session resumption, đừng tạo connection mới mỗi request ([nối TCP pool](../../01-Foundations-LowerLayers/lesson-08-tcp/)). Crypto cụ thể: [nối TLS/crypto basics](../../../Programming/lesson-47-tls-crypto-basics/).

### 6.2. 📝 Tóm tắt mục 6

- TLS/HTTPS mặc định: cert **Let's Encrypt** (free auto-renew), **TLS termination** ở LB (service nói HTTP nội bộ), mTLS cho zero-trust.
- Bẫy: **cert hết hạn** (site down → auto-renew+alert), \`InsecureSkipVerify\` (MITM, chỉ test), tắt TLS cũ.
- Handshake tốn RTT → tái dùng connection + TLS 1.3; đừng tạo connection mới mỗi request.
