# Lesson 06 — Hello World & Toolchain Go
...
> 💡 **Go toolchain (`go build/test/mod`, cross-compile, embed) là vũ khí thật khiến Go được chọn cho CLI, microservice, DevOps tooling.**

| Tính năng toolchain | Dùng thật ở đâu |
|---------------------|-----------------|
| **`go build` ra 1 binary tĩnh** | Deploy = copy 1 file, không cần runtime/dependency (khác Java/Node) |
| **Cross-compile** (`GOOS`/`GOARCH`) | Build trên Mac, chạy trên Linux server / ARM — một lệnh |
| **`//go:embed`** | Nhúng template/static/migration vào binary → 1 file tự đủ |
| **`-ldflags`** | Inject version/build-time vào binary lúc CI |
| **`go test`/`go bench`** | Test + benchmark có sẵn, không cần framework ngoài |

### 10.1. Ví dụ cụ thể — vì sao Docker image Go nhỏ xíu

Binary Go tĩnh + cross-compile cho phép Docker image **`FROM scratch`** (rỗng) chỉ chứa 1 binary ~10MB — so với image Node/Python kéo theo cả runtime + thư viện hàng trăm MB. Cùng lý do: deploy Go = scp 1 file rồi chạy, không lo "máy đích thiếu thư viện". Đây là vì sao Docker, Kubernetes, Terraform, Prometheus — toàn DevOps tooling — viết bằng Go.

> ❓ **"`//go:embed` thay CDN/file server không?"** Có cho asset nhỏ: nhúng HTML template, file migration SQL, config mặc định vào binary → triển khai chỉ 1 file, không lo "quên copy thư mục templates". Asset lớn (ảnh/video) vẫn để CDN. Đây là lý do nhiều CLI/web Go tự đủ trong một file.

### 10.2. 📝 Tóm tắt mục 10

- Toolchain Go: **1 binary tĩnh** (deploy = copy 1 file), **cross-compile** (build Mac→Linux/ARM), **`//go:embed`** (nhúng asset), **`-ldflags`** (inject version).
- Binary tĩnh → Docker `FROM scratch` ~10MB → vì sao DevOps tooling (Docker/K8s/Terraform) dùng Go.
- `go test`/`go bench` built-in, không cần framework ngoài.
