# Lesson 01 — Vũ trụ giãn nở (The Expanding Universe)

## Mục tiêu học tập

Sau bài này bạn sẽ:

- Hiểu **định luật Hubble (Hubble's law)** `v = H₀·d` và ý nghĩa của hằng số Hubble `H₀ ≈ 70 km/s/Mpc`.
...
- Tính **thời gian Hubble (Hubble time)** `t_H = 1/H₀` và hiểu vì sao nó xấp xỉ tuổi vũ trụ.
...
- Đơn vị **parsec** và megaparsec (Mpc): `1 Mpc ≈ 3.26 triệu năm ánh sáng ≈ 3.086 × 10¹⁹ km`.
...
- **(a) Là gì** — `H₀` là **tốc độ giãn nở hiện nay** của vũ trụ: với mỗi megaparsec khoảng cách tăng thêm, vận tốc lùi xa tăng thêm `~70 km/s`. Đơn vị `km/s/Mpc` = (vận tốc)/(khoảng cách) = nghịch đảo của thời gian.
- **(b) Vì sao cần** — Ta cần một con số duy nhất gói gọn quan hệ "xa hơn ⇒ nhanh hơn". `H₀` cho phép, chỉ cần đo redshift (suy ra `v`), tính ngay khoảng cách `d = v/H₀` — đây là **thước đo khoảng cách vũ trụ** cho các thiên hà quá xa để dùng cách khác.
- **(c) Ví dụ số** — Thiên hà cách `10 Mpc` lùi với `v = 70 × 10 = 700 km/s`; cách `100 Mpc` lùi `7000 km/s`; cách `200 Mpc` lùi `14000 km/s` (gấp đôi vì xa gấp đôi).
...
**Walk-through bằng số thật (verify):** Một thiên hà cách `d = 100 Mpc`.

```
v = H₀ · d = 70 km/s/Mpc × 100 Mpc = 7000 km/s
```

→ Thiên hà này lùi xa ta với **7000 km/s**. Kiểm tra ngược: `d = v/H₀ = 7000/70 = 100 Mpc` ✓.

**4 ví dụ số (với `H₀ = 70`):**

| d (Mpc) | v = H₀·d (km/s) | Diễn giải |
|---|---|---|
| 10 | 700 | Thiên hà gần |
| 100 | 7000 | Walk-through ở trên |
| 1000 | 70 000 | ~0.23c — bắt đầu cần hiệu chỉnh tương đối tính |
...
- **(c) Ví dụ số** — Vạch Hydro chuẩn `λ₀ = 656.3 nm`. Nếu đo được `λ = 663.0 nm` thì `Δλ = 6.7 nm`, `z = 6.7/656.3 ≈ 0.0102`.

**Công thức:**

```
z = Δλ / λ₀ = (λ_quan_sát − λ₀) / λ₀
```

Với redshift nhỏ (`z ≪ 1`, vận tốc nhỏ so với ánh sáng), vận tốc lùi xa:

```
v ≈ c · z          (c = 300 000 km/s)
```

**Walk-through bằng số thật (verify):** `z = 0.0102` (ví dụ trên):

```
v ≈ c · z = 300 000 km/s × 0.0102 = 3060 km/s
```

→ Suy ra khoảng cách: `d = v/H₀ = 3060/70 ≈ 43.7 Mpc`.

**4 ví dụ số:**

| λ₀ (nm) | λ đo (nm) | z = Δλ/λ₀ | v ≈ cz (km/s) |
|---|---|---|---|
...
> ⚠ **Lỗi thường gặp.** Dùng `v = cz` cho `z` lớn (vd `z = 3`). Sai — `v = cz` chỉ là xấp xỉ khi `z ≪ 1`. Với `z` lớn phải dùng công thức tương đối tính/vũ trụ học; `z` *không* bị chặn ở 1 dù `v < c`.
...
> 🔁 **Dừng lại tự kiểm tra.** Vạch chuẩn `λ₀ = 500 nm`, đo được `λ = 515 nm`. Tính `z` và `v`.
> <details><summary>Đáp án</summary>`z = (515−500)/500 = 15/500 = 0.03`. `v ≈ cz = 300 000 × 0.03 = 9000 km/s`. (Và `d = 9000/70 ≈ 129 Mpc`.)</details>
...
**Mô hình bánh mì nho khô (raisin bread):** Ổ bột nở gấp đôi sau khi nướng. Hai hạt nho ban đầu cách `1 cm` → sau khi nở cách `2 cm`; hai hạt cách `3 cm` → thành `6 cm`. Trong cùng thời gian nướng:

- Cặp gần: dịch ra `1 cm` → vận tốc nhỏ.
- Cặp xa: dịch ra `3 cm` → vận tốc lớn gấp 3.
...
> 💡 **Trực giác.** Nếu các thiên hà đang lùi xa, hãy "tua ngược phim": chúng từng chụm lại gần nhau hơn, và nếu lùi đều thì có một thời điểm **mọi thứ chồng lên nhau** — đó là lúc giãn nở bắt đầu. Khoảng thời gian từ lúc đó tới nay xấp xỉ `1/H₀`.

**Định nghĩa thời gian Hubble (3 phần):**

- **(a) Là gì** — `t_H = 1/H₀` là thời gian để vũ trụ giãn tới kích thước hiện nay **nếu** tốc độ giãn không đổi. Nó là nghịch đảo của hằng số Hubble.
- **(b) Vì sao cần** — Cho ta một **ước lượng tuổi vũ trụ** chỉ từ một con số đo được (`H₀`), không cần biết gì thêm.
- **(c) Ví dụ số** — Với `H₀ = 70 km/s/Mpc`, `t_H ≈ 14 tỉ năm` (tính dưới đây).

**Walk-through bằng số thật (verify):** Đổi `H₀` về đơn vị `1/giây` rồi nghịch đảo.

```
1 Mpc = 3.086 × 10¹⁹ km
H₀ = 70 km/s/Mpc = 70 / (3.086 × 10¹⁹) (km/s)/km = 2.268 × 10⁻¹⁸ s⁻¹

t_H = 1 / H₀ = 1 / (2.268 × 10⁻¹⁸) = 4.41 × 10¹⁷ s

Đổi ra năm: 4.41 × 10¹⁷ s ÷ (3.156 × 10⁷ s/năm) ≈ 1.40 × 10¹⁰ năm = 14.0 tỉ năm
```
...
> ❓ **Câu hỏi tự nhiên.** *"Vì sao `t_H = 14` tỉ nhưng tuổi thật là 13.8 tỉ? Sai do đâu?"* — Vì `t_H` giả định giãn nở **đều mãi mãi**. Thực tế hấp dẫn từng hãm giãn nở (làm vũ trụ trẻ hơn ước lượng đơn giản), rồi năng lượng tối lại tăng tốc (Lesson 03). Hai hiệu ứng gần triệt tiêu nên `1/H₀` trùng hợp rất sát tuổi thật.

> 🔁 **Dừng lại tự kiểm tra.** Nếu `H₀` đo được lớn hơn (vd `100 km/s/Mpc`), tuổi vũ trụ ước lượng sẽ lớn hơn hay nhỏ hơn?
> <details><summary>Đáp án</summary>Nhỏ hơn. `t_H = 1/H₀`, `H₀` lớn ⇒ `t_H` nhỏ. `H₀ = 100` cho `t_H ≈ 9.8` tỉ năm. (Đây từng là "khủng hoảng tuổi" lịch sử: H₀ lớn cho vũ trụ trẻ hơn cả các sao già nhất — vô lý — nên H₀ phải nhỏ hơn.)</details>

> 📝 **Tóm tắt toàn bài.**
> - **Định luật Hubble**: `v = H₀·d`, `H₀ ≈ 70 km/s/Mpc`. Thiên hà càng xa lùi càng nhanh.
> - **Redshift**: `z = Δλ/λ₀`; với z nhỏ `v ≈ cz`. Đo vạch phổ → suy vận tốc & khoảng cách.
> - **Không tâm**: bánh mì nho / quả bóng phồng — mọi điểm thấy mọi điểm lùi xa; Big Bang xảy ra ở mọi nơi.
> - **Thời gian Hubble**: `t_H = 1/H₀ ≈ 14 tỉ năm` ≈ tuổi vũ trụ (13.8 tỉ năm).

---

## Bài tập

1. **Vận tốc lùi xa.** Một thiên hà cách `d = 250 Mpc`. Dùng `H₀ = 70 km/s/Mpc`, tính vận tốc lùi xa.

2. **Khoảng cách từ redshift.** Đo vạch Hydro `λ₀ = 656.3 nm` của một quasar, thu được `λ = 689.1 nm`. Tính `z`, `v` (dùng `v ≈ cz`) và khoảng cách `d`.

3. **Thời gian Hubble với H₀ khác.** Một số phép đo cho `H₀ = 73 km/s/Mpc`. Ước lượng tuổi vũ trụ (tỉ năm). So sánh với `H₀ = 67`.

4. **Không có tâm.** Trong mô hình bánh mì nho, ổ bột nở sao cho mọi khoảng cách tăng 3 lần sau 1 giờ. Hai hạt cách nhau `2 cm` và hai hạt khác cách nhau `5 cm`. Tính vận tốc lùi của mỗi cặp và kiểm tra rằng `v` tỉ lệ với `d`.

5. **Giới hạn của v ≈ cz.** Một thiên hà rất xa có `z = 1.5`. Vì sao **không** được kết luận `v = 1.5c = 450 000 km/s`? Giải thích bằng lời (không cần công thức tương đối tính).
...
```
v = H₀ · d = 70 km/s/Mpc × 250 Mpc = 17 500 km/s
```

→ Thiên hà lùi xa với **17 500 km/s** (≈ 0.058c — vẫn nhỏ so với ánh sáng nên `v ≈ cz` còn dùng tốt).

### Bài 2 — Khoảng cách từ redshift

**Bước 1 — tính z:**
```
Δλ = 689.1 − 656.3 = 32.8 nm
z = Δλ/λ₀ = 32.8 / 656.3 = 0.0500
```
**Bước 2 — vận tốc:**
```
v ≈ c·z = 300 000 × 0.0500 = 15 000 km/s
```
**Bước 3 — khoảng cách:**
```
d = v/H₀ = 15 000 / 70 ≈ 214 Mpc
```
→ Quasar cách **~214 Mpc** (≈ 700 triệu năm ánh sáng).

### Bài 3 — Thời gian Hubble với H₀ khác

Dùng tỉ lệ: với `H₀ = 70` ta có `t_H ≈ 14.0` tỉ năm. Vì `t_H ∝ 1/H₀`:

- `H₀ = 73`: `t_H = 14.0 × (70/73) ≈ 13.4` tỉ năm.
- `H₀ = 67`: `t_H = 14.0 × (70/67) ≈ 14.6` tỉ năm.

→ `H₀` càng lớn, vũ trụ ước lượng càng **trẻ**. Chênh lệch giữa 67 và 73 (đúng "căng thẳng Hubble — Hubble tension" trong vũ trụ học hiện đại) cho tuổi lệch ~1.2 tỉ năm.

### Bài 4 — Không có tâm

Nở 3 lần trong 1 giờ ⇒ mỗi khoảng cách tăng thêm `Δd = 2·d` (từ `d` lên `3d`). Vận tốc lùi = `Δd / 1 giờ`:

- Cặp `2 cm`: `Δd = 2 × 2 = 4 cm` → `v = 4 cm/h`.
- Cặp `5 cm`: `Δd = 2 × 5 = 10 cm` → `v = 10 cm/h`.

Kiểm tra tỉ lệ: `v/d` cho cặp 1 là `4/2 = 2 (1/h)`; cặp 2 là `10/5 = 2 (1/h)`. **Bằng nhau** ⇒ `v = (2/h)·d`, đúng dạng `v = H·d` với "hằng số Hubble" của ổ bánh `= 2 /h`. Vận tốc tỉ lệ khoảng cách, không cần biết "tâm" ở đâu.

### Bài 5 — Giới hạn của v ≈ cz

`v ≈ cz` chỉ là **xấp xỉ cho z nhỏ** (`z ≪ 1`), suy ra từ Doppler bậc nhất. Với `z = 1.5` (không nhỏ), xấp xỉ này sai. Lý do bằng lời:
...
2. Nếu cứ áp `v = cz` cho mọi z thì khi `z > 1` sẽ ra `v > c` — mâu thuẫn với thuyết tương đối nếu hiểu đó là vận tốc *qua* không gian. Thực ra `z` có thể lớn tùy ý mà không vi phạm gì, vì nó đo *giãn nở của không gian*, không phải vật bay nhanh hơn ánh sáng.

→ Phải dùng công thức tương đối tính/vũ trụ học (ngoài phạm vi bài), không dùng `v = cz`.
