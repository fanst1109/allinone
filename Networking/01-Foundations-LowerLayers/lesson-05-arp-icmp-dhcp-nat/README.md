...
> 💡 **ARP/ICMP/DHCP/NAT là các giao thức "hậu trường" — bạn không gọi chúng trực tiếp nhưng chúng giải thích `ping`, "tại sao IP nội bộ", và cả lỗ hổng bảo mật mạng.**

| Giao thức | Dùng/gặp thật ở đâu |
|-----------|---------------------|
| **NAT** | Vì sao máy nhà chung 1 IP công cộng; vì sao "không connect thẳng vào máy sau NAT" |
| **ICMP** | `ping` (reachability), `traceroute`, Path MTU Discovery |
| **DHCP** | Tự cấp IP; DHCP reservation gán IP cố định theo MAC |
| **ARP** | IP→MAC nội bộ; ARP spoofing = tấn công MITM mạng LAN |

### 6.1. Ví dụ cụ thể — NAT và bài toán "kết nối từ ngoài vào"

Máy chủ game/app sau NAT (mạng nhà/cloud private): client ngoài **không** kết nối thẳng vào được vì NAT chỉ map connection **đi ra**. Đây là lý do: (1) cần **port forwarding** để host service tại nhà; (2) WebRTC/P2P cần **STUN/TURN** để "xuyên NAT" (NAT traversal); (3) cloud cần **public IP / load balancer / NAT Gateway** để private instance ra internet (ra được, vào phải qua LB). Hiểu NAT = hiểu vì sao "chạy localhost OK, deploy không ai vào được" và cách giải. Cùng lý do IPv4 vẫn sống dù cạn địa chỉ — NAT cho nhiều máy chung một IP công cộng.

> ⚠ **ARP spoofing — lỗ hổng bảo mật LAN thật.** ARP không xác thực → kẻ tấn công trong cùng mạng LAN gửi ARP giả "tôi là gateway" → traffic nạn nhân đi qua máy nó (man-in-the-middle) → nghe lén/sửa. Đây là lý do: (1) **không tin mạng LAN** (quán cafe/wifi công cộng) → luôn HTTPS/VPN ([nối TLS](../../02-Application-Services/lesson-06-tls/)); (2) mạng doanh nghiệp dùng dynamic ARP inspection. ICMP cũng bị lạm dụng (ICMP flood DoS, ping sweep dò máy) → nhiều firewall chặn/giới hạn ICMP.

### 6.2. 📝 Tóm tắt mục 6

- NAT: nhiều máy chung 1 IP công; "vào trong" cần port-forward/LB/STUN-TURN (vì sao localhost OK mà deploy không ai vào).
- ICMP = `ping`/`traceroute`/PMTUD; DHCP tự cấp IP (reservation theo MAC); ARP = IP→MAC nội bộ.
- Bảo mật: **ARP spoofing** (MITM trên LAN) → không tin wifi công cộng, dùng HTTPS/VPN.
