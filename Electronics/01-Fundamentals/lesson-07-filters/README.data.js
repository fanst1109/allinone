...
- Tính **tần số cắt** f_c = 1/(2πRC) cho bộ lọc thông thấp và thông cao bậc 1.
- Hiểu ý nghĩa −3 dB tại f_c: biên độ giảm còn 1/√2 ≈ 70.7%.
...
- [Lesson 04 — Tụ điện & Mạch RC](../lesson-04-capacitor-rc/) — trở kháng tụ Z_C = 1/(jωC).
- [Lesson 06 — AC, Trở kháng & Mạch RLC](../lesson-06-ac-impedance-rlc/) — chia áp phức, khái niệm ω = 2πf.
...
- Tín hiệu trước ADC (bộ chuyển đổi tương tự-số): nếu có tần số > f_sample/2 sẽ gây aliasing (tín hiệu ảo không thể phân biệt với tín hiệu thật).
...
Tụ C đóng vai trò **chia áp phụ thuộc tần số**. Nhớ lại từ [Lesson 04](../lesson-04-capacitor-rc/) và [Lesson 06](../lesson-06-ac-impedance-rlc/): trở kháng tụ Z_C = 1/(jωC) với ω = 2πf.

### 2.2. Công thức và ý nghĩa

**Hàm truyền** (transfer function) — tỉ số biên độ V_out/V_in theo tần số:

\`\`\`
H(f) = V_out/V_in = Z_C / (R + Z_C) = 1 / (1 + jωRC)
\`\`\`
...
💡 **Ý nghĩa vật lý**: Ở tần số thấp (f << f_c), Z_C rất lớn → hầu hết điện áp rơi trên C → V_out ≈ V_in. Ở tần số cao (f >> f_c), Z_C rất nhỏ → C như ngắn mạch, hầu hết điện áp rơi trên R → V_out ≈ 0.

**Tại f = f_c chính xác**: |H| = 1/√(1 + 1) = 1/√2 ≈ **0.707** → điện áp ra bằng **70.7% điện áp vào**.

**Tại sao gọi là −3 dB?** Vì:
- dB = 20·log10(|H|) = 20·log10(1/√2) = 20·(−0.5·log10(2)) = 20·(−0.5·0.301) ≈ **−3.01 dB ≈ −3 dB**.
...
→ f_c ≈ **159 Hz**. Tín hiệu 50 Hz (nhiễu lưới) đi qua gần như nguyên vẹn; tín hiệu 10 kHz bị suy giảm mạnh.
...
R = 1 / (2π × f_c × C)
  = 1 / (2π × 1000 × 10×10⁻⁹)
  = 1 / (6.283×10⁻⁵)
  ≈ 15,915 Ω → chọn điện trở tiêu chuẩn gần nhất 15 kΩ hoặc 16 kΩ
\`\`\`
...
⚠ **Lỗi thường gặp**: Nhầm f_c = 1/(RC) (thiếu 2π). Ví dụ: R = 1 kΩ, C = 1 µF → 1/(RC) = 1000 rad/s = ω_c (tần số góc), còn f_c = ω_c/(2π) = 1000/6.283 ≈ 159 Hz. Phân biệt rõ ω_c (rad/s) và f_c (Hz).
...
f_c = 1/(2π × 2200 × 47×10⁻⁹) = 1/(2π × 1.034×10⁻⁴) = 1/(6.497×10⁻⁴) ≈ **1,539 Hz ≈ 1.54 kHz**.
...
- f_c = 1/(2πRC). Tích RC quyết định f_c, không phải giá trị riêng lẻ.
- Tại f_c: |H| = 1/√2 ≈ 0.707, tức −3 dB.
...
💡 **Hình dung**: Tụ C chặn một chiều (DC) và cản tần số thấp. Khi tần số thấp, Z_C lớn → điện áp rơi chủ yếu trên C → V_out nhỏ. Khi tần số cao, Z_C nhỏ → C gần ngắn mạch → hầu hết điện áp rơi trên R → V_out ≈ V_in.
...
Tại f = f_c: |H| = 1/√2 ≈ 0.707 → **−3 dB** (giống LPF, nhưng đây là điểm chuyển từ suy giảm sang thông).

Tần số thấp hơn f_c → suy giảm (dốc +20 dB/decade từ trái sang). Tần số cao hơn f_c → đi qua với |H| ≈ 1.
...
→ Cùng f_c — lại một lần nữa tích RC = 10⁻² quyết định kết quả.

**Ví dụ 3**: R = 4.7 kΩ, C = 100 nF → f_c?

\`\`\`
f_c = 1 / (2π × 4700 × 100×10⁻⁹)
    = 1 / (2π × 4.7×10⁻⁴)
    = 1 / (2.953×10⁻³)
    ≈ 339 Hz
\`\`\`
...
R = 1 / (2π × 500 × 22×10⁻⁹)
  = 1 / (6.912×10⁻⁵)
  ≈ 14,469 Ω → chọn 15 kΩ (tiêu chuẩn)
\`\`\`
...
f_c = 1/(2π × 33000 × 47×10⁻⁹) = 1/(2π × 1.551×10⁻³) = 1/(9.745×10⁻³) ≈ 102.6 Hz.

Tại f = 100 Hz ≈ f_c: |H| ≈ 1/√2 ≈ 0.707 → **suy giảm −3 dB** (≈ 29.3%).
...
**Vì sao dùng hệ số 20?** Vì công suất tỉ lệ với V². Nếu P_dB = 10·log10(P_out/P_in) thì A_dB = 20·log10(V_out/V_in) (vì log(x²) = 2·log(x)).
...
| √2 ≈ 1.414 | +3 dB (tăng gấp √2) |
| 1/√2 ≈ 0.707 | −3 dB (tại f_c) |
| 0.5 | −6 dB |
| 0.1 | −20 dB |
| 0.01 | −40 dB |
| 10 | +20 dB |
...
**Độ dốc −20 dB/decade**: với LPF bậc 1, ở vùng f >> f_c, mỗi khi f tăng 10 lần, |H| giảm 10 lần → A_dB giảm 20 dB.

**Chứng minh** (ở vùng f >> f_c): |H| = 1/√(1 + (f/f_c)²) ≈ f_c/f. Tại f₁ và 10f₁: |H(f₁)| = f_c/f₁, |H(10f₁)| = f_c/(10f₁) = (1/10)·|H(f₁)|. Tỉ số = 1/10 → A = 20·log10(1/10) = **−20 dB**.
...
- dB điện áp = 20·log10(V_out/V_in). Tại f_c: −3 dB.
...
🔁 **Tự kiểm tra**: Tại f = 10·f_c (gấp 10 lần tần số cắt), độ suy giảm của LPF bậc 1 xấp xỉ bao nhiêu dB?

<details>
<summary>Đáp án</summary>

|H| = 1/√(1 + 10²) = 1/√101 ≈ 0.0995. A_dB = 20·log10(0.0995) ≈ 20·(−1.002) ≈ **−20 dB**.

(Đúng như dự đoán từ độ dốc −20 dB/decade — tại 10·f_c là 1 decade từ f_c.)

</details>

### 📝 Tóm tắt mục 5

- Giản đồ Bode: trục X là log(f), trục Y là A_dB.
...
Trước khi đưa tín hiệu vào bộ chuyển đổi tương tự-số (ADC), **bắt buộc** phải lọc bỏ tất cả tần số cao hơn f_sample/2 (điều kiện Nyquist). Bộ lọc này gọi là **anti-aliasing filter** — là một LPF với f_c ≤ f_sample/2.
...
Ví dụ thực tế: R = 10 kΩ, C = 10 µF → f_c = 1/(2π × 10⁴ × 10⁻⁵) ≈ **1.6 Hz** → loại bỏ hầu hết nhiễu điện tử, giữ lại biến động nhiệt độ chậm.

### 📝 Tóm tắt mục 6

- LPF trong nguồn: lọc gợn sóng ripple.
- Crossover loa: LPF cho woofer, HPF cho tweeter.
- Anti-aliasing: LPF bắt buộc trước ADC (f_c ≤ f_sample/2).
...
**Bài 7**: Tín hiệu V_in = 2 V đỉnh, tần số 1 kHz đi qua LPF RC với f_c = 500 Hz. Tính V_out.

### Lời giải chi tiết

**Bài 1**:

Bước 1 — Tính R từ f_c = 300 Hz, C = 100 nF = 100×10⁻⁹ F:
\`\`\`
R = 1 / (2π × f_c × C)
  = 1 / (2π × 300 × 100×10⁻⁹)
  = 1 / (2π × 3×10⁻⁵)
  = 1 / (1.885×10⁻⁴)
  ≈ 5,305 Ω → chọn 5.1 kΩ hoặc 5.6 kΩ tiêu chuẩn
\`\`\`
...
A_dB = 20 × log10(0.0995) ≈ −20 dB
\`\`\`

→ R ≈ **5.1 kΩ**, tại 3 kHz suy giảm **−20 dB** (tín hiệu treble còn ≈ 10% biên độ).

**Bài 2**:

Bước 1 — Tính f_c:
\`\`\`
f_c = 1 / (2π × 47000 × 22×10⁻⁹)
    = 1 / (2π × 1.034×10⁻³)
    = 1 / (6.497×10⁻³)
    ≈ 153.9 Hz ≈ 154 Hz
\`\`\`
...
(b) Tại f = 500 Hz:
\`\`\`
f/f_c = 500/1064 ≈ 0.470
|H| = 1 / √(1 + 0.470²) = 1/√(1.221) = 1/1.105 ≈ 0.905
→ Biên độ ra ≈ 90.5% (suy giảm nhẹ, ≈ −0.87 dB)
\`\`\`

(c) Tại f = 5 kHz:
\`\`\`
f/f_c = 5000/1064 ≈ 4.70
|H| = 1/√(1 + 4.70²) = 1/√(23.09) = 1/4.805 ≈ 0.208
...
- Tại f = 100 kHz (tức 10·f_c = 1 decade từ f_c): đo được −20 dB.
...
Với f_c = 5 Hz:
- Nếu C = 1 µF: R = 1/(2π × 5 × 1×10⁻⁶) = 1/(3.14×10⁻⁵) ≈ **31,831 Ω ≈ 32 kΩ** — giá trị tiêu chuẩn (33 kΩ). Thực tế, hợp lý.
- Nếu C = 10 µF: R = 1/(2π × 5 × 10⁻⁵) = 1/(3.14×10⁻⁴) ≈ **3,183 Ω ≈ 3.3 kΩ** — cũng tiêu chuẩn.
...
(A): f_c = 1/(2π × 1000 × 1×10⁻⁶) ≈ **159 Hz**.

(B): f_c = 1/(2π × 10 × 100×10⁻⁶) = 1/(2π × 10⁻³) ≈ **159 Hz**.
...
\`\`\`
|H| = 1/√(1 + 2²) = 1/√5 ≈ 0.4472
V_out = V_in × |H| = 2 × 0.4472 ≈ 0.894 V
A_dB = 20 × log10(0.4472) ≈ −7 dB
\`\`\`

→ **V_out ≈ 0.894 V đỉnh** (tín hiệu 1 kHz bị suy giảm về ≈ 44.7% biên độ, tức −7 dB).
...
2. **f_c = 1/(2πRC)** — tần số cắt; tại f_c, biên độ giảm còn 1/√2 ≈ 0.707 tức −3 dB.
3. **LPF**: R nối tiếp → lấy áp tại C. **HPF**: C nối tiếp → lấy áp tại R. Cùng f_c, khác hướng suy giảm.
4. **dB = 20·log10(V_out/V_in)**: thang đo tiện lợi cho dải rộng. −3 dB tại f_c, −20 dB/decade (bậc 1).
