...
// Source: Algorithms/lesson-07-merge-sort/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 07 — Merge Sort (Sắp xếp trộn)
...
- [Lesson 06 — Elementary Sorts](../lesson-06-elementary-sorts/README.md): bubble/insertion/selection đều \`O(n²)\`. Merge sort là bước nhảy đầu tiên xuống \`O(n log n)\`.
...
> 💡 **Trực giác / Hình dung.** Bạn có một cọc 8 bài thi cần xếp theo điểm. Cách "ngây thơ" (insertion sort): cầm từng bài, dò từ đầu cọc đã xếp để chèn đúng chỗ — mỗi lần chèn phải đẩy nhiều bài, tốn \`O(n²)\`.
>
...
>
> - *"Tại sao chia đôi mà không chia ba, chia bốn?"* — Chia đôi cho recurrence \`T(n) = 2T(n/2) + O(n)\`. Chia \`k\` phần cho \`T(n) = kT(n/k) + O(n·log k cho merge k chiều)\` — vẫn \`O(n log n)\` nhưng hằng số lớn hơn và code phức tạp hơn. Chia đôi là đơn giản nhất mà vẫn tối ưu tiệm cận. (Chia \`k\` chiều có ích cho external sort — xem mục 8.)
...
> ❓ **Câu hỏi tự nhiên của người đọc.**
> - *"Cây có bao nhiêu tầng?"* — Mỗi tầng chia đôi kích thước, từ \`n\` xuống \`1\` mất \`⌈log₂ n⌉\` tầng. Với \`n=5\`: \`5 → 2/3 → 1\` ≈ 3 tầng chia. Tổng quát \`log₂ n\`. (Xem mục 4.)
...
Đây đúng dạng Master Theorem (xem [Lesson 03](../lesson-03-recursion-recurrence/README.md)): \`T(n) = a·T(n/b) + f(n)\` với \`a = 2\`, \`b = 2\`, \`f(n) = Θ(n)\`.

- Tính \`n^(log_b a) = n^(log₂ 2) = n¹ = n\`.
- So sánh với \`f(n) = Θ(n) = Θ(n^(log_b a))\` → **Case 2** của Master Theorem.
- Kết quả: \`T(n) = Θ(n^(log_b a) · log n) = Θ(n log n)\`. ∎
...
Số tầng = \`log₂ 8 + 1 = 4\`. Tổng công = \`8 × 4 = 32 = n·(log₂ n + 1) = Θ(n log n)\`. ✓

> 💡 **Trực giác.** Mỗi tầng "chạm" mỗi phần tử đúng một lần khi merge → \`n\` công/tầng. Số tầng = số lần chia đôi \`n\` về \`1\` = \`log₂ n\`. Nhân lại: \`n log n\`. Hình dung như tờ giấy gấp đôi: gấp \`log₂ n\` lần thì còn 1 ô.

### 4.3 Tại sao best = average = worst đều \`n log n\`?

Khác hẳn quicksort (worst \`O(n²)\`), merge sort **luôn** chia đôi đều đặn bất kể dữ liệu, và merge luôn quét hết \`n\` phần tử bất kể chúng đã sort hay đảo ngược. Recurrence \`T(n) = 2T(n/2) + Θ(n)\` **không phụ thuộc input** → ba trường hợp bằng nhau.
...
> ⚠ **Lỗi thường gặp.** Tưởng "merge sort nhanh hơn quicksort vì không có worst \`O(n²)\`". Thực tế quicksort thường **nhanh hơn về hằng số** (in-place, cache-friendly). Merge sort thắng ở **đảm bảo** và **stability**, không phải tốc độ thô. Xem mục 10.

> 📝 **Tóm tắt mục 4.** \`T(n)=2T(n/2)+Θ(n)\` → Master Theorem case 2 → \`Θ(n log n)\` cho best=avg=worst. Cây đệ quy: \`log n\` tầng × \`n\` công/tầng.
...
Bước merge **không thể trộn tại chỗ** một cách dễ dàng: để chèn phần tử của mảng phải vào giữa mảng trái cần dịch chuyển → quay về \`O(n²)\`. Cách chuẩn là dùng **mảng tạm (temp)** kích thước \`O(n)\` để chứa kết quả merge rồi copy ngược lại.
...
Số pass = \`log₂ 8 = 3\`. Mỗi pass quét toàn bộ \`n\` phần tử → \`O(n log n)\`, khớp top-down.
...
**Nghịch thế (inversion)** của mảng \`a\` là cặp chỉ số \`(i, j)\` với \`i < j\` nhưng \`a[i] > a[j]\` — tức cặp "sai thứ tự". Số nghịch thế đo mức độ "lộn xộn" của mảng (0 = đã sort tăng; tối đa \`n(n−1)/2\` = sort giảm).
...
**Cách ngây thơ:** đếm mọi cặp → \`O(n²)\`. **Cách thông minh:** lồng việc đếm vào merge sort → \`O(n log n)\`.
...
> - **Tràn số:** với \`n\` lớn, số nghịch thế tối đa \`n(n−1)/2\` ≈ \`5×10¹⁷\` với \`n=10⁹\` → phải dùng \`int64\`, không \`int32\`.

> 🔁 **Dừng lại tự kiểm tra.** Mảng \`[5,4,3,2,1]\` có bao nhiêu nghịch thế?
>
> <details><summary>Đáp án</summary>
>
> Mọi cặp đều nghịch thế → \`n(n−1)/2 = 5·4/2 = 10\`. Đây là tối đa (mảng sort giảm hoàn toàn).
> </details>

> 📝 **Tóm tắt mục 9.** Lồng đếm vào merge: khi lấy phần tử phải trước, cộng số phần tử trái còn lại. \`O(n log n)\` thay vì \`O(n²)\`. Nhớ \`int64\` và \`mid - i + 1\`.
...
| **Worst** | \`O(n log n)\` ✓ đảm bảo | \`O(n²)\` (pivot tệ) ✗ |
...
> 📝 **Tóm tắt mục 10.** Merge sort = **ổn định + worst \`O(n log n)\` đảm bảo + hợp linked list/external** nhưng tốn \`O(n)\` space và chậm hơn về hằng số. Quicksort = in-place + nhanh thực tế nhưng worst \`O(n²)\` + không stable.
...
\`\`\`
**Phân tích.**
- **(a) Merge từng cặp tuần tự** (gộp list1+list2, rồi +list3, ...): list kết quả lớn dần. Phần tử của list đầu bị merge lại \`k\` lần → \`O(N·k)\` xấu nhất. Nếu merge theo kiểu cây (ghép đôi balanced) thì \`O(N log k)\`.
...
- **So sánh:** cả (b) và (a)-cây-cân đều \`O(N log k)\`, nhưng heap chỉ giữ \`O(k)\` trong RAM cùng lúc → đây chính là engine của **external k-way merge** (mục 8). Merge tuần tự ngây thơ \`O(N·k)\` tệ hơn nhiều khi \`k\` lớn.
...
**Đối chiếu cận:** \`n log₂ n = 5 · log₂ 5 ≈ 5 · 2.32 ≈ 11.6\`. Thực tế 8 ≤ 11.6 ✓ (số so sánh thực luôn ≤ \`n⌈log n⌉\`, thường nhỏ hơn vì nhiều merge dừng sớm khi một nửa hết).
...
→ [Lesson 08 — Quicksort](../lesson-08-quicksort/README.md): partition (Lomuto/Hoare), chọn pivot, randomization để né worst case \`O(n²)\`, vì sao quicksort thường nhanh hơn merge sort trong thực tế dù worst tệ hơn.
