...
- Với heap n phần tử, **chiều cao** là $\\lfloor \\log_2 n \\rfloor$ → mọi thao tác đi từ gốc xuống lá tốn tối đa O(log n) bước.
- Các phần tử từ index $\\lfloor n/2 \\rfloor$ đến $n-1$ đều là **lá** (không có con) — đây là điểm khởi đầu quan trọng cho build heap (mục 3).
...
>
> 1. Ở một trong các **lá** (index $\\lfloor n/2 \\rfloor..n-1$). Heap KHÔNG đảm bảo lá ngoài cùng bên phải là nhỏ nhất — chỉ chắc chắn nhỏ nhất là một lá nào đó, vì mọi nút không-lá đều ≥ con của nó nên không thể nhỏ nhất.
...
> - Lá nằm ở index $\\lfloor n/2 \\rfloor..n-1$.
...
**Ý tưởng**: Lá (index $\\lfloor n/2 \\rfloor..n-1$) đã là heap kích thước 1 — không cần làm gì. Ta gọi sift-down cho mọi node **không-lá**, đi từ node không-lá cuối cùng ($\\lfloor n/2 \\rfloor - 1$) lùi về gốc (index 0). Khi xử lý node \`i\`, hai cây con của nó đã được heapify trước rồi (vì ta đi từ dưới lên) → tiền đề của sift-down được thỏa.
...
Trong heap n phần tử (chiều cao $h = \\lfloor \\log_2 n \\rfloor$), một node ở **chiều cao $k$** (lá có chiều cao 0, gốc chiều cao h) tốn tối đa **$k$ bước sift-down** (đi xuống tối đa k tầng).

Số node ở chiều cao $k$ trong heap đầy đủ là **tối đa $\\left\\lceil n / 2^{k+1} \\right\\rceil$**. Trực giác: lá (k=0) khoảng n/2 node; node chiều cao 1 khoảng n/4; chiều cao 2 khoảng n/8; ... — mỗi tầng cao hơn thì số node giảm một nửa.
...
Chuỗi $\\sum_{k=0}^{\\infty} k / 2^k$ là chuỗi hội tụ nổi tiếng, giá trị = **2**:
...
> ❓ **Câu hỏi tự nhiên**
> - *"Vậy sao chèn n phần tử từng cái (push + sift-up) lại là O(n log n)?"* — Vì sift-up đi từ **lá lên gốc**: lá ở tầng đông nhất (n/2 node) lại tốn nhiều bước nhất (tới log n), ngược hẳn với sift-down. Tổng $\\sum (n/2^{k+1}) \\times (h-k)$ không hội tụ thành tuyến tính → O(n log n). Chính vì vậy build heap **luôn dùng sift-down từ dưới lên**, không dùng sift-up.
