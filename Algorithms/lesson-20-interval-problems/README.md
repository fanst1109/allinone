# Lesson 20 — Interval Problems (Greedy trên khoảng)
...
Một interval là một cặp `[s, e]` với `s ≤ e` (start, end). Hai interval **chồng nhau (overlap)** nếu chúng có điểm chung. Đa số bài toán interval thuộc vào một trong các dạng:
...
> - *"Có cần DP không?"* → Không. DP cũng giải được nhưng `O(n²)` hoặc tệ hơn; greedy cho `O(n log n)` và đơn giản hơn nhiều.
...
- Nếu `o ≠ g`: vì `g` có end nhỏ nhất toàn cục nên `e_g ≤ e_o`. Thay `o` bằng `g` trong `OPT` để được tập `OPT'`. Ta khẳng định `OPT'` vẫn hợp lệ (đôi một không chồng):
  - `g` so với mọi hoạt động `x` còn lại trong `OPT` (tức `x` đứng *sau* `o`): vì `OPT` hợp lệ và sắp theo end, `s_x ≥ e_o ≥ e_g`, nên `g` không chồng `x`. (Bước này **không** lươn lẹo: `s_x ≥ e_o` do `x` đứng sau `o` và `OPT` không chồng; `e_o ≥ e_g` do `g` end nhỏ nhất.)
  - `g` không trùng `o` nên việc thay 1-đổi-1 giữ nguyên kích thước: `|OPT'| = |OPT|`.
- Vậy `OPT'` cũng tối ưu **và** chứa `g`.

Sau khi cố định `g`, mọi hoạt động chồng `g` bị loại; bài toán thu về cùng dạng trên tập hoạt động bắt đầu `≥ e_g`. Quy nạp trên kích thước bài toán ⇒ greedy cho lời giải tối ưu. ∎
...
>
> 1. Sau khi chọn {A(1,4), D(5,7)} có `lastEnd = 7`; `G(7,8)` có `7 ≥ 7 ✓` (biên nửa mở) → chọn → **{A, D, G}**, 3 hoạt động.
> 2. Vì lập luận "thay `o` bằng `g` mà vẫn hợp lệ" dựa vào `e_g ≤ e_o` (g xong sớm hơn nên không đụng những cái sau `o`). Nếu chọn theo start nhỏ nhất, cái đó có thể end rất muộn (vd `C(0,6)`) và chặn mất nhiều cái khác — phản ví dụ ở mục 3.
> </details>
...
`O(n log n)`. **Chứng minh đúng** ngắn gọn: sau khi sort theo start, mọi khoảng chồng nhau tạo thành một "cụm" liên tục trong dãy (nếu `B` chồng `A` mà `A` đứng trước thì `s_B ≤ e_A`; mọi khoảng sau `B` có start `≥ s_B`, nên cụm chồng nhau luôn liền kề). Một lượt quét gộp đúng từng cụm. ∎
...
**Bài toán.** Mỗi quả bóng là một khoảng `[s, e]` *theo trục x*. Một mũi tên bắn dọc tại `x = p` làm nổ mọi bóng có `s ≤ p ≤ e`. Tìm **ít mũi tên nhất** để nổ hết.
...
`O(n log n)`. **Chứng minh đúng:** Xét bóng có end nhỏ nhất, gọi `b`. Mọi mũi tên nổ `b` phải nằm trong `[s_b, e_b]`. Đặt mũi tên đúng tại `e_b` *trội* mọi vị trí khác (phủ tập bóng ⊇ bất kỳ vị trí nào khác trong `[s_b, e_b]`, vì bóng nào có `s ≤ e_b ≤ e` đều bị phủ). Exchange argument y hệt mục 2.4 ⇒ greedy tối ưu. ∎
...
3. **Off-by-one ở điều kiện overlap.** Hai khoảng `[a,b]` và `[c,d]` (đã giả sử `a ≤ c`) chồng nhau khi:
   - Biên đóng: `c <= b`.
   - Biên nửa mở: `c < b`.
