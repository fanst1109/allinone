# Lesson 47 — Hình học tính toán (Computational Geometry)
...
- **(a) Là gì** — một số (scalar) đo *"hai vector cùng hướng tới mức nào"*. Công thức tương đương: $a \cdot b = |a|\cdot|b|\cdot\cos\theta$, với $\theta$ là góc giữa hai vector.
...
| $a$ | $b$ | $a\cdot b$ | Góc |
|-----|-----|------|-----|
| $(1,0)$ | $(1,0)$ | $1\cdot 1 + 0\cdot 0 = 1$ | 0° (cùng hướng) |
| $(1,0)$ | $(0,1)$ | $1\cdot 0 + 0\cdot 1 = 0$ | 90° (vuông) |
| $(1,0)$ | $(-1,0)$ | $1\cdot(-1) + 0\cdot 0 = -1$ | 180° (ngược) |
| $(2,3)$ | $(4,-1)$ | $2\cdot 4 + 3\cdot(-1) = 8-3 = 5$ | nhọn (>0) |
| $(3,1)$ | $(-1,3)$ | $3\cdot(-1)+1\cdot 3 = -3+3 = 0$ | vuông |
...
- Dot product $a\cdot b = a_X\cdot b_X + a_Y\cdot b_Y$ → dấu cho biết góc nhọn ($>0$) / vuông ($0$) / tù ($<0$).
...
1. **Diện tích có dấu** của hình bình hành căng bởi $u$ và $v$. Trị tuyệt đối $|u \times v|$ = diện tích hình bình hành; chia đôi = diện tích tam giác.
...
```
Kiểm tra: tam giác vuông cạnh 4 và 3 → diện tích $= \frac{1}{2}\cdot 4\cdot 3 = 6$ ✓.

❓ **Câu hỏi tự nhiên của người đọc.**
- *"Vì sao cross = 0 lại nghĩa là thẳng hàng?"* — Vì $u \times v = |u||v|\sin\theta$; $\sin\theta = 0$ khi $\theta = 0°$ hoặc $180°$, tức hai vector song song → ba điểm thẳng hàng.
...
- *"Cross product có tràn số (overflow) không?"* — Có thể! Nếu tọa độ tới $\pm 10^9$, tích lên tới $\sim 10^{18}$, vẫn vừa `int64` ($\sim 9.2\times 10^{18}$). Nhưng nếu lớn hơn phải dùng `big.Int` hoặc `int128`.
...
$\text{cross} = (3-1)\cdot(5-1) - (1-1)\cdot(2-1) = 2\cdot 4 - 0\cdot 1 = 8 > 0$ → CCW, `c` ở bên **trái** (đúng vì `b` đi sang phải, `c` ở trên).
</details>

📝 **Tóm tắt mục 2.**
- $u \times v = u_X\cdot v_Y - u_Y\cdot v_X$ (số trong 2D).
- $\text{cross}(a,b,c) = (b-a)\times(c-a)$: $>0$ trái/CCW, $<0$ phải/CW, $=0$ thẳng hàng.
- $|\text{cross}|$ = diện tích hình bình hành; $/2$ = diện tích tam giác.
...
| $c$ | $\text{cross} = (4)(c_Y) - (4)(c_X)$ | Orientation |
|-----|----------------------------|-------------|
| $(0,4)$ | $4\cdot 4 - 4\cdot 0 = 16 > 0$ | CCW (trái) |
| $(4,0)$ | $4\cdot 0 - 4\cdot 4 = -16 < 0$ | CW (phải) |
| $(2,2)$ | $4\cdot 2 - 4\cdot 2 = 0$ | Collinear |
| $(8,8)$ | $4\cdot 8 - 4\cdot 8 = 0$ | Collinear |
| $(1,3)$ | $4\cdot 3 - 4\cdot 1 = 8 > 0$ | CCW |
...
Mọi cross = 0 (thẳng hàng). `onSegment((0,0),(3,0),(2,0))`: $3 \in [0,2]$? Không ($3 > 2$). Các on-segment khác cũng false. → **Không giao** ✓.
</details>
...
```
(Tam giác vuông cạnh 4,3 → $\frac{1}{2}\cdot 4\cdot 3 = 6$. Khớp.)
...
**Hướng CW (đảo thứ tự hình vuông trên):** `(0,0),(0,5),(5,5),(5,0)` → $\Sigma = -50$ → dấu âm = CW, diện tích vẫn $25$.
...
$\Sigma = (1\cdot 1-5\cdot 1) + (5\cdot 4-3\cdot 1) + (3\cdot 1-1\cdot 4) = (1-5)+(20-3)+(3-4) = -4+17-1 = 12$. Diện tích $= 12/2 = 6$. (Đáy 4, cao 3 → $\frac{1}{2}\cdot 4\cdot 3 = 6$ ✓.)
</details>

📝 **Tóm tắt mục 5.** Shoelace: $A = \frac{1}{2}\left|\sum(x_i\cdot y_{i+1} - x_{i+1}\cdot y_i)\right|$. Dấu tổng = hướng duyệt (CCW dương). $O(n)$. Giữ tổng dưới dạng $2A$ integer để khỏi mất chính xác.
...
💡 **Trực giác.** Brute force so mọi cặp = $O(n^2)$. Chia để trị: cắt đôi tập điểm bằng đường dọc, tìm cặp gần nhất ở nửa trái, nửa phải (đệ quy), lấy $d = \min(d_\text{trái}, d_\text{phải})$. Cặp gần nhất *xuyên qua* đường cắt chỉ có thể nằm trong **dải (strip)** rộng $2d$ quanh đường cắt — và trong dải đó, mỗi điểm chỉ cần so với ≤ 7 điểm kế tiếp (sắp theo $y$). Đây là phần "ghép" thông minh khiến $O(n^2)$ rớt xuống $O(n\log n)$. (Recap chia để trị: [Lesson 17](../lesson-17-divide-and-conquer/).)

Sơ đồ thuật toán:
1. Sort điểm theo $x$. $O(n\log n)$.
2. Đệ quy chia đôi → tìm $d$ của hai nửa.
3. Lấy các điểm trong dải $|x - x_\text{mid}| < d$, sort theo $y$.
...
> **Vì sao chỉ 7 điểm?** Trong một ô vuông $d \times d$ chỉ chứa được tối đa vài điểm (vì hai điểm cùng nửa cách nhau ≥ $d$). Xét dải $2d$ cao $d$, hình học chứng minh mỗi điểm có ≤ 7 hàng xóm cần kiểm tra → bước strip là $O(n)$, không phải $O(n^2)$.
...
```
**Cho $(0,0),(2,2),(3,1)$:** $\text{cross} = (2-0)(1-0) - (2-0)(3-0) = 2\cdot 1 - 2\cdot 3 = 2 - 6 = -4 < 0$ → **"CW"** (rẽ phải). Trực giác: đi từ $(0,0)$ lên $(2,2)$ (đường chéo lên-phải), điểm $(3,1)$ nằm bên phải-dưới đường đó. **Big-O:** $O(1)$.
...
- **$(3,2)$** — tia phải từ $(3,2)$. Cạnh trái $(0,0)-(3,6)$ bắc qua $y=2$: tại $y=2$, $x = 0 + (3-0)\cdot(2-0)/(6-0) = 1$. $3 < 1$? Không. Cạnh phải $(3,6)-(6,0)$: tại $y=2$, $x = 3 + (6-3)\cdot(2-6)/(0-6) = 3 + 3\cdot(-4)/(-6) = 3+2 = 5$. $3 < 5$? Có → lật. Đáy $y=0$ không bắc qua $y=2$. Tổng 1 (lẻ) → **TRONG** ✓.
- **$(0,4)$** — tia phải. Cạnh trái tại $y=4$: $x = 3\cdot 4/6 = 2$. $0 < 2$? Có → lật. Cạnh phải tại $y=4$: $x = 3 + 3\cdot(4-6)/(-6) = 3+1 = 4$. $0<4$? Có → lật lại. Tổng 2 (chẵn) → **NGOÀI** ✓ (điểm ở bên trái tam giác).
...
```
**Big-O:** $O(n^2)$ (mỗi gốc quét $O(n)$ điểm). Đây là cách tối ưu thực tế; $O(n^2\log n)$ nếu dùng sort thay map. Chuẩn hóa `(dy,dx)` bằng gcd + dấu → tránh float, tránh chia 0 (đường thẳng đứng `dx=0` vẫn là key hợp lệ).
...
```
**Vì sao ≤ 7 điểm?** Trong dải rộng $2d$, các điểm cùng một nửa cách nhau ≥ $d$. Chia dải thành các ô vuông $d/2 \times d/2$ → mỗi ô chứa tối đa 1 điểm. Một cửa sổ $2d \times d$ chứa tối đa $4\times 2 = 8$ ô, trừ chính nó còn 7 ô → mỗi điểm chỉ cần so ≤ 7 điểm kế tiếp theo $y$. Vòng `j` tự dừng nhờ điều kiện $\Delta y < d$. **Big-O:** bước strip $O(n)$; tổng thuật toán $T(n)=2T(n/2)+O(n)=O(n\log n)$. (Dùng bình phương khoảng cách để giữ integer, tránh `sqrt`.)
