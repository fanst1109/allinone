...
// Source: Astronomy/02-StarsGalaxies/lesson-02-measuring-stars/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 02 — Đo sao: độ sáng & khoảng cách (Measuring Stars)
...
- Dùng công thức \`m₁ − m₂ = −2.5·log(F₁/F₂)\` để so sánh độ sáng (flux).
- Hiểu **độ trưng (luminosity)** vs **độ rọi/flux** và **định luật nghịch đảo bình phương**.
- Đo **khoảng cách bằng thị sai (parallax)**: \`d[pc] = 1/p["]\`, và phân biệt **parsec** với **năm ánh sáng**.
...
> <details><summary>Đáp án</summary>Sao **A** (m = 2) sáng hơn, vì số nhỏ hơn. Chênh 3 cấp ⇒ A sáng hơn B khoảng \`100^(3/5) ≈ 15.8\` lần.</details>

---

## 2. Thang logarit — công thức cấp sao

> 💡 **Trực giác.** "Chênh 5 cấp = 100 lần độ sáng" nghĩa là mỗi 1 cấp ứng với hệ số \`100^(1/5) = 10^(0.4) ≈ 2.512\`. Đi xuống 1 cấp (sáng hơn) → flux nhân 2.512; đi lên 1 cấp (mờ hơn) → flux chia 2.512.

> 📐 **Công thức Pogson:**
> \`m₁ − m₂ = −2.5 · log₁₀(F₁/F₂)\`
>
> trong đó \`F\` là **flux** (độ rọi, năng lượng nhận được trên mỗi m² ở Trái Đất). Dấu trừ phản ánh "số nhỏ = sáng". Đảo lại: \`F₁/F₂ = 10^[−0.4·(m₁−m₂)]\`.

**Walk-through bằng số thật (verify cả 2 vế):**

1. **Chênh đúng 5 cấp.** \`m₁ − m₂ = 5\`:
   \`\`\`
   F₁/F₂ = 10^(−0.4×5) = 10^(−2) = 0.01
   \`\`\`
   → sao có m lớn hơn 5 đơn vị thì **mờ gấp 100 lần** (flux = 1/100). Verify ngược: \`m₁−m₂ = −2.5·log(0.01) = −2.5×(−2) = 5\` ✓.

2. **Chênh 1 cấp.** \`Δm = 1\` → \`F₁/F₂ = 10^(−0.4) = 0.398 ≈ 1/2.512\`. Mỗi cấp = hệ số 2.512. ✓

3. **Sirius vs Mặt Trời** (\`m_Sirius = −1.46\`, \`m_Sun = −26.7\`):
   \`\`\`
   Δm = −1.46 − (−26.7) = 25.24
   F_Sun/F_Sirius = 10^(0.4×25.24) = 10^(10.10) ≈ 1.26×10¹⁰
   \`\`\`
   → Mặt Trời sáng hơn Sirius (nhìn từ Trái Đất) khoảng **13 tỉ lần**. (Hợp lý: Mặt Trời ở ngay cạnh ta.)

4. **Hai sao chênh 2.5 cấp.** \`Δm = 2.5\`:
   \`\`\`
   F₁/F₂ = 10^(−0.4×2.5) = 10^(−1) = 0.1
   \`\`\`
   → chênh 2.5 cấp = đúng **10 lần** độ sáng. (Mẹo nhớ: 2.5 cấp ↔ ×10.)

> ❓ **Câu hỏi tự nhiên của người đọc.**
> - *"Vì sao là 2.5 mà không phải số tròn?"* — Vì người ta *chốt* "5 cấp = 100 lần" trước (để khớp thang cổ của Hipparchus), rồi giải ra hệ số \`2.5 = 5/log₁₀(100) = 5/2\`. Con số 2.5 là hệ quả, không phải chọn tùy ý.
> - *"log cơ số mấy?"* — Cơ số 10 (\`log₁₀\`). Trong công thức Pogson luôn là log thập phân.
...
> <details><summary>Đáp án</summary>10 cấp = 2 lần "bậc 5 cấp" = \`100 × 100 = 10000\` lần. Hoặc: \`F₁/F₂ = 10^(0.4×10) = 10⁴ = 10000\`.</details>
...
- **Độ trưng L (luminosity)** — tổng công suất sao phát ra (W). **Thuộc tính nội tại** của sao, không đổi theo khoảng cách. (Đã gặp ở Lesson 01: \`L = 4πR²σT⁴\`.)
- **Flux F (độ rọi)** — năng lượng nhận được trên mỗi m² tại Trái Đất (W/m²). **Phụ thuộc khoảng cách**: sao càng xa, F càng nhỏ.

> 📐 **Định luật nghịch đảo bình phương:**
> \`F = L / (4π d²)\`
>
> với \`d\` là khoảng cách tới sao. Hệ quả: \`F ∝ 1/d²\`.

**Walk-through số thật (verify):**

Mặt Trời \`L☉ = 3.83×10²⁶ W\`, khoảng cách \`d = 1.496×10¹¹ m\` (1 AU):
\`\`\`
F = 3.83e26 / (4π × (1.496e11)²)
  = 3.83e26 / (4π × 2.238e22)
  = 3.83e26 / 2.812e23
  ≈ 1362 W/m²
\`\`\`
...
> \`m − M = 5·log₁₀(d) − 5\` (d tính bằng parsec).
...
> - *"Mặt Trời cấp tuyệt đối bao nhiêu?"* — \`M_Sun ≈ +4.83\`. Nghĩa là nếu đặt Mặt Trời ở 10 pc, nó chỉ là một sao mờ nhạt mắt thường vừa thấy — Mặt Trời thực ra là sao khá tầm thường.

> 🔁 **Dừng lại tự kiểm tra.** Một sao bị dời ra xa gấp 5 lần. Flux nhận được thay đổi thế nào?
> <details><summary>Đáp án</summary>\`F ∝ 1/d²\` nên xa gấp 5 → flux giảm \`1/5² = 1/25\` lần (chỉ còn 4%). Sao trông mờ đi nhiều.</details>
...
- **(c) Ví dụ trực giác bằng số** — Sao gần nhất Proxima Centauri có \`p = 0.769"\` → \`d = 1/0.769 = 1.30 pc\`. Không có sao nào có \`p ≥ 1"\` (tức không sao nào gần hơn 1 pc).

> 📐 **Công thức thị sai:** \`d [pc] = 1 / p ["]\`.
>
> Đây là *định nghĩa* của parsec: khoảng cách mà tại đó 1 AU chắn một góc đúng 1 giây cung. (Hình học: \`tan p = 1 AU / d\`; với p rất nhỏ, \`p[rad] ≈ 1 AU/d\`, rút gọn thành \`d[pc]=1/p["]\`.)

**Walk-through số thật (verify):**

1. **p = 0.1"**:
   \`\`\`
   d = 1/0.1 = 10 pc = 32.6 năm ánh sáng
   \`\`\`
   Verify ngược: ở 10 pc, 1 AU chắn \`p = 1/10 = 0.1"\` ✓.

2. **p = 0.769" (Proxima):** \`d = 1/0.769 = 1.30 pc = 4.24 ly\` — đúng khoảng cách đã biết tới sao gần nhất ✓.

3. **p = 0.379" (Sirius):** \`d = 1/0.379 = 2.64 pc = 8.6 ly\` — khớp giá trị quan trắc ✓.

4. **p = 0.001" (sao xa):** \`d = 1/0.001 = 1000 pc = 3260 ly\`. Góc cực nhỏ → cần vệ tinh chính xác cao.

> 📏 **Đơn vị khoảng cách:**
> - **Năm ánh sáng (light-year, ly)** = quãng đường ánh sáng đi trong 1 năm \`≈ 9.46×10¹² km\`.
> - **Parsec (pc)** = \`3.26 ly ≈ 3.086×10¹³ km\`. Thiên văn học chuyên nghiệp dùng pc vì gắn trực tiếp với phép đo thị sai (\`d=1/p\`), không cần đổi.
...
> - *"1 AU là gì?"* — Đơn vị thiên văn (astronomical unit) = khoảng cách Trái Đất–Mặt Trời trung bình ≈ 1.496×10⁸ km.

> ⚠ **Lỗi thường gặp.** Quên rằng \`p\` phải tính bằng **giây cung**, không phải độ. \`0.1°\` rất khác \`0.1"\` (1° = 3600"). Công thức \`d=1/p\` chỉ đúng với p tính bằng arcsecond.

> 🔁 **Dừng lại tự kiểm tra.** Sao Vega có \`p = 0.13"\`. Khoảng cách bằng bao nhiêu pc và ly?
> <details><summary>Đáp án</summary>\`d = 1/0.13 = 7.7 pc\`; đổi sang ly: \`7.7 × 3.26 = 25 ly\`. (Vega cách ~25 năm ánh sáng — khớp.)</details>
...
> - **Thang logarit**: 5 cấp = 100 lần; mỗi cấp = ×2.512; \`m₁−m₂ = −2.5·log(F₁/F₂)\`.
> - **Độ trưng L** (nội tại, không đổi) vs **flux F** (phụ thuộc khoảng cách): \`F = L/(4πd²)\`, nghịch đảo bình phương.
> - **Cấp tuyệt đối M**: cấp biểu kiến nếu đặt sao ở 10 pc; đo độ sáng nội tại. \`m−M = 5·log d − 5\`.
> - **Thị sai**: \`d[pc] = 1/p["]\`; parsec = 3.26 ly. Proxima p=0.769" → 1.30 pc.

---

## Bài tập

1. **So sánh độ sáng.** Sao A có \`m = 1.0\`, sao B có \`m = 6.0\`. Tỉ số flux \`F_A/F_B\` bằng bao nhiêu? Sao nào sáng hơn?

2. **Cấp ↔ tỉ số tổng quát.** Hai sao chênh \`Δm = 3.5\`. Tính tỉ số độ sáng.
...
\`Δm = m_A − m_B = 1.0 − 6.0 = −5.0\`.
\`\`\`
F_A/F_B = 10^(−0.4 × Δm) = 10^(−0.4×(−5)) = 10^(2) = 100
\`\`\`
...
\`\`\`
F₁/F₂ = 10^(−0.4 × (−3.5)) = 10^(1.4) = 25.1
\`\`\`
(Lấy sao sáng hơn làm tử số.) → chênh 3.5 cấp ⇒ sáng hơn **~25 lần**. Verify: \`−2.5·log(25.1) = −2.5×1.4 = −3.5\` ✓.

### Bài 3 — Nghịch đảo bình phương

**(a)** \`F ∝ 1/d²\`; xa gấp 4 → \`F → F/4² = F/16\`. Flux còn **1/16** (≈ 6.25%).

**(b)** Đổi tỉ số flux 1/16 sang cấp sao:
\`\`\`
Δm = −2.5·log(F_mới/F_cũ) = −2.5·log(1/16) = −2.5×(−1.204) = +3.01
\`\`\`
→ sao **mờ đi ~3 cấp** (m tăng thêm ~3). Trực giác: ×16 độ sáng ≈ chênh 3 cấp (vì \`2.512³ ≈ 15.85 ≈ 16\`).

### Bài 4 — Thị sai → khoảng cách (61 Cygni)

\`\`\`
d = 1/p = 1/0.286 = 3.50 pc
\`\`\`
Đổi sang ly: \`3.50 × 3.26 = 11.4 ly\`. (61 Cygni — sao đầu tiên đo được parallax, năm 1838 — cách ~11.4 năm ánh sáng. Khớp.)

### Bài 5 — Khoảng cách → thị sai

Đảo công thức: \`p = 1/d = 1/50 = 0.02"\`. → thị sai **0.02 giây cung** (rất nhỏ, cần thiết bị tốt).
...
d = 10² = 100 pc
\`\`\`
→ sao cách **100 pc** (≈ 326 ly). Kiểm tra trực giác: \`m−M = 5\` đúng nghĩa "ở xa gấp 10 lần khoảng cách chuẩn 10 pc" → \`10×10 = 100 pc\` ✓.
