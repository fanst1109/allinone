...
// Source: Databases/02-Intermediate/lesson-05-query-execution/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 05 — Query Execution & EXPLAIN
...
- **Độ phân biệt (distinct values / selectivity)**: cột \`city\` có 64 giá trị khác nhau → một giá trị cụ thể (\`'Hanoi'\`) khớp khoảng $1{.}000{.}000 / 64 \\approx 15{.}625$ dòng.
...
2. $\\approx 500{.}000 / 50 = \\mathbf{10{.}000}$ dòng (selectivity = 2%), giả sử phân bố đều.
</details>
...
**(b) Index seek/scan (truy cập qua index).** Dùng B-tree index để tìm các dòng khớp ($\\sim O(\\log n)$ bước), rồi với mỗi dòng tìm được phải **nhảy ngẫu nhiên** tới trang chứa dòng đó để đọc (random I/O). Random I/O **đắt hơn nhiều** so với sequential I/O.
...
**Plan A — Sequential scan:** đọc cả 10.000 trang tuần tự (luôn cố định, không phụ thuộc selectivity):

$$\\text{cost}_{\\text{seq}} = P \\times 1 = 10000 \\times 1 = 10000 \\text{ đơn vị}$$

**Plan B — Index scan:** với mỗi dòng khớp, phải làm ~1 random I/O (giả sử mỗi dòng khớp nằm một trang khác). Gọi $k$ = số dòng khớp (bỏ qua chi phí đi xuống B-tree, vốn nhỏ $\\sim \\log n$):

$$\\text{cost}_{\\text{idx}} \\approx k \\times 4$$

So sánh tại các mức selectivity:

| Selectivity | k (dòng khớp) | $\\text{cost}_{\\text{idx}} = k \\times 4$ | $\\text{cost}_{\\text{seq}}$ | Optimizer chọn |
| --- | --- | --- | --- | --- |
...
**Điểm hòa vốn (break-even):** $k \\times 4 = 10000 \\Rightarrow k = 2500$ dòng $\\approx$ **0,25%** của bảng. Dưới ngưỡng này index thắng; trên ngưỡng này quét tuần tự thắng.
...
1. $\\text{cost}_{\\text{idx}} = 1500 \\times 4 = 6000 < 10000 = \\text{cost}_{\\text{seq}}$ → chọn **Index scan**.
2. Random rẻ hơn → index hấp dẫn hơn → điểm hòa vốn **dịch lên cao hơn** (break-even khi $k \\times 1 = 10000 \\Rightarrow k = 10000$ dòng = 1%). Tức index được dùng cho nhiều query hơn.
</details>
...
**Chi phí:** $O(m \\times n)$ so sánh — đắt nếu cả hai bảng to. **Nhưng** nếu bảng inner có **index** trên cột join, không cần duyệt cả inner mà tra $\\sim O(\\log n)$ → chi phí còn $O(m \\times \\log n)$.

**Ví dụ số.** outer \`orders\` = 1.000 dòng, inner \`customers\` = 100.000 dòng.
- Không index: $1000 \\times 100000 = 100000000$ so sánh.
- Có index trên \`customers.id\`: $1000 \\times \\log_2 100000 \\approx 1000 \\times 17 = 17000$ bước. Nhanh hơn ~5.900 lần.
...
- Hash join: $100000$ (build) $+ 1000000$ (probe) $= 1100000$ thao tác.
- Nested loop không index: $1000000 \\times 100000 = $ **100 tỉ** so sánh.
...
- Merge join: chỉ $\\sim 1000000 + 100000 = 1100000$ bước trộn (không tốn sort).
- Nếu phải sort \`orders\`: thêm $1000000 \\times \\log_2 1000000 \\approx 20000000$ bước → đắt hơn hash join.
...
| --- | --- | --- | --- |
| **Nested loop** | $O(m \\times n)$, hoặc $O(m \\cdot \\log n)$ nếu inner có index | Bất kỳ (kể cả \`<\`, \`>\`) | 1 bảng nhỏ + bảng kia có index |
...
1. **Hash join** — equi-join hai bảng lớn không index, $O(m+n) \\approx$ 10 triệu thao tác, rẻ nhất.
2. **Không** — đây không phải equi-join. Phải dùng nested loop (hoặc merge nếu sắp theo \`ts\`).
</details>

📝 **Tóm tắt mục 4.** Nested loop $O(m \\times n)$ (hoặc $O(m \\cdot \\log n)$ với index inner) — hợp bảng nhỏ. Hash join $O(m+n)$ — hợp equi-join hai bảng lớn, build hash trên bảng nhỏ rồi probe. Merge join $O(m+n)$ khi input đã sorted — hợp khi có index theo thứ tự. Optimizer ước lượng cost cả ba rồi chọn rẻ nhất.
...
- **(a) Nested loop join.** \`users\` rất nhỏ (50 dòng) làm outer, \`orders\` có index trên \`user_id\` làm inner → $50 \\times \\log 2000000 \\approx 50 \\times 21 \\approx 1050$ bước. Bảng nhỏ + inner có index = đúng "sân nhà" của nested loop.
- **(b) Hash join.** Equi-join hai bảng lớn, không index, không sorted. Build hash trên \`payments\` (nhỏ hơn, 4 triệu) rồi probe bằng \`orders\` → $O(m+n) \\approx 9$ triệu thao tác. Nested loop sẽ là $5M \\times 4M = 20$ nghìn tỉ → loại. Merge phải sort cả hai → đắt hơn hash.
...
- $\\text{cost}_{\\text{seq}} = 20000 \\times 1 = 20000$ (cố định).
- **(a)** 3.000 dòng: $\\text{cost}_{\\text{idx}} = 3000 \\times 4 = 12000 < 20000$ → chọn **Index Scan**.
- **(b)** 800.000 dòng: $\\text{cost}_{\\text{idx}} = 800000 \\times 4 = 3200000 \\gg 20000$ → chọn **Seq Scan** (chênh 160 lần).
- **(c)** Điểm hòa vốn: $k \\times 4 = 20000 \\Rightarrow k = 5000$ dòng $= 5000 / 2000000 = $ **0,25%**. Dưới 5.000 dòng → index thắng; trên → seq scan thắng.
