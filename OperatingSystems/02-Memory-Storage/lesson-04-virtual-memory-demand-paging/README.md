# Lesson 04 — Bộ nhớ ảo & Demand Paging
...
Trong đó:
- `p` = page fault rate (tỷ lệ truy cập gây page fault, 0 ≤ p ≤ 1).
- `t_mem` = thời gian truy cập RAM bình thường.
- `t_fault` = thời gian xử lý một page fault (bao gồm I/O đĩa).
...
- t_mem = **100 ns**
- t_fault = **8 ms = 8,000,000 ns** (HDD)

```
EAT = (1 − 0.001) × 100 + 0.001 × 8,000,000
    = 0.999 × 100 + 0.001 × 8,000,000
    = 99.9 + 8,000
    = 8,099.9 ns
    ≈ 8.1 µs
```
...
- t_mem = 100 ns, t_fault = 8,000,000 ns.
...
**Yêu cầu:** EAT ≤ 200 ns, t_mem = 100 ns, t_fault = 8,000,000 ns (HDD).

```
(1-p) × 100 + p × 8,000,000 ≤ 200
100 - 100p + 8,000,000p ≤ 200
7,999,900p ≤ 100
p ≤ 100 / 7,999,900 ≈ 0.0000125
p ≤ 1.25 × 10^-5
```
...
- *"Page fault rate thực tế là bao nhiêu?"* — Với workload thông thường và RAM đủ: p ≈ 10^-6 đến 10^-5. Khi RAM đầy và thrashing xảy ra: p có thể tăng vọt lên 0.01–0.1.
...
- SSD giảm t_fault 80× → hệ thống virtual memory thực tế hơn nhiều.
...
**Working set W(t, Δ)** = tập các trang tiến trình truy cập trong Δ đơn vị thời gian gần nhất. Δ là **cửa sổ working set (working set window)**.

**Ví dụ:** Chuỗi truy cập trang: 1, 2, 3, 4, 3, 2, 1, 1, 2, 5. Với Δ=5:

- Tại t=10 (5 truy cập gần nhất: 1, 1, 2, 5, ...): W = {1, 2, 5} — cần 3 frame.

**Ý nghĩa:** Nếu OS cấp cho tiến trình ≥ |W| frame → gần như không có page fault (tất cả trang đang dùng đều nằm trong RAM).
...
```
Nếu: Σ |W(ti)| > số frame RAM
Thì: thiếu frame → page fault liên tục → CPU utilization giảm
...
Σ |W| = 5 + 6 + 5 = 16 frame. 16 ≤ 20 → đủ frame cho cả 3 tiến trình. **Không thrashing.** Còn dư 4 frame có thể dùng cho tiến trình mới hoặc buffer.
</details>

📝 **Tóm tắt mục 5:**
- Working set: tập trang tiến trình dùng trong cửa sổ thời gian Δ.
...
EAT = (1 − 0.0001) × 100 + 0.0001 × 10,000,000
    ...
    = 99.99 + 1,000
    = 1,099.99 ns ≈ 1,100 ns
```

**(b)** EAT / t_mem = 1100 / 100 = **11 lần** chậm hơn. Với p chỉ 0.01% (1/10000), hệ thống đã chậm hơn 11 lần!

### Bài 3 — Tính p tối đa

```
(1-p) × 150 + p × 5,000,000 ≤ 300
150 − 150p + 5,000,000p ≤ 300
4,999,850p ≤ 150
p ≤ 150 / 4,999,850 ≈ 3 × 10^-5
```
...
**Nếu swap out A** (working set 3 frame): còn lại B+C+D = 5+6+3 = 14 ≤ 16 frame → an toàn.
...
- **EAT = (1−p)×t_mem + p×t_fault**. HDD: p=0.001 → EAT tăng 81×. SSD: nhẹ hơn nhiều.
- **Thrashing**: Σ working set > RAM → fault liên tục → CPU util thấp. Giải pháp: giảm tiến trình hoặc tăng RAM.
