...
Kẻ tấn công thấy C_1 = C_2 → biết ngay P_1 = P_2, dù không biết key.
...
> 💡 **Trực giác**: Mỗi block trước khi encrypt được XOR với block ciphertext trước đó. IV (Initialization Vector) đóng vai trò C_0 để chain bắt đầu được. Hai plaintext block giống nhau → ciphertext khác nhau vì C_{i-1} khác.

### 3.2. Walk-through 3 block

Giả sử (ký hiệu đơn giản, giá trị hex):
...
Block 1: P_1 = "ATTACKATDAWN!!!!" (0x41 0x54 ... 0x21)
  → C_1 = E_K(P_1 ⊕ C_0)
         = E_K("ATTACKATDAWN!!!!" ⊕ 0xAA...)
         = 0xF3 0x82 ... (ciphertext block 1)

Block 2: P_2 = "ATTACKATDAWN!!!!" (giống P_1!)
  → C_2 = E_K(P_2 ⊕ C_1)
         ≠ C_1 vì C_1 ≠ C_0
         → C_2 trông khác hoàn toàn C_1
...
**Flip bit trong IV**: 1 bit thay đổi ở IV → bit tương ứng trong P_1 bị flip khi decrypt. Các block sau không ảnh hưởng.

**Flip bit trong C_i**: Khi decrypt:
- **P_i bị corrupt hoàn toàn** (vì D_K(C_i) thay đổi toàn bộ).
- **Cùng bit trong P_{i+1} bị flip** (vì P_{i+1} = D_K(C_{i+1}) ⊕ C_i, đúng bit đó trong C_i đã flip).

**Ví dụ attack**: Nếu C_1 = encrypt("Transfer $0100 to"), kẻ tấn công flip đúng 1 bit ở vị trí byte '$' → decrypt P_2 thay đổi giá trị tương ứng (P_2 corrupt, nhưng vẫn được xử lý nếu server bỏ qua lỗi).
...
2. Thử 256 giá trị cho byte cuối C_{i-1} → khi không có lỗi padding, suy ra byte cuối P_i.
3. Lặp lại cho từng byte → giải mã hoàn toàn plaintext mà không cần key.

**Độ phức tạp**: 256 × 16 × số_block = ~4096 request để giải mã 1 block.
...
- **Parallelizable hoàn toàn**: tính Keystream_1, Keystream_2, ... đồng thời.
...
**Vấn đề**: CTR không authenticate. Flip bit trong C_i → **cùng bit trong P_i bị flip**, không có lỗi nào xuất hiện.

**Ví dụ concrete**:

```
Ciphertext: ... C_amount_bytes ...

Biết plaintext có "$0100" ở vị trí nào đó.
'$' = 0x24, '0' = 0x30, '1' = 0x31, '0' = 0x30, '0' = 0x30

Muốn đổi thành "$0900" ('9' = 0x39):
Flip bit 3 (bit value 8) của byte '1' (0x31):
  0x31 ⊕ 0x08 = 0x39 = '9'

Kết quả: ciphertext byte đó flip → decrypt ra "Transfer $0900"
```
...
```
C = CTR_Encrypt(K, nonce, P)
T = GHASH(H, A, C)  // authentication tag 128 bit
  H = E_K(0...0)   // hash subkey
```

GHASH tính MAC bằng nhân đa thức trong GF(2¹²⁸) — nhanh với hardware.
...
- **Integrity + Authenticity**: GHASH tag đảm bảo — bất kỳ 1 bit thay đổi trong C hoặc AD → tag không khớp.
- **Nonce reuse là thảm họa**: nếu dùng lại nonce với cùng key → keystream trùng → XOR 2 ciphertext = XOR 2 plaintext → toàn bộ confidentiality mất. Nonce PHẢI unique cho mỗi message.

**Ví dụ verify fail**:

```
Alice gửi: (C, T) = encrypt("Transfer $1000 to Alice")
Mallory sửa C thành C' = encrypt thay "$1000" → "$9999"
Bob nhận (C', T): GHASH(C') ≠ T → reject!
```
...
**IV = 0 cố định**: Không sai về mặt kỹ thuật nếu mỗi message dùng key khác. Nhưng nếu Alice dùng cùng key nhiều lần, P_1 luôn XOR với 0x00 trước khi encrypt → nếu P_1 giống nhau ở nhiều message → C_1 giống nhau. Mất tính indistinguishability.

**IV sequential**: Chosen-plaintext attack. Nếu Mallory biết IV tiếp theo sẽ là IV_n và đang chọn 1 block để encrypt, cô có thể tạo plaintext = P_target ⊕ IV_n ⊕ IV_old → decrypt cho ra P_target. Đây là lý do IV phải **random** (không đoán được trước).

### Bài 3

```
Vị trí "$0100" trong plaintext = index i trong chuỗi byte.
Giả sử nằm trong block j, byte offset b.
Keystream_j[b] = C_j[b] ⊕ P_j[b]
Muốn P'_j[b] = '9' = 0x39:
  C'_j[b] = Keystream_j[b] ⊕ '9'
           = (C_j[b] ⊕ P_j[b]) ⊕ '9'
           = C_j[b] ⊕ P_j[b] ⊕ '9'
```

Biết P_j[b] = '$' = 0x24 và '9' = 0x39:
```
C'_j[b] = C_j[b] ⊕ 0x24 ⊕ 0x39 = C_j[b] ⊕ 0x1d
```

Flip bit ở đúng vị trí: XOR byte C_j[b] với 0x1d. Lặp lại cho '0','1','0','0' → '$','9','9','9'. **Đây là lý do CTR một mình không đủ**.

### Bài 4

Xác suất đoán đúng tag = 1/2¹²⁸ ≈ 3×10⁻³⁹. Nếu Mallory gửi 10⁹ tampered message/giây liên tục trong 100 năm = 3.15×10¹⁸ message → xác suất thành công ≈ 3×10¹⁸ / 2¹²⁸ ≈ 10⁻²⁰. Thực tế: hệ thống rate-limit và block IP sau vài lần fail → xác suất còn thấp hơn nhiều.
