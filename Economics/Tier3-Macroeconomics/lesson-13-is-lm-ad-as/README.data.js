...
E = C(Y) + I(r) + G + NX
\`\`\`
- \`C(Y) = a + b·Y\` (consumption tăng theo thu nhập, \`b\` = MPC marginal propensity to consume).
- \`I(r) = d − e·r\` (đầu tư giảm khi lãi suất tăng).

Cân bằng thị trường hàng: \`Y = E\`.

### 1.2. Đường IS

Giải \`Y = a + bY + d − e·r + G\` cho \`Y\`:
\`\`\`
Y = (a + d + G) / (1 − b) − e/(1−b) · r
\`\`\`
...
**Walk-through**: \`a = 100, b = 0.6, d = 200, e = 50, G = 50\`. IS: \`Y = (100 + 200 + 50)/0.4 − 50/0.4 · r = 875 − 125r\`.
...
Cầu tiền \`M_d = k·Y − h·r\` (cần tiền cho giao dịch ↑Y, nhưng giữ tiền tốn cơ hội ↑r → cầu giảm).
...
- **G tăng** (chính sách tài khóa mở rộng): IS dịch phải → \`Y*↑, r*↑\`.
- **Thuế giảm** → C tăng → IS dịch phải.
- **Xuất khẩu giảm** → IS dịch trái → suy thoái.

### 3.2. Sốc lên LM

- **M tăng** (NHTW mở rộng tiền): LM dịch xuống → \`Y*↑, r*↓\`.
- **M giảm** (siết tiền): LM dịch lên → \`Y*↓, r*↑\`.
...
Khi SR ≠ LR → áp lực điều chỉnh giá → SRAS dịch.
...
So sánh ΔY trong Bài 2 với ΔY khi LM ngang (r không đổi). Khoảng cách = crowding out.
...
\`800 − 100r = 200 + 200r\` → \`600 = 300r\` → \`r* = 2%\`. \`Y* = 600\`.

### Lời giải Bài 2

\`900 − 100r = 200 + 200r\` → \`700 = 300r\` → \`r* ≈ 2.33%\`. \`Y* ≈ 667\`. ΔY = \`67\`, Δr = \`0.33%\`.

### Lời giải Bài 3

Nếu r cố định 2%: ΔY chỉ do IS dịch = 100. Thực tế ΔY = 67. **Crowding out = 100 − 67 = 33** (33% G tăng bị đẩy ra do r tăng).

### Lời giải Bài 4

Giá dầu tăng → chi phí sản xuất tăng → **SRAS dịch lên** (sang trái nếu nhìn theo Y). Cân bằng mới: \`Y↓, P↑\` → vừa suy thoái vừa lạm phát = **stagflation**. NHTW kẹt: nới lỏng (AD phải) → P thêm cao; siết (AD trái) → Y thêm thấp. Đây là tình huống của Mỹ 1970s.
