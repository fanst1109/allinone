# Lesson T3-01 — K-means & Hierarchical Clustering
...
- [Vectors/Linear Algebra: khoảng cách Euclidean, trung bình vector](../../../Vectors/): `‖a − b‖₂`, mean vector.
...
**Input**: tập điểm `X = {x₁, …, xₙ}` ∈ ℝᵈ, số cluster `k`.

**Bước 1 — Khởi tạo**: chọn k centroids `μ₁, …, μₖ` (random hoặc K-means++).
...
Điểm: `x = [1, 1.5, 7, 8]`. Khởi tạo: `μ₁ = 1`, `μ₂ = 7`.

**Iter 1 — Assign**:
- `x₁=1`: dist(1,1)=0, dist(1,7)=6 → C1
- `x₂=1.5`: dist(1.5,1)=0.5, dist(1.5,7)=5.5 → C1
- `x₃=7`: dist(7,1)=6, dist(7,7)=0 → C2
- `x₄=8`: dist(8,1)=7, dist(8,7)=1 → C2
...
**Iter 2 — Assign**:
- `x₁=1`: dist(1,1.25)=0.25, dist(1,7.5)=6.5 → C1
- `x₂=1.5`: dist(1.5,1.25)=0.25, dist(1.5,7.5)=6 → C1
- `x₃=7`: dist(7,1.25)=5.75, dist(7,7.5)=0.5 → C2
- `x₄=8`: dist(8,1.25)=6.75, dist(8,7.5)=0.5 → C2

**Iter 2 — Update**: μ₁=1.25 (không đổi), μ₂=7.5 (không đổi) → **Hội tụ**.
...
Với mỗi điểm `xᵢ`:
- `a(i)` = trung bình khoảng cách từ `xᵢ` đến các điểm *cùng cluster*.
- `b(i)` = trung bình khoảng cách từ `xᵢ` đến các điểm của *cluster gần nhất khác*.
- **Silhouette**: `s(i) = (b(i) − a(i)) / max(a(i), b(i))`

Giá trị: `s(i) ∈ [−1, 1]`.
- `s(i) ≈ 1`: điểm nằm gọn trong cluster của nó, xa cluster khác — tốt.
- `s(i) ≈ 0`: điểm nằm trên ranh giới giữa 2 cluster.
- `s(i) < 0`: điểm bị gán nhầm cluster.

**Mean silhouette** `S(k) = mean(s(i))` — chọn k có `S(k)` lớn nhất.
...
>
> C1={0, 0.1, 0.2} (μ₁=0.1), C2={10, 10.3} (μ₂=10.15).
>
...
1. Chọn centroid đầu tiên `μ₁` ngẫu nhiên từ X.
2. Với mỗi điểm `xᵢ`, tính `D(xᵢ) = min_{j đã chọn} ‖xᵢ − μⱼ‖₂²`.
3. Chọn centroid tiếp theo theo xác suất tỷ lệ `D(xᵢ)²` (điểm xa centroid hiện tại được chọn với xác suất cao hơn).
4. Lặp bước 2–3 cho đến khi đủ k centroids.

**Walk-through số** (3 điểm: A=0, B=5, C=10, k=2):

Bước 1: chọn `μ₁ = A = 0` (ngẫu nhiên).
Bước 2: D(A)=0, D(B)=5²=25, D(C)=10²=100.
Tổng = 125. Xác suất: P(A)=0, P(B)=25/125=0.2, P(C)=100/125=0.8.
→ Rất có khả năng chọn C=10 làm `μ₂` — đúng nhất!
...
>
> Không — vẫn là local minimum. Nhưng **expectation** của inertia bị chặn trên bởi `O(log k) × optimal`. Tức là về lý thuyết, K-means++ không tệ hơn optimal quá log k lần (kỳ vọng).
...
> 📝 **Tóm tắt mục 1–4**: K-means đơn giản, nhanh (O(nkd·iter)), hoạt động tốt với cluster spherical, kích thước đều. K-means++ cải thiện init. Dùng Elbow + Silhouette để chọn k. Thất bại với non-convex shape, outlier, unequal cluster size.
...
|---------|-----------|-----------|
| **Single** (min) | `min_{a∈A, b∈B} d(a,b)` | Sensitive to outlier, tạo "chain" dài |
| **Complete** (max) | `max_{a∈A, b∈B} d(a,b)` | Compact cluster, nhạy với outlier |
| **Average** | `(1/|A||B|) Σ d(a,b)` | Cân bằng, ít nhạy outlier |
...
>
> - **Hierarchical tốt hơn**: khi muốn xem toàn bộ cấu trúc lồng nhau (taxonomy), data nhỏ (n < 10.000, vì O(n²) memory/time), muốn thử nhiều k mà không train lại.
...
| Time complexity | O(nkd·iter) | O(n² log n) |
...
| Data lớn (n>10⁶) | Khả thi | Không (O(n²) memory) |

> 📝 **Tóm tắt mục 5–6**: Hierarchical agglomerative: mỗi điểm là 1 cluster, merge cặp gần nhất. Dendrogram cho cái nhìn toàn cục; cắt theo height → ra k clusters tùy chọn. Ward linkage thường tốt nhất. Chi phí O(n²) — chỉ thực tế với n nhỏ.

---

## 7. Bài Tập

**Bài 1**: Data 1D: `{2, 4, 10, 12, 20, 22}`. Chạy K-means với k=3, khởi tạo `μ₁=2, μ₂=10, μ₃=20`. Bao nhiêu iteration đến hội tụ? Inertia cuối?
...
**Bài 4**: Cho data 2D sau với k=2: `P1=(0,0), P2=(1,0), P3=(5,0), P4=(6,0)`. Khởi tạo `μ₁=P1, μ₂=P3`. Sau bao nhiêu iteration hội tụ? Vì sao K-means++ có thể chọn init khác?
