...
// Source: Networking/01-Foundations-LowerLayers/lesson-02-link-ethernet/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 02 — Tầng liên kết & Ethernet
...
> 💡 **Tầng link nghe "phần cứng", nhưng MTU, VLAN, MAC ảnh hưởng trực tiếp bug mạng mà dev backend/devops gặp thật.**

| Khái niệm | Gặp ở đâu trong công việc |
|-----------|---------------------------|
| **MTU / jumbo frames** | Bug "connection hang" do MTU mismatch (VPN/overlay network K8s) |
| **VLAN** | Phân tách mạng trong datacenter/cloud (tách traffic prod/dev) |
| **MAC address** | DHCP reservation, license binding, ARP-based security |
| **Switch vs hub** | Vì sao mạng hiện đại không "nghe lén" được mọi gói (cần port mirror) |

### 6.1. Ví dụ cụ thể — MTU mismatch, bug mạng ẩn kinh điển

Service trong Kubernetes (overlay network như VXLAN) gửi packet lớn → bị "hang" hoặc chậm bất thường. Nguyên nhân: overlay thêm header → MTU thực < 1500, packet đúng kích thước Ethernet chuẩn lại **quá lớn** sau khi bọc → bị fragment hoặc drop (nếu DF bit set). Triệu chứng: kết nối nhỏ OK, transfer lớn treo. Đây là bug devops thật hay gặp với CNI plugin, VPN, tunnel. Hiểu MTU (§frame) → biết chỉnh MTU interface hoặc bật MSS clamping. Không hiểu → debug hàng giờ "tại sao curl nhỏ chạy, upload file treo".

> ❓ **"Dev app có cần quan tâm tầng link không?"** Phần lớn thời gian không (TCP/IP che hết). Nhưng khi gặp: connection hang theo kích thước (MTU), latency lạ trong cluster (VLAN/switch), hoặc thiết kế network cloud (subnet/MTU của VPC) → kiến thức này cứu hàng giờ debug. Đây là "biết để nhận ra khi cần".

### 6.2. 📝 Tóm tắt mục 6

- Tầng link ảnh hưởng dev/devops qua: **MTU** (bug hang theo size, overlay K8s/VPN), **VLAN** (tách mạng), **MAC** (DHCP/security).
- MTU mismatch = bug ẩn kinh điển: kết nối nhỏ OK, transfer lớn treo → chỉnh MTU/MSS clamping.
- Dev app thường không cần, nhưng "biết để nhận ra" khi gặp bug mạng theo kích thước/cluster.
