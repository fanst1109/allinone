...
// Source: Statistics/02-Inferential/lesson-07-resampling/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 07: Bootstrap & Permutation Test
...
**Tại sao "with replacement"?**: Nếu lấy mẫu không hoàn lại (without replacement), mỗi bootstrap sample = một hoán vị của mẫu gốc → tất cả x̄* đều bằng nhau → không có variance. Phải WITH replacement để bootstrap sample khác nhau.
...
Bootstrap medians: {7, 5, 5, 7, 5, 7}. Mean = 6. Std ≈ 1.09.
...
- Bootstrap distribution của mean: mean ≈ 74.6, SD ≈ 15.8
- 95% bootstrap CI cho mean: **[46.1, 108.4]** (rộng vì outlier 200)
- 95% bootstrap CI cho median: **[46.5, 71.0]** (hẹp hơn vì median robust hơn)

**So sánh với t-CI cho mean**:
- s = 52.3, SE = 52.3/√10 ≈ 16.5
- t*(9, 95%) = 2.262
- t-CI: 74.7 ± 2.262 × 16.5 = **[37.4, 112.0]**
...
**Dữ liệu nhỏ**: A = {5, 8, 7} (n₁=3), B = {3, 4, 6} (n₂=3).

D_obs = (5+8+7)/3 - (3+4+6)/3 = 20/3 - 13/3 = **7/3 ≈ 2.33**
...
P(|D*| ≥ 2.33) = số permutation có |D*| ≥ 2.33 / 20.

Liệt kê đầy đủ: các D* là {-3.0, -2.33, -2.33, -2.0, -1.67, -1.33, -1.0, -0.67, -0.33, 0, 0, 0.33, 0.67, 1.0, 1.33, 1.67, 2.0, 2.33, 2.33, 3.0}.

Số |D*| ≥ 2.33: {-3.0, -2.33, -2.33, 2.33, 2.33, 3.0} = **6 permutations**.

p-value = 6/20 = **0.30**. Với α=0.05 → Fail to reject H₀.

**Nhận xét**: n nhỏ (3 mỗi nhóm) → p-value tối thiểu đạt được là 1/20 = 0.05 → gần như không thể reject với α=0.05.
...
- Phân phối D* ~ N(0, ~3.3) do CLT.
- P(|D*| ≥ 5) ≈ P(|Z| ≥ 5/3.3) = P(|Z| ≥ 1.515) ≈ **0.13**.

So với Welch's t-test: p ≈ 0.13 → giống nhau! (Khi assumptions t-test gần đúng, hai test cho kết quả tương tự.)
...
4. Permutation test exact chỉ khả thi khi n nhỏ (C(n₁+n₂,n₁) nhỏ); n lớn dùng approximate (random shuffle B lần).
...
3. Hai nhóm: A={10, 12, 11}, B={7, 9, 8}. Liệt kê tất cả C(6,3)=20 permutations, tính D* cho từng cái, và tính exact p-value (two-sided) cho D_obs = mean(A)-mean(B).
...
- B1: {5, 5, 3, 2, 8} → mean = 4.6
- B2: {2, 4, 4, 3, 8} → mean = 4.2
- B3: {8, 5, 5, 5, 2} → mean = 5.0
- B4: {3, 3, 8, 4, 2} → mean = 4.0

Mean gốc = (2+5+3+8+4)/5 = **4.4**.

Độ biến thiên của 4 mean {4.6, 4.2, 5.0, 4.0} phản ánh **uncertainty của x̄** — đây là xấp xỉ thực nghiệm của sampling distribution của x̄. Nếu lấy đủ B=1000, SD của {mean bootstrap} ≈ SE = s/√n.

### Bài 2

**Median mẫu gốc**: Sort {8,9,9,10,11,12,14,15} → median = (10+11)/2 = 10.5.
...
**Vì sao không dùng t-CI cho median**: T-CI dựa trên phân phối của x̄, không phải median. Không có công thức closed-form cho SE của median trừ khi biết pdf tổng thể (SE_median ≈ 1/(2×f(m)×√n) — cần ước lượng f(m) là mật độ tại median). Bootstrap tránh được vấn đề này.

### Bài 3

A={10,12,11}, B={7,9,8}. D_obs = (10+12+11)/3 - (7+9+8)/3 = 11 - 8 = **3.0**.
...
|D*| ≥ 3.0: {-3.0, +3.0} → **2 permutations**.

p-value (two-sided) = 2/20 = **0.10**. Với α=0.05 → Fail to reject H₀.
