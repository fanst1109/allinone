...
// Source: Databases/02-Intermediate/lesson-02-index/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 02 — Index (chỉ mục)
...
- Hiểu vì sao truy vấn không có index phải **quét toàn bảng (full table scan)** với chi phí $O(n)$, và **index (chỉ mục)** giúp tìm xuống còn $\\sim O(\\log n)$.
...
| Số bước (1.000.000 dòng) | tới **1.000.000** so sánh | $\\log_2(1{.}000{.}000) \\approx \\mathbf{20}$ bước |
| Vì sao | Không có cách "nhảy thẳng" | Mỗi bước cắt đôi (hoặc hơn) không gian tìm kiếm |

Cụ thể: $2^{20} = 1{.}048{.}576 > 1{.}000{.}000$, nên cần khoảng **20** bước nếu cây phân nhánh đôi. Nhanh hơn $\\approx 1{.}000{.}000 / 20 = \\mathbf{50{.}000}$ lần. (Index thật phân nhánh hàng trăm nhánh mỗi tầng nên còn ít tầng hơn nữa — xem mục 2.)
...
📝 **Tóm tắt mục 1.** Không index → full table scan $O(n)$. Index → $\\sim O(\\log n)$. Với 1 triệu dòng là chênh lệch ~20 bước vs 1 triệu bước. Nhưng index chỉ có lợi với truy vấn chọn lọc.
...
- 1 tầng: tới 100 khóa.
- 2 tầng: tới $100 \\times 101 \\approx 10{.}100$ khóa.
- 3 tầng: $\\approx 1{.}020{.}000$ khóa.
...
2. $\\approx 100^4 = 100$ triệu khóa, tra chỉ cần 4 lần đọc page.
</details>
...
**(a) Là gì.** Hash index lưu các khóa qua một **hàm băm (hash function)** vào **bảng băm (hash table)** — xem [DataStructures — Hash Table](../../../DataStructures/01-Basic/lesson-06-hash-table/). Tra cứu \`key = giá trị\` → tính hash → nhảy thẳng tới bucket → $\\sim O(1)$.
...
> 💡 **Index là tối ưu hiệu năng DB #1 — và là nguyên nhân #1 khiến query chậm khi thiếu.** Mỗi kỹ sư backend phải biết đọc \`EXPLAIN\` và đặt index đúng.

| Tình huống thật | Index làm gì |
|-----------------|--------------|
| **\`WHERE email = ?\` (login)** | Index trên \`email\` → tra $O(\\log n)$ thay vì full scan $O(n)$ |
| **\`ORDER BY created_at\` + phân trang** | Index trên \`created_at\` → khỏi sort, đọc tuần tự |
| **\`JOIN\` trên foreign key** | Index trên FK → join nhanh (DB cần index FK, không tự tạo) |
| **\`WHERE status=? AND user_id=?\`** | Composite index \`(user_id, status)\` — đúng thứ tự cột |
| **Tìm kiếm text** | Full-text index / GIN (Postgres), không phải B-tree thường |

### 7.1. Ví dụ cụ thể — đọc EXPLAIN để thấy index làm việc

Bảng \`users\` 10 triệu dòng, query \`WHERE email = 'a@b.com'\`:

\`\`\`sql
EXPLAIN SELECT * FROM users WHERE email = 'a@b.com';
-- KHÔNG index:  Seq Scan, rows=10000000   ← quét cả bảng, ~vài giây
CREATE INDEX idx_email ON users(email);
-- CÓ index:     Index Scan, rows=1         ← nhảy thẳng tới dòng, ~mili giây
\`\`\`

Index là **B+ tree** (cùng nguyên lý [balanced trees](../../../DataStructures/02-Intermediate/lesson-04-balanced-trees/)): cao 3-4 tầng cho tỉ dòng → 3-4 lần đọc đĩa. Đây là lý do "app chậm dần khi data lớn" thường chỉ cần thêm một index.

> ⚠ **Index không miễn phí — đừng index tất cả.** Mỗi index: (1) **tốn dung lượng đĩa**, (2) **làm chậm INSERT/UPDATE/DELETE** (phải cập nhật cả index). Bảng ghi nhiều, index ít cột thật cần. Quy tắc: index cột xuất hiện trong \`WHERE\`/\`JOIN\`/\`ORDER BY\` thường xuyên; **composite index** đặt cột lọc bằng (\`=\`) trước cột lọc khoảng (\`>\`, \`<\`). Cột ít giá trị phân biệt (vd \`gender\`) index ít lợi.

### 7.2. 📝 Tóm tắt mục 7

- Index = tối ưu DB #1: \`WHERE\`/\`JOIN\`/\`ORDER BY\` từ $O(n)$ full scan xuống $O(\\log n)$ (B+ tree).
- Đọc **\`EXPLAIN\`**: \`Seq Scan\` = chưa dùng index, \`Index Scan\` = đang đi cây.
- Đánh đổi: tốn đĩa + chậm ghi → chỉ index cột thật cần; composite đặt cột \`=\` trước cột khoảng.
...
4. **Ước lượng số bước.** Bảng \`events\` có 64.000.000 dòng. (a) Full table scan mất bao nhiêu bước? (b) Với B-tree, ước lượng theo $\\log_2$. (c) Nếu B+tree fanout 256, cây cần khoảng mấy tầng (tức mấy lần đọc page)?
...
- **(b)** $\\log_2(64{.}000{.}000)$: vì $2^{26} = 67{.}108{.}864 > 64{.}000{.}000$, nên $\\approx \\mathbf{26}$ bước. Nhanh hơn $\\approx 64{.}000{.}000 / 26 \\approx \\mathbf{2{,}46}$ triệu lần.
- **(c)** Fanout 256: $256^2 = 65{.}536$; $256^3 = 16{.}777{.}216$; $256^4 = 4{,}29$ tỷ $> 64$ triệu → **3–4 tầng** là đủ (3 tầng phủ 16,7 triệu < 64 triệu nên cần **4 tầng**). Tức chỉ **~4 lần đọc page** cho 64 triệu dòng — minh họa rõ vì sao fanout cao thắng $\\log_2$.
