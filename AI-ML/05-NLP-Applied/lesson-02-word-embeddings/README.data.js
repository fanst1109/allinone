...
// Source: AI-ML/05-NLP-Applied/lesson-02-word-embeddings/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 06 — Word Embedding: Từ thưa sang dày, từ ký tự sang nghĩa
...
- Thực hiện **vector arithmetic** trên embedding: \`king − man + woman ≈ queen\` và lý giải vì sao có kết quả này.
...
Đây là cách đơn giản nhất để hiện thực hoá distributional hypothesis: với vocab |V|, lập ma trận \`X ∈ ℝ^{|V|×|V|}\` trong đó \`X[i][j]\` = số lần từ \`j\` xuất hiện trong window của từ \`i\`.
...
**Embedding** của một từ \`w\` là một vector dày \`e_w ∈ ℝ^d\` với:
...
Tập hợp tất cả embedding tạo thành **ma trận embedding** \`E ∈ ℝ^{|V| × d}\`. Hàng \`i\` của \`E\` = embedding của từ thứ \`i\` trong vocab.
...
1. \`king · queen = 0.6·0.6 + 0.7·0.7 + 0.4·(−0.4) = 0.36 + 0.49 − 0.16 = 0.69\`. Norm = \`√(0.36+0.49+0.16) = √1.01 ≈ 1.005\`. Cos = \`0.69 / (1.005·1.005) ≈ 0.683\`.
2. \`king · man = 0.6·0.5 + 0.7·0.1 + 0.4·0.4 = 0.30+0.07+0.16 = 0.53\`. \`‖king‖ ≈ 1.005\`, \`‖man‖ = √(0.25+0.01+0.16) = √0.42 ≈ 0.648\`. Cos ≈ \`0.53/(1.005·0.648) ≈ 0.814\`.
3. \`king · woman = 0.6·0.5 + 0.7·0.1 + 0.4·(−0.4) = 0.30+0.07−0.16 = 0.21\`. Cos ≈ \`0.21/(1.005·0.648) ≈ 0.323\`.
4. \`man · woman = 0.5·0.5 + 0.1·0.1 + 0.4·(−0.4) = 0.25+0.01−0.16 = 0.10\`. Cos ≈ \`0.10/(0.648·0.648) ≈ 0.238\`.
...
> 📝 **Tóm tắt mục 3.**
> - Embedding = vector dày \`e_w ∈ ℝ^d\`.
> - Toàn bộ vocab gói gọn trong ma trận \`E ∈ ℝ^{|V|×d}\`.
...
- \`h = W^T x\` đơn giản lấy ra **hàng \`i\` của W** (vì các ô khác trong x đều = 0). Đây là embedding center của từ \`i\`.
- \`y_j = softmax(W'^T h)_j = exp(w'_j · h) / Σ_k exp(w'_k · h)\`. Đây là xác suất từ \`j\` xuất hiện trong context của từ \`i\`.
...
>
> Đây là điểm rất tinh tế. Mục tiêu của Word2Vec **không phải dự đoán cực kỳ chính xác** — mà là **tạo ra embedding tốt**. Nếu thêm phi tuyến, mạng "nhớ" được nhiều luật phức tạp, nhưng embedding \`h = W^T x\` không còn là biểu diễn tuyến tính đẹp. Loại bỏ phi tuyến ⟹ embedding chính là hàm tuyến tính của input ⟹ vector arithmetic (mục 7) hoạt động.

### 5.4 Negative sampling — vấn đề tốc độ

> ⚠ **Vấn đề lớn của softmax.** Mẫu số \`Σ_k exp(w'_k · h)\` chạy qua **toàn bộ vocabulary** (|V| = 100 000+). Mỗi training step phải tính 100 000 dot product. Trên 1 tỷ cặp training → 10^14 dot product. Không khả thi.
...
Trong đó \`σ(x) = 1/(1 + e^{-x})\` là sigmoid (đã học ở Lesson 03).
...
Loss = \`-log(0.634) - log(0.475) - log(0.488) = 0.456 + 0.744 + 0.717 = 1.917\`. Gradient sẽ ép \`σ(w'_man · h_king)\` lên cao hơn, và \`σ(w'_laptop · h_king), σ(w'_pizza · h_king)\` xuống thấp hơn.

> ❓ **Câu hỏi tự nhiên.** *"Sampling negative ngẫu nhiên đều có công bằng không? Từ phổ biến (the, a) bị chọn quá nhiều?"*
>
> Đúng vậy — đó là vấn đề. Mikolov dùng **unigram distribution mũ 3/4**: \`P(w) ∝ count(w)^0.75\`. Mũ 0.75 nén bớt frequency cao và lift up frequency thấp, cân bằng training tốt hơn so với đều hoặc theo unigram thường.
...
- \`vec(king) − vec(man) ≈ vec(queen) − vec(woman)\` (offset "royalty")
- \`vec(Paris) − vec(France) ≈ vec(Rome) − vec(Italy)\` (offset "capital_of")
- \`vec(walking) − vec(walked) ≈ vec(running) − vec(ran)\` (offset "tense")

Suy ra: \`vec(king) − vec(man) + vec(woman) ≈ vec(queen)\`.
...
**Cách 1 — về cấu trúc loss.** Skip-gram học \`w'_o · w_c ≈ log P(o | c)\`. Levy & Goldberg (2014) chứng minh điều này tương đương với **factorization của ma trận PMI** (Pointwise Mutual Information). Ma trận PMI có cấu trúc tuyến tính, do đó embedding cũng có cấu trúc tuyến tính → hỗ trợ arithmetic.
...
> 2. \`Tokyo − Japan + Korea = ?\`. Vector offset "Tokyo − Japan" = vector "is_capital_of_inverse". Cộng vào Korea → "capital of Korea" = **Seoul**.
> </details>
...
Mục tiêu GloVe: tìm \`w_i, w̃_j ∈ ℝ^d\` sao cho:
...
GloVe sẽ học để dot product \`w_ice · w̃_water = 11.82 − b_ice − b̃_water\`. Sau training, \`w_ice\` và \`w_steam\` sẽ có offset rõ ràng (steam ↔ ice ≈ vapor ↔ solid).
...
>
> SVD phân tích \`X = UΣV^T\` cho phép giảm chiều: lấy top-d singular vector → embedding d chiều. GloVe cũng tìm \`w_i · w̃_j ≈ log X_{ij}\` — bản chất là **weighted matrix factorization** của ma trận log X (với weight \`f(X_{ij})\`).
>
...
Đánh giá: tính \`vec(b) − vec(a) + vec(c)\`, tìm nearest neighbor (trừ a, b, c), so với đáp án.
...
3. Minimize KL divergence \`KL(P ‖ Q)\` bằng gradient descent trên \`y_i\`.
...
Công thức: với mỗi điểm \`i\`, tìm \`σ_i\` sao cho **entropy** của phân phối \`p_{·|i}\` bằng \`log(perplexity)\`.

Ví dụ với perplexity = 5, entropy mong muốn = \`log(5) ≈ 1.609\`. Khi đó với mỗi điểm có "ảnh hưởng đáng kể" của khoảng 5 điểm xung quanh.
...
- \`vec(king) · g − vec(queen) · g ≈ −1.5\` (queen "female hơn" king).
- \`vec(woman) · g − vec(man) · g ≈ −1.5\`.
- \`vec(doctor) · g ≈ +0.3\` (lệch về male trong corpus 2013) → embedding **encode bias xã hội**!
...
Với static embedding, \`vec(bank)\` ở câu A và câu B **giống hệt nhau**. Không có cách nào phân biệt nghĩa từ embedding.
...
Bài này tập trung **Static dense** (Word2Vec/GloVe) vì nó là foundation. Hiểu được tại sao \`king − man + woman ≈ queen\` là chìa khoá hiểu mọi cái đến sau.
...
- **CLIP** (OpenAI 2021): ảnh + caption cùng không gian 512 chiều. \`cos(image_emb, text_emb)\` cao nếu ảnh khớp caption.
...
Tương tự \`sent(S2) ≈ (0.40, 0.55, 0.50)\` (giả sử pasta gần pizza, enjoy gần love).

Cosine \`cos(S1, S2) ≈ 0.98\` — cao, nhưng cũng cao tương đương cho 2 câu "I love eating pizza" và "I love programming pizza" (vô nghĩa) vì cũng dùng cùng từ.

**Cách 2 — Sentence-BERT.** Encode trực tiếp (768D). \`cos(S1, S2) ≈ 0.85\` — vẫn cao, nhưng cho cặp vô nghĩa \`cos ≈ 0.6\`. SBERT phân biệt được semantic, không chỉ overlap từ vựng.
...
(a) Score \`w'_i · h_A\` cho mỗi \`i\`.
(b) Softmax \`P(i | A)\` cho mỗi \`i\`.
...
(a) Tính \`Paris − France + Germany\`. So với Berlin (cosine).
(b) Tính \`Tokyo − Japan + France\`. Đáp án mong đợi là gì?
...
Cho \`X[ice][solid] = 1.9 × 10^4\`, \`X[steam][solid] = 2.2 × 10^2\`. Giả sử embedding hiện tại:
...
(a) Tính \`log X[ice][solid]\` và \`log X[steam][solid]\`.
(b) Tính dự đoán \`w_ice · w̃_solid + b_ice + b̃_solid\`.
(c) Tính weight \`f(X[ice][solid])\` (vì \`X > x_max\`, f = 1).
(d) Tính error² và loss.
...
(c) Loss = \`-log P(B | A) = -log(0.506) = 0.681\`.

### Lời giải 3

(a) \`Paris − France + Germany\`:
...
- Output: dot product \`h\` với mỗi hàng \`W'[j]\`, j = 1..|V|. Mỗi dot = \`d\` phép nhân. Tổng = \`|V|·d = 100 000 × 300 = 3·10^7\` phép nhân.

(Có thêm exp + sum cho softmax nhưng đó là O(|V|), bỏ qua.)

**Negative sampling k=5.** Mỗi step:

- 1 cặp dương + 5 cặp âm = 6 dot product \`d\`. Tổng = \`6·d = 1800\` phép nhân.

**Tỉ số tăng tốc** = \`3·10^7 / 1800 ≈ 16 667 lần\`. ✓

### Lời giải 5

(a) \`log X[ice][solid] = log(1.9·10^4) = log(19000) ≈ 9.852\`.

\`log X[steam][solid] = log(220) ≈ 5.394\`.

(b) Dự đoán \`w_ice · w̃_solid + b_ice + b̃_solid\`:
...
(c) Weight: \`X[ice][solid] = 19000 > x_max=100\` → f = 1.

(d) Error = \`1.25 − 9.852 = −8.602\`. Error² = \`73.99\`. Loss = \`1 · 73.99 = 73.99\`.
