# Lesson 01 — Game Loop & Timestep (vòng lặp game & bước thời gian)
...
- [Math — Tích phân xác định](../../../Math/04-Calculus-1var/lesson-07-definite-integral/) — (tùy chọn) cộng dồn các bước nhỏ `v·Δt` để ra quãng đường chính là **tổng Riemann** của vận tốc; cập nhật game = tích phân số (numerical integration). Sẽ đào sâu ở Lesson 03.
...
- Máy A chạy **30 fps** → 30 frame/giây → `x` tăng `30 × 2 = 60` px mỗi giây.
- Máy B chạy **60 fps** → 60 frame/giây → `x` tăng `60 × 2 = 120` px mỗi giây.
- Máy C chạy **144 fps** → `144 × 2 = 288` px mỗi giây.
...
Muốn "100 px mỗi **giây**" thật sự, ta đo thời gian trôi qua từ frame trước (`Δt`, đơn vị giây) rồi cộng `v · Δt`:
...
- 30 fps: `dt ≈ 1/30 ≈ 0.0333` s → mỗi frame `+100 × 0.0333 ≈ 3.33` px, một giây có 30 frame → `30 × 3.33 ≈ 100` px. ✓
- 60 fps: `dt ≈ 1/60 ≈ 0.0167` s → mỗi frame `+100 × 0.0167 ≈ 1.67` px, `60 × 1.67 ≈ 100` px. ✓
- 144 fps: `dt ≈ 1/144 ≈ 0.00694` s → mỗi frame `+0.694` px, `144 × 0.694 ≈ 100` px. ✓
...
> - **Cộng `v·Δt`** → tốc độ tính theo giây → mọi máy cùng tốc độ. Đây là ý tưởng cốt lõi của cả bài.
...
Một viên bi: vị trí `x`, vận tốc `v = 200` px/s, máy chạy 60 fps (`Δt = 1/60 ≈ 0.0167` s). Theo dõi 4 frame đầu (`x` khởi đầu = 0):
...
Sau 1 giây (60 frame): `x ≈ 60 × 3.33 ≈ 200` px ✓ — đúng `v · t = 200 × 1`.
...
Ký hiệu Δ (delta) trong toán/vật lý nghĩa là "sự thay đổi / hiệu". `Δt` = hiệu thời gian. (Cùng ký hiệu bạn gặp trong [động học](../../../Physics/01-Mechanics/lesson-01-kinematics/): vận tốc trung bình $= \Delta x / \Delta t$.)
...
Vật có `v = 100` px/s. Ta theo dõi quãng đường đi trong **đúng 1 giây** ở các fps khác nhau. Mỗi frame cộng `v · Δt`.

**Ví dụ 1 — 60 fps.** `Δt = 1/60 ≈ 0.01667` s.
- Mỗi frame: `100 × 0.01667 = 1.667` px.
- 60 frame trong 1 giây: `60 × 1.667 = 100.0` px. ✓

**Ví dụ 2 — 30 fps.** `Δt = 1/30 ≈ 0.03333` s.
- Mỗi frame: `100 × 0.03333 = 3.333` px (gấp đôi frame ở 60 fps, vì mỗi frame kéo dài gấp đôi).
- 30 frame trong 1 giây: `30 × 3.333 = 100.0` px. ✓

**Ví dụ 3 — 120 fps.** `Δt = 1/120 ≈ 0.008333` s.
- Mỗi frame: `100 × 0.008333 = 0.8333` px (nửa so với 60 fps).
- 120 frame: `120 × 0.8333 = 100.0` px. ✓

**Ví dụ 4 — 15 fps (máy yếu).** `Δt = 1/15 ≈ 0.06667` s.
- Mỗi frame: `100 × 0.06667 = 6.667` px (bước to vì frame thưa).
- 15 frame: `15 × 6.667 = 100.0` px. ✓

**Ví dụ 5 — fps dao động trong 1 giây.** Giả sử 1 giây gồm 3 frame có Δt lần lượt `0.5 s`, `0.3 s`, `0.2 s` (tổng = 1 s).
- Frame 1: `100 × 0.5 = 50` px → tổng 50.
- Frame 2: `100 × 0.3 = 30` px → tổng 80.
- Frame 3: `100 × 0.2 = 20` px → tổng 100. ✓

Dù fps lên xuống thất thường, **tổng quãng đường vẫn = `v × (tổng Δt) = 100 × 1 = 100` px**. Đây chính là vì cộng `v·Δt` thực chất đang **cộng dồn (tích phân số)** vận tốc theo thời gian.

> 💡 **Trực giác — liên hệ tích phân.** Quãng đường = "diện tích dưới đường vận tốc". Mỗi `v·Δt` là một **cột chữ nhật** rộng `Δt`, cao `v`. Cộng các cột lại = xấp xỉ diện tích = quãng đường. fps cao → nhiều cột mảnh; fps thấp → ít cột mập — nhưng tổng diện tích như nhau khi v hằng số. Đây đúng là **tổng Riemann** từ [Math — Tích phân xác định](../../../Math/04-Calculus-1var/lesson-07-definite-integral/). Sẽ đào sâu (Euler vs Verlet) ở **Lesson 03**.
...
> ❓ **Câu hỏi tự nhiên của người đọc.**
> - **"Δt có bao giờ to bất thường không?"** — Có. Khi tab bị ẩn rồi quay lại, hoặc máy lag, Δt có thể nhảy lên cả giây. Một bước `v·Δt` khổng lồ → vật "dịch chuyển tức thời" xuyên tường. Cách chữa: **kẹp Δt** (`dt = Math.min(dt, 0.1)`) hoặc dùng **fixed timestep** (§4).
...
> <details><summary>Đáp án</summary>
>
> 1. `Δt = 1/50 = 0.02` s → mỗi frame `250 × 0.02 = 5` px. 50 frame/giây → `50 × 5 = 250` px = `v × 1 s`. ✓
> 2. Vì tổng quãng đường = `v × Σ Δt = v × (thời gian thật trôi qua)`. Σ Δt trong 1 giây luôn = 1 giây bất kể chia thành 30 hay 60 mảnh. fps chỉ đổi *kích thước* mỗi bước, không đổi *tổng*.
> </details>

> 📝 **Tóm tắt mục 3.**
> - Δt = thời gian thực giữa 2 frame (giây). Đo bằng hiệu timestamp.
> - Mọi thay đổi theo thời gian = `(tốc độ/giây) × Δt`.
> - Cộng `v·Δt` = tích phân số → tổng quãng đường độc lập fps.
...
**(c) Ví dụ số — vì sao Δt to làm hỏng:** Lò xo kéo vật về gốc với gia tốc `a = -k·x` (`k = 50`). Vật bắt đầu ở `x = 1`, `v = 0`. Tích phân Euler `v += a·dt; x += v·dt`.

- **Δt nhỏ = 0.01 s:** mỗi bước `x` nhích nhẹ về 0, dao động đẹp quanh gốc (biên độ ổn định ≈ 1).
- **Δt to = 0.1 s:** bước 1: `a = -50×1 = -50`, `v = 0 + (-50)(0.1) = -5`, `x = 1 + (-5)(0.1) = 0.5`. Bước 2: `a = -50×0.5 = -25`, `v = -5 + (-25)(0.1) = -7.5`, `x = 0.5 + (-7.5)(0.1) = -0.25`. Bước 3: `a = -50×(-0.25)=12.5`, `v = -7.5+1.25 = -6.25`, `x = -0.25 + (-0.625) = -0.875` → đã vượt biên độ ban đầu! Biên độ **phình to** mỗi chu kỳ → vật "nổ tung". ✗

→ Δt to + vật lý cứng = **bùng nổ số học**. Đây là lý do cần fixed timestep.

### 4.2. Fixed timestep — bước cố định + accumulator

**(a) Là gì:** Vật lý luôn cập nhật với **một Δt cố định** (vd `FIXED = 1/60 ≈ 0.01667` s) bất kể frame thật dài bao lâu. Để khớp với frame thật (dài/ngắn thất thường), dùng một **accumulator (bộ tích lũy)**: dồn thời gian thật vào accumulator, rồi "tiêu" nó thành từng bước cố định.
...
**(c) Walk-through accumulator bằng số:** `FIXED = 1/60 ≈ 0.01667` s. Theo dõi qua các frame có Δt thật khác nhau:
...
**Ví dụ số:** vật bước từ `x_cũ = 100` tới `x_mới = 110` mỗi bước vật lý. Một frame render có `acc = 0.5 × FIXED` → `α = 0.5` → vẽ ở `100 + (110−100)×0.5 = 105`. Frame khác `α = 0.25` → vẽ ở `102.5`. Hình di chuyển *liên tục* dù vật lý chỉ nhảy nấc 100 → 110 → 120. (Cần lưu cả `x_cũ` và `x_mới`.) Đây là **linear interpolation (lerp)** — sẽ gặp lại nhiều ở các bài chuyển động/animation sau.

> 📝 **Tóm tắt mục 5.**
> - Tách update (logic/vật lý, nhịp cố định) khỏi render (hình ảnh, nhịp màn hình).
> - Phần dư accumulator → tỉ lệ `α = acc/FIXED` → nội suy `lerp(x_cũ, x_mới, α)` cho hình mượt giữa hai bước vật lý.
...
**Ví dụ:** `Δt = 0.0167` s → `1/0.0167 ≈ 60` fps. `Δt = 0.0333` → 30 fps. `Δt = 0.00694` → 144 fps. `Δt = 0.05` → 20 fps.
...
- **Δt là hằng số xuyên suốt GameDev.** Mọi cập nhật sau này đều dạng `đại lượng += tốc độ * dt`:
  ...
  - **Lesson 04 — Lực & gia tốc (Forces):** Newton `F = m·a` → `a = F/m` → `v += a·dt`. Trọng lực, lò xo, ma sát đều cộng vào `a` rồi nhân Δt.
...
**Bài 3.** (Accumulator) Cho `FIXED = 1/60 ≈ 0.01667` s, accumulator bắt đầu = 0. Các frame có Δt thật lần lượt: `0.030, 0.005, 0.040, 0.010`. Với mỗi frame, tính số bước fixed chạy và accumulator còn dư sau frame.
...
**Bài 6.** (Interpolation) Vật lý bước từ `x_cũ = 50` tới `x_mới = 62` mỗi bước FIXED. Một frame render có accumulator dư `acc = 0.012` s (FIXED = 0.0167 s). Tính `α` và vị trí vẽ nội suy `x_vẽ`.
...
`v = 300` px/s. Mỗi frame = `v · Δt = 300/fps`. Tổng/giây = `fps × (300/fps) = 300` px luôn.
...
`x += 4` mỗi frame → tốc độ = `4 × fps` px/s.
- 60 fps: `4 × 60 = 240` px/s (đề bài).
- 144 fps: `4 × 144 = **576** px/s` — nhanh gấp 2.4 lần. Đây là bug frame-rate dependence.
...
```
Kiểm tra ở 144 fps: `dt = 1/144`, mỗi frame `240/144 ≈ 1.667` px, `144 × 1.667 = 240` px/s ✓. Mọi máy giữ 240 px/s.
...
Tổng bước = `1+1+2+1 = 5`; `5 × 0.016667 = 0.08333` s ≈ tổng Δt `0.030+0.005+0.040+0.010 = 0.085` s trừ phần dư cuối `0.001667`. ✓

### Bài 4

FIXED đại diện 16.67 ms thời-gian-game, nhưng tính một bước mất 25 ms thời-gian-thật. Một frame thật = 50 ms:
- 50 ms thật → accumulator cần tiêu ~`50/16.67 ≈ 3` bước.
- 3 bước × 25 ms tính = **75 ms** thời gian thật để xử lý xong frame "lẽ ra 50 ms".
- Nhưng trong 75 ms đó, đồng hồ thật đã trôi 75 ms → frame *kế* nhận `75/16.67 ≈ 4-5` bước → cần 100–125 ms tính → frame kế nữa còn nhiều bước hơn...
...
`Δt = 0.0125` s.
- (a) FPS = `1/0.0125 = **80** fps`.
- (b) Budget 60 fps = 16.67 ms. Frame hiện tại tốn 12.5 ms → còn dư `16.67 − 12.5 = **4.17 ms**`.
- (c) Thêm 6 ms → tổng `12.5 + 6 = 18.5 ms > 16.67 ms` → **không** còn đạt 60 fps; fps mới ≈ `1/0.0185 ≈ 54` fps.

### Bài 6

`FIXED = 0.0167` s, `acc = 0.012` s.
- `α = acc / FIXED = 0.012 / 0.0167 ≈ **0.719**`.
- `x_vẽ = x_cũ + (x_mới − x_cũ)·α = 50 + (62 − 50) × 0.719 = 50 + 12 × 0.719 = 50 + 8.63 = **58.63**`.
