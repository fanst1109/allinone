# Lesson 04 — Tính đúng đắn & Loop Invariant
...
Phần 5.3 mới chỉ là partial correctness. Phải chứng minh loop không treo. **Variant: số phần tử trong đoạn `hi − lo + 1`.**

- Mỗi vòng (khi chưa tìm thấy) đều thực hiện `lo = mid+1` hoặc `hi = mid−1`, làm `hi − lo` **giảm chặt** ít nhất 1 (vì `lo ≤ mid ≤ hi`).
- Đại lượng `hi − lo + 1` là số nguyên, **giảm chặt mỗi vòng**, và bị chặn dưới (khi nó về ≤ 0 thì `lo > hi` → thoát).

Một số nguyên giảm chặt và có chặn dưới chỉ giảm được hữu hạn lần → loop dừng sau **tối đa** ~`log₂ n` vòng. ✓
...
> 📝 **Tóm tắt mục 5.** Invariant "target ∈ [lo,hi] nếu tồn tại". 3 phần partial correctness + variant `hi−lo+1` giảm chặt cho termination. Dùng `lo+(hi-lo)/2` tránh tràn.
...
Kiểm nhanh: `n=5` → 0,1,3,6,10,15 → trả 15 = `5·6/2`. ✓ (`n=1`→1; `n=0`→ loop không chạy, trả 0; `n=3`→6.)
...
Để chứng minh mệnh đề `P(n)` đúng với mọi số tự nhiên `n ≥ n₀`:
...
| Base case `P(n₀)` | **Initialization** (invariant đúng trước vòng đầu) |
| Inductive step `P(k) → P(k+1)` | **Maintenance** (`I(i)` đúng → `I(i+1)` đúng) |
| Kết luận `P(n) ∀n` | Invariant đúng ở **mọi** vòng, kể cả lúc thoát → **Termination** |

Nhìn lại insertion sort (mục 3): "Init" chứng minh `I(1)`, "Maintenance" chứng minh `I(i) → I(i+1)`. Theo nguyên lý quy nạp, `I(i)` đúng với mọi `i` từ 1 đến `n` — bao gồm `I(n)` lúc thoát. Đó **chính xác** là quy nạp.
...
1. **Giảm chặt:** mỗi lần lặp, `V` giảm ít nhất 1 (`V_sau < V_trước`).
2. **Chặn dưới:** `V ≥ 0` (hoặc một hằng số nào đó) luôn đúng.
...
|------------|-------------|-------------|-----------|
| Insertion sort (vòng ngoài) | `n − i` | mỗi vòng `i++` | `≥ 0`, dừng khi `=0` |
| Binary search | `hi − lo + 1` | mỗi vòng `lo↑` hoặc `hi↓` | `≥ 0`, thoát khi `<1` |
| Sum / Factorial | `n − i + 1` | mỗi vòng `i++` | `≥ 0` |
| Euclid GCD | `b` (số chia) | xem 8.3 | `≥ 0`, dừng khi `=0` |

### 8.3 Ví dụ — Euclid GCD

Thuật toán Euclid tìm ước chung lớn nhất: `gcd(a,b) = gcd(b, a mod b)`, dừng khi `b = 0`.
...
**Partial correctness — invariant `gcd(a,b)` bất biến.** Nhờ định lý `gcd(a,b) = gcd(b, a mod b)`: mỗi vòng thay `(a,b)` bằng `(b, a mod b)` giữ nguyên gcd. Khi thoát `b=0`, `gcd(a,0)=a` → trả `a` đúng.

**Termination — variant `b`.** Sau gán, `b` mới `= a mod b`, mà `0 ≤ a mod b < b` (định nghĩa phép mod với `b>0`). Vậy `b` **giảm chặt** mỗi vòng và **luôn ≥ 0**. Một số tự nhiên giảm chặt không thể giảm vô hạn → loop dừng. ✓
...
**Mệnh đề `P(n)`:** `Power(x, n) = xⁿ` với mọi `n ≥ 0`.

**Base case `n = 0`.** `Power(x,0)` trả `1 = x⁰`. ✓

**Inductive step.** Giả sử `P(m)` đúng với **mọi** `0 ≤ m < n` (quy nạp mạnh). Vì `n/2 < n` (chia nguyên, `n ≥ 1`), giả thiết cho `half = Power(x, n/2) = x^(n/2)`. Xét hai trường hợp:
- `n` chẵn (`n = 2k`): trả `half·half = x^(n/2)·x^(n/2) = x^(n/2 + n/2) = xⁿ`. ✓
- `n` lẻ (`n = 2k+1`, `n/2 = k`): trả `half·half·x = x^k·x^k·x = x^(2k+1) = xⁿ`. ✓

Vậy `P(n)` đúng. Theo quy nạp mạnh, `Power` đúng `∀ n ≥ 0`. ∎

**Termination của đệ quy.** Variant: kích thước `n`. Mỗi lời gọi giảm `n` về `n/2 < n`, chặn dưới `0` (base case). Số tự nhiên giảm chặt → đệ quy dừng sau ~`log₂ n` lời gọi. ✓

**Verify số:** `Power(2,10)`: `=Power(2,5)²`; `Power(2,5)=Power(2,2)²·2`; `Power(2,2)=Power(2,1)²`; `Power(2,1)=Power(2,0)²·2=1·1·2=2`; `Power(2,2)=2·2=4`; `Power(2,5)=4·4·2=32`; `Power(2,10)=32·32=1024=2¹⁰`. ✓ Thêm: `Power(3,0)=1` ✓; `Power(5,1)=5` ✓; `Power(2,3)=8` ✓.
...
Khi `hi = lo + 1`: `mid = lo + (1)/2 = lo`. Nếu `a[mid] < target`, `lo = mid = lo` → **variant `hi − lo` không giảm** → vòng lặp vô hạn. Đây là minh chứng vì sao **phải chứng minh variant giảm CHẶT** (mục 8): nếu không chứng minh được variant giảm, rất có thể loop treo.
...
**Termination.** Vòng ngoài dừng khi `k = n−1`. Invariant: `a[1..n−1]` là `n−1` phần tử lớn nhất đã sắp; phần còn lại `a[0]` ≤ tất cả chúng → `a[0]` là nhỏ nhất, đặt đúng chỗ. Cả mảng đã sắp xếp. ✓ (Variant vòng ngoài: `n−1−k`, giảm chặt, chặn dưới 0.)
...
**Termination (dừng):** variant `V = hi − lo + 1`, số nguyên ≥ 0, giảm chặt mỗi vòng (vì `lo↑` hoặc `hi↓`, và `lo ≤ mid ≤ hi`) → dừng sau ≤ `⌊log₂ n⌋ + 1` vòng. ✓ Độ phức tạp: **O(log n)** thời gian, **O(1)** bộ nhớ.
...
Xem mục 8.3. **Variant:** giá trị `b`. Sau gán `a, b = b, a%b`, giá trị mới của `b` là `a % b` (b cũ), thỏa `0 ≤ a%b < b` với `b > 0`. Vậy mỗi vòng `b` giảm **chặt** ít nhất 1 và luôn `≥ 0`. Một dãy số tự nhiên giảm chặt là **hữu hạn** (không tồn tại dãy giảm vô hạn trong ℕ — well-ordering principle) → loop chạy hữu hạn vòng → dừng. ∎ Số vòng ≤ O(log min(a,b)) (định lý Lamé). Trường hợp đặc biệt: nếu `b=0` ngay từ đầu, loop bỏ qua, dừng tức thì.
...
- *Base `n=0`:* trả `1 = x⁰`. ✓
- *Step:* giả sử đúng `∀ m < n`. `half = Power(x,n/2) = x^(n/2)` (vì `n/2<n`). `n` chẵn: trả `half² = x^n`; `n` lẻ: trả `half²·x = x^(2⌊n/2⌋+1) = x^n`. ✓
- *Dừng:* variant `n` giảm về `n/2 < n`, chặn dưới 0 (base) → ~log₂n lời gọi, dừng. ✓
...
- **Variant:** `hi − lo`, giảm 2 mỗi vòng, chặn dưới (thoát khi ≤ 0) → dừng sau ⌊n/2⌋ vòng. ✓
