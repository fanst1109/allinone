# Lesson 04 — Bias-Variance & Regularization
...
- Phát biểu được **bias-variance decomposition**: $E[(\hat{y} - y)^2] = \text{Bias}^2 + \text{Variance} + \text{Irreducible noise}$.
...
**Generalization gap** = $\text{val MSE} - \text{train MSE}$. Degree 10: gap = 8.699.
...
**(a) Là gì**: với model $\hat{f}$ học từ tập train ngẫu nhiên và target $y = f(x) + \epsilon$:
...
- **Irreducible noise $\sigma^2$**: nhiễu trong data, không model nào loại bỏ được.
...
**(c) Ví dụ số**: polynomial regression trên hàm $y = \sin(\pi x) + \epsilon, \epsilon \sim N(0, 0{,}1)$, n=20 điểm:

| Degree | Bias² | Variance | Total Expected MSE |
|:---:|:---:|:---:|:---:|
| 1 | 0.42 | 0.01 | $0{,}43 + \sigma^2$ |
| 3 | 0.08 | 0.05 | $0{,}13 + \sigma^2$ |
| 7 | 0.01 | 0.31 | $0{,}32 + \sigma^2$ |
| 10 | 0.001 | 1.42 | $1{,}42 + \sigma^2$ |
...
📝 **Tóm tắt mục 2**:
- Expected MSE = $\text{Bias}^2 + \text{Variance} + \sigma^2_{\text{irreducible}}$.
...
**(a) Là gì**: thêm penalty $\lambda \cdot \|w\|^2$ vào loss:
...
- $\|w\|^2 = 3{,}5^2 + 2{,}1^2 + 4{,}8^2 + 1{,}3^2 = 12{,}25 + 4{,}41 + 23{,}04 + 1{,}69 = 41{,}39$
...
**Nhận xét**: $\lambda$ tăng → $\|w\|^2$ giảm **monotonically**. Không có coefficient nào = 0 chính xác — Ridge **shrink** về 0 nhưng không bằng 0.

> ❓ **Câu hỏi**: Tại sao $(X^\top X + \lambda I)$ luôn invertible? — Vì $X^\top X$ positive semi-definite, cộng $\lambda I$ ($\lambda > 0$) → positive definite → luôn invertible. Đây là lý do Ridge cũng dùng để giải quyết **ill-conditioned** systems.

### 3.2. L1 Regularization — Lasso Regression

**(a) Là gì**: penalty là $\lambda \cdot \|w\|_1$ = $\lambda \cdot \sum |w_j|$:
...
$\lambda = 0{,}5$ → $w_2$ và $w_4$ trở thành 0 chính xác → **feature selection tự động**.

> 💡 **Tại sao L1 tạo sparsity còn L2 thì không?**: Hình học!
> - L1 constraint: vùng khả thi $|w_1| + |w_2| \leq t$ là hình thoi → các góc của hình thoi nằm trên trục toạ độ → solution hay nằm tại góc → một số $w_j = 0$ chính xác.
> - L2 constraint: $w_1^2 + w_2^2 \leq t$ là hình tròn → không có góc → solution trượt trên cạnh tròn → $w_j \to 0$ nhưng không bằng 0.
...
**(c) Ví dụ**: $x_1 \approx x_2$ (correlation 0.95). Lasso: chọn $x_1$ bỏ $x_2$. Ridge: giữ cả hai với $w$ nhỏ. Elastic Net: giữ cả hai nhưng với trọng số tương đương nhau.
...
Best $\lambda = 0{,}1$. Std giảm đều (regularization ổn định predictions), nhưng CV MSE có điểm tối ưu.
...
**Bài 2**: Ridge regression với features $w = [5, 0{,}1, 8, -3]$ và $\lambda = 2$. Biết rằng $(X^\top X + \lambda I)^{-1} X^\top y$ cho $w_{\text{ridge}} = w / (1 + 2\lambda/\|w\|_2)$ (approximation trong trường hợp orthogonal features). Tính $\|w\|^2$ và $\|w_{\text{ridge}}\|^2$ với $\lambda=2$.

**Bài 3**: Lasso trên dataset 5 features. Với $\lambda=0{,}3$, kết quả: $w = [2{,}1, 0, 3{,}4, 0, -1{,}2]$. Tính L1 norm $\|w\|_1$. Nếu tăng $\lambda$ lên 0.6 và features 1, 3, 5 thật sự quan trọng, dự đoán $\|w_{\text{new}}\|_1$ sẽ tăng hay giảm? Feature nào likely thành 0 đầu tiên?
...
**Tính w_ridge** (approximation orthogonal features, scale factor $s = \|w\|_2 / (\|w\|_2 + 2\lambda)$):
```
‖w‖₂ = √98.01 ≈ 9.90
...
**$\|w_{\text{new}}\|_1$ giảm** vì $\lambda$ tăng luôn tạo shrinkage thêm.
