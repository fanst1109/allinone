...
// Source: Statistics/01-Descriptive/lesson-04-distribution-viz/README.md
// Chạy lại: go run tools/build-readme-data.go
window.README_MD = `# Lesson 04: Trực quan hoá phân phối
...
Với $n = 100$: $k \\approx \\lceil 6{,}64 \\rceil + 1 = 8$ bin.
Với $n = 1000$: $k \\approx \\lceil 9{,}97 \\rceil + 1 = 11$ bin.
...
Với $n = 100$, $\\text{IQR} = 10$: $\\text{Bin width} = 2 \\times 10 \\times 100^{-1/3} = \\dfrac{20}{4{,}64} \\approx 4{,}3$

**Quy tắc Scott (1979):**

$$\\text{Bin width} = 3{,}49 \\times s \\times n^{-1/3}$$
...
- Sturges: $k = \\lceil \\log_2(20) \\rceil + 1 = 5+1 = 6$ bin.
- Range $= 95 - 55 = 40$. Bin width $\\approx 40/6 \\approx 7$ → dùng 10 cho đẹp.
...
> <details><summary>Đáp án</summary>
> $k = \\lceil \\log_2(64) \\rceil + 1 = \\lceil 6 \\rceil + 1 = 7$ bin.
> </details>
...
$$\\text{Outlier nếu: } x < Q_1 - 1{,}5 \\times \\text{IQR} \\quad \\text{hoặc} \\quad x > Q_3 + 1{,}5 \\times \\text{IQR}$$
...
- $Q_1$ = median nửa dưới [2,5,7,8,10,12] $= \\dfrac{7+8}{2} = 7{,}5$
- $Q_2$ (Median) $= \\dfrac{12+14}{2} = 13$
- $Q_3$ = median nửa trên [14,15,15,18,22,50] $= \\dfrac{15+18}{2} = 16{,}5$
- $\\text{IQR} = 16{,}5 - 7{,}5 = 9$
- Lower fence: $Q_1 - 1{,}5 \\times \\text{IQR} = 7{,}5 - 13{,}5 = -6$ (không có điểm dưới mức này)
- Upper fence: $Q_3 + 1{,}5 \\times \\text{IQR} = 16{,}5 + 13{,}5 = 30$
- Outlier: $50 > 30 \\to$ **50 là outlier**
...
- $Q_1 \\approx 7$, $Q_3 \\approx 17{,}5$, $\\text{IQR} = 10{,}5$. Upper fence $= 33{,}25$.
- Outlier: 35, 80.
...
> - "1.5×IQR fence — con số 1.5 từ đâu ra?" → John Tukey chọn 1.5 để fence nằm ở khoảng $\\pm 2{,}7\\sigma$ với Normal distribution (bắt ~0.7% điểm ngoài fence). Có thể dùng $3{,}0 \\times \\text{IQR}$ cho "extreme outlier fence".
...
**(a) Là gì:** $\\text{ECDF}(x)$ = tỉ lệ phần trăm dữ liệu $\\leq x$. Đây là hàm bước (step function) tăng dần từ 0 đến 1.
...
**(c) Công thức:** Với $n$ điểm, mỗi điểm đóng góp $\\dfrac{1}{n}$ vào CDF.
...
Đọc: "$\\text{ECDF}(10) = 0{,}667$" nghĩa là 66.7% dữ liệu $\\leq 10$.
...
**(a) Quartiles:**
- $Q_2$ (Median) = giá trị thứ 6 = **11**
- $Q_1$ = median của nửa dưới [4,7,8,9,10] = **8**
- $Q_3$ = median của nửa trên [12,13,14,16,45] = **14**
- $\\text{IQR} = 14 - 8 = \\mathbf{6}$

**(b) Tukey fence:**
- Lower $= Q_1 - 1{,}5 \\times \\text{IQR} = 8 - 9 = \\mathbf{-1}$
- Upper $= Q_3 + 1{,}5 \\times \\text{IQR} = 14 + 9 = \\mathbf{23}$

**(c) Outlier:** Giá trị $45 > 23 \\to$ **45 là outlier**. Giá trị $4 > -1 \\to$ không phải outlier.

**(d) Mô tả boxplot:**
- Hộp từ $Q_1=8$ đến $Q_3=14$, đường median tại 11 (hơi bên trái giữa hộp → hơi right-skewed bên trong hộp).
...
**(b) P(thời gian ≤ 30):**
$\\text{ECDF}(30) = \\dfrac{6}{10} = \\mathbf{0{,}60 = 60\\%}$. Ba trong năm đơn hàng giao trong 30 phút.

**(c) Median (50th percentile):**
$\\text{ECDF} = 0{,}50$ tại $x=28$ → Median = **28 phút** (cũng verify: số điểm trực tiếp = median của [18,22,25,25,28,30,32,35,40,52] $= \\dfrac{28+30}{2} = 29$ phút theo công thức trực tiếp — ECDF method và formula method cho kết quả gần nhau).

**(d) 90th percentile:**
$\\text{ECDF} = 0{,}90$ tại $x=40$ → P90 = **40 phút**. "90% đơn hàng được giao trong vòng 40 phút" — đây là SLA (Service Level Agreement) điển hình.
