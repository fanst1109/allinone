...
**Ví dụ số**: Si pha P với nồng độ $N_D = 10^{16}$ atom/cm³. Ở 300 K, nồng độ electron tự do $n \approx 10^{16}$ /cm³, nồng độ lỗ trống $p \approx n_i^2/n = (1.5 \times 10^{10})^2/10^{16} =$ **2.25×10⁴ /cm³**. Electron nhiều hơn lỗ trống $10^{12}/10^4 =$ **10¹² lần** — hoàn toàn áp đảo.
...
**Ví dụ số tương đương**: Si pha B với $N_A = 10^{16}$ /cm³ → $p \approx 10^{16}$ /cm³, $n \approx 2.25 \times 10^4$ /cm³.
...
- Nồng độ hạt tải thiểu số rất nhỏ: $p \cdot n = n_i^2 \approx (1.5 \times 10^{10})^2$ ở 300 K.
...
$$V_0 = \frac{kT}{q} \cdot \ln\!\left(\frac{N_A \cdot N_D}{n_i^2}\right)$$

Trong đó:
- $k = 1.38 \times 10^{-23}$ J/K (hằng số Boltzmann).
- $T$ = nhiệt độ tuyệt đối (K).
- $q = 1.6 \times 10^{-19}$ C (điện tích electron).
- $N_A$, $N_D$ = nồng độ chất pha tạp (acceptor, donor).
- $n_i$ = nồng độ hạt tải nội tại của Si tinh khiết $\approx 1.5 \times 10^{10}$ /cm³ ở 300 K.
...
**Ví dụ 4 — GaAs** ($n_i \approx 2 \times 10^6$ /cm³, $N_A = N_D = 10^{17}$ /cm³):
$V_0 \approx$ 1.2 V — cao hơn nhiều do $n_i$ cực nhỏ.

**Quy tắc ngón tay cái:**
- Si: $V_0 \approx$ **0.6–0.7 V** (rào thế điển hình).
- Ge: $V_0 \approx$ **0.2–0.3 V**.
- GaAs: $V_0 \approx$ **1.1–1.4 V** (dùng trong LED, laser diode).

⚠ **Lỗi thường gặp**: Nhiều người nhầm rào thế $V_0$ với "điện áp nguồn cần thiết". Thực ra $V_0$ là hiệu điện thế nội tại — nó **không xuất hiện** trên đầu đo voltmeter khi bạn đo hai cực của P-N junction không phân cực. Điện trường nội và điện trường trong vùng tiếp xúc kim loại-bán dẫn (ở đầu dây) bù nhau hoàn toàn → voltmeter đọc 0 V. $V_0$ chỉ có ý nghĩa khi ta áp điện áp ngoài để thu hẹp hoặc mở rộng vùng nghèo.

📝 **Tóm tắt mục 3:**
- Ghép P+N → khuếch tán → tái kết hợp hạt tải → vùng nghèo hình thành.
- Điện trường nội hướng N→P (từ + sang -), tạo rào thế $V_0$.
- $V_0$(Si) ≈ 0.6–0.7 V, $V_0$(Ge) ≈ 0.2–0.3 V.
...
💡 **Hình dung**: Rào thế $V_0$ như đập nước ngăn dòng chảy. Phân cực thuận = ta dùng máy bơm (nguồn điện ngoài) **đẩy** ngược chiều đập — khi áp suất bơm đủ mạnh ($V_{\text{ext}} > V_0$), nước vượt qua đập và chảy ào ào.
...
4. Khi $V_{\text{ext}} \approx V_0$ (≈ 0.7 V với Si): rào thế bị triệt tiêu phần lớn.
...
Trong đó:
- $I_S$ = dòng bão hòa ngược (reverse saturation current) $\approx 10^{-10}$ đến $10^{-14}$ A với Si ở 300 K.
- $n$ = hệ số ideality (1 đến 2, thường $\approx$ 1 với Si).
- $V_T = kT/q \approx 26$ mV ở 300 K.

**Ví dụ tính dòng** (Si, $I_S = 10^{-12}$ A, $n = 1$):
...
**Dòng rò điển hình:**
- Si: $I_S \approx 10^{-12}$ đến $10^{-10}$ A (1 pA đến 100 pA) — bỏ qua được.
- Ge: $I_S \approx 10^{-6}$ A (1 µA) — lớn hơn 100–1000 lần do $n_i$ của Ge cao hơn.
...
- *"Tại sao Ge lại ít dùng hơn Si nếu rào thế thấp hơn (0.3 V) — tức là cần ít năng lượng hơn để mở?"* — Chính vì $n_i$ của Ge cao hơn (${\approx}2.4 \times 10^{13}$ vs $1.5 \times 10^{10}$), dòng rò ngược $I_S$ lớn hơn Si ~1000 lần. Ở nhiệt độ cao (50–80°C), dòng rò Ge tăng mạnh làm mạch mất kiểm soát. Si ổn định hơn đến ~150°C.

- *"Điện áp phân cực thuận càng lớn thì dòng càng lớn — vậy tại sao không tăng V mãi?"* — Dòng càng lớn → công suất $P = V \cdot I$ → nhiệt tỏa ra → nhiệt độ diode tăng → $I_S$ tăng → dòng tăng thêm → vòng lặp nhiệt dương (thermal runaway). Diode thực tế có điện áp tối đa ($I_{F,\text{max}}$) và công suất tiêu thụ tối đa ($P_{D,\text{max}}$) ghi trên datasheet.

📝 **Tóm tắt mục 4:**
- Forward bias: vùng nghèo thu hẹp → dòng lớn khi $V > V_0 \approx$ 0.7 V (Si).
- Reverse bias: vùng nghèo mở rộng → dòng rò cực nhỏ $\approx I_S \approx$ 1 pA (Si).
- Tính chỉnh lưu = chỉ dẫn một chiều = cơ sở của diode.
- Phương trình Shockley: $I = I_S \cdot (e^{V/V_T} - 1)$.
...
**Bài 2 (Pha tạp)**: Silicon pha Phosphorus với nồng độ $N_D = 5 \times 10^{15}$ /cm³. Tính nồng độ lỗ trống (hole concentration) ở 300 K. Biết $n_i = 1.5 \times 10^{10}$ /cm³. Loại bán dẫn này là N hay P?

**Bài 3 (Rào thế)**: Tính rào thế $V_0$ của mối nối P-N có $N_A = 10^{16}$ /cm³ và $N_D = 2 \times 10^{16}$ /cm³ trong Si ở 300 K. Dùng $V_T = 26$ mV, $n_i = 1.5 \times 10^{10}$ /cm³.

**Bài 4 (Dòng forward)**: Một diode Si có $I_S = 2 \times 10^{-12}$ A, $n = 1$. Tính dòng khi áp $V = 0.65$ V lên mối nối ở 300 K ($V_T = 26$ mV).
...
Bán dẫn pha $N_D = 5 \times 10^{15}$ /cm³ (Phosphorus, nhóm V) → **bán dẫn loại N** (electron là hạt tải đa số).

Tính nồng độ lỗ trống (minority carriers), dùng luật tác dụng khối $n \cdot p = n_i^2$:

$$p = \frac{n_i^2}{n} \approx \frac{n_i^2}{N_D} = \frac{(1.5 \times 10^{10})^2}{5 \times 10^{15}} = \frac{2.25 \times 10^{20}}{5 \times 10^{15}} = 4.5 \times 10^4 \text{ /cm}^3$$

Kết quả: electron $n \approx 5 \times 10^{15}$ /cm³, lỗ trống $p \approx$ **4.5×10⁴ /cm³**. Electron nhiều hơn lỗ trống hơn $10^{11}$ lần.
...
Bước 2 — Tính $N_A \times N_D$:

$$N_A \times N_D = 10^{16} \times 2 \times 10^{16} = 2 \times 10^{32} \text{ /cm}^6$$

Bước 3 — Tính $V_0$:

$$\begin{aligned}
V_0 &= V_T \cdot \ln\!\left(\frac{N_A \cdot N_D}{n_i^2}\right) = 0.026 \times \ln\!\left(\frac{2 \times 10^{32}}{2.25 \times 10^{20}}\right) \\
    &= 0.026 \times \ln(8.89 \times 10^{11})
\end{aligned}$$

Tính $\ln(8.89 \times 10^{11})$: $\ln(10^{12}) = 12 \times \ln 10 = 12 \times 2.303 = 27.6$; $\ln(8.89 \times 10^{11}) \approx 27.6 - \ln(10/8.89) \approx 27.6 - 0.12 = 27.5$.

$$V_0 = 0.026 \times 27.5 \approx 0.715 \text{ V}$$

Kết luận: rào thế $V_0 \approx$ **0.72 V** — hơi cao hơn điển hình do cả hai phía đều có doping cao hơn mức trung bình.

**Bài 4:**

Dùng phương trình Shockley với $I_S = 2 \times 10^{-12}$ A, $V = 0.65$ V, $V_T = 0.026$ V:

$$\begin{aligned}
I &= I_S \cdot (e^{V/V_T} - 1) = 2 \times 10^{-12} \times (e^{0.65/0.026} - 1) \\
  &= 2 \times 10^{-12} \times (e^{25} - 1)
\end{aligned}$$

Tính $e^{25}$: $e^{10} \approx 22026$; $e^{25} = e^{10} \times e^{10} \times e^5 \approx 22026 \times 22026 \times 148.4 \approx 7.2 \times 10^{10}$.

$$I \approx 2 \times 10^{-12} \times 7.2 \times 10^{10} = 144 \times 10^{-3} \approx 0.144 \text{ A} = \textbf{144 mA}$$
...
$$V_R = V_{\text{nguồn}} - V_D = 5 - 0.7 = 4.3 \text{ V}$$

$$I = \frac{V_R}{R} = \frac{4.3}{220} \approx 0.0195 \text{ A} \approx \textbf{19.5 mA}$$

Kiểm tra: áp trên R = $19.5 \text{ mA} \times 220\ \Omega =$ **4.29 V** ✓ (cộng $V_D = 0.7$ V ≈ 5 V ✓).

Khi đảo chiều diode (phân cực ngược):

$$I \approx I_S \approx 10^{-12} \text{ A} = 1 \text{ pA}$$
...
4. **Forward bias** ($V > V_0$): vùng nghèo thu hẹp, dòng tăng theo hàm mũ $I = I_S \cdot e^{V/V_T}$.
5. **Reverse bias**: vùng nghèo mở rộng, dòng rò cực nhỏ $\approx I_S \approx$ 1 pA (Si).
6. **Breakdown ngược** ở $V_{BR}$ đủ lớn: avalanche hoặc Zener — nền tảng của diode Zener (Lesson 02).
