...
- Tính được số loại giao tử của cá thể có **n cặp gen dị hợp** (2ⁿ), tỉ lệ kiểu hình của lai 2 tính (9:3:3:1), và kết quả của **lai phân tích (test cross)**.
...
**Ví dụ 4 — Cây có 4 cặp gene dị hợp** \`AaBbCcDd\`. Đây không phải kiểu gen lạ — sau này dùng để tính số loại giao tử. **4 cặp dị hợp → 2⁴ = 16 loại giao tử khác nhau** (§5.3). Chỉ riêng việc này đã cho thấy độ đa dạng tăng cấp số nhân theo n.
...
- 2 cặp dị hợp (\`AaBb\`): 2 × 2 = **4** loại (AB, Ab, aB, ab).
- 3 cặp dị hợp (\`AaBbCc\`): 2³ = **8** loại.
- 4 cặp dị hợp (\`AaBbCcDd\`): 2⁴ = **16** loại.
- Tổng quát: **2ⁿ loại giao tử**. (Chứng minh: mỗi gene độc lập cho 2 lựa chọn → nhân lại.)
...
- Cá thể có n cặp gene dị hợp → 2ⁿ loại giao tử.
...
**Bài 5**:
- (a) 4 cặp dị hợp, mỗi cặp cho 2 loại allele độc lập → **2⁴ = 16** loại giao tử.
- (b) Mỗi giao tử cụ thể có xác suất 1/16 (vì tất cả 16 loại đồng xác suất). → P(\`aBcD\`) = **1/16**. Cách khác (nhân): P(a) × P(B) × P(c) × P(D) = (1/2)⁴ = 1/16 ✓.
- (c) P(con \`aabbccdd\`) = P(aa) × P(bb) × P(cc) × P(dd) (vì độc lập) = (1/4)⁴ = **1/256**. 4 gene độc lập, mỗi gene cho P(đồng hợp lặn) = 1/4 ở thế hệ con của \`Aa × Aa\`.
...
5. **Quy luật 2 — Phân ly độc lập**: gene ở NST khác nhau phân ly độc lập. n cặp dị hợp → 2ⁿ giao tử. Lai 2 tính \`AaBb × AaBb\` → kiểu hình **9:3:3:1**.
